...
public class ApplicationsProvider extends ContentProvider {
    ...
    //lijun add start for pinyin search
    private static final HashMap<String,String> sPinYinPattern = new HashMap<String,String>();
    ...
    public boolean onCreate() {
        ...
        sPinYinPattern.clear();//lijun add
        return true;
    }
    ...
    private Cursor searchApplications(String query, String[] projectionIn,
        ...
        //lijun add start for pinyin search
        StringBuffer aaa = new StringBuffer(searchWhere);
        ArrayList<String> wherePinYin = getTitleMatchedByPinYin(query);
        if (wherePinYin != null && wherePinYin.size() > 0) {
            for (String s : wherePinYin) {
                aaa.append(" or name='"+s+"'");
            }
        }
        searchWhere = aaa.toString();
        Log.d("lijun","search where : " + aaa);
        //lijun add end
    ...
    private synchronized void updateApplicationsList(String packageName) {
        ...
        try {
            ...
            for (int i = 0; i < activityCount; i++) {
                ...
                //lijun add start
//                IconManager iconManager = IconManager.getInstance(getContext(), false, false);
//                Uri uri = iconManager.getIconUri(activityPackageName,null,null);
//                Log.d("lijun","icon uri : " + uri);
//                if(uri!=null){
//                    icon = uri.toString();
//                }
                //lijun add end
                ...
                inserter.execute();
                //lijun add start for pinyin search
                String pinyin = Utilities.getRegExpBySpChinese(title);
//                Log.d("lijun","init sPinYinPattern : " + pinyin);
                if(pinyin!=null){
                    sPinYinPattern.put(title,pinyin);
                }
                //lijun add end
            }
            Log.d("lijun","sPinYinPattern size : " + sPinYinPattern.size());
    ...
    //lijun add start for pinyin search
    private ArrayList<String> getTitleMatchedByPinYin(String query) {
        if (query.trim().isEmpty()) {
            return null;
        }
        if(!Utilities.isLetter(query)){
            return null;
        }
        ...
        Iterator iter = sPinYinPattern.entrySet().iterator();
        while (iter.hasNext()) {
            Map.Entry entry = (Map.Entry) iter.next();
            String key = (String) entry.getKey();
            String pinyin = (String) entry.getValue();
            if (matches(key, queryWords, pinyin)) {
                list.add(key);
            }
        }
        return list;
    ...
    private boolean matches(String title, String[] queryWords, String patternString) {
        ...
        for (int qi = 0; qi < queryWords.length; qi++) {
            ...
            for (int i = 0; i < words.length; i++) {
                if (pattern != null && pattern.matcher(queryWords[qi]).matches()) {//words[i].indexOf(queryWords[qi]) != -1 ||
                    foundMatch = true;
                    break;
                }
            }

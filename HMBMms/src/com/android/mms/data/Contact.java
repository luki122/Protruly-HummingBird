...
//    };
    ...
    public static String formatNameAndNumber(String name, String number, String numberE164) {
         ...
         * }
         */
        if(DEBUG) Log.i(M_TAG, "formatNameAndNumber, name=" + name + ", number=" + number);
    ...
    private static class ContactsCache {
        ...
        //tangyisen fix bug642
        /*private static final String EMAIL_SELECTION = "UPPER(" + Email.DATA + ")=UPPER(?) AND "
                + Data.MIMETYPE + "='" + Email.CONTENT_ITEM_TYPE + "'";*/
        private static final String EMAIL_SELECTION = "UPPER(" + Email.DATA + ")=UPPER(?) AND "
                + Data.MIMETYPE + "='" + Email.CONTENT_ITEM_TYPE + "'" + " AND ((account_name ='" + MessageUtils.ACCOUNT_NAME_LOCAL_PHONE
                + "' AND account_type='" + MessageUtils.ACCOUNT_TYPE_LOCAL_PHONE + "') OR (account_name is null AND account_type is null))";
        //end tangyisen
        ...
        private List<Contact> getContactInfoForPhoneUrisInternal(StringBuilder idSetBuilder) {
            ...
            if (idSetBuilder.length() > 0) {
                //begin tangyisen fix bug642
                /*final String whereClause = Phone._ID + " IN (" + idSetBuilder.toString() + ")";*/
                final String whereClause = Phone._ID + " IN (" + idSetBuilder.toString() + ")" + " AND ((account_name ='" + MessageUtils.ACCOUNT_NAME_LOCAL_PHONE
                        + "' AND account_type='" + MessageUtils.ACCOUNT_TYPE_LOCAL_PHONE + "') OR (account_name is null AND account_type is null)) ";
        ...
        private boolean contactChanged(Contact orig, Contact newContactData) {
            ...
            if (orig.mPersonId != newContactData.mPersonId) {
                if (Log.isLoggable(LogTag.CONTACT, Log.DEBUG)) {
                    Log.d(LogTag.CONTACT, TAG+" person id changed");
                }
                return true;
            }
            ...
            if (orig.mPresenceResId != newContactData.mPresenceResId) {
                if (Log.isLoggable(LogTag.CONTACT, Log.DEBUG)) {
                    Log.d(LogTag.CONTACT, TAG+" presence changed");
                }
                return true;
            }
            ...
            if (!oldName.equals(newName)) {
                if (Log.isLoggable(LogTag.CONTACT, Log.DEBUG)) {
                    Log.d(LogTag.CONTACT, TAG+String.format(" name changed: %s -> %s", oldName, newName));
                }
                return true;
            }
            ...
            if (!oldLabel.equals(newLabel)) {
                if (Log.isLoggable(LogTag.CONTACT, Log.DEBUG)) {
                    Log.d(LogTag.CONTACT, TAG+String.format(" label changed: %s -> %s", oldLabel, newLabel));
                }
                return true;
            }
            ...
            if (!Arrays.equals(orig.mAvatarData, newContactData.mAvatarData)) {
                if (Log.isLoggable(LogTag.CONTACT, Log.DEBUG)) {
                    Log.d(LogTag.CONTACT, TAG+" avatar changed");
                }
                return true;
            }
...
//                    }
                    ...
                    if (!TextUtils.isEmpty(c.mNumber)) {
                        ...
                        for (UpdateListener l : iterator) {
                            if (Log.isLoggable(LogTag.CONTACT, Log.DEBUG)) {
                                Log.d(LogTag.CONTACT, TAG+" updating " + l);
                            }
                            l.onUpdate(c);
                        }
                    }
...
//                /// @}
            ...
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(matchNumber)) {
                //begin tangyisen fix bug642
               String selection = "raw_contacts.account_id IN (select _id from accounts where (account_name = '" + MessageUtils.ACCOUNT_NAME_LOCAL_PHONE
                       + "' AND account_type = '" + MessageUtils.ACCOUNT_TYPE_LOCAL_PHONE + "') OR (account_name is null AND account_type is null))";
                Cursor cursor = mContext.getContentResolver().query(
                        Uri.withAppendedPath(PhoneLookup.CONTENT_FILTER_URI, normalizedNumber), CALLER_ID_PROJECTION, selection, null, Phone.DISPLAY_NAME + " ASC");
                if (cursor == null) {
                    /// M:
        ...
        private void fillPhoneTypeContact(final Contact contact, final Cursor cursor) {
			...
			//lichao delete in 2017-06-26 for no need show Avatar
        ...
        */
        ...
        static final int STATIC_KEY_BUFFER_MAXIMUM_LENGTH = 15;//tangyisen modify 10 to 15 to fix bug2583
...
//        /// M: add for group contact
    ...
    */
    ...
    /*public void addToBlacklistTask(final Context context){
        ...
        if(TextUtils.isEmpty(number)){
            ...
            if(null != mMyAddBlackListener){
                mMyAddBlackListener.onAddBlackCompleted(false);
            }
            return ;
        }
        //lichao modify in 2017-05-22
        //AsyncTask<Params, Progress, Result>
        new AsyncTask<Void, Integer, Boolean>() {
            @Override
            protected void onPreExecute() {
                if(DEBUG) Log.i(TAG, "addToBlacklistTask, onPreExecute() called");
            }

            //void onProgressUpdate(Progress... values)
            @Override
            protected void onProgressUpdate(Integer... progresses) {
                if(DEBUG) Log.i(TAG, "addToBlacklistTask, onProgressUpdate() called");
            }

            //Result doInBackground(Params... params)
            protected Boolean doInBackground(Void... none) {
                return MessageUtils.addblack(context, number, name);
            }

            //void onPostExecute(Result result)
            protected void onPostExecute(Boolean isSuccess) {
                if(DEBUG) Log.d(TAG, "addToBlacklistTask, onPostExecute(), isSuccess = "+isSuccess);
                if(null != mMyAddBlackListener){
                    mMyAddBlackListener.onAddBlackCompleted(isSuccess);
                }
            }

            @Override
            protected void onCancelled() {
                if(DEBUG) Log.i(TAG, "addToBlacklistTask, onCancelled() called");
            }
        } .execute();
    }*/

    //HB. Comments :  , Engerineer : lichao , Date : 17-5-22 , begin
    /*public interface MyAddBlackListener {
        void onAddBlackCompleted(boolean isSeccess);
    }

    private MyAddBlackListener mMyAddBlackListener = null;

    public void setMyAddBlackListener(MyAddBlackListener myAddBlackListener) {
        this.mMyAddBlackListener = myAddBlackListener;
    }*/
    //HB. end

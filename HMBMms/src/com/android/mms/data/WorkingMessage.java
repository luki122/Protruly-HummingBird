...
public class WorkingMessage implements IWorkingMessageCallback {
    ...
    private static final String TAG_DRAFT = "Mms/Draft/WMessage";
...
//add for attachment enhance
    ...
    public static WorkingMessage loadDraft(final ComposeMessageActivity activity,
        ...
        /// @}
        //begin tangyisen
        boolean needReloadBeforeContent = false;
        String uriString = null;
        //end tangyisen
        if (conv.getThreadId() <= 0) {//tangyisen add needReloadBeforeContent
            //begin tangyisen add
            uriString = MessageUtils.getPreferenceValueString(activity.getApplicationContext(), MessageUtils.SMS_AND_MMS_URI_KEY, "");
            needReloadBeforeContent = !TextUtils.isEmpty( uriString ) ? true : false;
            if(!needReloadBeforeContent) {
            //end tangyisen
        ...
        //begin tangyisen
        final boolean needReloadBeforeContentFinal = needReloadBeforeContent;
        ...
        new Thread(new Runnable() {
            public void run() {
                String draftTmp = null;
                if(needReloadBeforeContentFinal) {
                    draftTmp = msg.readDraftSmsMessageNoRecipients( conv );
                } else {
                    draftTmp = msg.readDraftSmsMessage(conv);
                }
                final String draftText = draftTmp;
                //tangyisen begin add fix bug2384
                boolean isSmsUri = uriStringFinal == null ? false : (uriStringFinal.indexOf("sms") == -1 ? false : true );
                //tangyisen end
                Log.d(TAG_DRAFT, "[loadDraft] draftText : " + draftText);
                if (TextUtils.isEmpty(draftText) && !isSmsUri) {//tangyisen mms
                    ...
                    /// @}
                    //begin tangyisen
                    if(TextUtils.isEmpty( uriStringFinal ) || threadId > 0) {
                        DraftManager.getInstance().loadDraft(DraftManager.ASYNC_LOAD_ACTION,
                                threadId, null, activity, msg.mInterface);
                    } else {
                        MmsDraftData ddd = DraftManager.getInstance().loadDraft(DraftManager.ASYNC_LOAD_ACTION,
                                0, Uri.parse( uriStringFinal ), activity, msg.mInterface);
                        MmsDraftData dddd = ddd;
                    }
                    //end tangyisen
                } else if (!TextUtils.isEmpty(draftText)){//tangyisen
                    ...
                    MessageUtils.setPreferenceValueString(activity.getApplicationContext(), MessageUtils.SMS_AND_MMS_URI_KEY, "");
                }
            }
    ...
    private class DraftInterface implements IDraftInterface {
        ...
        public void loadFinished(MmsDraftData mdd) {
            if (mdd != null) {
                ...
                MessageUtils.setPreferenceValueString(mActivity.getApplicationContext(), MessageUtils.SMS_AND_MMS_URI_KEY, "");
            }
            /// M Fix CR : ALPS01234459 @{
            ...
            try {
                ...
                if(mConversation.getRecipients().isEmpty()) {
                    if(msgUri == null) {
                        ...
                        MessageUtils.setPreferenceValueString(mActivity.getApplicationContext(), MessageUtils.SMS_AND_MMS_URI_KEY, msgUri.toString());
                    }
                }
                //end tangyisen
    ...
    public void setText(CharSequence s) {
        if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
            Log.v(LogTag.APP, TAG+" setText: s = " + s);
        }
...
//                }
                   //begin tangyisen
                    //MessageUtils.setPreferenceValueString(mActivity.getApplicationContext(), MessageUtils.SMS_AND_MMS_URI_KEY, mMessageUri.toString());
                   //end tangyisen
            }
...
//            loadFromUri(uri);
    ...
    public void setHasMultipleRecipients(boolean hasMultipleRecipients, boolean notify) {
        //tangyisen delete tmp multi recipients not change to mms
    ...
    public static final int RESEND_NONE_TYPE = 0;
    public static final int RESEND_SMS_TYPE = 1;
    public static final int RESEND_MMS_TYPE = 2;
    private int mResendType = RESEND_NONE_TYPE;//0 -- no special  1 -- sms   2 -- mms
    public void setResendType(int type) {
        mResendType = type;
    }
    ...
    public void resetResend() {
        mResendType = RESEND_NONE_TYPE;
        /*mMessageUri = mBeforeResendUri;
        mSlideshow = mBeforeResendModel;
        mBeforeResendUri = null;
        mBeforeResendModel = null;*/
        mResendWorkingMessage = null;
    }

    /*private Uri mBeforeResendUri;
    private SlideshowModel mBeforeResendModel;*/
    private WorkingMessage mResendWorkingMessage;
    public void setResendMms(WorkingMessage wm) {
        /*mBeforeResendUri = mMessageUri;
        mBeforeResendModel = mSlideshow;*/
        mResendWorkingMessage = wm;
    }
    //end tangysien add for resend message
    ...
    public void send(final String recipientsInUI, final int subId) {
        ...
        prepareForSave(true /* notify */);
        checkConversationHasRecipients(recipientsInUI);//add by tangyisen
        ...
        final SplitToMmsAndSmsConversation spliter = new SplitToMmsAndSmsConversation(conv, msgTxt, mResendType);
    ...
    private class SplitToMmsAndSmsConversation implements ISplitToMmsAndSmsConversationCallback {
        ...
        //begin tangyisen add for resend msg
        public SplitToMmsAndSmsConversation(final Conversation conv, final String msgTxt, final int resendType) {
            mMmsConv = null;
            mSmsConv = null;
            Log.d(TAG, "setConversationToSplitAsSmsMms");
        ...
        //end tangyisen add for resend msg
...
//            mStatusListener.onMessageSent();
    ...
    private void markMmsMessageWithError(Uri mmsUri, int subId) {
        try {
            ...
            ContentValues valuePduTable = new ContentValues(2);
			...
			valuePduTable.put(Mms.FAILED_UNREAD, 1);//lichao add for failed_unread
    ...
    private void asyncUpdateDraftMmsMessage(final Conversation conv, final boolean isStopping, final IDraftInterface callback) {
        ...
        //tangyisen begin modify
        asyncUpdateDraftMmsMessageNoRecipients(conv, isStopping, callback, sendReq);
        /*DraftManager.getInstance().saveDraft(DraftManager.ASYNC_SAVE_ACTION, threadId,
                mMessageUri, mSlideshow, sendReq, mActivity, callback);*/
        //end tangyisen
    }
    ...
    private String readDraftSmsMessage(Conversation conv) {
        ...
        if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
            Log.v(LogTag.APP, TAG+" readDraftSmsMessage conv: " + conv);
        }
            ...
            public void run() {
                try {
                    ...
                    if (conv.getRecipients().isEmpty()) {
                        ...
                        updateDraftSmsMessageNoRecipients(conv, contents);
                        //end tangyisen
                        return;
                    }
    ...
    */
    ...
    //begin tangyisen
    private void updateDraftSmsMessageNoRecipients(final Conversation conv, String contents) {

        String uriString = MessageUtils.getPreferenceValueString(mActivity.getApplicationContext(), MessageUtils.SMS_AND_MMS_URI_KEY, "");
        if(!TextUtils.isEmpty( uriString )) {
            //maybe sms or mms,just delete
            asyncDelete(Uri.parse( uriString ), null, null);
        }
        ...
        ContentValues values = new ContentValues(3);
        //if no recipients,will not create thread,the thread id will send 0
        values.put(Sms.THREAD_ID, 0);
        values.put(Sms.BODY, contents);
        //values.put(Sms.TYPE, Sms.MESSAGE_TYPE_DRAFT);//next will change this
        Uri uri = SqliteWrapper.insert(mActivity, mContentResolver, Sms.CONTENT_URI, values);
        if(uri == null) {
            MessageUtils.setPreferenceValueString(mActivity.getApplicationContext(), MessageUtils.SMS_AND_MMS_URI_KEY, "");
        } else {
            MessageUtils.setPreferenceValueString(mActivity.getApplicationContext(), MessageUtils.SMS_AND_MMS_URI_KEY, uri.toString());
        }
        mMessageUri = null;
    }

    private String readDraftSmsMessageNoRecipients(Conversation conv) {
        String uriString = MessageUtils.getPreferenceValueString(mActivity.getApplicationContext(), MessageUtils.SMS_AND_MMS_URI_KEY, "");
        if(TextUtils.isEmpty( uriString )) {
            return null;
        }
        ...
        Cursor c = SqliteWrapper.query(mActivity, mContentResolver,
               uri, SMS_BODY_PROJECTION, null, null, null);
        boolean haveDraft = false;
        if (c != null) {
            try {
                if (c.moveToFirst()) {
        ...
        } /*else {//because has mms,so dont set ""
            MessageUtils.setPreferenceValueString(mActivity.getApplicationContext(), MessageUtils.SMS_AND_MMS_URI_KEY, "");
        }*/
    ...
    private boolean mIsSaveNoRecipientsMms = false;
    private void asyncUpdateDraftMmsMessageNoRecipients(final Conversation conv, final boolean isStopping, final IDraftInterface callback, SendReq sendReq) {
        if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
            LogTag.debug("asyncUpdateDraftMmsMessageNoRecipients conv=%s mMessageUri=%s", conv, mMessageUri);
        }

        mIsSaveNoRecipientsMms = true;
        if(conv.getRecipients().isEmpty()) {
            String uriString = MessageUtils.getPreferenceValueString(mActivity.getApplicationContext(), MessageUtils.SMS_AND_MMS_URI_KEY, "");
            if(!TextUtils.isEmpty( uriString )) {
                //maybe sms or mms,just delete
                if(!uriString.equals( mMessageUri.toString() )) {
                    asyncDelete(Uri.parse( uriString ), null, null);
                }
            }
        }
        ...
        DraftCache.getInstance().setSavingDraft(true);
        SendReq sendRequTmp = sendReq;
        if(sendReq == null) {
            sendRequTmp = makeSendReq(conv, mSubject);
        }
        MmsDraftData data = DraftManager.getInstance().saveDraft(DraftManager.ASYNC_SAVE_ACTION, 0,
                mMessageUri, mSlideshow, sendRequTmp, mActivity, callback);
        //becuanse DraftManager.ASYNC_SAVE_ACTION,so will not get result in here,will get in DraftInterface updateAfterSaveDraftFinished
        /*Uri uri = data.getMessageUri();
        if(uri == null) {
            MessageUtils.setPreferenceValueString(mActivity.getApplicationContext(), MessageUtils.SMS_AND_MMS_URI_KEY, "");
        } else {
            MessageUtils.setPreferenceValueString(mActivity.getApplicationContext(), MessageUtils.SMS_AND_MMS_URI_KEY, uri.toString());
    ...
    public void asyncDeleteDraftNoRecipient(final Uri uri) {
        String authority = uri.getAuthority();
        //if("sms".equals( authority )) {
            asyncDelete(uri, null, null);
        /*} else {

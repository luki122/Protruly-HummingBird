...
public class MyScrollListener implements OnScrollListener {
    ...
    //begin tangyisen
    private boolean mNeedRunThread = true;
    //end tangyisen

    //lichao add
    private Runnable mHideSoftKeyboardRunnable = null;
    ...
    public void destroyThread() {
        synchronized (runnable) {
            if(DEBUG) MmsLog.d(TAG, "destroy thread.");
            mNeedDestroy = true;
    ...
    //begin tangyisen
    public void setIsNeedRunThread(boolean need) {
        this.mNeedRunThread = need;
    }
    //end tangyisen
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        ...
        if(DEBUG) MmsLog.d(TAG, "OnScrollListener.onScrollStateChanged()...");
        ...
        //} else {
            if(view.getAdapter() instanceof MessageCursorAdapter){
        ...
        if (scrollState == OnScrollListener.SCROLL_STATE_TOUCH_SCROLL
            ...
            if (null != mHideSoftKeyboardRunnable) {
                if (DEBUG) MmsLog.d(TAG, "onScrollStateChanged >>>HideSoftKeyboardRunnable.run()");
                mHideSoftKeyboardRunnable.run();
            }
        }

        if (mlistAdapter == null) {
            if(DEBUG) MmsLog.w(TAG, "onScrollStateChanged() mlistAdapter is null, return !");
            return;
        }
        ...
        if (mlistAdapter != null && mlistAdapter.getCount() >= mMinCursorCount) { // run below code when threads' count more than 100.
            if (scrollState == OnScrollListener.SCROLL_STATE_TOUCH_SCROLL) { // on touch
                if(DEBUG) MmsLog.d(TAG, "OnScrollListener.onScrollStateChanged(): on touch state.");
                mlistAdapter.setIsScrolling(false);

            } else if (scrollState == OnScrollListener.SCROLL_STATE_FLING) { // scrolling
                if(DEBUG) MmsLog.d(TAG, "OnScrollListener.onScrollStateChanged(): scrolling...");
                mlistAdapter.setIsScrolling(true);
                if(mNeedRunThread) {//tangyisen add
                    synchronized (runnable) {
        ...
        if (mlistAdapter != null && scrollState == OnScrollListener.SCROLL_STATE_IDLE) { // not in scrolling
            //begin tangyisen
            /*mlistAdapter.setIsScrolling(false);
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        if (mContext != null) {
            if(DEBUG) MmsLog.d(TAG, "OnScrollListener.onScroll(): firstVisibleItem: " + firstVisibleItem
    ...
    private class MyRunnable implements Runnable {
        ...
        public void setNeedRun(boolean needRun) {
            if (mNeedRun) {
                if(DEBUG) MmsLog.d(TAG, "OnScrollListener.onScrollStateChanged(): stop scrolling!");
            }
            mNeedRun = needRun;
            if (!mNeedRun) {
                count = 0;
        ...
        public void run() {
            ...
            while (!mNeedDestroy) {
                while (mNeedRun) {
                    if(DEBUG) MmsLog.d(TAG, "OnScrollListener.run():count=" + count
                            + " mVelocity=" + mVelocity);
                    if (mNeedAllRefresh
                ...
                if(DEBUG) MmsLog.d(TAG, "OnScrollListener.run(): listener is wait.");
                synchronized (this) {
                    try {
                        this.wait();
                    } catch (InterruptedException ex) {
    ...
    public void setHideSoftKeyboardRunnable(Runnable hideSoftKeyboardRunnable){
        mHideSoftKeyboardRunnable = hideSoftKeyboardRunnable;
    }
}

 ...
 */
//tangyisen modify Hb
public class MmsPlayerActivity extends HbActivity
...
//    private static CellConnMgr mCellMgr = null;//csw
    ...
    public void onCreate(Bundle icicle) {
        ...
        //add for cmcc dir ui end
        //tangyisen modify to hb
        //setContentView(R.layout.mms_player_activity);
        setHbContentView(R.layout.mms_player_activity);
        initToolBar();
        //end tangyisen
    ...
    private void initListAdapter(Uri msg) {
        ...
        for (int i = 0; i < slideNum; i++) {
            ...
            int mTextWidth = 0;
            int mTextHeight = 0;
            boolean isContinue = true;//add tangyisen
        ...
        //if (mDirMode) {
            if (mSubject != null) {
               //tangyisen modify
                ...
                //addFileAttachment(null, getString(R.string.subject_label) + mSubject);
                attachments.add(new MmsPlayerActivityItemData(this, mSubject, null, null));
                //end tangyisen
            }
            ...
            ArrayList<FileAttachmentModel> fileSlide = slideshowModel.getAttachFiles();
            for(int i = 0; i < fileAttachmentCount; i ++) {
                //addFileAttachment( fileSlide.get( i ), null );
                attachments.add(new MmsPlayerActivityItemData(this, null, fileSlide.get( i ).getSrc(), fileSlide.get( i )));
            }
            //end tangyisen
        //}
    ...
    private void addFileAttachment(final FileAttachmentModel file, String subjectStr) {
       /*View header = LayoutInflater.from(this).inflate(R.layout.foldermode_mms_header, null);
        mRecipient = (TextView) header.findViewById(R.id.msg_recipent);
        mDate = (TextView) header.findViewById(R.id.msg_date);
        mThrough = (TextView) header.findViewById(R.id.through_id);
        ...
        mLockedInd.setVisibility(View.GONE);
        mListView.addHeaderView(header);*/
        if(file == null && TextUtils.isEmpty( subjectStr )) {
            return;
        }
        View header = LayoutInflater.from(this).inflate(R.layout.zzz_fileattachment_mms_header, null);
        TextView subject = (TextView) header.findViewById(R.id.zzz_msg_subject);
        ImageView attachmenticon = (ImageView) header.findViewById(R.id.zzz_has_file_attachment);
        TextView attachmentname = (TextView) header.findViewById(R.id.zzz_attach_Name);
        View attachmentParent = header.findViewById(R.id.zzz_attachment_parent);
        if(TextUtils.isEmpty( subjectStr ) ) {
            MessageUtils.hideView( subject );
        } else {
            MessageUtils.showView( subject );
            subject.setText( subjectStr );
        }
        if(file != null) {
            MessageUtils.showView( attachmenticon );
            MessageUtils.showView( attachmentname );
            MessageUtils.showView( attachmentParent );
            String src = file.getSrc();
            attachmentname.setText(src);
            attachmentname.setOnClickListener( new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    // TODO Auto-generated method stub
                    if (file.isVCard()) {
                        VCardUtils.importVCard(mContext, file);
                    } else if (file.isVCalendar()) {
                        try {
                            Intent intent = new Intent(Intent.ACTION_VIEW);
                            intent.setDataAndType(file.getUri(), file.getContentType().toLowerCase());
            ...
            MessageUtils.hideView( attachmenticon );
            MessageUtils.hideView( attachmentname );
            MessageUtils.hideView( attachmentParent );
        }
        mListView.addHeaderView( header );
    }
    ...
    protected void onDestroy() {
        ...
        if (mListAdapter != null && mClearCache) {
            ...
            //begin tangyisen add for fix bug 5641
            mListAdapter.resetAudio();
            //end tangyisen add for fix bug 5641
        }
        super.onDestroy();
    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (mDirMode) {
            ...
            if (mIsSmsEnabled) {
                ...
                } else if (isOutgoingMessage() && mShowResendsend fail) {
            ...
            if (((mMsgBox == Mms.MESSAGE_BOX_OUTBOX) || (mMsgBox == Mms.MESSAGE_BOX_SENT))
                && (mDeliveryReport == 128 || mReadReport == 128show only has been requested)) {
    ...
    private void markMmsRead() {
        new Thread(new Runnable() {
            public void run() {
                final ContentValues values = new ContentValues(3);//2
                values.put("read", 1);
                values.put("seen", 1);
                values.put("failed_unread", 0);//lichao add for failed_unread
    ...
    private void initToolBar() {
        ...
        myToolbar.setTitle(R.string.mms);
        //myToolbar.inflateMenu(R.menu.zzz_mms_player_toolbar_menu);
    ...
    public boolean onMenuItemClick(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case R.id.play_as_slideshow_menu:
                Intent intent = new Intent(this, SlideshowActivity.class);
                intent.setData(mMessageUri);
                this.startActivity(intent);
                return true;

...
public class AttachmentEditor extends LinearLayout implements IAttachmentEditorCallback {
    ...
    private SlideViewInterface createMediaView(
        ...
        //tangyisen add close btn
        mClose = (ImageView) view.findViewById(remove_button_id);
        if(null != mClose) {
            mClose.setOnClickListener(new MessageOnClick(removeMessage));
        }
        if(view_button_id != -1) {
            mPlay = (ImageView) view.findViewById( view_button_id );
            if(mPlay != null) {
                mPlay.setOnClickListener(new MessageOnClick(viewMessage));
            }
        }
        /// M: @{
        /// M: show Mms Size
    ...
    /// @}
       ...
       /// M: @{
        ...
        ImageView removeButton = (ImageView) view.findViewById(R.id.remove_slideshow_button);
        removeButton.setOnClickListener(new MessageOnClick(MSG_REMOVE_ATTACHMENT));

        //begin tangyisen
        final ImageView viewBtn = (ImageView) view.findViewById(
                R.id.view_slide_button);
        viewBtn.setOnClickListener(new MessageOnClick(MSG_PLAY_SLIDESHOW));
        //end tangyisen
        /// M: disable when non-default sms
        /*boolean smsEnable = MmsConfig.isSmsEnabled(mContext);

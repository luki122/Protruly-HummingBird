...
public class UriImage {
    ...
    public PduPart getResizedImageAsPart(int widthLimit, int heightLimit, int byteLimit) {
        ...
        if (data == null) {
            if (LOCAL_LOGV) {
                Log.v(LogTag.APP, TAG+" Resize image failed.");
            }
            return null;
        }
    ...
    public static byte[] getResizedImageData(int width, int height,
        ...
        if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
            Log.v(LogTag.APP, TAG+" getResizedImageData: wlimit=" + widthLimit +
        ...
        /// @}
                    ...
                    if (needReduce || isMustResize) {
                        ...
                        if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
                            Log.v(LogTag.APP, TAG+" getResizedImageData: retry scaling using " +
                        ...
                        if (b == null) {
                            if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
                                Log.v(LogTag.APP, TAG+" Bitmap.createScaledBitmap returned NULL!");
                            }
                            return null;
                        }
                    }
                    ...
                    while (jpgFileSize > byteLimit && count < MessageUtils.MAX_COMPRESS_TIMES) {
                        ...
                        /// @}
                            ...
                            if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
                                Log.v(LogTag.APP, TAG+" getResizedImageData: compress(2) w/ quality=" + quality);
                            }
                            if (os != null) {
                                try {
                                    os.close();
                ...
                if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
                    Log.v(LogTag.APP, TAG+" attempt=" + attempts
            ...
            if (Log.isLoggable(LogTag.APP, Log.VERBOSE) && resultTooBig) {
                Log.v(LogTag.APP, TAG+" getResizedImageData returning NULL because the result is too big: "

...
public class SlideshowPresenter extends Presenter {
    ...
    public SlideshowPresenter(Context context, ViewInterface view, Model model) {
        ...
        //begin tangyisen
        if(mModel instanceof SlideshowModel) {
            ArrayList<FileAttachmentModel> files = ((SlideshowModel)mModel).getAttachFiles();
            for(int i = 0; i < files.size(); i++) {
                files.get( i ).registerModelChangedObserver( this );
            }
        }
        //end tangyisen
    }
    ...
    //begin tangyisen add
    protected void presentFileAttachment(SlideViewInterface view, FileAttachmentModel model,
            boolean dataChanged) {
        if (dataChanged) {
            view.setText(model.getSrc(), model.getSrc());
        }
        view.setTextVisibility(true);
    }

    protected void presentSubject(SlideViewInterface view, String subject, boolean dataChanged) {
        if (dataChanged) {
            view.setText(subject, subject);
        }
        view.setTextVisibility(true);
    }
    //end tangyisen
    ...
    public void onModelChanged(final Model model, final boolean dataChanged) {
        ...
        if (model instanceof SlideshowModel) {
            ...
            // TODO:
        } else if(model instanceof FileAttachmentModel) {//tangyisen add
            mHandler.post(new Runnable() {
                public void run() {
                    presentFileAttachment( view, (FileAttachmentModel)model, dataChanged );
                }
            });
        }
    }

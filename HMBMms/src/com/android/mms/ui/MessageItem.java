...
public class MessageItem implements IMessageItemCallback {
    ...
    String mTimestamp;
    //begin tangyisen add for NOTIFICTION_IND
    ...
    MessageItem(Context context, ColumnsMap columnsMap, Cursor cursor) throws MmsException {
        ...
        //begin tangyisen
        mSmsGroupId = cursor.getLong(columnsMap.mColumnSmsGroupId);
        mSmsNotiCount = cursor.getInt(columnsMap.mColumnSmsNotiCount);
        mSmsFailsCount = cursor.getInt(columnsMap.mColumnSmsFailsCount);
        mSmsMmsReject = cursor.getInt(columnsMap.mColumnSmsMmsReject);
        mSmsMmsRejectTag = cursor.getString(columnsMap.mColumnSmsMmsRejectTag);
        //end tangyisen
    ...
    MessageItem(Context context, int boxId, int messageType, int subId, int errorType, int locked,
        ...
        if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType) {
            ...
            if (mMessageType == PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF) {
                ...
                /*long timestampSent = ((SendReq) msg).getDateSent() * 1000L;
                if(timestampSent != 0 && timestampSent < timestamp) {
        ...
        if (!isOutgoingMessage()) {
            if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType) {
                ...
                mExpireOnTimestamp = context.getString(getTimestampStrId(), MessageUtils
                        .formatTimeStampString(context, timestamp));
            } /*else {*///tangyisen delete
                //tangyisen
                //mTimestamp = MessageUtils.getShortTimeString(context, timestamp);
                mTimestamp = MessageUtils.formatTimeStampStringAndTimeForItem(context, timestamp);
            //}
        } else {//tangyisen add
            mTimestamp = MessageUtils.formatTimeStampStringAndTimeForItem(context, timestamp);
        }
    ...
    MessageItem(Context context, String type, Cursor cursor, ColumnsMap columnsMap,
        ...
        if ("sms".equals(type)) {
            ...
            //begin tangyisen
            long dateShow = cursor.getLong(columnsMap.mColumnSmsDateSent);
            if(dateShow > date || dateShow == 0) {
                dateShow = date;
            }
            //end tangyisen
            if (!isOutgoingMessage()) {
                    ...
                    if (isReceivedMessage()) {
                        if (mSubMsg) {
                            mTimestamp = String.format(context.getString(R.string.received_on),
                                    MessageUtils.formatTimeStampString(context, dateShow));//tangyisen change date to dateSent
                        } else {
                            //tangyisen
                            //mTimestamp = MessageUtils.getShortTimeString(context, date);
                            mTimestamp = MessageUtils.formatTimeStampStringAndTimeForItem(context, dateShow);
                        }
...
//                                MessageUtils.formatTimeStampString(context, date));
            ...
            //begin tangyisen 20170523 add for reject
            mSmsMmsReject = cursor.getInt(columnsMap.mColumnSmsMmsReject);
            mSmsMmsRejectTag = cursor.getString(columnsMap.mColumnSmsMmsRejectTag);
            //end tangyisen
        ...
        public void onItemLoaded(Object result, Throwable exception) {
            ...
            if (!isOutgoingMessage()) {
                if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType) {
                    ...
                    mExpireOnTimestamp = mContext.getString(getTimestampStrId(), MessageUtils
                            .formatTimeStampString(mContext, timestamp));
                } else {
                   //tangyisen delete
    ...
    public void cancelPduLoading() {
        ...
        if (mItemLoadedFuture != null && !mItemLoadedFuture.isDone()) {
            if (Log.isLoggable(LogTag.APP, Log.DEBUG)) {
                Log.d(LogTag.APP, TAG+" cancelPduLoading for: " + this);
            }
    ...
    public boolean isInboxMessage() {
        return mBoxId == Mms.MESSAGE_BOX_INBOX;//Mms.MESSAGE_BOX_INBOX and Sms. is the same value
    }

...
public class MmsPlayerActivityAdapter extends BaseAdapter {
    ...
    public View getView(int arg0, View arg1, ViewGroup arg2) {
        ...
        //begin tangyisen   show vcard,subject or vcalendar
        String fileName = item.getFileName();
        String subject = item.getSubject();
        final FileAttachmentModel model = item.getFileAttachmentModel();
        //end tangyisen
        ...
        if (audioName != null) {
            ...
            mAudioIcon.setImageResource( R.drawable.ic_mms_music );//tangyisen add
        ...
        } else if (fileName != null) {//tangyisen
            mAudioName.setText(fileName);
            mAudioName.setTextSize(18);
            mAudioIcon.setVisibility(View.VISIBLE);
            mAudioIcon.setImageResource( R.drawable.ic_dialog_attach );
            mAudio.setVisibility(View.VISIBLE);
            mAudioName.setVisibility(View.VISIBLE);
            mAudioName.setClickable(true);
            mAudioName.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    // TODO Auto-generated method stub
                    if (model.isVCard()) {
                        VCardUtils.importVCard(mContext, model);
                    } else if (model.isVCalendar()) {
                        try {
                            Intent intent = new Intent(Intent.ACTION_VIEW);
                            intent.setDataAndType(model.getUri(), model.getContentType().toLowerCase());
        ...
        } else if (subject != null) {//tangyisen add
    ...
    class mediaClick implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            mType = MessageUtils.getContentType(mType, mTitle);//tangyisen add here
            if(mType.indexOf( "audio" ) != -1) {
                if(mPlayingView == null && v != null) {
                    audioMediaPlay(v);
                    viewAudioAttachment(mUri);
                } else if(v == mPlayingView && mIsPlaying) {
                    audioMediaPlay(v);
                } else {
                    cleanupMediaPlayer();
                    audioMediaPlay(v);
    ...
    //begin tangyisen fix bug 5641
    private MediaPlayer mMediaPlayer;
    private boolean mIsPlaying;
    private boolean mIsPausing;
    private View mPlayingView;
    private void audioMediaPlay(View view) {
    ...
    private void viewAudioAttachment(Uri uri) {
        if(uri == null) {
            return;
        }

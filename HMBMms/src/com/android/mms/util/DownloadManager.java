...
public class DownloadManager implements IDownloadManagerCallback{
    private static final String TAG = "Mms/DownloadManager";
    ...
    public static final int STATE_SKIP_RETRYING = 0x82;// M:[ALPS01772380] 0x89;//tangyisen why the same with STATE_TRANSIENT_FAILURE?
    ...
    private final OnSharedPreferenceChangeListener mPreferencesChangeListener =
        ...
        public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
            if (MyMessagingPreferenceActivity.AUTO_RETRIEVAL.equals(key)
                    || MyMessagingPreferenceActivity.RETRIEVAL_DURING_ROAMING.equals(key)) {
                if (LOCAL_LOGV) {
    ...
    static boolean getAutoDownloadState(SharedPreferences prefs, boolean roaming) {
        boolean autoDownload = prefs.getBoolean(
                MyMessagingPreferenceActivity.AUTO_RETRIEVAL, true);
        ...
        if (autoDownload) {
            boolean alwaysAuto = prefs.getBoolean(
                    MyMessagingPreferenceActivity.RETRIEVAL_DURING_ROAMING, false);
    ...
    private boolean getAutoDownloadState(SharedPreferences prefs, boolean roaming, int subId) {
        ...
        //tangyisen begin not need subId
        /*autoDownload = prefs.getBoolean(Long.toString(subId) + "_" +
        MmsPreferenceActivity.AUTO_RETRIEVAL, true);*/
        autoDownload = prefs.getBoolean(MyMessagingPreferenceActivity.AUTO_RETRIEVAL, true);
        //end tangyisen
        if(DEBUG) Log.d(TAG, "getAutoDownloadState, roaming=" + roaming);
        /// @}
        if(DEBUG) Log.d(TAG, "subId " + subId + " auto download without roaming -> "
                + autoDownload);
        if (autoDownload) {
            ...
            boolean alwaysAuto = prefs.getBoolean(MyMessagingPreferenceActivity.RETRIEVAL_DURING_ROAMING, false);
            //end tangyisen
            if(DEBUG) Log.d(TAG, "subId " + subId + " auto download during roaming -> "
                    + alwaysAuto);
            if (!roaming || alwaysAuto) {
                return true;
            }

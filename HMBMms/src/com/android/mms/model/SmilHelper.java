...
public class SmilHelper {
    ...
    private static SMILDocument createSmilDocument(SlideshowModel slideshow) {
        ...
        //begin tangyisen
        /*ArrayList<FileAttachmentModel> fileModel = slideshow.getAttachFiles();
        for(int f = 0; f < fileModel.size(); f ++) {
            FileAttachmentModel file = fileModel.get( f );
            SMILMediaElement sme = SmilHelper.createMediaElement(SmilHelper.ELEMENT_TAG_TEXT, document, file.getSrc());//because just show text?
            SMILParElement par = addPar(document);
            txtRegionPresentInLayout = setRegion((SMILRegionMediaElement) sme,
                                                 smilRegions,
                                                 layoutElement,
                                                 LayoutModel.TEXT_REGION_ID,
                                                 txtRegionPresentInLayout);
            int begin = 0;//media.getBegin();tmp set
            if (begin != 0) {
                sme.setAttribute("begin", String.valueOf(begin / 1000));
            }
            int duration = 5000;//media.getDuration(); tmp set
            if (duration != 0) {
                sme.setDur((float) duration / 1000);
              /// M: add log for bug ALPS01276998
                MmsLog.d(TAG, "createSmilDocument media duration: " + duration
            ...
            addFileElementEventListeners((EventTarget) sme, file);
        }*/
        //end tangyisen
    ...
    //begin tangyisen add
    static void addFileElementEventListeners(
            EventTarget target, FileAttachmentModel file) {
        // To play the media with SmilPlayer, we should add them
        // as EventListener into an EventTarget.
        target.addEventListener(SMIL_MEDIA_START_EVENT, file, false);
        target.addEventListener(SMIL_MEDIA_END_EVENT, file, false);
        target.addEventListener(SMIL_MEDIA_PAUSE_EVENT, file, false);
        target.addEventListener(SMIL_MEDIA_SEEK_EVENT, file, false);
    }
    //end tangyisen add
}

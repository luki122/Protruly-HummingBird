...
public class RetryScheduler implements Observer {
    ...
    public void update(Observable observable) {
        try {
            ...
            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                Log.v(LogTag.TRANSACTION, TAG+" [RetryScheduler] update " + observable);
            }
            ...
            try {
                if ((cursor.getCount() == 1) && cursor.moveToFirst()) {
                    ...
                    if ((retryIndex < scheme.getRetryLimit()) && retry) {
                        ...
                        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                            Log.v(LogTag.TRANSACTION, TAG+" scheduleRetry: retry for " + uri + " is scheduled at "
                        ...
                        if (isRetryDownloading) {
							...
							stateValues.put(Mms.FAILED_UNREAD, 1);//lichao modify for failed_unread
    ...
    private int getRetrieveStatus(long msgID) {
        ...
        if (retrieveStatus != 0) {
            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                Log.v(LogTag.TRANSACTION, TAG+" Retrieve status is: " + retrieveStatus);
            }
        }
        ...
        if (cursor != null) {
            try {
                if (cursor.moveToFirst()) {
                    ...
                    if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                        Log.v(LogTag.TRANSACTION, TAG+" Next retry is scheduled at"
                                + (retryAt - System.currentTimeMillis()) + "ms from now");
                    }
                    /// M:

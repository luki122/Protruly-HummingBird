...
public class MmsSystemEventReceiver extends BroadcastReceiver {
    ...
    private static void wakeUpService(Context context) {
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            Log.v(LogTag.TRANSACTION, TAG+" wakeUpService: start transaction service ...");
        }
    ...
    public void onReceive(Context context, Intent intent) {
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            Log.v(LogTag.TRANSACTION, TAG+" Intent received: " + intent);
        }
        /// M:
        ...
        /// @}
            ...
            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                Log.v(LogTag.TRANSACTION, TAG+" ANY_DATA_STATE event received: " + state);
            }
    ...
    public static void registerForConnectionStateChanges(Context context) {
        ...
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            Log.v(LogTag.TRANSACTION, TAG+" registerForConnectionStateChanges");
        }
    ...
    public static void unRegisterForConnectionStateChanges(Context context) {
        ...
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            Log.v(LogTag.TRANSACTION, TAG+" unRegisterForConnectionStateChanges");
        }
        if (sMmsSystemEventReceiver != null) {
            try {
            ...
            try {
                ...
                while (cursor.moveToNext()) {
                    ...
                    if (type == PduHeaders.MESSAGE_TYPE_SEND_REQ) {
						...
						readValues.put(Mms.FAILED_UNREAD, 1);//lichao modify for failed_unread
            ...
            try {
                ...
                while (cursor.moveToNext()) {
                    ...
                    if (DownloadManager.STATE_DOWNLOADING == (status & ~DownloadManager.DEFERRED_MASK)) {
                        values = new ContentValues(2);//1
                        values.put(Mms.STATUS, DownloadManager.STATE_PERMANENT_FAILURE);
                        //values.put(Mms.READ, 0);
						values.put(Mms.FAILED_UNREAD, 1);//lichao modify for failed_unread

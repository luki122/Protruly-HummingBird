...
public class TransactionService extends Service implements Observer, ITransactionServiceCallback {
    ...
    public void onNewIntent(Intent intent, int serviceId) {
        ...
        if (mConnMgr == null //|| !SubStatusResolver.isMobileDataEnabledOnAnySub(getApplicationContext())
            ...
            stopSelf(serviceId);
            Log.e(TAG, "onNewIntent(), not support SMS, is not the default sms app");
            return;
        }
        ...
        if (ACTION_ONALARM.equals(action) || ACTION_ENABLE_AUTO_RETRIEVE.equals(action) ||
            ...
            if (cursor != null) {
                try {
                    ...
                    if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                        Log.v(LogTag.TRANSACTION, TAG+" onNewIntent: cursor.count=" + count + " action=" + action);
                    }

                    if (count == 0) {
                        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                            Log.v(LogTag.TRANSACTION, TAG+" onNewIntent: no pending messages. Stopping service.");
                        }
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                            Log.v(LogTag.TRANSACTION, TAG+" onNewIntent: msgType=" + msgType + " transactionType=" +
                                    transactionType);
                        }
                        if (noNetwork) {
                        ...
                        switch (transactionType) {
                            ...
                            case Transaction.RETRIEVE_TRANSACTION:
                                ...
                                if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                                    Log.v(LogTag.TRANSACTION, TAG+" onNewIntent: failureType=" + failureType +
                                ...
                                if (!autoDownload) {
                                    ...
                                    // transaction.
                                    if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                                        Log.v(LogTag.TRANSACTION, TAG+" onNewIntent: skipping - autodownload off");
                                    }
                                ...
                                if (!(failureType == MmsSms.NO_ERROR ||
                                    ...
                                    if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                                        Log.v(LogTag.TRANSACTION, TAG+" onNewIntent: skipping - permanent error");
                                    }
                                    break;
                                }
                                if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                                    Log.v(LogTag.TRANSACTION, TAG+" onNewIntent: falling through and processing");
                                }
                            ...
                            default:
                                ...
                                if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                                    Log.v(LogTag.TRANSACTION, TAG+" onNewIntent: launchTransaction uri=" + uri);
                                }
            ...
            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                Log.v(LogTag.TRANSACTION, TAG+" onNewIntent: launch transaction...");
            }
    ...
    private void stopSelfIfIdle(int startId) {
        synchronized (mProcessing) {
            if (mProcessing.isEmpty() && mPending.isEmpty()) {
                if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                    Log.v(LogTag.TRANSACTION, TAG+" stopSelfIfIdle: STOP!");
                }
    ...
    private void launchTransaction(int serviceId, TransactionBundle txnBundle, boolean noNetwork,
        ...
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            Log.v(LogTag.TRANSACTION, TAG+" launchTransaction: sending message " + msg);
        }
        mServiceHandler.sendMessage(msg);
    }
    ...
    private void onNetworkUnavailable(int serviceId, int transactionType) {
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            Log.v(LogTag.TRANSACTION, TAG+" onNetworkUnavailable: sid=" + serviceId + ", type=" + transactionType);
        }
    ...
    public void update(Observable observable) {
        ...
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            Log.v(LogTag.TRANSACTION, TAG+" update transaction " + serviceId);
        }
        ...
        try {
            synchronized (mProcessing) {
                ...
                if (mPending.size() > 0) {
                    if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                        Log.v(LogTag.TRANSACTION, TAG+" update: handle next pending transaction...");
                    }
            ...
            switch (result) {
                case TransactionState.SUCCESS:
                    ...
                    if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                        Log.v(LogTag.TRANSACTION, TAG+" Transaction complete: " + serviceId);
                    }

                    //lichao refactor in 2017-07-26
                    Uri contentUri = state.getContentUri();
                    //Mms/TransactionService Transaction complete, contentUri = content://mms/inbox/22
                    Log.v(LogTag.TRANSACTION, TAG+" Transaction complete, contentUri = " + contentUri);
                    intent.putExtra(STATE_URI, contentUri);
                    ...
                    switch (transaction.getType()) {
                        ...
                        case Transaction.RETRIEVE_TRANSACTION:
                            ...
                            //收到被拦截短信后不应该再执行[重新查询是否有未读信息并弹出通知]的方法了
                            String address = AddressUtils.getFrom(this, contentUri);
                            Log.v(LogTag.TRANSACTION, TAG + " Transaction complete, address = " + address);
                            if (true == RejectUtil.isRejectMms(this, address)) {
                                Log.w(LogTag.TRANSACTION, TAG + " Transaction complete, isRejectMms, " +
                                        "don't call blockingUpdateNewMessageIndicator, break");
                                break;
                            }
                            ...
                            if (NotificationPreferenceActivity.isPopupNotificationEnable()) {
                                new MmsDialogNotifyUtils(this).notifyNewSmsDialog(contentUri);
                            }
                ...
                case TransactionState.FAILED:
                    ...
                    if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                        Log.v(LogTag.TRANSACTION, TAG+" Transaction failed: " + serviceId);
                    }
                    break;
                default:
                	MmsLog.d(MmsApp.TXN_TAG, "update: result=default");
                    if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                        Log.v(LogTag.TRANSACTION, TAG+" Transaction state unknown: " +
                                serviceId + " " + result);
                    }
                    break;
            }
            ...
            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                Log.v(LogTag.TRANSACTION, TAG+" update: broadcast transaction result " + result);
            }
    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                Log.v(LogTag.TRANSACTION, TAG+" Handling incoming message: " + msg + " = " + decodeMessage(msg));
            }
            ...
            switch (msg.what) {
                ...
                case EVENT_TRANSACTION_REQUEST:
                    ...
                    try {
                        ...
                        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                            Log.v(LogTag.TRANSACTION, TAG+" handle EVENT_TRANSACTION_REQUEST: transactionType=" +
                        ...
                        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                            Log.v(LogTag.TRANSACTION, TAG+" Started processing of incoming message: " + msg);
                        }
                    } catch (Exception ex) {
                        ...
                        if (transaction == null) {
                            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                                Log.v(LogTag.TRANSACTION, TAG+" Transaction was null. Stopping self: " + serviceId);
                            }
                            stopSelf(serviceId);
                        }
                    }
        ...
        public void processPendingTransaction(Transaction transaction) {
            ...
            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                Log.v(LogTag.TRANSACTION, TAG+" processPendingTxn: transaction=" + transaction);
            }
            ...
            if (transaction != null) {
                ...
                try {
                    ...
                    if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                        Log.v(LogTag.TRANSACTION, TAG+" processPendingTxn: process " + serviceId);
                    }

                    if (processTransaction(transaction)) {
                        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                            Log.v(LogTag.TRANSACTION, TAG+" Started deferred processing of transaction  "
                                    + transaction);
                        }
                    } else {
                        transaction = null;
        ...
        private boolean processTransaction(Transaction transaction)
            ...
            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                Log.v(LogTag.TRANSACTION, TAG+" processTransaction: starting transaction " + transaction);
            }
    ...
    private Transaction getLastTransaction() {
        ...
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            Log.v(LogTag.TRANSACTION, TAG+" getLastTransaction, transaction = " + lastTransaction);
        }

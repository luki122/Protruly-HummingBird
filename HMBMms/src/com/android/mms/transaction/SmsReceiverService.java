...
import com.zzz.provider.Telephony.Sms.Intents;
import com.zzz.provider.Telephony.Sms.Outbox;
...
public class SmsReceiverService extends Service implements ISmsReceiverServiceCallback {
    private static final String TAG = "Mms/SmsReceiverService";
...
//            Log.v(LogTag.TRANSACTION, TAG+" onCreate");
//        }
    ...
    public void onDestroy() {
        // Temporarily removed for this duplicate message track down.
//        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE) || LogTag.DEBUG_SEND) {
//            Log.v(LogTag.TRANSACTION, TAG+" onDestroy");
//        }
    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                Log.v(LogTag.TRANSACTION, TAG+" handleMessage serviceId: " + serviceId + " intent: " + intent);
            }
            // KK migration, for default MMS function.
            boolean isSmsEnabled = MmsConfig.isSmsEnabled(getApplicationContext());
            MmsLog.d(MmsApp.TXN_TAG, "Sms handleMessage, isSmsEnabled = " + isSmsEnabled);
            if (intent != null && isSmsEnabled) {
                String action = intent.getAction();

                int error = intent.getIntExtra("errorCode", 0);

                if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                    Log.v(LogTag.TRANSACTION, TAG+" handleMessage action: " + action + " error: " + error);
                }
            ...
            try {
                if (c.moveToFirst()) {
                    ...
                    if (LogTag.DEBUG_SEND ||
                        ...
                        Log.v(LogTag.TRANSACTION, TAG+" sendFirstQueuedMessage " + msgUri +
                                ", address: " + address +
                                ", threadId: " + threadId +
                                /// M: print more info
                                ", body: " + msgText);
    ...
    private void handleSmsSent(Intent intent, int error) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            if (LogTag.DEBUG_SEND || Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                Log.v(LogTag.TRANSACTION, TAG+" handleSmsSent move message to sent folder uri: " + uri);
            }
            ...
            /// @}
            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
                Log.v(LogTag.TRANSACTION, TAG+" handleSmsSent: no service, queuing message w/ uri: " + uri);
            }
    ...
    private void messageFailedToSend(Uri uri, int error) {
        ...
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE) || LogTag.DEBUG_SEND) {
            Log.v(LogTag.TRANSACTION, TAG+" messageFailedToSend msg failed uri: " + uri + " error: " + error);
        }
        //这里会调用 values.put(Sms.READ, Integer.valueOf(0));
    ...
    private void handleSmsReceived(Intent intent, int error) {
        ...
        MmsLog.d(MmsApp.TXN_TAG, "SmsReceiverService, handleSmsReceived()");
        /// @}
        ...
        //lichao refactor in 2017-07-26 begin
        ...
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE) || LogTag.DEBUG_SEND) {
            Log.v(LogTag.TRANSACTION, TAG + " handleSmsReceived" + (isReplace ? "(replace)" : "") +
        ...
        MmsLog.d(MmsApp.TXN_TAG, "handleSmsReceived" + (isReplace ? "(replace)" : "")
                + " messageUri: " + messageUri
                + ", originatingAddress: " + originatingAddress
                + ", displayOriginatingAddress: " + displayOriginatingAddress
                + ", body: " + msgBody);
        /// @}
        //lichao refactor in 2017-07-26 end

        //lichao add for bug 5697 in 2017-07-26 begin
        //收到被拦截短信后不再执行[重新查询是否有未读信息并弹出通知]的方法了
        RejectSmsResult result = RejectUtil.isRejectSms(this, originatingAddress, msgBody);
        if(null != result && result.getReject() > 0){
            Log.w(TAG, "handleSmsReceived, isRejectSms, don't call blockingUpdateNewMessageIndicator, return");
            return;
        }
        //lichao add for bug 5697 in 2017-07-26 end
        ...
        //class 0短信会弹框显示，并且对于联通随机登录码除了即显短信还有有一条普通短信，所以再次弹通知显得有点多余。
        else {
            //SmsMessage sms = msgs[0];
            SmsMessage msg = SmsMessage.createFromPdu(sms.getPdu(), format);
            if (msg == null) {
                MmsLog.e(MmsApp.TXN_TAG, "handleSmsReceived, createFromPdu return null.");
                return;
            }
            if (TextUtils.isEmpty(msgBody)) {
                MmsLog.e(MmsApp.TXN_TAG, "handleSmsReceived, msgBody isEmpty, return");
                return;
            }
            int subId = msgs[0].getSubId();
            Log.d(MmsApp.TXN_TAG, "handleSmsReceived, >>>notifyClassZeroMessage");
            MessagingNotification.notifyClassZeroMessage(this, originatingAddress, msgBody, subId);
        }
        /// @}
    }
    ...
    private int moveOutboxMessagesToQueuedBox() {
        ...
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE) || LogTag.DEBUG_SEND) {
            Log.v(LogTag.TRANSACTION, TAG+" moveOutboxMessagesToQueuedBox messageCount: " + messageCount);
        }
        return messageCount;
    }
    ...
    private int moveOutboxMessagesToFailedBox() {
		...
		values.put(Sms.FAILED_UNREAD, 1);//lichao modify for failed_unread

        int messageCount = SqliteWrapper.update(
                getApplicationContext(), getContentResolver(), Outbox.CONTENT_URI,
                values, "type = " + Sms.MESSAGE_TYPE_OUTBOX, null);
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE) || LogTag.DEBUG_SEND) {
            Log.v(LogTag.TRANSACTION, TAG+" moveOutboxMessagesToFailedBox messageCount: " + messageCount);
        }
        return messageCount;
    }
    ...
    private Uri replaceMessage(Context context, SmsMessage[] msgs, int error) {
        ...
        if (pduCount == 1) {
            ...
            smsBody = replaceFormFeeds(sms.getDisplayMessageBody());
            ...
            smsBody = replaceFormFeeds(body.toString());
            values.put(Inbox.BODY, smsBody);
        }
        ...
        if (!TextUtils.isEmpty(address)) {
            RejectSmsResult result = RejectUtil.isRejectSms(context, address, smsBody);
            if(null != result && result.getReject() > 0){
                RejectUtil.sendRejectNotification(context, true, address, smsBody);
                //HashSet重复put会覆盖之前put的
...
//    private static int count = 0;
    ...
    private Uri storeMessage(Context context, SmsMessage[] msgs, int error) {
        /// M:
        MmsLog.v(TAG, "Sms storeMessage begin...");
        SmsMessage sms = msgs[0];
        ...
        values.put(Sms.ERROR_CODE, error);
        int pduCount = msgs.length;
        MmsLog.v(TAG, "Sms storeMessage, SmsMessage[] length = "+pduCount);

        String smsBody = "";
        if (!mOpSmsReceiverServiceExt.storeMessage(msgs, sms, values)) {
            smsBody = extractSmsBody(msgs, sms, values);
        }
...
//        }
        ...
        //begin tangyisen validate reject before insert sms 20170523
        //now handler in provider
        /*if (!TextUtils.isEmpty(address)) {
            String number = address.replace("-", "").replace(" ", "");
            values.put(Sms.REJECT, RejectUtil.isRejectSms(context, number, values.getAsString( Sms.BODY )));
        }*/
        //end tangyisen
...
//        }
        ...
        Log.v(TAG, "Sms storeMessage, after SqliteWrapper.insert, insertedUri = "+insertedUri);
    ...
    private void registerForServiceStateChanges(int subId) {
        if (sNoServiceSimSet.isEmpty()) {
            ...
            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE) || LogTag.DEBUG_SEND) {
                Log.v(LogTag.TRANSACTION, TAG+" registerForServiceStateChanges");
            }
    ...
    private void unRegisterForServiceStateChanges() {
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE) || LogTag.DEBUG_SEND) {
            Log.v(LogTag.TRANSACTION, TAG+" unRegisterForServiceStateChanges");
        }
        try {
            if (sNoServiceSimSet.isEmpty()) {
                Context context = getApplicationContext();
    ...
    private int moveQueuedMessagesToFailedBox() {
        ...
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE) || LogTag.DEBUG_SEND) {
            Log.v(LogTag.TRANSACTION, TAG+" moveQueuedMessagesToFailedBox messageCount: " + messageCount);
        }
        return messageCount;
    }
    ...
    public String extractSmsBody(SmsMessage[] msgs, SmsMessage sms, ContentValues values) {
        String smsBody = "";
        int pduCount = msgs.length;

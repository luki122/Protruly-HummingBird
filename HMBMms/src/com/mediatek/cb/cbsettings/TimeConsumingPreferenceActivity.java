...
public class TimeConsumingPreferenceActivity extends PreferenceActivity
    ...
    public final static int DIALOG_DELAY_TIME = 1000;
    ...
    public void onResume() {
        ...
        super.onResume();
        mHandler = new Handler();//lichao add in 2017-05-03
        mIsForeground = true;
        if (mReadBusyList.size() == 1) {
            showDialog(BUSY_READING_DIALOG);
            MmsLog.d(LOG_TAG, "onResume >>>showDialog(BUSY_READING_DIALOG)");
        }

        if (mSaveBusyList.size() == 1) {
            showDialog(BUSY_SAVING_DIALOG);
            MmsLog.d(LOG_TAG, "onResume >>>showDialog(BUSY_SAVING_DIALOG)");
        }
    }
    ...
    public void onStarted(final Preference preference, boolean reading) {
        if (DBG) {
            dumpState();
        }
        if (DBG) {
        ...
        if (reading) {
            ...
            if ((mReadBusyList.size() == 1) && mIsForeground) {
                showDialog(BUSY_READING_DIALOG);
                MmsLog.d(LOG_TAG, "onStarted >>>showDialog(BUSY_READING_DIALOG)");
                preference.setEnabled(false);//lichao add in 2017-05-03
            }
        } else {
            mSaveBusyList.add(preference.getKey());
            if ((mSaveBusyList.size() == 1) && mIsForeground) {
                showDialog(BUSY_SAVING_DIALOG);
                MmsLog.d(LOG_TAG, "onStarted >>>showDialog(BUSY_SAVING_DIALOG)");
                preference.setEnabled(false);//lichao add in 2017-05-03
            }
        }
    }

    public void onFinished(final Preference preference, boolean reading) {
        if (DBG) {
            dumpState();
        }
        if (DBG) {
        ...
        //lichao moidfy in 2017-05-03 begin
        if (reading) {
            ...
            if (mReadBusyList.isEmpty()) {
                mHandler.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        MmsLog.d(LOG_TAG, "removeDialog(BUSY_READING_DIALOG) now");
            ...
            if (mSaveBusyList.isEmpty()) {
                mHandler.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        MmsLog.d(LOG_TAG, "removeDialog(BUSY_SAVING_DIALOG) now");
                        preference.setEnabled(true);
                    }
                }, DIALOG_DELAY_TIME);
        ...
        //lichao moidfy in 2017-05-03 end
    }

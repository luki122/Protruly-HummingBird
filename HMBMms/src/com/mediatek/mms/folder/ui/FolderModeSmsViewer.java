...
//            }
    ...
    private void markSmsRead(int type) {
        Uri readUri = null;
        final ContentValues values = new ContentValues(3);//1
        values.put("read", 1);
        values.put("seen", 1);
        //当type == 4时候会更新到cellbroadcast,所以在数据库里需要对cellbroadcast增加failed_unread字段
        //Log.d(TAG, "\n\n markSmsRead(), values.put(failed_unread, 0)");

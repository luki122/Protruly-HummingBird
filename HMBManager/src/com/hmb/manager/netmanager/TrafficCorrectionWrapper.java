...
public class TrafficCorrectionWrapper {
	...
	public void setTrafficCorrectionListener(ITrafficCorrectionListener listener) {
		...
		Log.e(TAG, "mTrafficCorrectionListener>>>>" + listener);
	}
	...
	public void clearTrafficInfo(Context context, int simIndex) {
		if (context == null) {
			return;
		}
		mPreferences = context.getSharedPreferences("trafficInfo", Context.MODE_PRIVATE);
		...
		if (simIndex == IDualPhoneInfoFetcher.FIRST_SIM_INDEX) {
			...
			// 常规流量
			editor.putInt("SIM1_COMMON_LEFT_KBYTES", -1);
			editor.putInt("SIM1_COMMON_USED_KBYTES", -1);
			editor.putInt("SIM1_COMMON_TOTAL_KBYTES", -1);
			// 闲时
			editor.putInt("SIM1_FREE_LEFT_KBYTES", -1);
			editor.putInt("SIM1_FREE_USED_KBYTES", -1);
			editor.putInt("SIM1_FREE_TOTAL_KBYTES", -1);
			// 4G
			editor.putInt("SIM1_4G_LEFT_KBYTES", -1);
			editor.putInt("SIM1_4G_USED_KBYTES", -1);
			editor.putInt("SIM1_4G_TOTAL_KBYTES", -1);

		} else if (simIndex == IDualPhoneInfoFetcher.SECOND_SIM_INDEX) {
			// 常规流量
			editor.putInt("SIM2_COMMON_LEFT_KBYTES", -1);
			editor.putInt("SIM2_COMMON_USED_KBYTES", -1);
			editor.putInt("SIM2_COMMON_TOTAL_KBYTES", -1);
			// 闲时
			editor.putInt("SIM2_FREE_LEFT_KBYTES", -1);
			editor.putInt("SIM2_FREE_USED_KBYTES", -1);
			editor.putInt("SIM2_FREE_TOTAL_KBYTES", -1);
			// 4G
			editor.putInt("SIM2_4G_LEFT_KBYTES", -1);
			editor.putInt("SIM2_4G_USED_KBYTES", -1);
			editor.putInt("SIM2_4G_TOTAL_KBYTES", -1);

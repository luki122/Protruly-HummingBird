...
import com.protruly.music.local.LocalPlayer;
import com.protruly.music.online.HBXiamiPlayer;
import com.protruly.music.ui.HBPlayerActivity;
import com.protruly.music.ui.lock.HBLockActivity;
...
public class MediaPlaybackService extends Service {
    ...
    //protruly hujianwei 20170830  modify add new values start
    public static final String PLAYSTATE_CHANGED = "com.protruly.music.playstatechanged";
    public static final String META_CHANGED = "com.protruly.music.metachanged";
    public static final String SHUFFLE_CHANGED = "org.codehb.music.shuffle";
    public static final String REPEAT_CHANGED = "org.codehb.music.repeat";
    public static final String QUEUE_CHANGED = "com.protruly.music.queuechanged";
    public static final String SERVICECMD = "com.protruly.music.musicservicecommand";
    ...
    private static final String PLAYSTATUS_REQUEST = "org.codehb.android.music.playstatusrequest";
    private static final String PLAYSTATUS_RESPONSE = "org.codehb.music.playstatusresponse";
    private static final String PLAYERSETTINGS_REQUEST = "org.codehb.music.playersettingsrequest";
    private static final String PLAYERSETTINGS_RESPONSE = "org.codehb.music.playersettingsresponse";
    private static final String SET_ADDRESSED_PLAYER = "org.codehb.music.setaddressedplayer";
    private static final String EXTRA_SHUFFLE_VAL = "shuffle";
    private static final String EXTRA_REPEAT_VAL = "repeat";

    public static final String HBLRC_CHANGED = "com.android.music.lrcchanged";
    ...
    private static final int PLAYFAILED = 500001;
    ...
    //protruly hujianwei 20170830  modify add new values end
    ...
    //protruly hujianwei 20170829 modify add private LocalPlayer mLocalPlayer start
    private LocalPlayer mLocalPlayer;
    private boolean mSingleSong = false;
    private ArrayList<HBListItem> mMusicList = null;
    private HBXiamiPlayer mHBXiamiPlayer;
    private String mLrcPath = null;

    private int mPlayStatus = 0; // 0为下一曲 1为上一曲 2为点击播放
    private int mNotifyWidth = 0;
    private boolean mSdMount = false;
    private boolean mbHasEar = false;
    //protruly hujianwei 20170829 modify add private LocalPlayer mLocalPlayer end
    ...
    // add by wxue for appwidget 20170918 start
    public static final String PLAYPROGRESS_CHANGED = "com.protruly.music.palyprogresschanged";
    public static final String ALBUM_ART_CHANGED = "com.protruly.music.albumartchanged";
    public static final String LOCK_SCREEN_CHANGED = "com.protruly.music.lockscreenchanged";
    ...
    // add by wxue for appwidget 20170918 end
    ...
    // add by wxue for update widget 20170918 start
    private Runnable mUpdateProgressRunnable = new Runnable() {
        @Override
        public void run() {
            mAppWidgetProvider.notifyChange(MediaPlaybackService.this,PLAYPROGRESS_CHANGED);
            mMediaplayerHandler.postDelayed(mUpdateProgressRunnable,1000);
        }
    };
    // add by wxue for update widget 20170918 end
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case TRACK_WENT_TO_NEXT:
                    ...
                    updateNotification(MediaPlaybackService.this);
                    // add by wxue for getting album art image 20170918 start
                    getAlbumArt();
                    // add by wxue for getting album art image 20170918 end
                    setNextTrack();
                    break;
                case TRACK_ENDED:
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            // add by wxue for lockactivity 20170918 start
            else if(LOCK_SCREEN_CHANGED.equals(action)){
                mIsLocked = intent.getBooleanExtra("islock", true);
            } else if(Intent.ACTION_SCREEN_OFF.equals(action)){
                if(isPlaying() && !mIsLocked){
                    Intent lockIntent = new Intent(MediaPlaybackService.this, HBLockActivity.class);
                    startActivity(lockIntent);
                }
            }
            // add by wxue for lockactivity 20170918 end
        }
    };
    ...
    // add by wxue for getting the album art image 20170918 start
    private void getAlbumArt(){
        LogUtil.d(TAG,"---getAlbumArt()--mPreAudioId = " + mPreAudioId + " getAudioId() = " + getAudioId());
        if(mPreAudioId == getAudioId()){
            return;
        }
        if(mAsyncAlbumArtWorker != null){
            mAsyncAlbumArtWorker.cancel(true);
            mAsyncAlbumArtWorker = null;
        }
        mAlbumArt = null;
    ...
    private class AlbumArtWorker extends AsyncTask<Long, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(Long... albumId) {
            ...
            try {
                ...
                LogUtil.e(TAG, "AlbumArtWorker called with wrong parameters");
                return null;
            }
            LogUtil.d(TAG, "AlbumArtWorker: getArtwork returns " + bm + " for album " + id);
            return bm;
        }
        ...
        protected void onPostExecute(Bitmap bm) {
            LogUtil.d(TAG, "AlbumArtWorker.onPostExecute: " + mIsSupposedToBePlaying);
            mAlbumArt = bm;
            if (mIsSupposedToBePlaying) {
                ...
                notifyChange(ALBUM_ART_CHANGED);
                updateNotification(MediaPlaybackService.this);
            }
            mAppWidgetProvider.notifyChange(MediaPlaybackService.this, ALBUM_ART_CHANGED);
        }
    }
    // add by wxue for getting the album art image 20170918 end
    ...
    public void onCreate() {
        ...
        reloadQueue();
        // add by wxue for getting album art 20170919 start
        getAlbumArt();
        // add by wxue for getting album art 20170919 end
        ...
        commandFilter.addAction(LOCK_SCREEN_CHANGED);
        // add by wxue for lockactivity 2017098 end
    ...
    protected BluetoothProfile.ServiceListener mBluetoothProfileServiceListener = new BluetoothProfile.ServiceListener() {
        ...
        public void onServiceConnected(int profile, BluetoothProfile proxy) {
            ...
            LogUtil.d(TAG, "BluetoothHeadset:" + mBluetoothHeadset);
        }
    ...
    private boolean isBluetoothAvailable() {
        ...
        LogUtil.d(TAG, "isBluetoothAvailable:" + isConnected);
        return isConnected;
    }
    ...
    public void onDestroy() {
        // add by wxue for Cancel album art worker 20170918 start
        if (mAsyncAlbumArtWorker != null) {
            mAsyncAlbumArtWorker.cancel(true);
        }
        mAlbumArt = null;
        // add by wxue for Cancel album art worker 20170918 end
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            LogUtil.i(TAG,"onStartCommand: " + action + " / " + cmd);
            if (CMDNEXT.equals(cmd) || NEXT_ACTION.equals(action)) {
                gotoNext(true);
            } else if (CMDPREVIOUS.equals(cmd) || PREVIOUS_ACTION.equals(action)) {
                if (position() < 2000) {
            ...
            // add by wxue for close notification 20170921 start
            else if (CLOSE_ACTION.equals(action)){
                pause();
                stopForeground(true);
            }
            // add by wxue for close notification 20170921 end
        }
    ...
    private void getNowPlayingEntries() {
        Log.i(TAG,  "getNowPlayingEntries: num of items: " + mPlayListLen);
    ...
    private void playItem(int scope, long playItemUid) {
        ...
        Log.i(TAG,  "playItem uid: " + playItemUid + " scope: " + scope);
        if (playItemUid < 0) {
            ...
            for (int index = 0; index < mPlayListLen; index++) {
                if (mPlayList[index] == playItemUid) {
                    Log.i(TAG, "Now Playing list contains UID at " + index);
                    success = true;
                    break;
                }
            }
    ...
    private boolean openItem (long playItemUid) {
        ...
        if (mCursor != null) {
            ...
            enqueue(list, NOW);
            Log.i(TAG, "Opened UID: " + playItemUid);
            isSuccess = true;
        } else {
                ...
                if (action.equals(SET_ADDRESSED_PLAYER)) {
                    ...
                    if (CMDGET.equals(cmd)) {
                        ...
                        switch (getCommand) {
                            ...
                            break;
                            default:
                               Log.e(TAG, "invalid getCommand"+getCommand);
                            break;
                        }
     ...
     * "com.protruly.music.metachanged"
     * "com.protruly.music.queuechanged",
     * "com.protruly.music.playbackcomplete"
     * "com.protruly.music.playstatechanged"
    ...
    private void notifyChange(String what) {
        ...
        if (what.equals(PLAYSTATE_CHANGED)) {
            ...
            try {
                ...
                Log.e(TAG, "METADATA_KEY_NUM_TRACKS: failed: " + e);
            }
        ...
        // add by wxue for update appwidget playback progress 20170918 start
        if(isPlaying() && !mIsUpdatingProgress){
            mIsUpdatingProgress = true;
            mMediaplayerHandler.post(mUpdateProgressRunnable);
        }else if(!isPlaying()){
            mIsUpdatingProgress = false;
            mMediaplayerHandler.removeCallbacks(mUpdateProgressRunnable);
        }
        // add by wxue for update appwidget playback progress 20170918 end
    }
    ...
    /**
     * 随机播放 ShuffleMode = SHUFFLE_NORMAL(4) RepeatMode = REPEAT_ALL(2)
     * @param list
     * @param position
     */
    ...
    //protruly hujianwei 20170830 modify add start
        ...
        synchronized (this) {
            ...
            if (mbOnline) {
                ...
                if (mCursor != null && mCursor.getCount() != 0 && open(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI + "/" + mCursor.getLong(IDCOLIDX))) {
                    ...
                    setNextTrack();
                    mPlayStatus = 2;
                    LogUtil.d(TAG, "openCurrent mCursor mPlayPos:" + mPlayPos);
                } else {
                    LogUtil.d(TAG, "openCurrent mCursor mQuietMode:" + mQuietMode);
                    if (!mQuietMode) {
    ...
    //protruly hujianwei 20170830 modify add end
    ...
    //protruly hujianwei modify for add api start
    public boolean HBOpen(String path) {
        mbOnline = false;

        if (!open(path)) {
            LogUtil.d(TAG, "1HBOpen  path:" + path);
            long[] mPlayList = getQueue();
            open(mPlayList, getQueuePosition());
            return true;
        } else {
            LogUtil.d(TAG, "2HBOpen  path:" + path);
            mSingleSong = true;
        }
        return true;
    }
    ...
    private long secondaryPosition() {
        if (mbOnline && mHBXiamiPlayer != null) {
            return mHBXiamiPlayer.secondaryPosition();
        }
        return -1;
    }
    ...
    private void notifyLrcPath(String path) {
        if (path != null && !path.isEmpty()) {
            mLrcPath = path;
            notifyChange(HBLRC_CHANGED);
        }
        return;
    }

    public void gotoSomeSong(int position) {
        LogUtil.d(TAG, "gotoSomeSong position:" + position + "  mPlayListLen:" + mPlayListLen);
        synchronized (this) {
            mSingleSong = false;
            if (mPlayListLen <= 0) {
                LogUtil.e(TAG, " No play queue");
                return;
            }
            if (position >= mPlayListLen) {
			...
			/*
			 * play(); if (!mbOnline) { notifyChange(META_CHANGED); }
			 */
        }
    }
    ...
    private void saveBookmarkIfNeeded(boolean isOnline) {
        if (isOnline) {
            return;
        }
        try {
    ...
    //protruly hujianwei modify for add api end
    ...
    public void play() {
        ...
        if (mPlayer.isInitialized()) {
            ...
            // modify by wxue for getting album art 20170919 start
            ...
            updateNotification(this);
            getAlbumArt();
            // modify by wxue for getting album art 20170919 end
    ...
    // add by wxue for setting notification layout 20170921 start
    private void setRemoteViews(Context context, boolean bigContent, RemoteViews views){
        views.setImageViewResource(R.id.iv_no_album_art, R.drawable.album_art_default);
        if (getAudioId() < 0) {
            // streaming
            views.setTextViewText(R.id.txt_no_trackname, getPath());
            views.setTextViewText(R.id.txt_no_artistalbum, null);
        } else {
            String artist = getArtistName();
            views.setTextViewText(R.id.txt_no_trackname, getTrackName());
            ...
            views.setTextViewText(R.id.txt_no_artistalbum,
                    getString(R.string.notification_artist_album, artist, album)
            );
            if (mAlbumArt != null) {
                views.setImageViewBitmap(R.id.iv_no_album_art, mAlbumArt);
            }
        }
        if (isPlaying()) {
            views.setImageViewResource(R.id.iv_no_music_play, R.drawable.hb_appwidget_play);
        } else {
            views.setImageViewResource(R.id.iv_no_music_play, R.drawable.hb_appwidget_pause);
        }
        Intent intent;
        ...
        views.setOnClickPendingIntent(R.id.iv_no_music_play, pIntent);

        intent = new Intent(NEXT_ACTION);
        intent.setClass(context, MediaPlaybackService.class);
        pIntent = PendingIntent.getService(context, 0, intent, 0);
        views.setOnClickPendingIntent(R.id.iv_no_music_next, pIntent);

        intent = new Intent(CLOSE_ACTION);
        intent.setClass(context, MediaPlaybackService.class);
        pIntent = PendingIntent.getService(context, 0, intent, 0);
        views.setOnClickPendingIntent(R.id.iv_no_music_close, pIntent);

        if(bigContent){
            intent = new Intent(PREVIOUS_ACTION);
            intent.setClass(context, MediaPlaybackService.class);
            pIntent = PendingIntent.getService(context, 0, intent, 0);
            views.setOnClickPendingIntent(R.id.iv_no_music_prev, pIntent);
        }

    }
    // add by wxue for setting notification layout 20170921 end

    private void updateNotification(Context context) {
        RemoteViews views = new RemoteViews(getPackageName(), R.layout.hb_notification_content);
        RemoteViews bigViews = new RemoteViews(getPackageName(), R.layout.hb_notification_content_big);
        setRemoteViews(context,false,views);
        setRemoteViews(context,true,bigViews);
        ...
        status.contentIntent = PendingIntent.getActivity(this, 0,
                new Intent(this, HBPlayerActivity.class)
    ...
    public void pause() {
        synchronized(this) {
            ...
            if (isPlaying()) {
                ...
                gotoIdleState();
                mIsSupposedToBePlaying = false;
                // add by wxue for notification 20170921 start
                updateNotification(this);
                // add by wxue for notification 20170921 end
                notifyChange(PLAYSTATE_CHANGED);
                saveBookmarkIfNeeded();
            }
        }
    ...
    private void gotoIdleState() {
        ...
        Message msg = mDelayedStopHandler.obtainMessage();
        mDelayedStopHandler.sendMessageDelayed(msg, IDLE_DELAY);
        // modify by wxue for notification 20170921 start
        //stopForeground(true);
        stopForeground(false);
        // modify by wxue for notification 20170921 end
    }
    ...
    private HBListItem getItemByMusicId(long id) {
        if (mMusicList == null) {
            return null;
        }
        int len = mMusicList.size();
        for (int i = 0; i < len; i++) {
            HBListItem item = mMusicList.get(i);
    ...
    public void addToFavorite(long id) {
        HBListItem item = getItemByMusicId(id);
        if (item == null) {
            LogUtil.e(TAG, "addToFavorite fail by no data");
            return;
        }
    ...
    public void removeFromFavorites(long id) {
        HBListItem item = getItemByMusicId(id);
        if (item == null) {
            LogUtil.e(TAG, "  removeFromFavorites fail by no data");
            return;
        }
    ...
    private String getLrcUri() {
        ...
        LogUtil.d(TAG, " db--pathString:" + pathString);
        ...
        pathString = getHBSongLrc();
        LogUtil.d(TAG, " sd--pathString:" + pathString);
        if (!TextUtils.isEmpty(pathString)) {
            mLrcPath = pathString;
    ...
    private void notifyValueIDs(String what, byte attribute) {
        ...
        switch (attribute) {
            ...
            break;
            default:
                Log.e(TAG,"unsupported attribute"+attribute);
    ...
    private void notifyAttributesText(String what, byte [] attrIds) {
        ...
        for (int i = 0; i < attrIds.length; i++) {
            if (attrIds[i] >= AttrStr.length) {
                Log.e(TAG, "attrib id is"+attrIds[i]+"which is not supported");
                AttribStrings[i] = "";
            } else {
    ...
    private void notifyAttributeValuesText(String what, int attribute,
        ...
        for (int i = 0; i < valIds.length; i++) {
            if ((valueStrs == null) ||
                ...
                Log.e(TAG, "value id is" + valIds[i] + "which is not supported");
                ValueStrings[i] = "";
            } else {
    ...
    private void setValidAttributes(byte [] attribValuePairs) {
        ...
        for (int i = 0; i < (attribValuePairs.length-1); i += 2) {
           ...
           switch(attrib) {
                ...
                break;
                default:
                   Log.e(TAG,"Unknown attribute"+attrib);
                   notifyAttributeValues(PLAYERSETTINGS_RESPONSE,
                            mAttributePairs, SET_ATTRIBUTE_VALUES);
                break;
           }
    ...
    //protruly hujianwei 20170829 modify add API start
    ...
    public void setListInfo(List<HBListItem> list) {
        if (mMusicList == null) {
            mMusicList = new ArrayList<HBListItem>();
        }
    ...
    public List<HBListItem> getListInfo() {
        if (mMusicList != null) {
            return mMusicList;
        }
        return null;
    ...
    private boolean isNet(long id) {
        ...
        if (mMusicList != null) {
            ...
            for (int i = 0; i < len; i++) {
                HBListItem item = mMusicList.get(i);
                if (item.getSongId() == id) {
                    return (item.getIsDownLoadType() == 1);
                }
            }
        }
        return false;
    ...
    //protruly hujianwei 20170829 modify add API end
    ...
    static class ServiceStub extends IMediaPlaybackService.Stub {
        ...
        @Override
        public void setListInfo(List<HBListItem> list) throws RemoteException {
            mService.get().setListInfo(list);
        }

        @Override
        public List<HBListItem> getListInfo() throws RemoteException {
    ...
    private void initXiamiPlayer(Handler handler) {
        mHBXiamiPlayer = new HBXiamiPlayer(getApplicationContext(), handler);
        return;
    }

    private void releaseXiamiPlayer() {
        if (mHBXiamiPlayer != null) {
            mHBXiamiPlayer.release();
            mHBXiamiPlayer = null;
        }
        return;
    }
    ...
    private String getImgUrl(int size) {
        if (mHBXiamiPlayer == null || !mbOnline) {
            return null;
        }
        return mHBXiamiPlayer.getImgUrl(size);
    }
    ...
    private String getLryFile() {
        ...
        return mHBXiamiPlayer.getLyricFile();
    }

...
public class StickyListHeadersListView extends HbListView {
    ...
    /**
     * true 表示listView滑动过程中title悬停 false 表示listView滑动过程中title不会悬停
     */
    ...
    private void scrollChanged(int reportedFirstVisibleItem) {
        ...
        if (childCount != 0) {
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                if (viewToWatch == null
                        || (!viewToWatchIsFooter && (viewToWatch
                        .findViewById(R.id.tv_artist_header)) == null)
                        || ((childIsFooter || child
                        .findViewById(R.id.tv_artist_header) != null) && childDistance < watchingChildDistance)) {
                    viewToWatch = child;
    ...
    private void updateHeaderVisibilities() {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            View header = child.findViewById(R.id.tv_artist_header);
            if (header != null) {
                if (child.getTop() < top) {
    ...
    public void setSelection(int position, int offset) {
        super.setSelectionFromTop(position + 1, offset);
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_DOWN
            ...
            if (mHeader.findViewById(R.id.tv_playmode)!=null&&ev.getX() < mHeader.findViewById(R.id.tv_playmode).getRight()+20) {
        ...
        if (mHeaderBeingPressed && mHeader != null) {

...
import hb.provider.ContactsContract.CommonDataKinds;
...
public class ShareContactViaSMSActivity extends Activity {
	...
	public void onCreate(Bundle icicle) {
		...
		Uri extraUri=null;
		if(mIntent.getBooleanExtra("hbflag", false)){
			extraUri = (Uri) mIntent.getExtra("lookupUri");
		}else{
			extraUri = (Uri) mIntent.getExtra(Intent.EXTRA_STREAM);
		}
		mLookUpUris = null;
		if (null != extraUri) {
			...
			if (!mNumbers.isEmpty()) {
				if (mNumbers.size() > 1) {
					...
					for (String number : mNumbers) {
						textVCardString += getString(R.string.hb_phoneNumber) + i + ": " + number + "\n";
						i++;
					}
				} else {
					textVCardString +=getString(R.string.hb_phoneNumber) + ": " + mNumbers.get(0) + "\n";
				}
			}
			if (!mOmails.isEmpty()) {
				if (mOmails.size() > 1) {

...
public class PinyinUtil {
	...
	public static String getFullSpell(String chinese) { 
		...
		if(arr==null) return chinese;

		for (int i = 0; i < arr.length; i++) {   
			if (arr[i] > 128) {   
				try {   
					String[] result=PinyinHelper.toHanyuPinyinStringArray(arr[i], format);
					if(result==null||result.length==0) return chinese;
		...
		Log.d(TAG,"pybf.toString():"+pybf.toString());

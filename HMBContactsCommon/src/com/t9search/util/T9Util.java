...
public class T9Util {
	...
	public static boolean matchByPinyin(PinyinSearchUnit pinyinSearchUnit,String keyword){
		if ((null == pinyinSearchUnit) || (null == keyword)) {
			return false;
		}
		...
		for(int i=0; i<pinyinUnitsLength; i++){
			...
			searchBuffer.delete(0, searchBuffer.length());
			searchBuffer.append(keyword);
			boolean found = findPinyinUnitsByPinyin(pinyinSearchUnit.getPinyinUnits(), i, j, pinyinSearchUnit.getBaseData(),
					searchBuffer, pinyinSearchUnit.getMatchKeyword());
			if (true == found) {
				return true;
			}
	 ...
	 * @return true if find,false otherwise.
	 */
	private static boolean findPinyinUnitsByPinyin(final List<PinyinUnit> pinyinUnits,int pinyinUnitIndex,int t9PinyinUnitIndex,final String baseData, StringBuffer searchBuffer,StringBuffer matchKeyword ){
		if((null==pinyinUnits)||(null==baseData)||(null==searchBuffer)||(null==matchKeyword)){
			return false;
		}
		...
		if(pyUnit.isPinyin()){
			...
			if(search.startsWith(String.valueOf(pinyinBaseUnit.getPinyin().charAt(0)))){// match pinyin first character
				searchBuffer.delete(0,1);//delete the match character
				matchKeyword.append(baseData.charAt(pyUnit.getStartPosition()));
				boolean found=findPinyinUnitsByPinyin(pinyinUnits, pinyinUnitIndex+1, 0, baseData, searchBuffer, matchKeyword);
				if(true==found){
					return true; 
				}else{
			...
			if(pinyinBaseUnit.getPinyin().startsWith(search)){
				//The string of "search" is the string of t9PinyinUnit.getPinyin() of a subset. means match success.
				matchKeyword.append(baseData.charAt(pyUnit.getStartPosition()));
				searchBuffer.delete(0, searchBuffer.length());	
				return true;
				
			}else if(search.startsWith(pinyinBaseUnit.getPinyin())){ //match quanpin  success
				//The string of t9PinyinUnit.getPinyin() is the string of "search" of a subset.
				...
				boolean found=findPinyinUnitsByPinyin(pinyinUnits, pinyinUnitIndex, t9PinyinUnitIndex+1, baseData, searchBuffer, matchKeyword);
				if(found==true){
					return true;
				}
			}
			...
			if(pinyinBaseUnit.getPinyin().startsWith(search)){
				//The string of "search" is the string of t9PinyinUnit.getPinyin() of a subset.
				int startIndex=0; 
				matchKeyword.append(baseData.substring(startIndex+pyUnit.getStartPosition(),startIndex+pyUnit.getStartPosition()+ search.length()));
				searchBuffer.delete(0, searchBuffer.length());
				return true;

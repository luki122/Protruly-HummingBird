...
public class ImportVCardActivity extends Activity {
	...
	// Runs on the UI thread.
	private class HbDialogDisplayer implements Runnable {
		private String title,message;
		private int type=0;
		public HbDialogDisplayer(String title,String message) {
			this.title=title;
			this.message=message;
		}

		public HbDialogDisplayer(int type) {
			this.type=type;
		}
		...
		public void onReceive(Context context, Intent intent) {
			...
			if(TextUtils.equals(action, "CONTACTS_IMPORT_FROM_SD_DOING")){
				...
				if(index==1){
					...
					mProgressDialog.setButton(getString(R.string.hb_cancel), new DialogInterface.OnClickListener() {
						...
						final Intent intent = new Intent(ImportVCardActivity.this, CancelActivity.class);
	...
	implements DialogInterface.OnClickListener, DialogInterface.OnCancelListener {
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			Toast.makeText(ImportVCardActivity.this, getString(R.string.hb_cancel_msg), Toast.LENGTH_LONG).show();
			setResult(Activity.RESULT_CANCELED);
			finish();
		}
		@Override
	...
	implements DialogInterface.OnCancelListener {
		...
		public void renameFile(String oldname,String newname){ 
			if(!oldname.equals(newname)){//新的文件名和以前文件名不同时,才有必要进行重命名 
				...
				Log.d(TAG,"renameFile0,oldfile:"+oldfile+" newfile:"+newfile);
				if(!oldfile.exists()){
					Log.d(TAG,"renameFile1,oldfile:"+oldfile+" newfile:"+newfile);
					return;//重命名文件不存在
				}
				if(newfile.exists())//若在该目录下已经有一个文件和新文件名相同，则不允许重命名 
					...
					Log.d(TAG,"renameFile2,oldfile:"+oldfile+" newfile:"+newfile);
				} 
			}else{
		...
		private Uri copyTo(final Uri sourceUri, String filename) throws IOException {
			Log.i(LOG_TAG, String.format("Copy     a Uri to app local storage (%s -> %s)",
					sourceUri, filename));
			String oldFilePath=sourceUri.toString().substring(sourceUri.toString().indexOf("///"));
			String newFilePath="";
			Uri newUriString=null;
			if(sourceUri.toString().contains("#")){
				String replaceString="_"+System.currentTimeMillis()+"_";
				newFilePath=oldFilePath.replace("#", replaceString);
				newUriString=Uri.parse(sourceUri.toString().replace("#", replaceString));
				renameFile(oldFilePath,newFilePath);
			}else {
				newFilePath=oldFilePath;
				newUriString=sourceUri;
			}
			
			Log.d(TAG,"oldFilePath:"+oldFilePath+" newFilePath:"+newFilePath);
			...
			Uri destUri = null;
			try {
				inputChannel = Channels.newChannel(resolver.openInputStream(newUriString));
				...
				if(!TextUtils.equals(newFilePath, oldFilePath)) renameFile(newFilePath,oldFilePath);
			}
			return destUri;
		}
	...
	private void startVCardSelectAndImport() {
		...
		if (getResources().getBoolean(R.bool.config_import_all_vcard_from_sdcard_automatically) ||
			...
			if (size == 1) {
				runOnUiThread(new HbDialogDisplayer(getString(R.string.hb_menu_import_from_sd),getString(R.string.hb_import_from_sd_found_vcard_dialog_message,
			...
			runOnUiThread(new HbDialogDisplayer(1));
	...
	protected void onCreate(Bundle bundle) {
		...
		if(uri!=null){
			...
			new AlertDialog.Builder(ImportVCardActivity.this)
			.setTitle(null) 
			.setMessage(ImportVCardActivity.this.getString(R.string.hb_confirm_import_vcard_message,fileName))
			.setPositiveButton(getString(com.hb.R.string.ok), new DialogInterface.OnClickListener() {

				@Override
				public void onClick(DialogInterface dialog, int which) {
					Toast.makeText(ImportVCardActivity.this, getString(R.string.hb_begin_vcard_import_message), Toast.LENGTH_LONG).show();
					startImport();
				}
			})
			.setNegativeButton(getString(com.hb.R.string.cancel), new DialogInterface.OnClickListener() {
	...
	public String getRealPathFromUri(Context context, Uri contentUri) {
		...
		try {
			...
			Log.d(TAG,"cursor:"+cursor+" contentUri:"+contentUri+" proj:"+Arrays.toString(proj));
	...
	private void startImport() {
		...
		if (uri != null) {
			...
			//			fileName = uri.getLastPathSegment();
			Log.i(LOG_TAG, "Starting vCard import using Uri " + uri+" fileName:"+fileName);
			importVCard(uri);
		} else {
	...
	private void doScanExternalStorageAndImportVCard() {
		...
		if (!file.exists() || !file.isDirectory() || !file.canRead()) {
			...
			//			prepareProgressDialogSpinner(getString(R.string.hb_import_from_sd_searching_vcard),getString(R.string.hb_import_from_sd_searching_vcard));
			beginTime=System.currentTimeMillis();
			prepareProgressDialogSpinner(null,getString(R.string.hb_import_from_sd_searching_vcard_title));
			mVCardScanThread.start();
		}
	}

	long beginTime=0L;
	private void waitUtilOneSecond(){

...
import com.android.contacts.common.interactions.MultiChoiceService.HbDeleteRequestListener;
...
public class DeleteProcessor extends ProcessorBase {
	...
	private HbDeleteRequestListener hbDeleteRequestListener=null;
	...
	public DeleteProcessor(final MultiChoiceService service,
			final MultiChoiceHandlerListener listener, final List<MultiChoiceRequest> requests,
			final int jobId,final HbDeleteRequestListener hbDeleteRequestListener) {
		Log.i(TAG, "[DeleteProcessor]new.");
		mService = service;
		mResolver = mService.getContentResolver();
		mListener = listener;
		this.hbDeleteRequestListener=hbDeleteRequestListener;
	...
	private void runInternal() {
		...
		for (MultiChoiceRequest request : mRequests) {
			...
			if(preContactId!=contactId){
				currentCount++;
				/*mListener*/hbDeleteRequestListener.onProcessed(MultiChoiceService.TYPE_DELETE, mJobId, currentCount, totalItems,

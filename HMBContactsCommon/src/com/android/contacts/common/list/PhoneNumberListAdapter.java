...
//		sliderView.addTextButton(1,context.getString(R.string.hb_delete));
...
//    	Log.d(TAG,"bindview");
        ...
        String snippet=cursor.getString(PhoneQuery.PHONE_NUMBER);
		setDisplayName(TextUtils.isEmpty(displayName)?"":displayName.replace(" ", ""), mHighlightedPrefix,
				viewHolder.name);
		
//		 final DirectoryPartition directory = (DirectoryPartition) getPartition(partition);
//		bindPhoneNumber(viewHolder.secondLine, cursor, directory.isDisplayNumber());
...
//        view.setLabel(label);
    ...
    protected void bindPhoneNumber(TextView view, Cursor cursor, boolean displayNumber) {
        CharSequence label = null;
        if (displayNumber &&  !cursor.isNull(PhoneQuery.PHONE_TYPE)) {
            final int type = cursor.getInt(PhoneQuery.PHONE_TYPE);
            final String customLabel = cursor.getString(PhoneQuery.PHONE_LABEL);
...
//        view.setLabel(label);
        ...
        setPhoneNumber(text, mCountryIso, view);
    }

...
import com.hmb.manager.tms.TmsServiceManager;
...
//    private static final Uri ICC_URI_FOUR = Uri.parse("content://sms/icc4");
    ...
    */
    ...
    public boolean onCreate() {
        ...
        //begin tangyisen init reject 20170523
        initReject();
        //end tangyisen
        return true;
    }

   //begin tangyisen 20170523 init reject
    public void initReject() {
        TmsServiceManager manager = TmsServiceManager.getInstance(getContext());
        if(manager != null) {
            manager.init();
        }
        //register broadcast and others
    }
    //end tangyisen
    ...
    public Cursor query(Uri uri, String[] projectionIn, String selection,
        ...
        Log.d(TAG, "query begin, uri = " + uri + ", selection = " + selection);
...
//                return getAllMessagesFromIccInternational(uri, PhoneConstants.SIM_ID_1);
        ...
        /*boolean selectionEmpty = TextUtils.isEmpty(selection);
        if(qb.getTables().equals(TABLE_SMS) || qb.getTables().equals(VIEW_SMS_RESTRICTED)) {
    ...
    private Uri insertInner(Uri url, ContentValues initialValues, int callerUid, String callerPkg) {
        ...
        try {
            if (table.equals(TABLE_SMS)) {
                ...
                } //tangyisen add for validation tools
                else if(!addType){
                    type = initialValues.getAsInteger(Sms.TYPE);
                }
                ...
                } else if(ProviderUtil.isSpecifiedPackageForReject( callerPkg, callerUid)){     //begin tangyisen 20170523
                    if(!TextUtils.isEmpty(address)) {
                        String smsBody = values.getAsString( Sms.BODY );
                        RejectSmsResult result = RejectUtil.isRejectSms(getContext(), address, smsBody);
                        if(result != null && result.getReject() > 0) {
                            values.put(Sms.REJECT, result.getReject());
                            values.put(Sms.REJECT_TAG, result.getRejectTag());
                            //lichao add in 2017-07-17 begin
                            values.put(Sms.READ, 1);//temp for bug5697
                            values.put(Sms.SEEN, 1);
                            Log.i(TAG, "sms insert(), >>>sendRejectNotification, address = "+address);
                            RejectUtil.sendRejectNotification(getContext(), true, address, smsBody);
            ...
            /// M: Code analyze 005, fix bug ALPS00245352, it cost long time to restore messages.
            /// remove useless operation and add transaction while import sms. @{
            Log.d(TAG, "insert table body end");
            // if (!importSms){
...
//                setThreadStatus(db, values, 0);
            ...
            // row is incorrect.
            if (table == TABLE_SMS /*&& !isReject*/) {//tangyisen add isReject 20170523
...
//                        }
    ...
    private final static String TAG = "MmsProvider/Sms";
...
//        }
    ...
    private ArrayList<String> getConcatSmsIndexAndBody(ArrayList<SmsMessage> messages, int index) {
        ...
        Log.d(TAG, "concatenation sms index:" + smsIndex.toString());
        Log.d(TAG, "concatenation sms body:" + smsBody.toString());
    ...
    private long getThreadIdInternal(String recipient, SQLiteDatabase db, boolean importSms) {
        ...
        Log.d(TAG, "sms insert, getThreadIdInternal, recipientId = " + recipientId);
    ...
    private Cursor getThreadIdWithoutInsert(String recipient, SQLiteDatabase db) {
        ...
        Log.d(TAG, "getThreadIdWithoutInsert, recipientId = " + recipientId);
        if (recipientId != -1L) {
            String[] selectionArgs = new String[] { String.valueOf(recipientId) };
    ...
    private long getSingleAddressId(String address, SQLiteDatabase db, boolean needInsert) {
        ...
        try {
            ...
            if (retVal == -1L) {
                ...
                Log.d(TAG, "getSingleAddressId: get exist id=" + retVal + ", refinedAddress="
    ...
    private long insertCanonicalAddresses(SQLiteDatabase db, String refinedAddress) {
        Log.d(TAG, "sms insert insertCanonicalAddresses for address = " + refinedAddress);

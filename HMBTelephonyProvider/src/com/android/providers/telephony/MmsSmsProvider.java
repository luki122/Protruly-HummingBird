...
import com.zzz.provider.Telephony.CanonicalAddressesColumns;
...
public class MmsSmsProvider extends ContentProvider {
    ...
    private static final String LOG_TAG = "MmsProvider/MmsSms";
    /// M: Code analyze 001, new feature, add Xlog flag.
    private static final String WAPPUSH_TAG = "MmsProvider/WapPush";
    private static final boolean DEBUG = true;
    private static final boolean DBG_SEARCH = false;
    ...
    private static final int URI_SEARCH_CONTENT                    = 102;
    //begin tangyisen 20170523
    private static final int URI_BLACK_RECOVERED_SMSMMS            = 103;
    //end tangyisen
    //HB. Comments :  , Engerineer : lichao , Date : 17-6-9 , begin
    private static final int URI_FIRST_REJECTED_MESSAGE_ALL          = 104;
    private static final int URI_FIRST_REJECTED_MESSAGE_BY_THREAD_ID = 105;
    //HB. end
    ...
    // lichao delete for no use in 2017-06-29
    ...
    private static final String[] MMS_ONLY_COLUMNS = {
        ...
        Mms.TRANSACTION_ID, Mms.MMS_VERSION, Telephony.Mms.SERVICE_CENTER, Mms.TEXT_ONLY, Mms.REJECT, Mms.REJECT_TAG };//tangyisen add 20170523
    ...
    // table.
	//tangyisen add group_id,noti_count and fails_count, reject and reject_tag
    private static final String[] SMS_ONLY_COLUMNS =
            { "address", "body", "person", "reply_path_present",
              "service_center", "status", "subject", "type", "error_code", "ipmsg_id",
                    "group_id", "noti_count", "fails_count", "reject", "reject_tag"};
    /// M: Code analyze 003, new feature, support for cellbroadcast. @{
    ...
    //see com/android/mms/data/Conversation.java String[] ALL_THREADS_PROJECTION
    public static final String[] THREADS_COLUMNS = {
            Threads._ID,//0
            Threads.DATE,//1
            Threads.MESSAGE_COUNT,//2
            Threads.RECIPIENT_IDS,//3
            Threads.SNIPPET,//4
            Threads.SNIPPET_CHARSET,//5
            Threads.READ,//6
            Threads.ERROR,//7
            Threads.HAS_ATTACHMENT,//8
            Threads.TYPE,//9 //COMMON_THREAD or BROADCAST_THREAD, MTK add WAPPUSH_THREAD
            //Threads.READ_COUNT,//10 /// MTK add
            //Threads.STATUS,//11 /// MTK add
            //ThreadSettings._ID,//12 /// MTK add
            //Threads.DATE_SENT//13 /// MTK add for common
            Threads.SUBSCRIPTION_ID,//10,lichao add
            Threads.DATE_TOP,//11,lichao add
            Threads.IS_TOP,//12,lichao add
            Threads.LATEST_IS_MMS,//13,lichao add
            Threads.LATEST_IS_NOTIFI,//14,lichao add
            Threads.LATEST_HAS_ATTACH,//15,lichao add
            Threads.LATEST_PDU_TEXT,//16,lichao add
            Threads.LATEST_HAS_ERROR,//17,lichao add
            Threads.FAILED_UNREAD//18,lichao add for failed_unread
    };
...
//            "snippet FROM words WHERE index_text MATCH ? ORDER BY snippet LIMIT 50;";
    ...
    //lichao add in 2017-04-10 begin
    private static String getTextSearchQuery(String smsTable, String pduTable) {
        ...
        // 非群发信息的group_id=0, 群发短信中头目短信的group_id=-1,
        // 群发短信中其他的信息的group_id=头目短信的_id(有效的_id都大于0)
        final String smsQuery = "SELECT "
                ...
                + "AND " + Sms.TYPE + " != " + Sms.MESSAGE_TYPE_DRAFT + " "
                + "AND " + Sms.REJECT + " <= 0 "
                + "AND " + Sms.GROUP_ID + " <= 0 "//lichao add for group SMS(only show one to user)
    ...
    //lichao move mtk codes to the getTextSearchQuery() method
    private String getTextSearchQuery(String smsTable, String pduTable, String pattern) {
        HashMap<String, String> contactRes = getContactsByNumber(pattern);
        String searchContacts = searchContacts(pattern, contactRes);
        String smsProjection = smsTable + "._id as _id,"
                +"thread_id,"
                +"address,"
                +"body,"
                +"date,"
        ...
        String mmsProjection = pduTable + "._id,"
                + "thread_id,"
                + "addr.address,"
                + "pdu.sub as body,"
                + "pdu.date,"
  ...
  /// M: add for op09 of new feature cc @{
    ...
    static {
        ...
        // URI for search messages with obtained search mode
        URI_MATCHER.addURI(AUTHORITY, "search-content", URI_SEARCH_CONTENT);
        ...
        /// @}
        //begin tangyisen 20170523
        URI_MATCHER.addURI(AUTHORITY, "blackRecoveredSmsmms", URI_BLACK_RECOVERED_SMSMMS);
        //end tangyisen

        //HB. Comments :  , Engerineer : lichao , Date : 17-6-9 , begin
        URI_MATCHER.addURI(AUTHORITY, "rejected", URI_FIRST_REJECTED_MESSAGE_ALL);

        URI_MATCHER.addURI(AUTHORITY, "rejected/#", URI_FIRST_REJECTED_MESSAGE_BY_THREAD_ID);
        //HB. Comments :  , Engerineer : lichao , Date : 17-6-9 , end
        ...
        switch (match) {
            ...
            case URI_CONVERSATIONS:
                ...
                if ((simple != null) && simple.equals("true")) {
                    ...
                    //lichao modify for Blacklist and setTop begin=========================
                    //lichao add for Blacklist in 2017-05-02
                    String tables = TABLE_THREADS;//"threads"
                    String groupBy = null;
                    String orderBy = " date DESC";
                    //HB. Comments :  , Engerineer : lichao , Date : 17-6-29 , begin
                    //仅仅包含被拦截信息的thread的msg_count_in_draft=0, 需要排除掉
                    String noReject = uri.getQueryParameter("no_reject");
                    if ((!TextUtils.isEmpty(noReject)) && noReject.equals("true")) {
                        //String MESSAGE_COUNT_INCLUDE_DRAFT = "msg_count_in_draft";
                        String reject_match = " (msg_count_in_draft > 0) ";
                        selection = concatSelections(selection, reject_match);
                    }
                    //HB. end

                    //lichao modify in 2017-04-19 begin
                    String isTop = uri.getQueryParameter("is_top");
                    if ((!TextUtils.isEmpty(isTop)) && isTop.equals("true")) {
                        Cursor[] cursors = new Cursor[2];
                        String selection1 = concatSelections(
                                selection, Threads.IS_TOP + "=" + 1);
                        if(DEBUG) Log.d(LOG_TAG, "query(), selection1 = " + selection1);
                        //Sorting is not neat when use date_top DESC
                        cursors[0] = getSimpleConversations(tables,
                                projection, selection1, selectionArgs, groupBy, orderBy);

                        String selection2 = concatSelections(
                                selection, Threads.IS_TOP + "=" + 0);
                        if(DEBUG) Log.d(LOG_TAG, "query(), selection2 = " + selection2);
                        cursors[1] = getSimpleConversations(tables,
                                projection, selection2, selectionArgs, groupBy, orderBy);
                        cursor = getMergeCursor(cursors);
                    }else{
                        cursor = getSimpleConversations(tables,
                                projection, selection, selectionArgs, groupBy, orderBy);
                    }
                    //lichao modify in 2017-04-19 end
					//lichao modify for Blacklist and setTop end==========================
            ...
            case URI_CONVERSATIONS_MESSAGES:
                ...
                //lichao add is_top in 2017-04-20
                String isTop = uri.getQueryParameter("is_top");
                if (((isRcse != null) && isRcse.equals("true"))
                        || ((isTop != null) && isTop.equals("true"))) {
            ...
            case URI_SEARCH: {
                ...
                * both queries. @{*/
                ...
                //lichao refactor mtk codes to a method
                String rawQuery = null;
                rawQuery = getTextSearchQuery(smsTable, pduTable, pattern);
                try {
            ...
            case URI_SEARCH_CONTENT:
                ...
                * both queries. @{*/
                ...
                try {
				    //lichao refactor mtk codes to a method and delete it
                    //cursor = db.rawQuery(getTextSearchQuery(smsTable, pduTable, pattern),
				    //new String[] { searchString,searchString, pduPattern });
            ...
            //HB. Comments :  , Engerineer : lichao , Date : 17-6-9 , begin
            case URI_FIRST_REJECTED_MESSAGE_BY_THREAD_ID: {
                long threadId;
                try {
                    threadId = Long.parseLong(uri.getLastPathSegment());
                } catch (NumberFormatException e) {
                ...
                cursor = getFirstRejectedMessage(projection, "thread_id=" + Long.toString(threadId),
                        sortOrder, smsTable, pduTable);
                break;
            }
            case URI_FIRST_REJECTED_MESSAGE_ALL: {
                cursor = getFirstRejectedMessage(
                        projection, selection, sortOrder, smsTable, pduTable);
                break;
            }
            //HB. end
    ...
    private void insertThread(String recipientIds, List<String> recipients) {
        ...
        //lichao add in 2017-04-19 begin
        values.put(ThreadsColumns.DATE_TOP, date - date % 1000);
        //lichao add in 2017-04-19 end
        ...
        if (MTK_RCS_SUPPORT) {
            ...
            //lichao add in 2017-04-19 begin
            v1.put(ThreadsColumns.DATE_TOP, 0);
            //lichao add in 2017-04-19 end
...
//            Collections.sort(recipients);
    ...
    //begin tangyisen add for reject just return query thread ID and not insert when not found 20170523
    private synchronized Cursor getSimpleThreadId(List<String> recipients) {
        Set<Long> addressIds = getAddressIds(recipients);
        String recipientIds = "";

        if (addressIds.size() == 0) {
            Log.e(LOG_TAG, "getSimpleThreadId: NO receipients specified -- NOT creating thread",
                    new Exception());
            return null;
        } else if (addressIds.size() == 1) {
            // optimize for size==1, which should be most of the cases
        ...
        if (Log.isLoggable(LOG_TAG, Log.VERBOSE)) {
            Log.d(LOG_TAG, "getSimpleThreadId: recipientIds (selectionArgs) =" + recipientIds);
        }
        ...
        if (cursor != null && cursor.getCount() > 1) {
            Log.w(LOG_TAG, "getSimpleThreadId: why is cursorCount=" + cursor.getCount());
        }
        return cursor;
    }
    ...
    private void updateSmsmmsBlack(ArrayList<Long> threadIds) {
        ArrayList<ContentProviderOperation> ops = new ArrayList<ContentProviderOperation>();
        for (long threadId : threadIds) {
            ops.add(getSmsUpdateOperation(threadId));
            ops.add(getMmsUpdateOperation(threadId));
        }
        try {
            applyBatch(ops);
    ...
    private ContentProviderOperation getSmsUpdateOperation(long threadId) {
        return ContentProviderOperation.newUpdate(Sms.CONTENT_URI)
                .withValue("reject", 0)
                .withValue("reject_tag", "")
                .withSelection("thread_id = " + threadId + " and reject = 1", null)
                .withYieldAllowed(true)
                .build();
    }

    private ContentProviderOperation getMmsUpdateOperation(long threadId) {
        return ContentProviderOperation.newUpdate(Mms.CONTENT_URI)
                .withValue("reject", 0)
                .withValue("reject_tag", "")
                .withSelection("thread_id = " + threadId + " and reject = 1", null)
                .withYieldAllowed(true)
    ...
    //lichao add begin
    private Cursor getSimpleConversations(String table, String[] projection, String selection,
        ...
        if(DEBUG){
            String sql = SQLiteQueryBuilder.buildQueryString(
                    false, table, projection, selection, groupBy, null, sortOrder, null);
            Log.d(LOG_TAG, "getSimpleConversations(), sql = "+sql);
        }
    ...
    //HB. Comments :  , Engerineer : lichao , Date : 17-6-9 , begin
	//add this like getFirstLockedMessage()
    private Cursor getFirstRejectedMessage(String[] projection, String selection,
                                         String sortOrder, String smsTable, String pduTable) {
        SQLiteQueryBuilder mmsQueryBuilder = new SQLiteQueryBuilder();
        SQLiteQueryBuilder smsQueryBuilder = new SQLiteQueryBuilder();
        /// M: Code analyze 013, new feature, support for wappush.
        ...
        if (DEBUG) {
            Log.v(LOG_TAG, "getFirstRejectedMessage query: " + unionQuery);
            Log.v(LOG_TAG, "cursor count: " + cursor.getCount());
        }
        return cursor;
    }
    ...
    public int update(Uri uri, ContentValues values,
        ...
        switch(URI_MATCHER.match(uri)) {
            ...
            case URI_CONVERSATIONS_MESSAGES:
                ...
                String isRcse = uri.getQueryParameter("isRcse");
                //lichao add is_top in 2017-04-18
                String isTop = uri.getQueryParameter("is_top");
                if (((isRcse != null) && isRcse.equals("true"))
                        || ((isTop != null) && isTop.equals("true"))) {
            ...
            /// @}
            //begin tangyisen
            case URI_BLACK_RECOVERED_SMSMMS:
                //applyBatch will handler batch recovered
                List<String> blackRecipients = uri.getQueryParameters("recipient");
                //if(blackRecipients.size() == 1) {
                Cursor black= null;
                if(blackRecipients != null && blackRecipients.size() != 0) {
                    ArrayList<String> nums = new ArrayList<String>();
                    try {
                        for(String number : blackRecipients) {
                            nums.clear();
                            nums.add( number );
                            black = getSimpleThreadId( nums );
                            long blackThreadId = 0;
                            if(black != null && black.getCount() != 0) {
                                if(black.moveToFirst()) {
                                    blackThreadId = black.getLong(black.getColumnIndex( "_id" ));
                                    black.close();
                                    black = null;
                                }
                                affectedRows = db.update(SmsProvider.TABLE_SMS,
                                        values, "thread_id = " + blackThreadId + " and reject = 1", null);
                                affectedRows += db.update(MmsProvider.TABLE_PDU,
                                        values, "thread_id = " + blackThreadId + " and reject = 1", null);
                            }
                        }
                    } finally {
                    ...
                    ArrayList<String> nums = new ArrayList<String>();
                    Cursor blacktmp = null;
                    for(String number : blackRecipients) {
                        nums.clear();
                        nums.add( number );
                        blacktmp = getSimpleThreadId(nums);
                        long blackThreadId = 0;
                        if(blacktmp != null && blacktmp.getCount() != 0) {
                            if(blacktmp.moveToFirst()) {
                                threadIds.add( blacktmp.getLong(blacktmp.getColumnIndex( "_id" )) );
                            }
                        }
                    }
                    updateSmsmmsBlack( threadIds );
                }*/
                break;
            //end tangyisen
            default:
    ...
    private Cursor getSearchThreads(Uri uri, SQLiteDatabase db) {
        ...
        //if (DBG_SEARCH) Log.d(LOG_TAG, "\n\n getSearchThreads : searchMode=" + searchMode);

        String keyStr = uri.getQueryParameter("key_str");
        //if (DBG_SEARCH) Log.d(LOG_TAG, "getSearchThreads : keyStr=" + keyStr);

        int matchWhole = Integer.parseInt(uri.getQueryParameter("match_whole"));
        //if (DBG_SEARCH) Log.d(LOG_TAG, "getSearchThreads : matchWhole=" + matchWhole);

        String separatedAddress = uri.getQueryParameter("addresses");
        //if (DBG_SEARCH) Log.d(LOG_TAG, "getSearchThreads : separatedAddress=" + separatedAddress);
    ...
    private String getThreadQueryStringMatchByAddress() {
        ...
        String address_match =" (canonical_address LIKE ? )";
        String recipient_match =" (recipient_ids = canonical_id) ";
        //lichao modify in 2017-06-29 for
        //仅仅包含被拦截信息的thread的msg_count_in_draft=0, 需要排除掉
        //String MESSAGE_COUNT_INCLUDE_DRAFT = "msg_count_in_draft";
        String reject_match = " (msg_count_in_draft > 0) ";
    ...
    private static final String[] CANONICAL_ADDRESSES_PROJECTION_ARRAY = {
            "canonical_addresses._id AS canonical_id",
            "canonical_addresses.address AS canonical_address"
    };
    //HB. end

    //HB. Comments :  , Engerineer : lichao , Date : 17-5-20 , begin
    public static String[] makeProjectionWithCanonicalAddresses(
            String[] original_projection) {
        return concat(original_projection, CANONICAL_ADDRESSES_PROJECTION_ARRAY);
    }
    //HB. end
    ...
    //lichao add in 2017-03-31
    //refer to ALL_THREADS_PROJECTION in com/android/mms/data/Conversation.java
    //+ "threads.attachment_info AS attachment_info, "//lichao delete in 2017-04-10
    private static final String THREADS_CANONICAL_ADDRESSES_PROJECTION =
            "threads._id AS _id, "//0:Threads._ID
                    ...
                    + "threads.type AS type, "//9:Threads.TYPE
                    + "threads.sub_id AS sub_id, "//10:Threads.SUBSCRIPTION_ID
                    + "threads.date_top AS date_top, "//11:Threads.DATE_TOP
                    + "threads.is_top AS is_top, "//12:Threads.IS_TOP
                    + "threads.latest_is_mms AS latest_is_mms, "//13:Threads.LATEST_IS_MMS
                    + "threads.latest_is_notifi AS latest_is_notifi, "//14:Threads.LATEST_IS_NOTIFI
                    + "threads.latest_has_attach AS latest_has_attach, "//15:Threads.LATEST_HAS_ATTACH
                    + "threads.latest_pdu_text AS latest_pdu_text, "//16:Threads.LATEST_PDU_TEXT
                    + "threads.latest_has_error AS latest_has_error,"//17:Threads.LATEST_HAS_ERROR
                    + "threads.failed_unread AS failed_unread,"//18:Threads.FAILED_UNREAD
                    + "threads.msg_count_in_draft AS msg_count_in_draft,"//MESSAGE_COUNT_INCLUDE_DRAFT
                    + "canonical_addresses._id AS canonical_id, "
                    + "canonical_addresses.address AS canonical_address ";
    ...
    private String getThreadQueryStringMatchByThreadId(String threadIdString) {
        ...
        String threadQuery = String.format(
                "SELECT %s FROM threads WHERE (_id in (%s) AND "+reject_match+")" +
                        " group by threads._id",
...
//                Set<String> recipients = searchRecipients(cursor.getString(1));
    ...
    private long insertCanonicalAddresses(SQLiteOpenHelper openHelper, String refinedAddress) {
        //begin tangyisen 20170523
        /*ProviderUtil.BlackEntry entry = ProviderUtil.getRejectEntryFromAddress( getContext(), refinedAddress );
        ContentValues contentValues = new ContentValues(3);
        if(entry != null) {
            contentValues.put(CanonicalAddressesColumns.IS_BLACK, entry.isBlack);
            contentValues.put(CanonicalAddressesColumns.BLACK_NOTE, entry.blackNote);
        } else {
            contentValues.put(CanonicalAddressesColumns.IS_BLACK, 0);
            contentValues.put(CanonicalAddressesColumns.BLACK_NOTE, "");
        }*/
        //end tangyisen
    ...
    public static void broadcastUnreadMessageNumberToMmsApp(Context context, int unreadMsgNumber) {
        Intent intent = new Intent();
        intent.setAction("com.android.mms.UNREAD_CHANGED");
        intent.putExtra(Intent.EXTRA_UNREAD_NUMBER, unreadMsgNumber);
        intent.putExtra(Intent.EXTRA_UNREAD_COMPONENT,
                new ComponentName("com.android.mms", "com.android.mms.ui.BootActivity"));
        context.sendBroadcast(intent);
    ...
    public static void notifyUnreadMessageNumberChanged(final Context context) {
        ...
        //Log.d(LOG_TAG, "notifyUnreadMessageNumberChanged, unreadNumber = "+unreadNumber);
        recordUnreadMessageNumberToSys(context, unreadNumber);
        broadcastUnreadMessageNumber(context, unreadNumber);
        broadcastUnreadMessageNumberToMmsApp(context, unreadNumber);
    }
...
//        long date = System.currentTimeMillis();
        if (!values.containsKey(ThreadsColumns.DATE)) {
            ...
            //lichao add in 2017-04-19 begin
            values.put(ThreadsColumns.DATE_TOP, 0);
            //lichao add in 2017-04-19 end
        }
    ...
    private Cursor getMergeCursor(Cursor[] cursors){
        if(null == cursors || 0 == cursors.length){
            Log.w(LOG_TAG, "getMergeCursor(), cursors[] is empty, return null");
            return null;
        }
        int count = 0;
        for(Cursor cursor : cursors){
            if(null != cursor){
                cursor.setNotificationUri(
                        getContext().getContentResolver(), MmsSms.CONTENT_URI);
                ++count;
            }
        }
        if(0 == count){
            Log.w(LOG_TAG, "getMergeCursor(), cursors count is 0, return null");
            return null;
        }
        if(cursors.length == count){
            Cursor mergeCursor = new MergeCursor(cursors);
            return mergeCursor;
        }
        // 0 < count < cursors.length

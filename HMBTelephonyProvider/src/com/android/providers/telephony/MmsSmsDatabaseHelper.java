...
import com.zzz.provider.Telephony.CellBroadcasts;
import com.zzz.provider.Telephony.Mms;
import com.zzz.provider.Telephony.Mms.Addr;
import com.zzz.provider.Telephony.Mms.Part;
import com.zzz.provider.Telephony.Mms.Rate;
...
import com.zzz.provider.Telephony.ThreadSettings;
import com.zzz.provider.Telephony.Threads;
import com.zzz.provider.Telephony.WapPush;
...
public class MmsSmsDatabaseHelper extends SQLiteOpenHelper {
    private static final String TAG = "MmsProvider/DBHelper";
    ...
    private static final String PDU_UPDATE_THREAD_STATUS =
                        ...
                        "AND threads._id=new.thread_id AND (new.reject = 0) ; ";
    // @}
    ...
    //=============================================================================================================
	//====================================lichao add trigger codes begin===========================================
	//=============================================================================================================
    //---------------------lichao add trigger for sub_id(111) begin ------------------------------
    private static final String SMS_PDU_UPDATE_THREAD_SUB_ID_ON_NEW =
            " UPDATE threads SET sub_id =" +
                    "    ( SELECT sub_id FROM " +
                    "        ( SELECT sub_id, thread_id, date * 1000 AS date FROM pdu " +
                    "             WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (thread_id = new.thread_id) AND (reject = 0) " +
                    "          UNION " +
                    "          SELECT sub_id, thread_id, date FROM sms " +
                    "             WHERE thread_id = new.thread_id AND (reject = 0) " +
                    "        ) " +
                    "      WHERE thread_id = new.thread_id ORDER BY date DESC LIMIT 1 "+
                    "    ) " +
                    " WHERE threads._id = new.thread_id; ";

    private static final String SMS_PDU_UPDATE_THREAD_SUB_ID_ON_OLD =
            " UPDATE threads SET sub_id =" +
                    "    ( SELECT sub_id FROM " +
                    "        ( SELECT sub_id, thread_id, date * 1000 AS date FROM pdu " +
                    "             WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (thread_id = old.thread_id) AND (reject = 0) " +
                    "          UNION " +
                    "          SELECT sub_id, thread_id, date FROM sms " +
                    "             WHERE thread_id = old.thread_id AND (reject = 0) " +
                    "        ) " +
                    "      WHERE thread_id = old.thread_id ORDER BY date DESC LIMIT 1 "+
                    "    ) " +
                    " WHERE threads._id = " + "old.thread_id; ";
    //---------------------lichao add trigger for sub_id(111) end --------------------------------

    //---------------------lichao add trigger for latest_is_mms(222) begin ------------------------------
    private static final String SMS_PDU_UPDATE_THREAD_LATEST_IS_MMS_ON_NEW =
            " UPDATE threads SET latest_is_mms =" +
                    "    (SELECT is_mms FROM" +
                    "        (SELECT 1 AS is_mms, date * 1000 AS date, thread_id FROM pdu " +
                    "         WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (thread_id = "
                    + "new.thread_id) AND (reject = 0) " +
                    "         UNION SELECT 0 AS is_mms, date, thread_id FROM sms " +
                    "         WHERE thread_id = " + "new.thread_id AND (reject = 0) )" +
                    "     WHERE thread_id = " + "new.thread_id ORDER BY date DESC LIMIT 1) " +
                    "  WHERE threads._id = " + "new.thread_id; ";

    private static final String SMS_PDU_UPDATE_THREAD_LATEST_IS_MMS_ON_OLD =
            " UPDATE threads SET latest_is_mms =" +
                    "    (SELECT is_mms FROM" +
                    "        (SELECT 1 AS is_mms, date * 1000 AS date, thread_id FROM pdu " +
                    "         WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (thread_id = "
                    + "old.thread_id) AND (reject = 0) " +
                    "         UNION SELECT 0 AS is_mms, date, thread_id FROM sms " +
                    "         WHERE thread_id = " + "old.thread_id AND (reject = 0) )" +
                    "     WHERE thread_id = " + "old.thread_id ORDER BY date DESC LIMIT 1) " +
                    "  WHERE threads._id = " + "old.thread_id; ";
    //---------------------lichao add trigger for latest_is_mms(222) end --------------------------------

    //---------------------lichao add trigger for latest_is_notifi(333) begin ------------------------------
    private static final String SMS_PDU_UPDATE_THREAD_LATEST_IS_NOTIFI_ON_NEW =
            "  UPDATE threads SET latest_is_notifi = " +
                    "    CASE (SELECT m_type FROM " +
                    "             (SELECT m_type, date * 1000 AS date, thread_id FROM pdu " +
                    "                 WHERE (m_type=132 OR m_type=130 OR m_type=128)  AND (thread_id = new.thread_id) " +
                    "                        AND (reject = 0) " +
                    "              UNION SELECT 0 AS m_type, date, thread_id FROM sms " +
                    "                 WHERE thread_id = new.thread_id AND (reject = 0) " +
                    "             )" +
                    "          WHERE thread_id = " + "new.thread_id" + " ORDER BY date DESC LIMIT 1" +
                    "         )" +
    ...
    private static final String SMS_PDU_UPDATE_THREAD_LATEST_IS_NOTIFI_ON_OLD =
            "  UPDATE threads SET latest_is_notifi = " +
                    "    CASE (SELECT m_type FROM " +
                    "             (SELECT m_type, date * 1000 AS date, thread_id FROM pdu " +
                    "                 WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (thread_id = old.thread_id) " +
                    "                        AND (reject = 0) " +
                    "              UNION SELECT 0 AS m_type, date, thread_id FROM sms " +
                    "                 WHERE thread_id = old.thread_id AND (reject = 0) " +
                    "             )" +
                    ...
                    "    END" +
                    "  WHERE threads._id = old.thread_id; ";
    //---------------------lichao add trigger for latest_is_notifi(333) end --------------------------------

    //---------------------lichao add trigger for latest_has_attach(444) begin ------------------------------
    //每次新建或者更新 part 表数据时候，都更新对应的 pdu 表数据里的 has_attach 信息
    //便于在删除一条pdu数据后，可以直接获取最新一条pdu数据的 has_attach 信息作为latest_has_attach
    private static final String PART_UPDATE_PDU_HAS_ATTACH_ON_NEW =
            "  UPDATE pdu SET has_attach = " +
                    "             CASE (SELECT COUNT(*) FROM part " +
                    "                       WHERE part.mid=new.mid AND part.ct != 'text/plain' AND part.ct != 'application/smil' ) " +
                    "             WHEN 0 " +
                    "             THEN 0 " +
                    "             ELSE 1 " +
                    "             END " +
                    " WHERE (m_type=132 OR m_type=130 OR m_type=128) AND _id=new.mid; ";

    //恢复被拦截的sms或pdu时候
    private static final String SMS_PDU_UPDATE_THREAD_LATEST_HAS_ATTACH_ON_UPDATE =
            "  UPDATE threads SET latest_has_attach = " +
                    "   ( SELECT has_attach FROM" +
                    "       ( SELECT date * 1000 AS date, has_attach, thread_id FROM pdu " +
                    "             WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (thread_id = new.thread_id) AND (reject = 0) " +
                    "                   AND (reject = 0) " +
                    "         UNION " +
                    "         SELECT date, 0 AS has_attach, thread_id FROM sms" +
                    "             WHERE thread_id = new.thread_id AND (reject = 0) ORDER BY date DESC LIMIT 1" +
                    "       )" +
                    "     WHERE thread_id = new.thread_id ORDER BY date DESC LIMIT 1" +
                    "   ) " +
                    " WHERE threads._id=new.thread_id; ";

    //part表直接更新latest_has_attach(对应的pdu不被拦截的情况下)
    //part表里除了ct等于text/plain和application/smil之外没有其他行了，说明就是纯文本彩信（不含附件）
    private static final String PART_UPDATE_THREAD_LATEST_HAS_ATTACH_ON_NEW =
            "  UPDATE threads SET latest_has_attach = " +
                    "   (SELECT latest_has_attach FROM" +
                    "      ( SELECT date * 1000 AS date, " +
                    "             CASE (SELECT COUNT(*) FROM part JOIN pdu ON pdu._id=new.mid AND part.mid=pdu._id " +
                    "                                                         AND (pdu.reject = 0) " +
                    ...
                    "             ELSE 1 " +
                    "             END " +
                    "             AS latest_has_attach, thread_id FROM pdu " +
                    "          WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (reject = 0) " +
                    "        UNION SELECT date, 0 AS latest_has_attach, thread_id FROM sms " +
                    "                  WHERE thread_id = (SELECT thread_id FROM pdu WHERE pdu._id=new.mid AND (pdu.reject = 0)) " +
                    "        ORDER BY date DESC LIMIT 1 " +
                    "       )" +
                    "     WHERE thread_id = (SELECT thread_id FROM pdu WHERE pdu._id=new.mid) ORDER BY date DESC LIMIT 1 " +
                    "   ) " +
                    "  WHERE threads._id = (SELECT thread_id FROM pdu WHERE pdu._id=new.mid); ";

    //删除sms或者pdu时候，从最新的pdu获取has_attach的值作为latest_has_attach
    private static final String SMS_PDU_UPDATE_THREAD_LATEST_HAS_ATTACH_ON_OLD =
            "  UPDATE threads SET latest_has_attach = " +
                    "   ( SELECT has_attach FROM" +
                    "       ( SELECT date * 1000 AS date, has_attach, thread_id FROM pdu " +
                    "             WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (thread_id = old.thread_id) " +
                    ...
                    "         UNION " +
                    "         SELECT date, 0 AS has_attach, thread_id FROM sms" +
                    "             WHERE thread_id = old.thread_id AND (reject = 0) ORDER BY date DESC LIMIT 1" +
                    "       )" +
                    "     WHERE thread_id = old.thread_id ORDER BY date DESC LIMIT 1" +
                    "   ) " +
                    " WHERE threads._id=old.thread_id; ";
    //---------------------lichao add trigger for latest_has_attach(444) end --------------------------------

    //---------------------lichao add trigger for latest_pdu_text(555) begin ----------------------------
    //PART表更新pdu_text后 PDU表再去更新threads表的latest_pdu_text，第一次保存长文本彩信草稿时候时间会来不及
    //所以改为PART表直接跟新threads表的latest_pdu_text
    //恢复被拦截的彩信时候，mms表中只有reject字段更新，而这应当引起latest_pdu_text的更新
    private static final String PDU_UPDATE_THREAD_LATEST_PDU_TEXT_ON_UPDATE =
            " UPDATE threads SET latest_pdu_text =" +
                    "    (SELECT pdu_text FROM" +
                    "        (SELECT pdu_text, thread_id, date * 1000 AS date FROM pdu " +
                    "             WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (thread_id = new.thread_id) AND (reject = 0) " +
                    "        ) " +
                    "     WHERE thread_id = new.thread_id ORDER BY date DESC LIMIT 1 " +
                    "    ) " +
    ...
    //part表的text更新pdu表的pdu_text，用于删除pdu时候，直接去最新的pdu表里获取pdu_text作为threads表的latest_pdu_text
    private static final String PART_UPDATE_PDU_TEXT_ON_NEW =
            "  UPDATE pdu SET pdu_text = new.text " +
                    "    WHERE (m_type=132 OR m_type=130 OR m_type=128) " +
                    "          AND new.ct = 'text/plain' " +
                    "          AND pdu._id=new.mid; ";

    //PART表直接跟新threads表的latest_pdu_text
    private static final String PART_UPDATE_THREAD_LATEST_PDU_TEXT_ON_NEW =
            "  UPDATE threads SET latest_pdu_text = " +
                    "   (SELECT pdu_text FROM" +
                    "      ( SELECT date * 1000 AS date, " +
                    "             (SELECT text FROM part JOIN pdu ON pdu._id=new.mid AND part.mid=pdu._id " +
                    "                       WHERE part.ct = 'text/plain' ) " +
                    "             AS pdu_text, thread_id FROM pdu " +
                    "          WHERE (m_type=132 OR m_type=130 OR m_type=128) " +
                    "                AND thread_id = (SELECT thread_id FROM pdu WHERE pdu._id=new.mid AND (pdu.reject = 0) ) " +
                    "       )" +
                    "     WHERE thread_id = (SELECT thread_id FROM pdu WHERE pdu._id=new.mid) ORDER BY date DESC LIMIT 1 " +
                    "   ) " +
                    "  WHERE threads._id = (SELECT thread_id FROM pdu WHERE pdu._id=new.mid); ";

    private static final String PDU_UPDATE_THREAD_LATEST_PDU_TEXT_ON_OLD =
            " UPDATE threads SET latest_pdu_text =" +
                    "    (SELECT pdu_text FROM" +
                    "        (SELECT pdu_text, thread_id, date * 1000 AS date FROM pdu " +
                    "             WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (thread_id = old.thread_id) AND (reject = 0) " +
                    ...
                    "    ) " +
                    " WHERE threads._id = old.thread_id; ";
    //---------------------lichao add trigger for latest_pdu_text(555) end ------------------------------

    //---------------------lichao add trigger for latest_has_error(666) begin ------------------------------
    //这里应该不需要加入(sms.reject = 0)的判断，因为被拦截的短信sms.has_error不会影响threads.latest_has_error
    //这个逻辑适用于非群发短信, 所以约束条件把 group_id = 0 加进去
    private static final String SMS_UPDATE_SMS_HAS_ERROR_ON_UPDATE =
            "  UPDATE sms SET has_error = " +
                    "    CASE " +
                    "    WHEN ( new.type = 5 OR new.status >= 64 ) " +
                    "    THEN 1 " +
                    "    ELSE 0 " +
                    "    END " +
                    " WHERE _id=new._id; ";

    //仅仅监听group_id == -1短信的fails_count的变化（即仅仅针对群发短信头目）
    //第一步，更新group_id == -1短信自身的 has_error
    private static final String SMS_UPDATE_SMS_ITSELF_HAS_ERROR_ON_UPDATE =
            "  UPDATE sms SET has_error = " +
                    "    CASE " +
                    "    WHEN ( new.fails_count > 0 ) " +
                    "    THEN 1 " +
                    "    ELSE 0 " +
                    "    END " +
                    " WHERE _id=new._id; ";
    //第二步，更新同一群组里其他短信(group_id等于那条group_id == -1短信的_id)的 has_error
    private static final String SMS_UPDATE_GROUP_SMS_HAS_ERROR_ON_UPDATE =
            "  UPDATE sms SET has_error = " +
                    "    CASE " +
                    "    WHEN ( new.fails_count > 0 ) " +
                    "    THEN 1 " +
                    "    ELSE 0 " +
                    "    END " +
                    " WHERE group_id=new._id; ";

    // Refer to TRIGGER update_threads_error_on_update_mms
    private static final String PENDING_MSGS_UPDATE_PDU_HAS_ERROR_CONSTRAINTS =
            "  WHEN   ( OLD.err_type < 10 AND NEW.err_type >= 10 " +
                    "   AND NEW.proto_type = " + MmsSms.MMS_PROTO +
                    "   AND NEW.msg_type = " + PduHeaders.MESSAGE_TYPE_SEND_REQ +
                    " )" +
                    " OR (OLD.err_type >= 10 AND NEW.err_type < 10) ";

    //这里应该不需要加入(pdu.reject = 0)的判断，因为被拦截的彩信pdu.has_error不会影响threads.latest_has_error
    // Refer to TRIGGER update_threads_error_on_update_mms
    private static final String PENDING_MSGS_UPDATE_PDU_HAS_ERROR_ON_UPDATE =
            "  UPDATE pdu SET has_error = " +
                    "    CASE " +
                    "    WHEN ( new.err_type >= 10 )" +
                    "    THEN 1 " +
                    "    ELSE 0 " +
                    "    END " +
                    " WHERE (m_type=132 OR m_type=130 OR m_type=128) AND pdu._id=new.msg_id; ";

    // 更新pdu的has_error不需要这个约束条件
    // 更新threads的error需要触发条件仅仅是为了让 error-1,没有error+1的情况
    // 而has_error需要根据实际情况赋值为1或者0
    /*private static final String PDU_UPDATE_PDU_HAS_ERROR_CONSTRAINTS =
            "  WHEN    (  (OLD.msg_box = 4 OR OLD.msg_box = 5) " +
                    "     AND NEW.msg_box != 4 AND NEW.msg_box != 5" +
                    "  ) ";*/

    //lichao add this like TRIGGER update_threads_error_on_move_mms
    private static final String PDU_UPDATE_PDU_HAS_ERROR_ON_UPDATE =
            "  UPDATE pdu SET has_error = " +
                    " (  CASE " +
                    "      WHEN ( NEW._id IN (SELECT DISTINCT msg_id FROM pending_msgs WHERE err_type >= 10)) " +
                    "      THEN 1 " +
                    "      ELSE 0 " +
                    "      END " +
                    " ) " +
                    " WHERE (m_type=132 OR m_type=130 OR m_type=128) AND _id=new._id; ";

    private static final String SMS_PDU_UPDATE_THREAD_LATEST_HAS_ERROR_ON_NEW =
            " UPDATE threads SET latest_has_error =" +
                    "    ( SELECT has_error FROM " +
                    "        ( SELECT has_error, thread_id, date * 1000 AS date FROM pdu " +
                    "             WHERE (m_type=132 OR m_type=130 OR m_type=128) " +
                    "                   AND (thread_id = new.thread_id) " +
                    "                   AND (reject = 0) " +
                    "          UNION " +
                    "          SELECT has_error, thread_id, date FROM sms " +
                    "          WHERE thread_id = new.thread_id AND (reject = 0) " +
                    "        ) " +
                    "      WHERE thread_id = new.thread_id ORDER BY date DESC LIMIT 1 "+
                    "    ) " +
    ...
    private static final String SMS_PDU_UPDATE_THREAD_LATEST_HAS_ERROR_ON_OLD =
            " UPDATE threads SET latest_has_error =" +
                    "    ( SELECT has_error FROM " +
                    "        ( SELECT has_error, thread_id, date * 1000 AS date FROM pdu " +
                    "             WHERE (m_type=132 OR m_type=130 OR m_type=128) " +
  ...
  /// M: Add for CT, Update thread date by the latest sms date sent time. @{
    ...
    private static final String SMS_UPDATE_THREAD_DATE_SENT =
          " UPDATE threads SET date_sent =" +
                  ...
                  "             WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (thread_id = new." + Sms.THREAD_ID + ") AND (reject = 0) " +
	...
	//lichao add 2 " AND (reject = 0)" and modify “ORDER BY”
    ...
    private static final String UPDATE_THREAD_READ_COUNT =
            "  UPDATE threads SET readcount = " +
                    ...
                    "            WHERE ((read=1) AND thread_id = new.thread_id AND (type != 3) AND (reject = 0)) " +
                    "        UNION " +
                    "        SELECT DISTINCT date * 1000 AS normalized_date, pdu._id, read FROM pdu " +
                    "            LEFT JOIN pending_msgs ON pdu._id = pending_msgs.msg_id " +
                    "            WHERE ( (read=1) AND thread_id = new.thread_id AND msg_box != 3 " +
                    "                    AND (msg_box != 3) " +
                    ...
                    "            WHERE ( (read=1) AND thread_id = new.thread_id ) " +
    ...
    private static final String UPDATE_THREAD_READ_COUNT_OLD =
            "  UPDATE threads SET readcount = " +
                    "  (SELECT count(_id) FROM " +
                    "    ( SELECT DISTINCT date * 1 AS normalized_date, _id, read FROM sms " +
                    "          WHERE ((read=1) AND thread_id = old.thread_id AND (type != 3) AND (reject = 0)) " +
                    "      UNION " +
                    "      SELECT DISTINCT date * 1000 AS normalized_date, pdu._id, read FROM pdu " +
                    "          LEFT JOIN pending_msgs ON pdu._id = pending_msgs.msg_id " +
                    "          WHERE ( (read=1) AND thread_id = old.thread_id AND msg_box != 3 " +
                    "                AND (msg_box != 3) " +
                    "                AND (m_type = 128 OR m_type = 132 OR m_type = 130) " +
                    "                AND (reject = 0) " +
                    "              ) " +
                    "      UNION " +
                    "      SELECT DISTINCT date * 1 AS normalized_date, _id, read FROM cellbroadcast " +
                    "          WHERE ((read=1) AND thread_id = old.thread_id) " +
                    "      ORDER BY normalized_date ASC " +
                    "    ) " +
                    "  ) " +
                    "  WHERE threads._id = old.thread_id; ";
...
//            "    END" +
//            "  WHERE threads._id = new." + Sms.THREAD_ID + "; ";
    //Sms.READ等于0表示已读为为false,即包含未读信息
	//lichao add 2 " AND (reject = 0)"
    private static final String SMS_UPDATE_THREAD_READ_BODY =
            "  UPDATE threads SET read = " +
                    "    CASE( (SELECT COUNT(*) FROM sms " +
                    "               WHERE " + Sms.READ + " = 0 " +
                    "                 AND " + Sms.THREAD_ID + " = threads._id " +
                    "                 AND (reject = 0) " +
                    "          )" +
                    "          + " +
                    "          (SELECT COUNT(*) FROM " + MmsProvider.TABLE_PDU +
                    "               WHERE " + Mms.READ + " = 0 " +
    ...
    //HB. Comments :  , Engerineer : lichao , Date : 17-7-6 , begin
    private static final String SMS_UPDATE_THREAD_FAILED_UNREAD_ON_NEW =
            "  UPDATE threads SET failed_unread = " +
                    "    CASE( (SELECT COUNT(*) FROM sms " +
                    "               WHERE failed_unread = 1 " +
    ...
    //Sms.TYPE: 3=Mms.MESSAGE_BOX_DRAFTS
	//lichao add 2 " AND (reject = 0)"
    ...
    //lichao add for don't show conversation when all messages was blocked begin
    // refer to UPDATE_THREAD_COUNT_ON_NEW
    private static final String SMS_PDU_UPDATE_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_ON_NEW =
            "  UPDATE threads SET msg_count_in_draft = " +
                    "     (SELECT COUNT(sms._id) FROM sms LEFT JOIN threads " +
                    "          ON threads._id = " + Sms.THREAD_ID +
                    "      WHERE thread_id = new.thread_id" +
                    //"        AND sms." + Sms.TYPE + " != 3 " +//3=Sms.MESSAGE_TYPE_DRAFT
                    "        AND (reject = 0) " +
                    "     )" +
                    "     + " +
                    "     (SELECT COUNT(pdu._id) FROM pdu LEFT JOIN threads " +
                    ...
                    "      WHERE " + Mms.THREAD_ID + " = new.thread_id" +
                    "        AND (m_type=132 OR m_type=130 OR m_type=128)" +
                    //"        AND " + Mms.MESSAGE_BOX + " != 3 " +//3=Mms.MESSAGE_BOX_DRAFTS
                    "        AND (reject = 0) " +
                    "     )" +
                    "  WHERE threads._id = new.thread_id; ";

    // refer to UPDATE_THREAD_COUNT_ON_OLD
	//加到 upgradeDatabaseToVersion560000() 里实际上没任何用了
    /*private static final String SMS_PDU_UPDATE_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_ON_OLD =
            "  UPDATE threads SET msg_count_in_draft = " +
                    "     (SELECT COUNT(sms._id) FROM sms LEFT JOIN threads " +
                    "          ON threads._id = " + Sms.THREAD_ID +
                    ...
                    "     )" +
                    "  WHERE threads._id = old.thread_id; ";*/
    //lichao add for don't show conversation when all messages was blocked end
    ...
    private static final String SMS_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE =
            ...
            UPDATE_THREAD_READ_COUNT +
                    SMS_PDU_UPDATE_THREAD_SUB_ID_ON_NEW + //lichao add in 2017-04-21
                    SMS_PDU_UPDATE_THREAD_LATEST_IS_MMS_ON_NEW + //lichao add in 2017-05-16
                    SMS_PDU_UPDATE_THREAD_LATEST_IS_NOTIFI_ON_NEW + //lichao add
                    SMS_PDU_UPDATE_THREAD_LATEST_HAS_ATTACH_ON_UPDATE + //lichao add
                    //sms no need at all update latest_pdu_text
                    //SMS_PDU_UPDATE_THREAD_LATEST_HAS_ERROR_ON_NEW + //lichao add
                    SMS_PDU_UPDATE_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_ON_NEW +//lichao add
                    SMS_UPDATE_THREAD_FAILED_UNREAD_ON_NEW + //lichao add in 2017-07-06
            " END;";
    ...
    private static final String SMS_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_INSERT =
            ...
            UPDATE_THREAD_READ_COUNT +
                    SMS_PDU_UPDATE_THREAD_SUB_ID_ON_NEW + //lichao add in 2017-04-21
                    SMS_PDU_UPDATE_THREAD_LATEST_IS_MMS_ON_NEW + //lichao add in 2017-05-16
                    SMS_PDU_UPDATE_THREAD_LATEST_IS_NOTIFI_ON_NEW + //lichao add in 2017-05-16
                    SMS_PDU_UPDATE_THREAD_LATEST_HAS_ERROR_ON_NEW + //lichao add in 2017-05-27
                    SMS_PDU_UPDATE_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_ON_NEW + //lichao add
                    SMS_UPDATE_THREAD_FAILED_UNREAD_ON_NEW + //lichao add in 2017-07-06
            " END;";
    /// @}

    //lichao add "(...) AND (new." + Mms.REJECT + " = 0) "
    private static final String PDU_UPDATE_THREAD_CONSTRAINTS =
            " WHEN    ( new." + Mms.MESSAGE_TYPE + "=" + PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF +
                    "   OR new." + Mms.MESSAGE_TYPE + "=" + PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND +
                    "   OR new." + Mms.MESSAGE_TYPE + "=" + PduHeaders.MESSAGE_TYPE_SEND_REQ +
                    " ) " +
                    " AND (new." + Mms.REJECT + " = 0) ";
...
//            "  WHERE threads._id = new." + Mms.THREAD_ID + "; ";
    ...
    // Mms.READ理解为是否所有已读的意思, 等于0表示已读为为false,即包含未读信息
	//lichao add 2 "AND (reject = 0)"
    private static final String PDU_UPDATE_THREAD_READ_BODY =
            "  UPDATE threads SET read = " +
    ...
    //HB. Comments :  , Engerineer : lichao , Date : 17-7-6 , begin
    private static final String PDU_UPDATE_THREAD_FAILED_UNREAD_ON_NEW =
            "  UPDATE threads SET failed_unread = " +
                    "    CASE (SELECT COUNT(*) FROM pdu" +
                    "            WHERE failed_unread = 1" +
                    "                  AND thread_id = threads._id " +
                    "                  AND (m_type=132 OR m_type=130 OR m_type=128)" +
                    "                  AND (reject = 0) " +
...
//            " AND threads._id = new." + Mms.THREAD_ID + "; " +
            ...
            UPDATE_THREAD_READ_COUNT +
                    ...
                    SMS_PDU_UPDATE_THREAD_LATEST_IS_NOTIFI_ON_NEW + //lichao add
                    SMS_PDU_UPDATE_THREAD_LATEST_HAS_ATTACH_ON_UPDATE + //lichao add
                    PDU_UPDATE_THREAD_LATEST_PDU_TEXT_ON_UPDATE + //lichao add
                    //SMS_PDU_UPDATE_THREAD_LATEST_HAS_ERROR_ON_NEW + //lichao add
                    SMS_PDU_UPDATE_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_ON_NEW + //lichao add
                    PDU_UPDATE_THREAD_FAILED_UNREAD_ON_NEW + //lichao add in 2017-07-06
            " END;";
    ...
    /// @{
	//lichao add AND 2 "AND (new.reject = 0)"
	//实际跟SMS_UPDATE_THREAD_DATE一样，因为Mms.THREAD_ID和Sms.THREAD_ID都等于"thread_id"
    private static final String PDU_UPDATE_THREAD_DATE =
            "UPDATE threads SET date = " +
            "    (SELECT date FROM" +
            "        (SELECT date * 1000 AS date, thread_id FROM pdu " +
            "         WHERE (new.m_type=132 OR new.m_type=130 OR new.m_type=128) AND (thread_id = "
            + "new." + Mms.THREAD_ID + ") AND (new.reject = 0) " +
            "         UNION SELECT date, thread_id FROM sms " +
            "         WHERE thread_id = " + "new." + Mms.THREAD_ID + " AND (new.reject = 0) ) " +
...
//            + Mms.THREAD_ID + "; " +
            ...
            PDU_UPDATE_THREAD_STATUS +
                    SMS_PDU_UPDATE_THREAD_SUB_ID_ON_NEW + //lichao add in 2017-04-21
                    SMS_PDU_UPDATE_THREAD_LATEST_IS_MMS_ON_NEW + //lichao add in 2017-05-17
                    SMS_PDU_UPDATE_THREAD_LATEST_IS_NOTIFI_ON_NEW + //lichao add in 2017-05-17
                    SMS_PDU_UPDATE_THREAD_LATEST_HAS_ERROR_ON_NEW + //lichao add in 2017-05-27
                    SMS_PDU_UPDATE_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_ON_NEW + //lichao add
                    PDU_UPDATE_THREAD_FAILED_UNREAD_ON_NEW + //lichao add in 2017-07-06
            "  END;";
	...
	//lichao add 3"AND (reject = 0) " and 3 "WHERE reject = 0 "
    private static final String UPDATE_THREAD_SNIPPET_SNIPPET_CS_ON_DELETE =
            "  UPDATE threads SET snippet = " +
                    ...
                    "            WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (reject = 0) " +
                    "         UNION " +
                    "         SELECT date, body AS snippet, thread_id FROM sms WHERE (reject = 0) " +
                    "      )" +
                    "      WHERE thread_id = OLD.thread_id ORDER BY date DESC LIMIT 1" +
                    "   ) " +
                    " WHERE threads._id = OLD.thread_id; " +
            "  UPDATE threads SET snippet_cs = " +
                    ...
                    "          WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (reject = 0) " +
                    "      UNION SELECT date, 0 AS snippet_cs, thread_id FROM sms WHERE (reject = 0) " +
                    "     )" +
                    "     WHERE thread_id = OLD.thread_id ORDER BY date DESC LIMIT 1" +
                    "   ) " +
                    "  WHERE threads._id = OLD.thread_id; " +
            "  UPDATE threads SET date_sent = " +
                    ...
                    "       (SELECT date_sent * 1000 as date_sent, date * 1000 AS date, thread_id FROM pdu " +
                    "          WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (reject = 0) " +
                    "        UNION SELECT date_sent, date, thread_id FROM sms WHERE (reject = 0) " +
                    "       )" +
                    "       WHERE thread_id = OLD.thread_id ORDER BY date DESC LIMIT 1" +
                    "   ) " +
                    "  WHERE threads._id = OLD.thread_id; ";
    ...
    /// M: Add for ip message @{
	//lichao add 6"AND (reject = 0) " and 6 "WHERE reject = 0 "
    private static final String UPDATE_THREAD_LIDATE_LISNIPPET_LISNIPPETSC_ON_UPDATE_AND_DELETE =
            "  UPDATE threads SET li_date = " +
                    ...
                    "                WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (reject = 0) " +
                    "            UNION SELECT date, body AS snippet, thread_id, locked FROM sms WHERE reject = 0 " +
                    "           )" +
                    ...
                    "    ELSE " +
                    "      (SELECT snippet_cs FROM" +
                    "        (SELECT date * 1000 AS date, sub_cs AS snippet_cs, thread_id, locked FROM pdu " +
                    "             WHERE (m_type=132 OR m_type=130 OR m_type=128) AND (reject = 0) " +
                    "         UNION SELECT date, 0 AS snippet_cs, thread_id, locked FROM sms WHERE reject = 0 " +
                    "        )" +
                    "       WHERE thread_id = OLD.thread_id AND locked=1 ORDER BY date DESC LIMIT 1" +
                    "      )" +
                    "    END" +
    ...
    private static final String PART_UPDATE_THREADS_ON_INSERT_TRIGGER =
            ...
            "   (SELECT pdu.thread_id FROM part JOIN pdu ON pdu._id=part.mid AND (pdu.reject = 0) " +
    ...
    private static final String PART_UPDATE_THREADS_ON_DELETE_TRIGGER =
            ...
            "     ON pdu._id=old.mid AND part.mid=pdu._id AND (pdu.reject = 0) " +
            "     WHERE part.ct != 'text/plain' AND part.ct != 'application/smil') " +
            "   WHEN 0 THEN 0 " +
            "   ELSE 1 " +
            "   END " +
    ...
    private static final String PDU_UPDATE_THREADS_ON_UPDATE_TRIGGER =
            ...
            " BEGIN " +
            "  UPDATE threads SET has_attachment=1 WHERE _id IN " +
            "   (SELECT pdu.thread_id FROM part JOIN pdu on (pdu.reject = 0) " +
            "     WHERE part.ct != 'text/plain' AND part.ct != 'application/smil' " +
    ...
    //lichao refactor 
    private static final String CB_UPDATE_THREAD_READ_BODY =
            "  UPDATE threads SET read = "
                    + "    CASE (SELECT COUNT(*)"
                    + "          FROM cellbroadcast"
                    + "          WHERE "+ Telephony.SmsCb.READ+ " = 0"
                    + "            AND "+ Telephony.SmsCb.THREAD_ID+ " = threads._id)"
                    + "      WHEN 0 THEN 1"
                    + "      ELSE 0"
                    + "    END"
                    + "  WHERE threads._id = new." + Telephony.SmsCb.THREAD_ID + "; ";

    //lichao refactor 
    private static final String CB_UPDATE_THREAD_READ_BODY_DELETE =
            "  UPDATE threads SET read = "
                    + "    CASE (SELECT COUNT(*)"
                    + "          FROM cellbroadcast"
                    + "          WHERE "+ Telephony.SmsCb.READ+ " = 0"
    ...
    //lichao refactor 
    private static final String CB_UPDATE_THREAD_COUNT_ON_NEW =
            "UPDATE threads SET message_count =" +
                    "  (  SELECT COUNT(cellbroadcast._id) FROM cellbroadcast LEFT JOIN threads " +
                    "         ON threads._id = " + Telephony.SmsCb.THREAD_ID +
                    "         WHERE " + Telephony.SmsCb.THREAD_ID + " = new.thread_id " +
                    "  )" +
                    "  WHERE threads._id = new.thread_id; ";

    //HB. Comments :  , Engerineer : lichao , Date : 17-6-21 , begin
    private static final String CB_UPDATE_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_ON_NEW =
            "UPDATE threads SET msg_count_in_draft =" +
                    "  (  SELECT COUNT(cellbroadcast._id) FROM cellbroadcast LEFT JOIN threads " +
                    "         ON threads._id = " + Telephony.SmsCb.THREAD_ID +
                    "         WHERE " + Telephony.SmsCb.THREAD_ID + " = new.thread_id " +
    ...
    private static final String CB_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE = "BEGIN"
            ...
            + CB_UPDATE_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_ON_NEW //lichao add
    ...
    //HB. Comments :  , Engerineer : lichao , Date : 17-6-21 , begin
    private static final String CB_UPDATE_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_ON_OLD =
            "UPDATE threads SET msg_count_in_draft =" +
                    "  (  SELECT COUNT(cellbroadcast._id) FROM cellbroadcast LEFT JOIN threads " +
                    "         ON threads._id = " + Telephony.SmsCb.THREAD_ID +
                    "         WHERE " + Telephony.SmsCb.THREAD_ID + " = old.thread_id " +
    ...
    //HB. Comments :  , Engerineer : lichao , Date : 17-6-21 , begin
    // refor to SMS_UPDATE_THREAD_MESSAGE_COUNT_CT
    private static final String SMS_UPDATE_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_CT =
            " UPDATE threads SET msg_count_in_draft = " +
                    " (  SELECT count(_id) FROM " +
                    "   (  SELECT _id FROM sms WHERE thread_id = new.thread_id " +
                    "      UNION " +
                    "      SELECT _id FROM pdu WHERE thread_id = new.thread_id " +
                    "                                AND (m_type=132 OR m_type=130 OR m_type=128)" +
                    "   )" +
                    " ) " +
                    " WHERE _id = new.thread_id;";
    ...
    //lichao refactor in 2017-06-21
    private static final String SMS_UPDATE_THREAD_MESSAGE_COUNT_CT =
            " UPDATE threads SET message_count = " +
                    " (  SELECT count(_id) FROM " +
                    "   (  SELECT _id FROM sms WHERE thread_id = new.thread_id " +
                    "      UNION " +
    ...
    //lichao refactor 
    private static final String SMS_UPDATE_THREAD_SNIPPET_CT =
            "UPDATE threads SET snippet = " +
                    " ( SELECT snippet FROM " +
                    "   ( SELECT max(date), snippet FROM " +
                    "       ( SELECT max(date) * 1000 as date, sub as snippet FROM pdu " +
                    "           WHERE (thread_id = new.thread_id AND (m_type=132 OR m_type=130 OR m_type=128) ) " +
                    "         UNION SELECT max(date) as date, body as snippet FROM sms " +
                    "           WHERE (thread_id = new.thread_id)" +
                    "       ) " +
            ...
            "UPDATE threads SET snippet_cs = " +
                    " ( SELECT snippet_cs FROM " +
                    "      ( SELECT max(date), snippet_cs FROM " +
                    "          ( SELECT max(date) * 1000 as date, sub_cs as snippet_cs FROM pdu " +
                    "              WHERE (thread_id = new.thread_id AND (m_type=132 OR m_type=130 OR m_type=128) ) " +
                    "            UNION SELECT max(date), 0 as snippet_cs FROM sms " +
                    "              WHERE (thread_id = new.thread_id)" +
                    "          )" +
                    "      )" +
                    " ) " +
    ...
    //lichao refactor in 2017-06-16 begin
    private static final String SMS_UPDATE_THREAD_ERROR_CT =
            "UPDATE threads SET error = " +
                    "  (  SELECT count(_id) FROM " +
                    "     (  SELECT _id FROM sms WHERE thread_id = new.thread_id AND type = 5 " +
                    "        UNION " +
                    "        SELECT _id FROM pdu WHERE thread_id = new.thread_id AND st = 135 " +
                    "                                  AND (m_type=132 OR m_type=130 OR m_type=128)" +
                    "     )" +
                    "  ) " +
                    "  WHERE _id = new.thread_id;";
    //lichao refactor in 2017-06-16 end
    ...
    //HB. Comments :  , Engerineer : lichao , Date : 17-6-21 , begin
    // refor to SMS_UPDATE_THREAD_MESSAGE_COUNT_CT
    private static final String SMS_UPDATE_OLD_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_CT =
            " UPDATE threads SET msg_count_in_draft = " +
                    " (  SELECT count(_id) FROM " +
                    "   (  SELECT _id FROM sms WHERE thread_id = old.thread_id " +
                    "      UNION " +
                    "      SELECT _id FROM pdu WHERE thread_id = old.thread_id " +
                    "                                AND (m_type=132 OR m_type=130 OR m_type=128)" +
                    "   )" +
                    " ) " +
                    " WHERE _id = old.thread_id;";
    ...
    //lichao refactor
    private static final String SMS_UPDATE_OLD_THREAD_ERROR_CT =
            "UPDATE threads SET error = " +
                    "  (  SELECT count(_id) FROM " +
                    "     (  SELECT _id FROM sms WHERE thread_id = old.thread_id AND type = 5 " +
                    "        UNION " +
                    "        SELECT _id FROM pdu WHERE thread_id = old.thread_id AND st = 135 " +
                    "                                  AND (m_type=132 OR m_type=130 OR m_type=128)" +
                    "     )" +
                    "  ) " +
                    "  WHERE _id = old.thread_id;";
    ...
    // lichao modify in 2017-03-31, 2017-07-04
    //static final int DATABASE_VERSION = 601022;
    static final int DATABASE_VERSION = 601029;
            ...
            if (rows > 0) {
                ...
                db.execSQL(
                        "  UPDATE threads SET message_count = " +
                                ...
                                "        AND sms." + Sms.TYPE + " != 3 " +
                                "        AND (sms.reject = 0) " +//lichao add reject
                                "     )" +
                                "     + " +
                                "     (SELECT COUNT(pdu._id) FROM pdu LEFT JOIN threads " +
                                "          ON threads._id = " + Mms.THREAD_ID +
                                ...
                                "        AND (m_type=132 OR m_type=130 OR m_type=128)" +
                                "        AND " + Mms.MESSAGE_BOX + " != 3 " +
                                "        AND (pdu.reject = 0) " +//lichao add reject
                                "     ) " +
                ...
                db.execSQL(
                        "  UPDATE threads " +
                                ...
                                "                   AND (type != 3) " +
                                "                   AND (reject = 0) " +//lichao add reject
                                "                 ) " +
                                "       UNION " +
                                ...
                                "                   AND thread_id = " + thread_id +
                                "                   AND msg_box != 3 " +
                                "                   AND ( msg_box != 3 AND (m_type = 128 OR m_type = 132 OR m_type = 130) ) " +
                                "                   AND (reject = 0) " +//lichao add reject
                                "                 ) " +
                                "       UNION " +
                                "       SELECT DISTINCT date * 1 AS normalized_date, _id, read FROM cellbroadcast " +
                                ...
                                "      (SELECT date_sent * 1000 as date_sent, date * 1000 AS date, thread_id FROM pdu " +
                                "           WHERE (m_type=132 OR m_type=130 OR m_type=128) " +
                                "                 AND thread_id = " + thread_id + " " +//lichao add thread_id
                                "                 AND (reject = 0) " +//lichao add reject
                                "       UNION SELECT date_sent, date, thread_id FROM sms" +
                                "           WHERE thread_id = " + thread_id + " " +//lichao add thread_id
                                "                 AND (reject = 0)" +//lichao add reject
                                "      )" +
                                "     WHERE thread_id = " + thread_id + " " +
                                "     ORDER BY date DESC LIMIT 1" +
                ...
                db.execSQL(
                        "  UPDATE threads" +
                                ...
                                "  date =" +
                                "    (SELECT date FROM" +
                                "        (SELECT date * 1000 AS date, thread_id, reject FROM pdu" +//lichao add reject
                                "             WHERE (m_type=132 OR m_type=130 OR m_type=128) " +//lichao add WHERE
                                "                   AND thread_id = " + thread_id + " " +//lichao add thread_id
                                "                   AND (reject = 0) " +//lichao add reject
                                "         UNION SELECT date, thread_id, reject FROM sms" +//lichao add reject
                                "             WHERE thread_id = " + thread_id + " " +//lichao add thread_id
                                "                   AND (reject = 0)" +//lichao add reject
                                "        )" +
                                ...
                                "     WHERE thread_id = " + thread_id + " ORDER BY date DESC LIMIT 1" +
                                "    )," +
                                //lichao add for UPDATE threads set sub_id in 2017-04-21 begin
                                "  sub_id =" +
                                "    (SELECT sub_id FROM" +
                                "        (SELECT date * 1000 AS date, sub_id, thread_id FROM pdu " +
                                "             WHERE (m_type=132 OR m_type=130 OR m_type=128) " +
                                "                   AND thread_id = " + thread_id + " " +
                                "                   AND (reject = 0) " +
                                "         UNION SELECT date, sub_id, thread_id FROM sms" +
                                "             WHERE thread_id = " + thread_id + " " +
                                "                   AND (reject = 0)" +
                                "        )" +
                                "     WHERE thread_id = " + thread_id + " ORDER BY date DESC LIMIT 1" +
                                "    )," +
                                //lichao add for UPDATE threads set sub_id in 2017-04-21 end
                                "  has_attachment = " +
                                "   CASE " +
                                "   ( SELECT COUNT(*) FROM part JOIN pdu " +
                    ...
                    String query = "SELECT thread_id FROM sms WHERE (type = " +
                        Telephony.TextBasedSmsColumns.MESSAGE_TYPE_FAILED + " OR status >= " +
                            ...
                            + thread_id;
//                                            + " LIMIT 1";
                    // this setError is the error Count will be set
                    int setError = 0;
    ...
    private void createMmsTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + MmsProvider.TABLE_PDU + " (" +
                   ...
                   ");");
        //begin tangyisen
        addColumn_mms_group_column(db);
        addColumn_Mms_reject(db);
        //end tangyisen
        //HB. Comments :  , Engerineer : lichao , Date : 17-5-24 , begin
        addColumn_mms_has_attach(db);
        addColumn_mms_text(db);
        addColumn_mms_has_error(db);//2017-05-27 add
        addColumn_mms_failed_unread(db);//lichao add in 2017-07-04
        //HB. end
    ...
    private void createMmsTriggers(SQLiteDatabase db) {
     ...
     // Updates threads table whenever a message in pdu is updated.
        //lichao add Mms.SUBSCRIPTION_ID in 2017-04-21
        //因为这里有监听Mms.MESSAGE_BOX，如果这里包含了对SMS_PDU_UPDATE_THREAD_LATEST_HAS_ERROR_ON_NEW
        //的触发，那么就会让latest_has_error直接从pdu的has_error获取
        //但是实际上我们需要先修改pdu的has_error,之后再修改latest_has_error
        ...
        db.execSQL("CREATE TRIGGER pdu_update_thread_date_subject_on_update AFTER" +
                   ...
                   ", " + Mms.SUBSCRIPTION_ID + ", " + Mms.REJECT +//lichao add SUBSCRIPTION_ID and REJECT
    ...
    /// @}
     ...
     // Updates threads table whenever a message in pdu is updated.
        ...
        //HB. Comments :  , Engerineer : lichao , Date : 17-7-6 , begin
        db.execSQL("DROP TRIGGER IF EXISTS pdu_update_thread_failed_unread_on_update");
        db.execSQL("CREATE TRIGGER pdu_update_thread_failed_unread_on_update AFTER" +
                "  UPDATE OF " + Mms.FAILED_UNREAD +
                "  ON " + MmsProvider.TABLE_PDU + " " +
                PDU_UPDATE_THREAD_CONSTRAINTS +
                "BEGIN " +
                PDU_UPDATE_THREAD_FAILED_UNREAD_ON_NEW +
                "END;");
        //HB. end
     ...
     // Update the error flag of threads when delete pending message.
        ...
        db.execSQL("CREATE TRIGGER update_threads_error_on_delete_mms " +
                ...
                "  WHEN ( ( OLD._id IN ( SELECT DISTINCT msg_id " +//lichao add "( ("
                "                        FROM pending_msgs" +
                "                        WHERE err_type >= 10" +
                "                      )" +
                "         ) " +//lichao add ")"
                "         AND (OLD.reject = 0) " +//lichao add "reject"
                "       )" +//lichao add ")"
                "BEGIN " +
        ...
        db.execSQL("CREATE TRIGGER update_threads_error_on_move_mms " +
                ...
                "          AND NEW.msg_box != 4 AND NEW.msg_box != 5" +
                "        ) " +
                "        AND ( OLD._id IN (SELECT DISTINCT msg_id FROM pending_msgs WHERE err_type >= 10) ) " +
                "        AND ( NEW.reject = 0 )" + //lichao add reject
                "BEGIN " +
                "  UPDATE threads SET error = error - 1" +
                "  WHERE threads._id = OLD.thread_id; " + //lichao add "threads."
                "END;");
				
		//===============lichao add trigger codes in createMmsTriggers() begin===============
        //HB. Comments :  , Engerineer : lichao , Date : 17-5-19 , begin
        db.execSQL("DROP TRIGGER IF EXISTS part_update_thread_latest_has_attach_on_update");
        db.execSQL("CREATE TRIGGER part_update_thread_latest_has_attach_on_update " +
                " AFTER UPDATE of " + Part.MSG_ID+ ", " + Part.CONTENT_TYPE + ", " + Part._DATA +
                "  ON part " +
                " BEGIN " +
                PART_UPDATE_PDU_HAS_ATTACH_ON_NEW +
                PART_UPDATE_THREAD_LATEST_HAS_ATTACH_ON_NEW +
                " END");
        //HB. end
        //HB. Comments :  , Engerineer : lichao , Date : 17-5-24 , begin
        db.execSQL("DROP TRIGGER IF EXISTS part_update_pdu_text_on_update");
        db.execSQL("CREATE TRIGGER part_update_pdu_text_on_update " +
                " AFTER UPDATE of " + Part.MSG_ID + ", " + Part.CONTENT_TYPE + ", " + Part.TEXT +
                "  ON part " +
                " BEGIN " +
                PART_UPDATE_PDU_TEXT_ON_NEW +
                PART_UPDATE_THREAD_LATEST_PDU_TEXT_ON_NEW+
                " END");
        //HB. end
				
        //HB. Comments :  , Engerineer : lichao , Date : 17-5-19 , begin
        db.execSQL("DROP TRIGGER IF EXISTS part_update_thread_latest_on_insert");
        db.execSQL("CREATE TRIGGER part_update_thread_latest_on_insert " +
                " AFTER INSERT ON part " +
                " BEGIN " +
                PART_UPDATE_PDU_HAS_ATTACH_ON_NEW +
                PART_UPDATE_THREAD_LATEST_HAS_ATTACH_ON_NEW +
        ...
        //HB. Comments : Engerineer : lichao , Date : 17-5-17 , begin
        db.execSQL("DROP TRIGGER IF EXISTS pdu_update_thread_on_delete222");
        db.execSQL("CREATE TRIGGER pdu_update_thread_on_delete222 " +
                "AFTER DELETE ON " + MmsProvider.TABLE_PDU + " " +
                " WHEN (OLD.reject = 0) " +
                "BEGIN " +
                SMS_PDU_UPDATE_THREAD_SUB_ID_ON_OLD +
                SMS_PDU_UPDATE_THREAD_LATEST_IS_MMS_ON_OLD +
                SMS_PDU_UPDATE_THREAD_LATEST_IS_NOTIFI_ON_OLD +
                SMS_PDU_UPDATE_THREAD_LATEST_HAS_ATTACH_ON_OLD +
                PDU_UPDATE_THREAD_LATEST_PDU_TEXT_ON_OLD +//lichao add in 2017-05-25
                SMS_PDU_UPDATE_THREAD_LATEST_HAS_ERROR_ON_OLD +//lichao add in 2017-05-27
                "END;");
        //HB. end

        //HB. Comments :  , Engerineer : lichao , Date : 17-6-16 , begin
        //lichao add this like TRIGGER update_threads_error_on_move_mms
        db.execSQL("DROP TRIGGER IF EXISTS pdu_update_pdu_has_error_on_update");
        db.execSQL("CREATE TRIGGER pdu_update_pdu_has_error_on_update" +
                " AFTER UPDATE OF " + Mms.MESSAGE_BOX + ", " + Mms.DATE +
                "  ON " + MmsProvider.TABLE_PDU + " " +
                //PDU_UPDATE_PDU_HAS_ERROR_CONSTRAINTS +//不需要
                "BEGIN " +
                PDU_UPDATE_PDU_HAS_ERROR_ON_UPDATE +
                "END;");
        //HB. end

        //HB. Comments :  , Engerineer : lichao , Date : 17-6-7 , begin
        db.execSQL("DROP TRIGGER IF EXISTS pdu_update_thread_latest_has_error_on_update");
        db.execSQL("CREATE TRIGGER pdu_update_thread_latest_has_error_on_update" +
                " AFTER UPDATE OF " + Mms.HAS_ERROR + ", " + Mms.REJECT +
                "  ON " + MmsProvider.TABLE_PDU + " " +
                PDU_UPDATE_THREAD_CONSTRAINTS +
                "BEGIN " +
                SMS_PDU_UPDATE_THREAD_LATEST_HAS_ERROR_ON_NEW +
                "END;");

        db.execSQL("DROP TRIGGER IF EXISTS pdu_update_thread_latest_is_notifi_on_update");
        db.execSQL("CREATE TRIGGER pdu_update_thread_latest_is_notifi_on_update" +
                " AFTER UPDATE OF " + Mms.MESSAGE_TYPE +
                "  ON " + MmsProvider.TABLE_PDU + " " +
                PDU_UPDATE_THREAD_CONSTRAINTS +
                "BEGIN " +
                SMS_PDU_UPDATE_THREAD_LATEST_IS_NOTIFI_ON_NEW +
                "END;");
        //HB. end
		//===============lichao add trigger codes in createMmsTriggers() end===============
    }//end of createMmsTriggers
    ...
    private void createSmsTables(SQLiteDatabase db) {
        ...
        //begin tangyisen
        addColumn_sms_group_column(db);
        addColumn_Sms_reject(db);
        //end tangyisen

        //HB. Comments :  , Engerineer : lichao , Date : 17-5-27 , begin
        addColumn_sms_has_error(db);
        //HB. end

        addColumn_sms_failed_unread(db);//lichao add in 2017-07-04
        /**
    ...
    private void createCommonTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE canonical_addresses (" +
                   ...
                   "address TEXT);");
        //begin tangyisen add for black 20170523
        addColumn_addresses_black_column(db);
        //end tangyisen
        ...
        db.execSQL("CREATE TABLE threads (" +
				   ...
				   // lichao add in 2017-03-31
                   ","+Threads.SUBSCRIPTION_ID + " INTEGER DEFAULT -1" +
                   //lichao add in 2017-04-17 begin
                   ","+Threads.DATE_TOP + " INTEGER DEFAULT 0" +
                   ","+Threads.IS_TOP + " INTEGER DEFAULT 0" +//lichao: top status 0:untop,1:top
                   ","+Threads.LATEST_IS_MMS + " INTEGER DEFAULT 0" +//lichao add
                   ","+Threads.LATEST_IS_NOTIFI + " INTEGER DEFAULT 0" +//lichao add
                   ","+Threads.LATEST_HAS_ATTACH + " INTEGER DEFAULT 0" +//lichao add
                   ","+Threads.LATEST_PDU_TEXT + " TEXT" +//lichao add in 2017-05-25
                   ","+Threads.LATEST_HAS_ERROR + " INTEGER DEFAULT 0" +//lichao add in 2017-05-26
                   ","+Threads.MESSAGE_COUNT_INCLUDE_DRAFT + " INTEGER DEFAULT 0" +//lichao add
                   ","+Threads.FAILED_UNREAD + " INTEGER DEFAULT 0" +//lichao add in 2017-07-05
                   //lichao add in 2017-04-17 end
				   ");");
    ...
    private void createCommonTriggers(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TRIGGER sms_update_thread_read_on_update AFTER" +
                ...
                "     WHEN (NEW.reject = 0) " +//lichao add
                   ...
                   "END;");
        //HB. Comments :  , Engerineer : lichao , Date : 17-7-6 , begin
        db.execSQL("CREATE TRIGGER sms_update_thread_failed_unread_on_update AFTER" +
                "  UPDATE OF " + Sms.FAILED_UNREAD +
                "  ON sms " +
                "     WHEN (NEW.reject = 0) " +
                "BEGIN " +
                SMS_UPDATE_THREAD_FAILED_UNREAD_ON_NEW +
                "END;");
        //HB. end

        /// M: for CT new feature of split message into new thread. @{
        db.execSQL("CREATE TRIGGER sms_update_thread_on_add AFTER" + 
		"  UPDATE OF " + Sms.THREAD_ID + " ON sms " + 
        "  WHEN (NEW.reject = 0) " + //lichao add reject
			"BEGIN " + 
			SMS_UPDATE_THREAD_DATE_CT + 
			SMS_UPDATE_THREAD_MESSAGE_COUNT_CT + 
            SMS_UPDATE_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_CT + //lichao add for reject
            SMS_UPDATE_THREAD_SNIPPET_CT + 
			SMS_UPDATE_THREAD_ERROR_CT + 
			"END;");
        ...
        "  WHEN (NEW.reject = 0) " + //lichao add reject
			"BEGIN " + 
			SMS_UPDATE_OLD_THREAD_DATE_CT + 
            SMS_UPDATE_OLD_THREAD_MESSAGE_COUNT_CT +
            SMS_UPDATE_OLD_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_CT + //lichao add for reject
            SMS_UPDATE_OLD_THREAD_SNIPPET_CT + 
			SMS_UPDATE_OLD_THREAD_ERROR_CT + 
			"END;");
        /// M: @}
...
//                   "END;");
        ...
        db.execSQL("CREATE TRIGGER update_threads_error_on_update_mms " +
                ...
                "    FROM pdu" +
                "    WHERE _id = NEW.msg_id AND (reject = 0) ); " +//lichao add "AND (reject = 0) "
                "END;");

        //HB. Comments :  , Engerineer : lichao , Date : 17-5-27 , begin
        //Refer to TRIGGER update_threads_error_on_update_mms
        db.execSQL("DROP TRIGGER IF EXISTS pending_msgs_update_pdu_has_error_on_update");
        db.execSQL("CREATE TRIGGER pending_msgs_update_pdu_has_error_on_update " +
                " AFTER UPDATE OF " + PendingMessages.ERROR_TYPE +
                "  ON " + MmsSmsProvider.TABLE_PENDING_MSG + " " +
                PENDING_MSGS_UPDATE_PDU_HAS_ERROR_CONSTRAINTS +
                " BEGIN " +
                PENDING_MSGS_UPDATE_PDU_HAS_ERROR_ON_UPDATE +
                " END");
        //HB. end
        ...
        // 这个通过触发器约束条件（date > li_date）来设置最新信息的时间和摘要的代码值得借鉴
        // 不过这是建立在有 li_date 并且li_date正确的基础上
        ...
        db.execSQL("CREATE TRIGGER cb_update_thread_on_delete " +
                   ...
                   CB_UPDATE_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_ON_OLD + //lichao add
        ...
        //===============lichao add trigger codes in createCommonTriggers() begin===============
        ////HB. Comments : Engerineer : lichao , Date : 17-5-17 , begin
        db.execSQL("DROP TRIGGER IF EXISTS sms_update_thread_on_delete222");
        db.execSQL("CREATE TRIGGER sms_update_thread_on_delete222 " +
                "AFTER DELETE ON sms " +
                " WHEN (OLD.reject = 0) " +
                "BEGIN " +
                SMS_PDU_UPDATE_THREAD_SUB_ID_ON_OLD +
                ...
                SMS_PDU_UPDATE_THREAD_LATEST_IS_NOTIFI_ON_OLD +
                SMS_PDU_UPDATE_THREAD_LATEST_HAS_ATTACH_ON_OLD +
                SMS_PDU_UPDATE_THREAD_LATEST_HAS_ERROR_ON_OLD +//add in 2017-05-27
                "END;");
        //HB. end

        //HB. Comments :  , Engerineer : lichao , Date : 17-5-27 , begin
        db.execSQL("DROP TRIGGER IF EXISTS sms_update_sms_has_error_on_update");
        //这里不需要加入(NEW.reject = 0)的判断，因为被拦截的短信sms.has_error不会影响threads.latest_has_error
        db.execSQL("CREATE TRIGGER sms_update_sms_has_error_on_update AFTER " +
                "  UPDATE OF " + Sms.TYPE + ", " + Sms.STATUS +
                "  ON sms " +
                "  WHEN ( (OLD.type != 5 AND NEW.type = 5)" +
                "         OR (OLD.type = 5 AND NEW.type != 5) " +
                "         OR (OLD.status >= 64 AND NEW.status < 64) " +
                ...
                "       AND (NEW.group_id = 0) " +
                "BEGIN " +
                SMS_UPDATE_SMS_HAS_ERROR_ON_UPDATE +
                "END;");
        //HB. end

        //HB. Comments :  , Engerineer : lichao , Date : 17-6-27 , begin
        db.execSQL("DROP TRIGGER IF EXISTS sms_update_group_sms_has_error_on_update");
        db.execSQL("CREATE TRIGGER sms_update_group_sms_has_error_on_update AFTER " +
                "  UPDATE OF " + Sms.FAILS_COUNT +
                "  ON sms " +
                "  WHEN (NEW.group_id = -1) " +
                "BEGIN " +
                SMS_UPDATE_SMS_ITSELF_HAS_ERROR_ON_UPDATE +
                SMS_UPDATE_GROUP_SMS_HAS_ERROR_ON_UPDATE +
                "END;");
        //HB. end

        //HB. Comments :  , Engerineer : lichao , Date : 17-6-7 , begin
        //lichao delete  Sms.REJECT for it already in TRIGGER sms_update_thread_date_subject_on_update
        db.execSQL("DROP TRIGGER IF EXISTS sms_update_thread_latest_has_error_on_update");
        db.execSQL("CREATE TRIGGER sms_update_thread_latest_has_error_on_update AFTER " +
                "  UPDATE OF " + Sms.HAS_ERROR + ", " + Sms.REJECT +
                "  ON sms " +
                "  WHEN (NEW.reject = 0) " +
                "BEGIN " +
                SMS_PDU_UPDATE_THREAD_LATEST_HAS_ERROR_ON_NEW +
        ...
        //begin tangyisen
        add_sms_group_column_trigger(db);
        //end tangyisen
        //===============lichao add trigger codes in createCommonTriggers() begin===============
    }//end of createCommonTriggers()
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int currentVersion) {
        ...
        case 601021:
            ...
            //HB. Comments :  , Engerineer : lichao , Date : 17-5-26 , begin
            case 601022: //oldVersion
            case 601023: //oldVersion
            case 601024: //oldVersion
            case 601025: //oldVersion
            case 601026: //oldVersion
            case 601027: //oldVersion
                if (currentVersion <= 601027) {
                    return;
                }
                db.beginTransaction();
                try {
                    upgradeDatabaseToVersion601028(db);
                    db.setTransactionSuccessful();
                } catch (Throwable ex) {
                    Log.e(TAG, ex.getMessage(), ex);
                    break;
            ...
            case 601028:
                if (currentVersion <= 601028){
                    return;
                }
                db.beginTransaction();
                try {
                    upgradeDatabaseToVersion601029(db);
                    db.setTransactionSuccessful();
                } catch (Throwable ex) {
                    Log.e(TAG, ex.getMessage(), ex);
                    break;
    ...
    private void upgradeDatabaseToVersion560000(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TRIGGER pdu_update_thread_on_delete " +
            ...
            "  WHERE threads._id = old." + Mms.THREAD_ID + "; " +
            UPDATE_THREAD_COUNT_ON_OLD +
            //SMS_PDU_UPDATE_THREAD_MESSAGE_COUNT_INCLUDE_DRAFT_ON_OLD + //lichao add
            UPDATE_THREAD_READ_COUNT_OLD +
            UPDATE_THREAD_SNIPPET_SNIPPET_CS_ON_DELETE +
            "END;");
    ...
    private void upgradeDatabaseToVersion560200(SQLiteDatabase db) {
        ...
        //lichao refactor and add nothing
        db.execSQL("CREATE TRIGGER sms_update_thread_on_add AFTER" + 
    ...
    private void upgradeDatabaseToVersion600000(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE sms_temp (" +
                ...
                "total_len INTEGER," +
                "rec_len INTEGER" +
                ",reject INTEGER DEFAULT 0" +//lichao add
                ",failed_unread INTEGER DEFAULT 0"+//lichao add in 2017-07-05
                ");");
        ...
        if (MTK_WAPPUSH_SUPPORT) {
            db.execSQL("CREATE TABLE wappush_temp ("
                    ...
                    + "siid TEXT,"
                    + "url TEXT,"
                    + "action INTEGER,"//lichao refactor
                    + "created INTEGER,"//lichao refactor
                    + "expiration INTEGER,"
                    + "text TEXT"//lichao refactor
                    + ",reject INTEGER DEFAULT 0"//lichao add
                    + ",failed_unread INTEGER DEFAULT 0"//lichao add in 2017-07-05
                    + ");");
        ...
        db.execSQL("CREATE TABLE cellbroadcast_temp "
                + "(_id INTEGER PRIMARY KEY,"//lichao refactor
                + Mms.SUBSCRIPTION_ID + " INTEGER DEFAULT -1,"
                + "locked INTEGER DEFAULT 0,"
                + "body TEXT," + "channel_id INTEGER,"
                + "thread_id INTEGER,"//lichao refactor
                + "read INTEGER DEFAULT 0,"
                + "seen INTEGER DEFAULT 0,"//lichao refactor
                + "date_sent INTEGER DEFAULT 0,"
                + "date INTEGER"//lichao refactor
                + ",reject INTEGER DEFAULT 0"//lichao add
                + ",failed_unread INTEGER DEFAULT 0"//lichao add in 2017-07-05
                + ");");
...
//        createRCSTable(db);
    ...
    private void upgradeThreadsTableToAutoIncrement(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE threads_temp (" +
                   ...
                   ","+Threads.SUBSCRIPTION_ID + " INTEGER DEFAULT -1" +
                   //lichao add in 2017-04-17 begin
                   ","+Threads.DATE_TOP + " INTEGER DEFAULT 0" +//lichao add
                   ","+Threads.IS_TOP + " INTEGER DEFAULT 0" +//lichao add
                   //lichao add in 2017-04-17 end
                   ","+Threads.LATEST_IS_MMS + " INTEGER DEFAULT 0" +//lichao add
                   ","+Threads.LATEST_IS_NOTIFI + " INTEGER DEFAULT 0" +//lichao add
                   ","+Threads.LATEST_HAS_ATTACH + " INTEGER DEFAULT 0" +//lichao add
    ...
    private void updateThreadsAttachmentColumn(SQLiteDatabase db) {
        ...
        db.execSQL("UPDATE threads SET has_attachment=1 WHERE _id IN " +
                   ...
                   "   JOIN pdu ON pdu._id=part.mid AND (pdu.reject = 0) " +//lichao add reject
                   "   WHERE part.ct != 'text/plain' AND part.ct != 'application/smil')");
    }
    ...
    public static void updateThreadReadAfterDeleteMessage(SQLiteDatabase db, long thread_id) {
        if (MTK_WAPPUSH_SUPPORT) {
            db.execSQL(
                    ...
                    "              WHERE sms.thread_id = " + thread_id +
                    "              AND sms.read=0 AND (sms.reject = 0)) + " +//lichao add reject
                    "          (SELECT COUNT(pdu._id) FROM pdu " +
                    "              WHERE pdu.thread_id = " + thread_id +
                    "              AND (pdu.m_type=132 OR pdu.m_type=130 OR pdu.m_type=128) " +
                    "              AND pdu.read=0 AND (pdu.reject = 0)) + " +//lichao add reject
    ...
    private void createCBTables(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + TABLE_CELLBROADCAST
                ...
                + "locked INTEGER DEFAULT 0,"
                + "body TEXT,"
                + "channel_id INTEGER,"//lichao refactor
                + "thread_id INTEGER,"//lichao refactor
                + "read INTEGER DEFAULT 0,"
                + "seen INTEGER DEFAULT 0,"//lichao refactor
                + "date_sent INTEGER DEFAULT 0,"
    ...
    private void upgradePduTableToAutoIncrement(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE pdu_temp (" +
                ...
                Mms.CREATOR + " TEXT," +
                Mms.TEXT_ONLY + " INTEGER DEFAULT 0" +
                ","+Mms.FAILED_UNREAD + " INTEGER DEFAULT 0" +//lichao add
                ");");
    ...
    //lichao add in 2017-03-31 begin
    private void addColumn_threads_sub_id(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsSmsProvider.TABLE_THREADS +" ADD COLUMN "
                + Threads.SUBSCRIPTION_ID + " INTEGER DEFAULT -1");
    }

    private void addColumn_threads_date_top(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsSmsProvider.TABLE_THREADS +" ADD COLUMN "
                + Threads.DATE_TOP + " INTEGER DEFAULT 0");
    }

    private void addColumn_threads_is_top(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsSmsProvider.TABLE_THREADS +" ADD COLUMN "
                + Threads.IS_TOP + " INTEGER DEFAULT 0");
    }
    //lichao add in 2017-03-31 end

    //begin tangyisen add group table
    private void addColumn_sms_group_column(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + SmsProvider.TABLE_SMS +" ADD COLUMN "
                + Sms.GROUP_ID + " INTEGER DEFAULT 0");
        db.execSQL("ALTER TABLE " + SmsProvider.TABLE_SMS +" ADD COLUMN "
                + Sms.NOTI_COUNT + " INTEGER DEFAULT 0");
        db.execSQL("ALTER TABLE " + SmsProvider.TABLE_SMS +" ADD COLUMN "
                + Sms.FAILS_COUNT + " INTEGER DEFAULT 0");
    }

    /*private static final String SMS_NOTICOUNT_UPDATE_ON_SMS_TYPE_UPDATE_TRIGGER =
            "CREATE TRIGGER update_sms_noticount_on_update_sms_type " +
            " AFTER UPDATE of " + Sms.TYPE + " ON sms " +
            " WHEN (old.type = 2 OR old.type = 5) AND old.group_id = -1 " +
            " BEGIN " +
            "  UPDATE sms SET new.noti_count=new.noti_count+1 WHERE new._id = old._id;" +
            " END";
    private static final String SMS_NOTICOUNT2_UPDATE_ON_SMS_TYPE_UPDATE_TRIGGER =
            "CREATE TRIGGER update_sms_noticount2_on_update_sms_type " +
            " AFTER UPDATE of " + Sms.TYPE + " ON sms " +
            " WHEN (old.type = 2 OR old.type = 5) AND old.group_id > 0 " +
            " BEGIN " +
            "  UPDATE sms SET new.noti_count=new.noti_count+1 WHERE new._id = old.group_id;" +
            " END";
    private static final String SMS_FAILSCOUNT_UPDATE_ON_SMS_TYPE_UPDATE_TRIGGER =
            "CREATE TRIGGER update_sms_failscount_on_update_sms_type " +
            " AFTER UPDATE of " + Sms.TYPE + " ON sms " +
            " WHEN old.type = 5 AND old.group_id = -1 " +
            " BEGIN " +
            "  UPDATE sms SET new.fails_count=new.fails_count+1 WHERE new._id = old._id;" +
            " END";
    private static final String SMS_FAILSCOUNT2_UPDATE_ON_SMS_TYPE_UPDATE_TRIGGER =
            "CREATE TRIGGER update_sms_failscount2_on_update_sms_type " +
            " AFTER UPDATE of " + Sms.TYPE + " ON sms " +
            " WHEN old.type = 5 AND old.group_id > 0 " +
            " BEGIN " +
            "  UPDATE sms SET new.fails_count=new.fails_count+1 WHERE new._id = old.group_id;" +
            " END";*/
    
    private static final String SMS_GROUP_UPDATE_ON_SMS_TYPE_UPDATE_TRIGGER =
            "CREATE TRIGGER update_sms_group_on_update_sms_type " +
            " AFTER UPDATE OF " + Sms.TYPE + " ON sms " +
            " WHEN (new.type = 2 OR new.type = 5) AND new.group_id != 0 " +
            " BEGIN " +
            " UPDATE sms SET noti_count=noti_count+1 " + 
            " WHERE ((new.group_id = -1 AND _id = new._id) OR (new.group_id > 0 AND _id = new.group_id)) AND (new.type = 2 OR new.type = 5) ; " +
            " UPDATE sms SET fails_count=fails_count+1 " + 
            " WHERE ((new.group_id = -1 AND _id = new._id) OR (new.group_id > 0 AND _id = new.group_id)) AND new.type = 5; " +
            " END";
    private static final String SMS_GROUP_UPDATE_ON_SMS_TYPE_UPDATE_TRIGGER_QUEUE =
            "CREATE TRIGGER update_sms_group_on_update_sms_type_queue " +
            " AFTER UPDATE OF " + Sms.TYPE + " ON sms " +
            " WHEN new.type = 6 AND new.group_id != 0 " +
            " BEGIN " +
            " UPDATE sms SET noti_count=noti_count-1, fails_count=fails_count-1 " + 
            " WHERE ((new.group_id = -1 AND _id = new._id) OR (new.group_id > 0 AND _id = new.group_id)); " +
            " END";
    /*private static final String SMS_GROUP_UPDATE_ON_SMS_TYPE_UPDATE_TRIGGER =
            "CREATE TRIGGER update_sms_group_on_update_sms_type " +
            " AFTER UPDATE OF " + Sms.TYPE + " ON sms " +
            ...
            " BEGIN " +
            " UPDATE sms SET noti_count=noti_count+1 " + 
            " WHERE old.group_id = -1 AND (new.type = 2 OR new.type = 5) AND _id = old._id; " +
            " END";*/

    private void add_sms_group_column_trigger(SQLiteDatabase db) {
       /* db.execSQL("DROP TRIGGER IF EXISTS update_sms_noticount_on_update_sms_type");
        db.execSQL(SMS_NOTICOUNT_UPDATE_ON_SMS_TYPE_UPDATE_TRIGGER);
        db.execSQL("DROP TRIGGER IF EXISTS update_sms_noticount2_on_update_sms_type");
        db.execSQL(SMS_NOTICOUNT2_UPDATE_ON_SMS_TYPE_UPDATE_TRIGGER);
        db.execSQL("DROP TRIGGER IF EXISTS update_sms_failscount_on_update_sms_type");
        db.execSQL(SMS_FAILSCOUNT_UPDATE_ON_SMS_TYPE_UPDATE_TRIGGER);
        db.execSQL("DROP TRIGGER IF EXISTS update_sms_failscount2_on_update_sms_type");
        db.execSQL(SMS_FAILSCOUNT2_UPDATE_ON_SMS_TYPE_UPDATE_TRIGGER);*/
        db.execSQL("DROP TRIGGER IF EXISTS update_sms_group_on_update_sms_type");
        db.execSQL(SMS_GROUP_UPDATE_ON_SMS_TYPE_UPDATE_TRIGGER);
        db.execSQL("DROP TRIGGER IF EXISTS update_sms_group_on_update_sms_type_queue");
        db.execSQL(SMS_GROUP_UPDATE_ON_SMS_TYPE_UPDATE_TRIGGER_QUEUE);
    }

    private void addColumn_mms_group_column(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU +" ADD COLUMN "
                + Sms.GROUP_ID + " INTEGER DEFAULT -1");
    }

    private void addColumn_addresses_black_column(SQLiteDatabase db) {
        /*db.execSQL("ALTER TABLE canonical_addresses ADD COLUMN " + CanonicalAddressesColumns.IS_BLACK + " INTEGER DEFAULT 0");
        db.execSQL("ALTER TABLE canonical_addresses ADD COLUMN " + CanonicalAddressesColumns.BLACK_NOTE + " TEXT");*/
    }
    ...
    private void addColumn_Sms_reject(SQLiteDatabase db) {
        ...
        db.execSQL("ALTER TABLE " + SmsProvider.TABLE_SMS + " ADD COLUMN " + Sms.REJECT_TAG +" TEXT");
    }
    ...
    private void addColumn_Mms_reject(SQLiteDatabase db) {
        ...
        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU + " ADD COLUMN " + Mms.REJECT_TAG +" TEXT");
    }
    //end tangyisen

    //HB. Comments :  , Engerineer : lichao , Date : 17-6-12 , begin
    private void addColumn_CB_reject(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + TABLE_CELLBROADCAST + " ADD COLUMN " + CellBroadcasts.REJECT +" INTEGER DEFAULT 0");
    }
    //HB. end

    //lichao add in 2017-05-16 begin
    private void addColumn_threads_latest_is_mms(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsSmsProvider.TABLE_THREADS +" ADD COLUMN "
                + Threads.LATEST_IS_MMS + " INTEGER DEFAULT 0");
    }
    private void addColumn_threads_latest_is_notifi(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsSmsProvider.TABLE_THREADS +" ADD COLUMN "
                + Threads.LATEST_IS_NOTIFI + " INTEGER DEFAULT 0");
    }
    private void addColumn_threads_latest_has_attach(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsSmsProvider.TABLE_THREADS +" ADD COLUMN "
                + Threads.LATEST_HAS_ATTACH + " INTEGER DEFAULT 0");
    }
    //lichao add in 2017-05-16 end

    //HB. Comments :  , Engerineer : lichao , Date : 17-5-24 , begin
    private void addColumn_mms_has_attach(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU +" ADD COLUMN "
                + Mms.HAS_ATTACH + " INTEGER DEFAULT 0");
    }
    private void addColumn_mms_text(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU +" ADD COLUMN "
                + Mms.PDU_TEXT + " TEXT");
    }
    private void addColumn_threads_latest_pdu_text(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsSmsProvider.TABLE_THREADS +" ADD COLUMN "
                + Threads.LATEST_PDU_TEXT + " TEXT");
    }

    //HB. Comments :  , Engerineer : lichao , Date : 17-5-26 , begin
    private void addColumn_threads_latest_has_error(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsSmsProvider.TABLE_THREADS +" ADD COLUMN "
                + Threads.LATEST_HAS_ERROR + " INTEGER DEFAULT 0");
    }
    private void addColumn_mms_has_error(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU +" ADD COLUMN "
                + Mms.HAS_ERROR + " INTEGER DEFAULT 0");
    }
    private void addColumn_sms_has_error(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + SmsProvider.TABLE_SMS +" ADD COLUMN "
                + Sms.HAS_ERROR + " INTEGER DEFAULT 0");
    }
	
    //lichao add in 2017-06-21
    private void addColumn_threads_msg_count_in_draft(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsSmsProvider.TABLE_THREADS + " ADD COLUMN "
                + Threads.MESSAGE_COUNT_INCLUDE_DRAFT + " INTEGER DEFAULT 0");
    }

    //HB. Comments :  , Engerineer : lichao , Date : 17-7-4 , begin
    private void addColumn_mms_failed_unread(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + MmsProvider.TABLE_PDU +" ADD COLUMN "
                + Mms.FAILED_UNREAD + " INTEGER DEFAULT 0");
    }
    private void addColumn_sms_failed_unread(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE " + SmsProvider.TABLE_SMS +" ADD COLUMN "
                + Sms.FAILED_UNREAD + " INTEGER DEFAULT 0");
    }
    private void addColumn_CB_failed_unread(SQLiteDatabase db) {
        //use "failed_unread INTEGER DEFAULT 0" in someplace when CREATE TABLE
        db.execSQL("ALTER TABLE " + TABLE_CELLBROADCAST + " ADD COLUMN "
                + CellBroadcasts.FAILED_UNREAD +" INTEGER DEFAULT 0");
    }
    private void addColumn_threads_failed_unread(SQLiteDatabase db) {
	    //use Threads.FAILED_UNREAD + " INTEGER DEFAULT 0" in someplace when CREATE TABLE
        db.execSQL("ALTER TABLE " + MmsSmsProvider.TABLE_THREADS + " ADD COLUMN "
                + Threads.FAILED_UNREAD + " INTEGER DEFAULT 0");
    }
	
    private void upgradeDatabaseToVersion601028(SQLiteDatabase db) {
		//upgradeDatabaseToVersion601023
        addColumn_threads_sub_id(db);

		//upgradeDatabaseToVersion601024
        addColumn_threads_date_top(db);
        addColumn_threads_is_top(db);
		
		//upgradeDatabaseToVersion601025
        addColumn_sms_group_column(db);
        addColumn_mms_group_column(db);
        add_sms_group_column_trigger(db);
		
        //upgradeDatabaseToVersion601026
        addColumn_threads_latest_is_mms(db);
        addColumn_threads_latest_is_notifi(db);
        addColumn_threads_latest_has_attach(db);

        //upgradeDatabaseToVersion601027
        addColumn_mms_has_attach(db);
        addColumn_mms_text(db);
        addColumn_threads_latest_pdu_text(db);
		
		//upgradeDatabaseToVersion601028
        addColumn_mms_has_error(db);
        addColumn_sms_has_error(db);
        addColumn_threads_latest_has_error(db);
        //upgradeDatabaseAboutReject(db);
        addColumn_addresses_black_column(db);
        addColumn_Sms_reject(db);
        addColumn_Mms_reject(db);
        addColumn_CB_reject(db);
        addColumn_threads_msg_count_in_draft(db); //lichao add in 2017-06-21
    }
    //HB. end
	
    private void upgradeDatabaseToVersion601029(SQLiteDatabase db) {
        addColumn_mms_failed_unread(db);
        addColumn_sms_failed_unread(db);
        addColumn_CB_failed_unread(db);
        addColumn_threads_failed_unread(db);
    }
    //HB. end
}

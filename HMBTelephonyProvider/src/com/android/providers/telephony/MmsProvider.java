...
public class MmsProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection,
        ...
        Log.d(TAG, "\n query begin, uri = " + uri + ", selection = " + selection);
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        Log.d(TAG, "\n insert begin, uri = " + uri + ", values = " + values);
        ...
        if (table.equals(TABLE_PDU)) {
            Log.d(TAG, "insert TABLE_PDU begin");
            ...
            //HB. Comments : modify for bug5435 , Engerineer : lichao , Date : 17-7-24 , begin
            if(DEBUG) Log.i(TAG, "mms insert(), threadId = "+threadId);
            if(DEBUG) Log.i(TAG, "mms insert(), address = "+address);
            if (ProviderUtil.isSpecifiedPackageForReject(callerPkg, callerUid)
                    && msgBox == Mms.MESSAGE_BOX_INBOX) {
                String address2 = address;
                if (TextUtils.isEmpty(address) && threadId > 0) {
                    String sql = "select address FROM canonical_addresses where _id = " +
                            "(select recipient_ids FROM threads where _id = " + threadId + ")";
                    Cursor cursor = db.rawQuery(sql, null);
                    if (cursor != null) {
                        try {
                            if (cursor.moveToFirst()) {
                                address2 = cursor.getString(cursor.getColumnIndex("address"));
                                if(DEBUG) Log.i(TAG, "mms insert(), after Query, address2 = " + address2);
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        } finally {
                ...
                if (!TextUtils.isEmpty(address2)) {
                    if (RejectUtil.isRejectMms(getContext(), address2)) {
                        finalValues.put(Mms.REJECT, 1);
                        //lichao add in 2017-07-17 begin
                        finalValues.put(Mms.READ, 1);//temp for bug5697
                        finalValues.put(Mms.SEEN, 1);
                        int m_type_value = values.getAsInteger(Mms.MESSAGE_TYPE);
                        if(DEBUG) Log.i(TAG, "mms insert(), m_type_value = "+m_type_value);
                        if(m_type_value == PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND){
                            Log.i(TAG, "mms insert(), >>>sendRejectNotification, address2 = "+address2);
                            RejectUtil.sendRejectNotification(getContext(), false, address2, null);
                        }
                        //lichao add in 2017-07-17 end
                    }
                } else {
                    Log.w(TAG, "mms insert(), both address and address2 isEmpty!");
                }
            }
            //HB. Comments : modify for bug5435 , Engerineer : lichao , Date : 17-7-24 , end
            //end tangyisen
            ...
            /// @}
            Log.d(TAG, "insert TABLE_PDU end");
        }
        else if (table.equals(TABLE_ADDR)) {
            Log.d(TAG, "insert TABLE_ADDR begin");
            finalValues = new ContentValues(values);
            ...
            Log.d(TAG, "insert TABLE_ADDR end");
        } else if (table.equals(TABLE_PART)) {
            Log.d(TAG, "insert TABLE_PART begin");
            finalValues = new ContentValues(values);
            ...
            if (plainText) {
                ...
                // table;
                //begin tangyisen add reject 20170523
                ...
                //if (!isReject) {//tangyisen 20170523
                    ContentValues cv = new ContentValues();
            ...
            Log.d(TAG, "insert TABLE_PART end");
        } else if (table.equals(TABLE_RATE)) {
            Log.d(TAG, "insert TABLE_RATE begin");
            ...
            Log.d(TAG, "insert TABLE_RATE end");
        } else if (table.equals(TABLE_DRM)) {
            Log.d(TAG, "insert TABLE_DRM begin");
            String path = getContext().getDir(PARTS_DIR_NAME, 0).getPath()
            ...
            Log.d(TAG, "insert TABLE_DRM end");
        } else {
 ...
 public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        Log.d(TAG, "update begin, uri = " + uri + ", values = " + values
                + ", selection = " + selection);
        boolean notify = false;
        String msgId = null;
        String table;
    ...
    private void notifyChange(Uri uri) {
        ...
        /// number after insert operation.
        if (!notifyUnread) {
            Log.d(TAG, "notifyChange, >>>notifyUnreadMessageNumberChanged");
            MmsSmsProvider.notifyUnreadMessageNumberChanged(getContext());
        }
    }

    private final static String TAG = "MmsProvider/Mms";

...
final class CallLogManager extends CallsManagerListenerBase {
    ...
    private static class AddCallArgs {
        ...
        public AddCallArgs(Context context, CallerInfo callerInfo, String number,
                ...
                long conferenceCallLogId /* M: For Conference call */, int subId/*add by lgy*/) {
    ...
    public void onCallStateChanged(Call call, int oldState, int newState) {
        ...
        if (isNewlyDisconnected &&
            ...
            if (oldState != CallState.DIALING && oldState != CallState.RINGING
                    ...
                    && !IMS_MERGED_SUCCESSFULLY.equals(call.getDisconnectCause().getReason())) {
    ...
    void logCall(Call call, int callLogType) {
        ...
        logCall(call.getCallerInfo(), logNumber, call.getHandlePresentation(),
                callLogType, callFeatures, accountHandle, age > 0 ? connectedTime :creationTime, age, null,
                call.getConferenceCallLogId() /* M: For Volte Conference call */, call.getSubId() /*add by lgy*/);
    }
    ...
    private void logCall(
        ...
        if (isOkToLogThisCall) {
            ...
            AddCallArgs args = new AddCallArgs(mContext, callerInfo, number, presentation,
                    ...
                    conferenceCallLogId /* M: For Conference call */, subId/*add by lgy*/);
            logCallAsync(args);
        } else {
        ...
        protected Uri[] doInBackground(AddCallArgs... callList) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                try {
                    ...
                    if (c.callType == Calls.MISSED_TYPE && InterceptionUtils.isNeedReject(mContext, c.number)) {
                        isReject = true;
                        values.put("reject", InterceptionUtils.getLastRejectMode());
						if (InterceptionUtils.getLastRejectMode() == InterceptionUtils.MODE_SMART) {
							values.put("mark", InterceptionUtils.getLastRejectName());
							values.put("user_mark", InterceptionUtils.getLastMarkCount());
						} else {
			                   values.put("black_name",
		                                InterceptionUtils.getLastRejectName());
						}
                    }
                    
                    values.put("sub_id", c.mSubId);
    ...
    private void showCallDuration(Call call) {
        final long callDuration = System.currentTimeMillis() - call.getConnectTimeMillis();

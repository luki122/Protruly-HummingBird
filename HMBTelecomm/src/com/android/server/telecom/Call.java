...
public class Call implements CreateConnectionResponse {
    ...
    public void setState(int newState, String tag) {
        if (mState != newState) {
            ...
            /// M: add for emergency call retry
            // when call disconnected, reset the "mResponse" to null
            // to avoid memory leak.
            if (mCreateConnectionProcessor != null
                && getHandle() != null
                && PhoneNumberUtils.isEmergencyNumber(getHandle().getSchemeSpecificPart())
                && newState == CallState.DISCONNECTED) {
                mCreateConnectionProcessor.resetReponseObj();
            }
            /// @}
    ...
    //zhangcj add start
    public boolean nameIsNum() {
         if (mCallerInfo == null) {
        	 return false;
         }
         if(mCallerInfo. name == null) {
        	 return true;
         }
         return mCallerInfo.phoneNumber.equals( mCallerInfo.name);
    }
    //zhangcj add end
    ...
    void setConnectionCapabilities(int connectionCapabilities, boolean forceUpdate) {
        ...
        if (forceUpdate || mConnectionCapabilities != connectionCapabilities) {
            if (TelecomSystem.getInstance().getCallsManager().okToRecordVoice(this)) {
                connectionCapabilities |= Connection.CAPABILITY_VOICE_RECORD;
            }
            mConnectionCapabilities = connectionCapabilities;
            /// M: For block certain ViLTE @{
            // remove hold capability
            mConnectionCapabilities = updateConnectionCapabilites(mConnectionCapabilities);
            /// @}
            for (Listener l : mListeners) {
            ...
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    mCallerInfoAsyncQueryFactory.startQuery(
                            ...
                            Call.this,
                            TelecomUtils.getSubIdByAccount(mContext, mTargetPhoneAccountHandle));
                }
            });
        }
    }
    ...
    public void updateExtras(Bundle bundle) {
        ...
        // Only for MT
        if (isIncoming() && TelecomVolteUtils.isPauFieldChanged(bundle, mPauField)) {
            mPauField = TelecomVolteUtils.getPauFieldFromBundle(bundle);
            Uri handle = TelecomVolteUtils.getHandleFromPauField(mPauField);
            // if "from"(connection.getAddress()) is null, always show "unknown", so do nothing.
            // if "from" is not null, and pau is not null, use number in pau to replace.
            if (getHandle() != null && !Objects.equals(handle, getHandle())) {
                // setHandle() will trigger InCallController.updateCall();
    ...
    public boolean isVideoCall(){
        Log.i(this, "isVideoCall = " + VideoProfile.isBidirectional(getVideoState()));
        return VideoProfile.isBidirectional(getVideoState());
    }
    /// @}
    ...
    public boolean is3GVideoCall() {
        ...
        if (isVideoCall() & (!can(Connection.CAPABILITY_VOLTE))) {
            is3GVTCall = true;
        }
    ...
    /// M: Add for the case when answer action failed @{
    void setUnanswered() {
        mIsAnswered = false;
    }
    /// @}
     ...
     * @param defaultConnectionCapabilities
     * @return
     */
    private int updateConnectionCapabilites(int defaultConnectionCapabilities) {
        int capabilities = defaultConnectionCapabilities;
        if (TelecomUtils.isFeatureEnabled(mContext, getTargetPhoneAccount(),
                FeatureType.ViLTE_BLOCK_HOLD)) {
    ...
    //add by lgy
    public int getSubId() {
        PhoneAccountHandle ph = getTargetPhoneAccount();
        if (ph != null) {
            TelecomManager telecomManager = (TelecomManager) mContext.getSystemService(Context.TELECOM_SERVICE);
            PhoneAccount phoneAccount = telecomManager.getPhoneAccount(ph);

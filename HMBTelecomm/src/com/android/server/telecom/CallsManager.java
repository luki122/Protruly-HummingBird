...
import com.hb.interception.InterceptionUtils;
...
public class CallsManager extends Call.ListenerBase implements VideoProviderProxy.Listener {
    ...
    public void onSuccessfulIncomingCall(Call incomingCall) {
        ...
        //add by lgy 
        String number = incomingCall.getHandle() == null ?
                null : incomingCall.getHandle().getSchemeSpecificPart();
        if(InterceptionUtils.isNeedRejectQuick(mContext, number)) {
            incomingCall.reject(false, null);
            mCallLogManager.logCall(incomingCall, Calls.MISSED_TYPE);
            Intent intent = new Intent("com.hb.interception.ACTION_NOTIFY_CALL");
     ...
     * @param extras The optional extras Bundle passed with the intent used for the incoming call.
     */
    Call startOutgoingCall(Uri handle, PhoneAccountHandle phoneAccountHandle, Bundle extras, Intent intent) {
        ...
        */
        ...
        //add by lgy start
     		if(accounts.size() > 1) {
                 ...
                 if(slot > -1) {
                     PhoneAccountHandle hbPath = getPhoneAccountHandleBySlot(mContext, slot);
                     Log.i(this, "PhoneAccountHandle = " + hbPath);
                     if(hbPath != null) {
                         phoneAccountHandle = hbPath;
                     }
                 } else {
//                     if(!call.isEmergencyCall()) {
//                         phoneAccountHandle = null;
//                     }
        ...
        if (!makeRoomForOutgoingCall(call, isEmergencyCall)) {
            ...
            //add by lgy for 3519 start
            Call liveCall = getFirstCallWithState(call, OUTGOING_CALL_STATES);
            if(liveCall != null) {
            	liveCall.disconnect();
            }
            //add by lgy for 3519 end
            return null;
        }
...
//        placeOutgoingCall(call, handle, gatewayInfo, speakerphoneOn, videoState);
    ...
    private boolean isPotentialMMICode(Uri handle) {
        ...
        //modify by lgy for cts
        return (number.contains("#") && !number.contains("@")) && !number.equals("*1234#");
    }
...
//        }
    ...
    private PhoneAccountHandle getPhoneAccountHandleBySlot(Context context, int phoneId) {
        Log.i(this, "getPhoneAccountHandleBySlot  start ");
        if (!SubscriptionManager.isValidPhoneId(phoneId)) {
            return null;
        }
        int subId = getSubIdbySlot(context, phoneId);
        Log.i(this, "subId: = " + subId);
        ...
        for (PhoneAccountHandle account : accounts) {
            ...
            Log.i(this, "phoneAccount: = " + phoneAccount);
            if (phoneAccount == null) {
                continue;
            }
        	int accountSubId  =  telephonyManager.getSubIdForPhoneAccount(phoneAccount) ;
            Log.i(this, "accountSubId: = " + accountSubId);
            if (subId == accountSubId) {
                return account;
            }
        }
        return null;

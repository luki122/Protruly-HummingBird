...
public abstract class SQLiteContentProvider extends ContentProvider
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        if (!applyingBatch) {
            ...
            onEndTransaction(!isCallerSyncAdapter && shouldSyncFor(uri), shouldSyncFor(uri));
        } else {
            result = insertInTransaction(uri, values, isCallerSyncAdapter);
            if (result != null) {
                mNotifyChange = true;
    ...
    protected void onEndTransaction(boolean syncToNetwork, boolean notifyChange) {
        if (!notifyChange) {
            mNotifyChange = false;
            return;
        }

        onEndTransaction(syncToNetwork);
    }

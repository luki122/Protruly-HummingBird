...
public class PhoneGlobals extends ContextWrapper {
    ...
    public static final boolean sCdmaVersion = false;
    public static final boolean sCmccVersion = SystemProperties.getInt("ro.mtk_cmcc_ft_precheck_support", 0) == 1;
    ...
    private final SubscriptionManager.OnSubscriptionsChangedListener mOnSubscriptionsChangeListener
        ...
        public void onSubscriptionsChanged() {
            ...
            //add by lgy
//            SubscriptionManager sm = SubscriptionManager.from(getInstance());
//            if(sm.getDefaultDataSubId() == SubscriptionManager.INVALID_SUBSCRIPTION_ID) {
//            	if(mSubscriptionInfos.size() > 0) {
//            		sm.setDefaultDataSubId(mSubscriptionInfos.get(0).getSubscriptionId());
//            	}
//            }
            mHandler.removeCallbacks(mCmccRunnable);
           	mHandler.postDelayed(mCmccRunnable, 10*1000);
    ...
    private boolean mPreProvisioning = false;
    public void processProvisioning() {
    	mPreProvisioning = true;
        getContentResolver().registerContentObserver(Settings.Global.CONTENT_URI, true, mSettingObserver);        
    }
	...
	public class SettingObserver extends ContentObserver {
		...
		public void onChange(boolean selfChange) {
	        ...
	        if(mPreProvisioning != isInProvisioning && !isInProvisioning) {
	            SubscriptionManager sm = SubscriptionManager.from(getInstance());
	            if(sm.getDefaultDataSubId() == SubscriptionManager.INVALID_SUBSCRIPTION_ID) {
	            	if(mSubscriptionInfos.size() > 0) {
	                    Intent newIntent = new Intent(getInstance(), SimDialogActivity.class);
	                    newIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
	                    newIntent.putExtra(SimDialogActivity.DIALOG_TYPE_KEY, SimDialogActivity.SET_DATA);
	        ...
	        mPreProvisioning = isInProvisioning;
		}
	}
	
	//add by lgy for DeviceRegister
    private DmAgent mDMAgent;
    private void setCTAValue(String cta) {
        if (mDMAgent == null) {
            Log.v(LOG_TAG, "setCTAValue DMAgent null");
            return;
        }
        ...
        try {
            ...
            Log.e(LOG_TAG, "setCTAValue RemoteException!");
        }
        Log.i(LOG_TAG, "save CTA [" + cta + "]");
    }
    
    //add for cmcc
    private Runnable mCmccRunnable = new Runnable(){
		public void run(){
			handleCmccDataSub();
		}
	};
    
    private void handleCmccDataSub() {
        SubscriptionManager sm = SubscriptionManager.from(getInstance());
        int SimMode = Settings.System.getInt(getContentResolver(),
                Settings.System.MSIM_MODE_SETTING, -1);        
        if(sCmccVersion && sm.getActiveSubscriptionInfoCount() > 1 && SimMode == 3) {
          	int currentDataSlot = sm.getDefaultDataPhoneId();
   
          	boolean isSimCmcc1 = false;
          	boolean isSimCmcc2 = false;
          	
//            mSubscriptionInfos = PhoneUtils.getActiveSubInfoList();    
//          	SubscriptionInfo s1 =  mSubscriptionInfos.get(0);
//         	SubscriptionInfo s2 =  mSubscriptionInfos.get(1);
//         	int mcc1= s1.getMcc();
//         	int mcc2= s2.getMcc();
//         	int mnc1= s1.getMnc();
//         	int mnc2= s2.getMnc();
//            Log.i(LOG_TAG, "handleCmccDataSub mcc1 = " + mcc1 + ", mnc1 = " + mnc1);
//            Log.i(LOG_TAG, "handleCmccDataSub mcc2 = " + mcc2 + ", mnc2 = " + mnc2);
//         	
//         	if(mcc1 == 460 && (mnc1 == 0 || mnc1 == 2 || mnc1 == 7 || mnc1 == 8)) {
//         		isSimCmcc1 = true;
//         	}
//          	
//         	if(mcc2 == 460 && (mnc2 == 0 || mnc2 == 2 || mnc2 == 7 || mnc2 == 8)) {
//         		isSimCmcc2 = true;
//         	}
         	
         	String simOperator1 = TelephonyManager.from(this).getSimOperatorNameForPhone(0);
          	String simOperator2 = TelephonyManager.from(this).getSimOperatorNameForPhone(1);
			if (!TextUtils.isEmpty(simOperator1) && simOperator1.equalsIgnoreCase("CMCC")) {
				isSimCmcc1 = true;
			}
			if (!TextUtils.isEmpty(simOperator2) && simOperator2.equalsIgnoreCase("CMCC")) {
				isSimCmcc2 = true;
			}
	        Log.i(LOG_TAG, "handleCmccDataSub currentDataSlot = " + currentDataSlot);
	        Log.i(LOG_TAG, "handleCmccDataSub simOperator1 = " + simOperator1 + ", simOperator2 = " + simOperator2);
	        
			if(isSimCmcc1 && isSimCmcc2) {
				if(currentDataSlot != 0 && currentDataSlot != 1) {
			 		sm.setDefaultDataSubId(sm.getSubIdUsingPhoneId(0));
				}
			} else if(isSimCmcc1) {
				if(currentDataSlot != 0) {
			 		sm.setDefaultDataSubId(sm.getSubIdUsingPhoneId(0));
				}
			} else if(isSimCmcc2) {
				if(currentDataSlot != 1) {
			 		sm.setDefaultDataSubId(sm.getSubIdUsingPhoneId(1));
				}
			} else {
				if(currentDataSlot != 0 && currentDataSlot != 1) {
			 		sm.setDefaultDataSubId(sm.getSubIdUsingPhoneId(0));

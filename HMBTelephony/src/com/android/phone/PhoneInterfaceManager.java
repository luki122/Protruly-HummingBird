...
public class PhoneInterfaceManager extends ITelephony.Stub {
    ...
    public void setDataEnabled(int subId, boolean enable) {
    	...
    	//add by lgy for 1013
		if (subId == SubscriptionManager.INVALID_SUBSCRIPTION_ID && enable) {
			SubscriptionManager mSubscriptionManager = SubscriptionManager
					.from(PhoneGlobals.getInstance());
			List<SubscriptionInfo> mSubscriptionInfos = PhoneUtils
					.getActiveSubInfoList();
			if (mSubscriptionInfos.size() > 0) {
				if(PhoneGlobals.sCdmaVersion && mSubscriptionInfos.size() > 1 && CdmaUtils.isCdmaCard(1)) {				
					subId = mSubscriptionInfos.get(1).getSubscriptionId();					
				}  else {
					subId = mSubscriptionInfos.get(0).getSubscriptionId();
				}

...
public class SpecialCharSequenceMgr {
    ...
    static private boolean handlePinEntry(Context context, String input,
        ...
        if ((input.startsWith("**04") || input.startsWith("**05"))
            ...
            /// M: Support multi-sim case. @{
            /* Original code:
            boolean isMMIHandled = app.mCM.getFgPhone().handlePinMmi(input);
            */
            boolean isMMIHandled = handlePinMmi(input, context);
            /// @}
    ...
    /**
     * Handle PinMmi by the given input.
     * @param input The mmi to be handled
     * @param context
     * @return return true if this MMI has been handled, otherwise false
     */
    private static boolean handlePinMmi(String input, Context context) {
        SubscriptionManager subscriptionManager = SubscriptionManager.from(context);
        int[] subIdList = subscriptionManager.getActiveSubscriptionIdList();
        // Used to mark whether the Pin entry has been handled.
        boolean isMMIHandled = false;
        // Try to unlock the first account which has been locked.
        for (int subId : subIdList) {
            int simState = SubscriptionManager.
                    getSimStateForSlotIdx(SubscriptionManager.getSlotId(subId));
            if (TelephonyManager.SIM_STATE_PIN_REQUIRED == simState
                    || TelephonyManager.SIM_STATE_PUK_REQUIRED == simState) {
                isMMIHandled = handlePinMmiForSubscriber(subId, input);
                log("handlePinMmi, isMMIHandled:" + isMMIHandled + "; subId: " + subId);
                return isMMIHandled;
            }
        }
        return isMMIHandled;
     ...
     */
    private static boolean handlePinMmiForSubscriber(int subId, String input) {

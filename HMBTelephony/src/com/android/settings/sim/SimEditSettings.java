...
public class SimEditSettings extends PreferenceActivity implements   PhoneServiceStateHandler.Listener {
    ...
    private void maybeSwitchSlot(boolean isChecked) {   
        ...
        Log.d(LOG_TAG, "maybeSwitchSlot otherSlot = " + otherSlot);
    	...
    	if(!isChecked) {
            ...
            int SimMode = Settings.System.getInt(getContentResolver(),
                    Settings.System.MSIM_MODE_SETTING, -1);
            Log.d(LOG_TAG, "maybeSwitchSlot SimMode = " + SimMode);
            Log.d(LOG_TAG, "maybeSwitchSlot otherSubId = " + otherSubId);
            int modeSlot = MODE_PHONE1_ONLY << otherSlot;
    		if(otherSubId > SubscriptionManager.INVALID_SUBSCRIPTION_ID && (SimMode & modeSlot) > 0) {
                ...
                telephonyManager.setDataEnabled(otherSubId, dataEnable);
    		}
    	} else {
            //add by lgy for cmcc
            if(PhoneGlobals.sCmccVersion) {
            	String currentOperator = TelephonyManager.from(this).getSimOperatorNameForPhone(mSlotId);
              	String otherOperator = TelephonyManager.from(this).getSimOperatorNameForPhone(otherSlot);
                Log.d(LOG_TAG, "currentOperator = " + currentOperator);
                Log.d(LOG_TAG, "otherOperator = " + otherOperator);
              	if(!TextUtils.isEmpty(currentOperator) 
              			&& !TextUtils.isEmpty(otherOperator) 
              			&& currentOperator.equalsIgnoreCase("CMCC")
              			&& !otherOperator.equalsIgnoreCase("CMCC")) {
              		int subId = SubscriptionManager.getSubIdUsingPhoneId(mSlotId);

...
public class SimDialogActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        switch (dialogType) {
            ...
            //add by lgy
            case  SET_DATA:
            	displayDataDialog(extras.getInt(PREFERRED_SIM, -1));
            	break;
            default:
    ...
    //add by lgy
    private void displayDataDialog(final int slotId) {
        final Resources res = getResources();
        final Context context = getApplicationContext();
		...
		if (slotId == -1) {
			...
			if (mSubscriptionInfos.size() > 1) {
				storeSimInfo(mSubscriptionInfos.get(
						getOtherSlot(sir.getSimSlotIndex()))
						.getSubscriptionId());
			}
		} else {
			if (sir != null) {

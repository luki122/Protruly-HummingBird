...
public class SimDialogActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        switch (dialogType) {
            ...
            //add by lgy
            case  SET_DATA:
            	boolean cdma = getIntent().getBooleanExtra("cdma", false);
            	displayDataDialog(extras.getInt(PREFERRED_SIM, -1), cdma);
            	break;
            default:
    ...
    private void setDefaultDataSubId(final Context context, final int subId) {
        ...
        boolean dataEnable =   telephonyManager.getDataEnabled();
        ...
        storeSimInfo(subId);
        //add by lgy 
        telephonyManager.setDataEnabled(subId, dataEnable);
    ...
    private class SelectAccountListAdapter extends ArrayAdapter<String> {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (sir == null) {
                ...
                holder.title.setText(sir.getDisplayName().toString() + (slot +1));
    ...
    //add by lgy
    private void displayDataDialog(final int slotId, boolean cdma) {
        final Resources res = getResources();
        final Context context = getApplicationContext();
		...
		if (slotId == -1) {
			List<SubscriptionInfo> mSubscriptionInfos = PhoneUtils
					.getActiveSubInfoList();
			//modify by lgy for cdma
			if (mSubscriptionInfos.size() > 0) {
				if(cdma && mSubscriptionInfos.size() > 1 && CdmaUtils.isCdmaCard(1)) {				
	        		sir = mSubscriptionInfos.get(1);					
				}  else {
					sir = mSubscriptionInfos.get(0);
				}
				isNeedShowDialog = cdma ? false : isNeedShowDialog(sir);
				setDefaultDataSubIdNoToast(context, sir.getSubscriptionId());
				if(mSubscriptionInfos.size() > 1) {
					storeSimInfo(mSubscriptionInfos.get(getOtherSlot(sir.getSimSlotIndex())).getSubscriptionId());
				}
			}
		} else {
			if (sir != null) {
				isNeedShowDialog = isNeedShowDialog(sir);
				int subId = sir.getSubscriptionId();
				PhoneAccountHandle phoneAccountHandle = subscriptionIdToPhoneAccountHandle(subId);
				setDefaultDataSubIdNoToast(context, subId);
				setDefaultSmsSubId(context, subId);
				setUserSelectedOutgoingPhoneAccount(phoneAccountHandle);
			}
		}		

        if (sir != null && isNeedShowDialog) {        
        	
	        AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(this);
	
	        alertDialogBuilder.setTitle(R.string.data_dialog_title);
	        alertDialogBuilder.setMessage(R.string.data_dialog_message);
	        /// @}
    ...
    private void storeSimInfo(int subid) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(PhoneGlobals.getInstance());
        SharedPreferences.Editor editor = prefs.edit();
        editor.putBoolean(subid + "", true);
        editor.commit();
    }
    
    private boolean isNeedShowDialog(SubscriptionInfo sir) {
    	SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(PhoneGlobals.getInstance());
        return !prefs.getBoolean(sir.getSubscriptionId() + "", false);	
    }
    
    //add by lgy
    private void setDefaultDataSubIdNoToast(final Context context, final int subId) {
        final SubscriptionManager subscriptionManager = SubscriptionManager.from(context);
        /// M: for plug-in, need to call before setDefaultDataSubId
        mSimManagementExt.setDataState(subId);
        subscriptionManager.setDefaultDataSubId(subId);
...
//                        R.string.data_switch_started), subId), Toast.LENGTH_LONG).show();
    ...
    private int getOtherSlot (int slot) {
    	return slot == 1 ? 0 : 1;
    }
}

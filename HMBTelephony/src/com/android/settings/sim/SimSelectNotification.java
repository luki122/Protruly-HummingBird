...
public class SimSelectNotification extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (numSlots < 2 || isInProvisioning) {
            if(PhoneGlobals.sCdmaVersion && isInProvisioning) {
                Intent newIntent = new Intent(context, SimDialogActivity.class);
                newIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                newIntent.putExtra(SimDialogActivity.DIALOG_TYPE_KEY, SimDialogActivity.SET_DATA);
                newIntent.putExtra("cdma", true);
                context.startActivity(newIntent);
            	PhoneGlobals.getInstance().processProvisioning();
            }
            return;
        }
...
//                .setColor(context.getColor(R.color.sim_noitification))
        ...
        Intent resultIntent = new Intent(context, SimSettings.class);

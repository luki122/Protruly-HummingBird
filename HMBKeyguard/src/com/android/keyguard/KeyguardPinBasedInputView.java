...
public abstract class KeyguardPinBasedInputView extends KeyguardAbsKeyInputView
    ...
    // modify by wxue 20170317 start
	//protected PasswordTextView mPasswordEntry;
    protected BasePasswordTextView mPasswordEntry;
    // modify by wxue 20170317 end
    ...
    protected int getPromtReasonStringRes(int reason) {
        switch (reason) {
            case PROMPT_REASON_RESTART:
            	// modify by wxue for input hints when restart device 20170313 start
                //return R.string.kg_prompt_reason_restart_pin;
            	return R.string.kg_prompt_reason_restart_password;
               // modify by wxue for input hints when restart device 20170313 end
            default:
                return 0;
        }
    }
    ...
    private void performClick(View view) {
    	//HB. Comments : modify for NullPointerException , Engerineer : wxue , Date : 2017年8月21日 ,begin
        //view.performClick();
    	if(view != null){
    		view.performClick();
    	}
    ...
    protected void onFinishInflate() {
    	...
    	mPasswordEntry = (BasePasswordTextView) findViewById(getPasswordTextViewId());
        // modify by wxue 20170317 end
        ...
        mPasswordEntry.setUserActivityListener(new BasePasswordTextView.UserActivityListener() {
            @Override
            public void onUserActivity() {
            	if (mPasswordEntry.isEnabled() && mPasswordEntry.getText().length() == 6&&mPasswordEntry instanceof PinPasswordTextView) {
                    Log.d(TAG, "check password automatically") ;
                    setPasswordEntryEnabled(false);
                    verifyPasswordAndUnlock();
                }
                onUserInput();
        ...
        if(mPasswordEntry instanceof PasswordTextView){
        	mOkButton = findViewById(R.id.key_enter);
            if (mOkButton != null) {
                mOkButton.setOnClickListener(new View.OnClickListener() {
                    @Override

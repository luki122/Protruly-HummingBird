...
public class SeekBarVolumizer implements OnSeekBarChangeListener, Handler.Callback {
    ...
    public SeekBarVolumizer(Context context, int streamType, Uri defaultUri,
        ...
        Log.d("@M_" + TAG, "" + mStreamType + " get Original SYSTEM Volume: "
                + mSystemVolume);

        //mOriginalStreamVolume = mAudioManager.getStreamVolume(mStreamType);
        mOriginalStreamVolume = mProfileManager.getStreamVolume(mKey, mStreamType);
        Log.d("@M_" + TAG, "Profile keys: " + mKey + " " + mStreamType + " get Original Volume: "
        ...
        // system
        if (mProfileIsActive || !isRingStream()) {
            if (mSystemVolume != mOriginalStreamVolume) {
                Log.d("@M_" + TAG, " sync " + mStreamType + " original Volume to"
                        + mSystemVolume);
    ...
    public void revertVolume() {
        Log.d("@M_" + TAG, "" + mStreamType + " revert Last Volume "
                + mOriginalStreamVolume);
        ...
        if (mProfileManager.isActiveProfile(mKey)) {
            Log.d("@M_" + TAG, "" + mStreamType + " Active, Revert system Volume "
                    + mOriginalStreamVolume);
            setVolume(mStreamType, mOriginalStreamVolume, false);
        } else {
            if (!isSilentProfileActive()) {
                Log.d("@M_" + TAG, "revertVolume: " + mStreamType
     ...
     *
     * @param volumeStore
     *            Object that stored instance state
     */
    ...
    private void setVolume(int streamType, int volume, boolean flag) {
        if (streamType == AudioProfileManager.STREAM_RING) {
            ...
            if (flag) {
                ...
                mAudioManager.setStreamVolume(AudioProfileManager.STREAM_RING,
                        volume, 0);
    ...
    public void saveVolume() {
        Log.d("@M_" + TAG, "" + mStreamType + " Save Last Volume " + mLastProgress);

//        if (mLastProgress > 0) {
//            resetNormalModeIfNeeded();
//        }
        ...
        if (mProfileManager.isActiveProfile(mKey) || !isRingStream()) {
            Log.d("@M_" + TAG, "" + mStreamType + " Active, save system Volume "
                    + mLastProgress);
    ...
    public void ringtoneChanged() {
        ...
        if (mStreamType == AudioProfileManager.STREAM_RING) {
            ...
            newRingtoneUri = RingtoneManager.getActualDefaultRingtoneUri(mContext, RingtoneManager.TYPE_ALARM);
        }
    ...
    private final class Receiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (!AudioManager.VOLUME_CHANGED_ACTION.equals(intent.getAction())
                    && !AudioManager.STREAM_MUTE_CHANGED_ACTION.equals(intent.getAction())) {
                return;
            }
            ...
            if (AudioManager.STREAM_MUTE_CHANGED_ACTION.equals(intent.getAction())) {
                if (AudioManager.STREAM_MUSIC == mStreamType) {
                    if (!mAudioManager.isStreamMute(mStreamType)) {
//                    mLastProgress = mOriginalStreamVolume = mSystemVolume = streamValue;
                        mLastProgress = mOriginalStreamVolume = mSystemVolume
                                = mAudioManager.getLastAudibleStreamVolume(mStreamType);
                        mSeekBar.setProgress(mOriginalStreamVolume);
                    } else {
                        mLastProgress = mOriginalStreamVolume = mSystemVolume = 0;
                        mSeekBar.setProgress(mOriginalStreamVolume);
                    }
                } else if (AudioManager.STREAM_RING == mStreamType) {
                    if (!mAudioManager.isStreamMute(mStreamType)) {
                        mLastProgress = mOriginalStreamVolume = mSystemVolume
    ...
    /**
     * 铃声音量从0开启时如果是振动模式先切换到正常模式
     *
     * @date Liuqin on 2017-05-03
     */
    private void resetNormalModeIfNeeded() {
        if (mProfileManager == null
                || mStreamType != AudioProfileManager.STREAM_RING) {
            return;
        }
        String activateProfile = mProfileManager.getActiveProfileKey();
        if (MEETING_PREF_KEY.equals(activateProfile)) {
     ...
     *
     * @return the boolean
     * @date Liuqin on 2017-05-23
     */
    private boolean isRingStream() {
        return mStreamType == AudioManager.STREAM_RING;
    }
...
//            mAudioManager.setStreamVolume(mStreamType, 0, 0);
            mAudioManager.setStreamMute(mStreamType, true);
            mLastProgress = mOriginalStreamVolume = mSystemVolume = 0;
            mSeekBar.setProgress(mOriginalStreamVolume);
        }
    }

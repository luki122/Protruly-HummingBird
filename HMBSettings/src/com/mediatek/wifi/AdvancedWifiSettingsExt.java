                ...
                for (int i = 0; i < ipAddressesLength; i++) {
                    if (ipAddresses[i].indexOf(":") == NOT_FOUND_STRING) {
                        ...
                        if (ipAddressesLength == ONLY_ONE_IP_ADDRESS) {
                            mWlanSettingsCategory.removePreference(mIpv6AddressPref);
                        }
                    } else  {
                        ...
                        //mActivity.getString(R.string.status_unavailable) : ipAddresses[i]);
                        if (ipAddressesLength == ONLY_ONE_IP_ADDRESS) {
                            mWlanSettingsCategory.removePreference(mIpAddressPref);
                        }
                    }
                }
            } else {
...
//        }
        ...
        if (FeatureOption.MTK_DHCPV6C_WIFI) {
            ...
            mIpv6AddressPref.setOrder(6);
            mWlanSettingsCategory.addPreference(mIpv6AddressPref);
        }
    }

    public void setWlanSettingsCategory(PreferenceCategory wlanSettingsCategory) {
        this.mWlanSettingsCategory = wlanSettingsCategory;
    }

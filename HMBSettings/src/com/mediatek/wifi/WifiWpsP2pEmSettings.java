...
public class WifiWpsP2pEmSettings {
    ...
    public WifiWpsP2pEmSettings(Context context, WifiP2pManager manager,
        WifiP2pManager.Channel channel) {
        Log.d("@M_" + TAG, "WifiWpsP2pEmSettings, WifiP2pManager");
        mContext = context;
    ...
    public WifiWpsP2pEmSettings(Context context, WifiManager manager) {
        Log.d("@M_" + TAG, "WifiWpsP2pEmSettings, WifiManager");
        mContext = context;
    ...
    public void resume() {
        if (mWpsOrP2p == WIFI_WPS_EM) {
            // Wifi Wps EM
            mIsWifiWpsEmOpen = Settings.System.getInt(mContext.
                    getContentResolver(), WPS_WIFI_ENABLE, 0) == 1;
        } else if (mWpsOrP2p == WIFI_P2P_EM) {
            // Wifi P2P EM
            mIsWifiP2pEmOpen = Settings.System.getInt(mContext.
                    getContentResolver(), WPS_P2P_ENABLE, 0) == 1;
        }
    }
    ...
    public void prepareOptionsMenu(Menu menu) {
        ...
        if (mWpsOrP2p == WIFI_P2P_EM && mIsWifiP2pEmOpen) {
            ...
            autoGoMenu.setChecked(Settings.System.getInt(mContext.
                    getContentResolver(), AUTONOMOUS_GO, 0) == 1);
        }
    }
    ...
    public boolean optionsItemSelected(MenuItem item) {
        if (mWpsOrP2p == WIFI_WPS_EM && mIsWifiWpsEmOpen) {
            ...
            case MENU_ID_DEVPIN:
                ...
                return true;
            case MENU_ID_DEVPBC:
                Log.d("@M_" + TAG, "click menu item: WPS Register PBC");
    ...
    public void createContextMenu(ContextMenu menu, Object selectedAccessPoint) {
        // Wifi Wps EM
        if ((selectedAccessPoint instanceof AccessPoint)
                && mWpsOrP2p == WIFI_WPS_EM && mIsWifiWpsEmOpen) {
    ...
    public boolean contextItemSelected(MenuItem item) {
        if (mWpsOrP2p == WIFI_WPS_EM && mIsWifiWpsEmOpen) {
            ...
            case MENU_ID_WPSPIN:
                if (mSelectedAccessPoint.networkId == INVALID_NETWORK_ID) {
                    showWifiWpsEmDialog(WIFI_WPS_CONFIG_CONFIRM_DIALOG_ID,
            ...
            case MENU_ID_WPSPBC:
                Log.d("@M_" + TAG, "click context item: wps_pbc");
                //showDialog(mSelectedAccessPoint, true);
                mWpsListener = new WifiManager.WpsCallback() {
                    public void onStarted(String pin) {
                    }
    ...
    // Wifi Wps EM
    private void showWifiWpsEmDialog(int dialogId,
    ...
    public static AccessPoint build(String apSsid, String apBssid,
            int apNetworkId, boolean apWpsAvailable) {
...
class AccessPoint {
    ...
    public AccessPoint(String apSsid, String apBssid, int apNetworkId, boolean apWpsAvailable) {
        Log.d("@M_" + TAG, "AccessPoint, ssid = " + apSsid + " ,bssid = " + apBssid

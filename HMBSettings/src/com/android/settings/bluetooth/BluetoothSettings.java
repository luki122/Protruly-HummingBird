...
public final class BluetoothSettings extends DeviceListPreferenceFragment implements Indexable {
    ...
    public static final boolean IS_SHOW_TIPS_IN_BOTTOM = false;
    ...
    private PreferenceScreen mPhoneNamePreference;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case MENU_ID_RENAME_DEVICE:
                ...
                showChangePhoneBluetoothNameDialog();
                return true;
...
//                preferenceScreen.setOrderingAsAdded(true);
                ...
                if (mAvailableDevicesCategory == null) {
                    ...
                    ((BluetoothProgressCategory)mAvailableDevicesCategory).setIsShowProgress(false);
                } else {
                    mAvailableDevicesCategory.removeAll();
                }
...
//                getActivity().invalidateOptionsMenu();
                ...
                mPhoneNamePreference.setEnabled(true);
                return; // not break
...
//        setDeviceListGroup(preferenceScreen);
        ...
        if (!isUiRestricted()) {
//            getActivity().invalidateOptionsMenu();
              updateRefreshMenuVisible(false);
        }
        mPhoneNamePreference.setEnabled(false);
    }
...
//        setDeviceListGroup(getPreferenceScreen());
     ...
     * Init
     *
     * @date Liuqin on 2017-03-29
     */
    private void init() {
        final PreferenceScreen preferenceScreen = getPreferenceScreen();
        preferenceScreen.setOrderingAsAdded(true);

        SwitchPreference togglePreference =
                (SwitchPreference) preferenceScreen.findPreference("key_toggle_enabler");
        if (IS_ENABLER_INLINE) {
            mBluetoothEnabler.setTogglePreference(togglePreference);
        } else {
            preferenceScreen.removePreference(togglePreference);
        }

        mPhoneNamePreference =
                (PreferenceScreen) preferenceScreen.findPreference("key_bluetooth_phone_name");
        mPhoneNamePreference.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
        ...
        mPhoneNamePreference.setStatus(getPhoneBluetoothName());
    }

    /**
     * Remove preference without custom
     *
     * @date Liuqin on 2017-03-29
     */
    private void removePreferenceWithoutCustom() {
        final PreferenceScreen preferenceScreen = getPreferenceScreen();
        if (mAvailableDevicesCategory != null) {
    ...
    /**
     * Gets phone bluetooth name.
     *
     * @return the phone bluetooth name
     * @date Liuqin on 2017-03-29
     */
    private String getPhoneBluetoothName() {
        String name = null;
    ...
    /**
     * Update phone bluetooth name
     *
     * @date Liuqin on 2017-03-29
     */
    private void updatePhoneBluetoothName() {
        if (mPhoneNamePreference != null) {
            mPhoneNamePreference.setStatus(getPhoneBluetoothName());
        }
    }

    private void showChangePhoneBluetoothNameDialog() {
        new BluetoothNameDialogFragment().show(
                getFragmentManager(), "rename device");
    }

    /**
     * Update refresh menu state
     *
     * @param started the started
     * @date Liuqin on 2017-05-17
     */
    public void updateRefreshMenuState(boolean started) {
        if (mRefreshView != null) {
            boolean isAnimating = LocalSettings.isAnimating(mRefreshView);
            if (!started) {
                LocalSettings.stopAnimation(mRefreshView);
            } else if (!isAnimating) {
                LocalSettings.startRotateAnimate(mRefreshView);
            }
        }
    }

    /**
     * Update refresh menu visible
     *
     * @date Liuqin on 2017-05-17
     */
    private void updateRefreshMenuVisible(boolean visible) {
        if (mRefreshView != null) {
            mRefreshView.setVisibility(visible ? View.VISIBLE : View.INVISIBLE);
        } else {
            getActivity().invalidateOptionsMenu();
        }
    }

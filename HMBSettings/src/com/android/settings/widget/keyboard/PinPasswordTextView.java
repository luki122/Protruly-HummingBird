...
import com.android.settings.widget.keyboard.BasePasswordTextView;

import static com.android.internal.R.string.reset;
...
//        mDrawPaint.setColor(0x80ffffff);
        mDrawPaint.setColor(0xff999999);
...
//        mDrawPaint.setColor(0xffffffff);
    ...
    public void append(char c) {
        if (mText.length() == PASSWORD_LENGTH) {
            return;
        }
...
//    }
    ...
    public void updateInputCount(int mInputCount) {
        int currentCount = mText.length();
        int delta = mInputCount - currentCount;
        if (delta != 0) {
            boolean added = delta > 0;
            int N = added ? delta : -delta;
            for (int i = 0; i< N; i++) {
                if (added) {
                    append('*');
                } else {
    ...
    public OnEditorActionListener getOnEditorActionListener() {
        return mOnEditorActionListener;
    }

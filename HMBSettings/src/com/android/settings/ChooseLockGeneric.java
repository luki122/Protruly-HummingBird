...
public class ChooseLockGeneric extends SubSettings {
    public static final String CONFIRM_CREDENTIALS = "confirm_credentials";
    private static final boolean IS_ENABLE_SETUP_ASK_PASSWORD_ON_BOOT = false;
    ...
    public static class ChooseLockGenericFragment extends SettingsPreferenceFragment {
        ...
        private void maybeEnableEncryption(int quality, boolean disabled) {
            ...
            if (Process.myUserHandle().isOwner() && LockPatternUtils.isDeviceEncryptionEnabled()
                    && !dpm.getDoNotAskCredentialsOnBoot() && IS_ENABLE_SETUP_ASK_PASSWORD_ON_BOOT) {
                ...
                final boolean accEn = true; //AccessibilityManager.getInstance(context).isEnabled();
...
//                preference.setSummary(R.string.current_screen_lock);
                preference.setSummary(getSummaryFroCurrent()
                        + " - " + getString(R.string.current_screen_lock));
            }
        }
        ...
        private String getSummaryFroCurrent() {
            switch (mLockPatternUtils.getKeyguardStoredPasswordQuality(UserHandle.myUserId())) {
                case DevicePolicyManager.PASSWORD_QUALITY_SOMETHING:
                    return getString(R.string.unlock_set_unlock_pattern_summary);
                case DevicePolicyManager.PASSWORD_QUALITY_NUMERIC:
                case DevicePolicyManager.PASSWORD_QUALITY_NUMERIC_COMPLEX:
                    return getString(R.string.unlock_set_unlock_pin_summary);
                case DevicePolicyManager.PASSWORD_QUALITY_ALPHABETIC:
                case DevicePolicyManager.PASSWORD_QUALITY_ALPHANUMERIC:
                    return getString(R.string.unlock_set_unlock_password_summary);
                case DevicePolicyManager.PASSWORD_QUALITY_UNSPECIFIED:
                    return "";
            }
            return "";
        ...
        private void updatePreferenceSummaryIfNeeded() {
            if (!LocalSettings.Enabler.ENABLE_TIP_IN_CHOOSE_LOCKTYPE_ACCESSIBILITY_ON) {
                return;
            }
        ...
        void updateUnlockMethodAndFinish(int quality, boolean disabled) {
            ...
            if (quality >= DevicePolicyManager.PASSWORD_QUALITY_NUMERIC) {
                ...
                // add password length limit when select digital unlock method, by wxue 20170314 start
               //final int maxLength = mDPM.getPasswordMaximumLength(quality);
                int maxLength = mDPM.getPasswordMaximumLength(quality);
                if(quality == DevicePolicyManager.PASSWORD_QUALITY_NUMERIC){
                	minLength = maxLength = 6;
                }
                // add password length limit when select digital unlock method, by wxue 20170314 end
                Intent intent;
                /// M: voice unlock fallback password @ {
                if (isFallback) {
...
//                        getGuideCompat().guideLaunchWifiSettings();

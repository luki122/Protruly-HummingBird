...
public class WifiSettings extends RestrictedSettingsFragment
    ...
    public static final boolean IS_SHOW_PROGRESSBAR_SWITCHING = false;
    public static final boolean IS_SHOW_ORIGINAL_MENU = false;
    public static final boolean IS_ENABLER_INLINE = true;
    ...
    public void onDestroy() {
        ...
        if (isGuide()) {
            getGuideCompat().guideRegisterNetworkStateListener(
                    getContext().getApplicationContext(), false, null);
        }
    }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        if (!isGuide()) {
            ...
            getGuideCompat().guideAddOptionMenu(menu, inflater);
            return;
        }
    ...
    void addOptionsMenuItems(Menu menu) {
        ...
        menu.add(Menu.NONE, MENU_ID_SCAN, 0, R.string.menu_stats_refresh)
                ...
                .setIcon(LocalSettings.Drawable.IC_MENU_REFRESH)
                .setShowAsAction(MenuItem.SHOW_AS_ACTION_IF_ROOM);
        mRefreshView = getActivity().findViewById(MENU_ID_SCAN);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case MENU_ID_WPS_PIN:
                ...
                return true;
            case MENU_ID_SCAN:
                if (!LocalSettings.isAnimating(mRefreshView)) {
                    MetricsLogger.action(getActivity(), MetricsLogger.ACTION_WIFI_FORCE_SCAN);
                    mWifiTracker.forceScan();
                    LocalSettings.startRotateAnimate(mRefreshView, 1);
                }
                return true;
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen screen, Preference preference) {
        ...
        } else if (preference.getKey().equals("key_wlan_add_network")) {
            if (!isUiRestricted()) {
                if (mWifiTracker.isWifiEnabled()) {
                    onAddNetworkPressed();
                }
    ...
    private void setOffMessage() {
        ...
        if (mWifiEnabler != null) {
            mWifiEnabler.setSummary(briefText + "");
        }
    }
    ...
    private void addMessagePreference(int messageId) {
        ...
        mWifiSettingsExt.emptyScreen(getPreferenceScreen());
        if (mWifiEnabler != null) {
            mWifiEnabler.setSummary(getString(messageId));
        }
    }

    protected void setProgressBarVisible(boolean visible) {
        if (mProgressHeader != null && IS_SHOW_PROGRESSBAR_SWITCHING) {
    ...
    private void init() {
        ...
        SwitchPreference wifiTogglePreference = (SwitchPreference) preferenceScreen
                .findPreference("key_toggle_enabler");
        if (IS_ENABLER_INLINE) {
                mWifiEnabler.setTogglePreference(wifiTogglePreference);

            if (GuideCompat.checkGuide(getActivity())) {
                preferenceScreen.removePreference(wifiTogglePreference);

                setGuideCompat(new GuideCompat(getActivity()));
                guideInit();
            }
        } else {
            preferenceScreen.removePreference(wifiTogglePreference);
    ...
    // 开机引导移动网络开关
    private SwitchPreference togglePreference;

    private void guideInit() {
        Context context = getContext();
        GuideCompat guideCompat = getGuideCompat();
        guideCompat.guideOpenSimCard();
        guideCompat.guideEnsureWifiOn();

        // init toolbar
        guideCompat.guideInitToolbar(this, new GuideCompat.GuideMenuListener() {
            @Override
        ...
        // mobile network enabler
        togglePreference = new SwitchPreference(getContext());
        togglePreference.setKey("key_mobile_toggle_enabler");
        togglePreference.setTitle(R.string.network_settings_title);
        togglePreference.setPersistent(false);
        togglePreference.setOrder(1);
        togglePreference.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            @Override
            public boolean onPreferenceChange(Preference preference, Object o) {
//                if (!GuideCompat.guideIsMobileDataSwitchClickable(getContext())) {
//                    return false;
//                }
                boolean isChecked = (boolean)o;
                return GuideCompat.guideSetMobileDataEnabled(getContext(), isChecked);
            }
        });
        togglePreference.setChecked(GuideCompat.guideIsMobileNetworkEnabled(context));
        togglePreference.setEnabled(GuideCompat.guideIsSimCardReady(context));
        guideAddOrRemoveMobileDataPreference();

        // remove wlan settings
        removePreference("key_wlan_settings_category");

        // add header
        ViewGroup mainLayoutView = (ViewGroup) getActivity().findViewById(R.id.main_layout);
        View headerView = getActivity().getLayoutInflater()
                .inflate(R.layout.guide_header, null);
        TextView titleView = (TextView) headerView.findViewById(R.id.guide_title);
        titleView.setText(getString(R.string.guide_network_title));
        mainLayoutView.addView(headerView, 0);

        // add footer
        View footerView = getActivity().getLayoutInflater()
                .inflate(R.layout.guide_one_button, null);
        final View nextBtn = footerView.findViewById(R.id.guide_positive_btn);
        ...
        ((ViewGroup)mainLayoutView.getParent()).addView(footerView);

        nextBtn.setEnabled(GuideCompat.guideIsNetworkConnected(context));
        guideCompat.guideRegisterNetworkStateListener(context.getApplicationContext(), true,
                new GuideCompat.GuideNetworkStateChangeListener() {
                    @Override
                    public void onNetworkConnectedChange(boolean isConnected) {
                        if (nextBtn.isEnabled() != isConnected) {
                            nextBtn.setEnabled(isConnected);
                        }

                        if (togglePreference.isEnabled()
                                && togglePreference.isChecked() != GuideCompat
                                    .guideIsMobileNetworkEnabled(getContext())) {
                            togglePreference.setChecked(!togglePreference.isChecked());
                        }
                    ...
                    public void onSimCardStateChange(boolean isReady) {
                        togglePreference.setEnabled(isReady);
                        if (!isReady && togglePreference.isChecked()) {
                            togglePreference.setChecked(false);
                        }
                        if (isReady) {
                            getGuideCompat().guideOpenSimCard();
                            togglePreference.setChecked(GuideCompat.guideIsMobileNetworkEnabled(getContext()));
                        }
        ...
        LocalSettings.initOnFirstBoot(context);
    }

    private void guideAddOrRemoveMobileDataPreference() {
        if (togglePreference.isEnabled()) {
            if (getPreferenceScreen().findPreference("key_mobile_toggle_enabler") == null) {
                if (GuideCompat.isOpenMobileData() || togglePreference.isChecked()) {
                    getPreferenceScreen().addPreference(togglePreference);
                    GuideCompat.setOpenMobileData();
                }
            }
        } else {
            getPreferenceScreen().removePreference(togglePreference);
        }
    }
}

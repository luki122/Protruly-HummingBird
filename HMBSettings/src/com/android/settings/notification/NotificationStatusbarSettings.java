...
import hb.preference.SeekBarVolumizer;
import hb.preference.SwitchPreference;
import hb.preference.TwoStatePreference;

public class NotificationStatusbarSettings extends SettingsPreferenceFragment implements Indexable, OnPreferenceChangeListener{
    private static final String TAG = "NotificationSettings";
    private static final boolean IS_DISPLAY_SOUND = false;
    private static final boolean IS_DISPLAY_ADVANCED = false;
    ...
    private static final String KEY_TOGGLE_LOCK_SCREEN_NOTIFICATIONS = "toggle_lock_screen_notification";
    private static final String KEY_STATUSBAR = "statusbar";
    private static final String KEY_NOTIFICATIONBAR = "notificationbar";
    private static final String KEY_TOGGLE_FOLD_NON_PRIORITY_NOTIFICATION = "toggle_fold_non_priority_notification";
    private static final String KEY_TOGGLE_SHOW_NETWORK_SPEED = "toggle_show_network_speed";
    private static final String KEY_TOGGLE_SHOW_BATTERY_PERCENTAGE = "toggle_show_battery_percentage";
    private static final String KEY_TOGGLE_DRAG_DOWN_ON_LOCK_SCREEN = "toggle_drag_down_on_lock_screen";
    private static final String KEY_TOGGLE_USE_MESSAGE_BOX = "toggle_use_message_box";
    ...
    private SwitchPreference mToggleLockscreen;
    private SwitchPreference mToggleFoldNonPriorityNotification;
    private SwitchPreference mToggleShowNetworkSpeed;
    private SwitchPreference mToggleShowBatteryPercentage;
    private SwitchPreference mToggleDragDownOnLockScreen;
    private SwitchPreference mToggleUseMessageBox;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        addPreferencesFromResource(R.xml.notification_statusbar_settings);
        ...
        if (!IS_DISPLAY_SOUND) {
           removePreference(KEY_SOUND);
        }
        if (!IS_DISPLAY_ADVANCED) {
            removePreference(KEY_ADVANCED);
        }
        removePreference(KEY_NOTIFICATION);

        initToggleLockscreenNotifications(getPreferenceScreen());
        initOtherSettings();
    }
    ...
    private final class SettingsObserver extends ContentObserver {
        ...
        private final Uri NOTIFICATION_FOLD_NON_PRIORITY_NOTIFICATION =
                Settings.Secure.getUriFor(LocalSettings.Secure.NOTIFICATION_FOLD_NON_PRIORITY_NOTIFICATION);
        private final Uri NOTIFICATION_SHOW_BATTERY_PERCENTAGE =
                Settings.Secure.getUriFor(LocalSettings.Secure.NOTIFICATION_SHOW_BATTERY_PERCENTAGE);
        private final Uri NOTIFICATION_SHOW_NETWORK_SPEED =
                Settings.Secure.getUriFor(LocalSettings.Secure.NOTIFICATION_SHOW_NETWORK_SPEED);
        private final Uri NOTIFICATION_DRAG_DOWN_ON_LOCK_SCREEN =
                Settings.Secure.getUriFor(LocalSettings.Secure.NOTIFICATION_DRAG_DOWN_ON_LOCK_SCREEN);
        private final Uri NOTIFICATION_USE_MESSAGE_BOX =
                Settings.Secure.getUriFor(LocalSettings.Secure.NOTIFICATION_USE_MESSAGE_BOX);
        ...
        public void register(boolean register) {
            ...
            if (register) {
                ...
                cr.registerContentObserver(NOTIFICATION_FOLD_NON_PRIORITY_NOTIFICATION, false, this);
                cr.registerContentObserver(NOTIFICATION_SHOW_BATTERY_PERCENTAGE, false, this);
                cr.registerContentObserver(NOTIFICATION_SHOW_NETWORK_SPEED, false, this);
                cr.registerContentObserver(NOTIFICATION_DRAG_DOWN_ON_LOCK_SCREEN, false, this);
                cr.registerContentObserver(NOTIFICATION_USE_MESSAGE_BOX, false, this);
            } else {
        ...
        public void onChange(boolean selfChange, Uri uri) {
            ...
            if (LOCK_SCREEN_PRIVATE_URI.equals(uri) || LOCK_SCREEN_SHOW_URI.equals(uri)) {
                updateLockscreenNotifications();
                updateToggleLockscreenNotification();
            }

            if (NOTIFICATION_FOLD_NON_PRIORITY_NOTIFICATION.equals(uri)) {
                updateToggleReversely(mToggleFoldNonPriorityNotification,
                        LocalSettings.Secure.NOTIFICATION_FOLD_NON_PRIORITY_NOTIFICATION, LocalSettings.FLAG_ENABLED);
            } else if(NOTIFICATION_SHOW_BATTERY_PERCENTAGE.equals(uri)) {
                updateToggle(mToggleShowBatteryPercentage,
                        LocalSettings.Secure.NOTIFICATION_SHOW_BATTERY_PERCENTAGE, LocalSettings.FLAG_ENABLED);
            } else if(NOTIFICATION_SHOW_NETWORK_SPEED.equals(uri)) {
                updateToggle(mToggleShowNetworkSpeed,
                        LocalSettings.Secure.NOTIFICATION_SHOW_NETWORK_SPEED, LocalSettings.FLAG_DISABLED);
            } else if(NOTIFICATION_DRAG_DOWN_ON_LOCK_SCREEN.equals(uri)) {
                updateToggle(mToggleDragDownOnLockScreen,
                        LocalSettings.Secure.NOTIFICATION_DRAG_DOWN_ON_LOCK_SCREEN, LocalSettings.FLAG_ENABLED);
            } else if(NOTIFICATION_USE_MESSAGE_BOX.equals(uri)) {
                updateToggle(mToggleUseMessageBox,
                        LocalSettings.Secure.NOTIFICATION_USE_MESSAGE_BOX, LocalSettings.FLAG_ENABLED);
            }
        }
    }
    ...
    public static final BaseSearchIndexProvider SEARCH_INDEX_DATA_PROVIDER =
        ...
        public List<SearchIndexableResource> getXmlResourcesToIndex(
            ...
            sir.xmlResId = R.xml.notification_statusbar_settings;
    ...
    /**
     * Init toggle lockscreen notifications
     *
     * @param parent the parent
     * @date Liuqin on 2017-03-23
     */
    private void initToggleLockscreenNotifications(PreferenceScreen parent) {
        mToggleLockscreen = (SwitchPreference)parent.findPreference(KEY_TOGGLE_LOCK_SCREEN_NOTIFICATIONS);

        boolean isSecureNotificationsDisabled = isSecureNotificationsDisabled();
        boolean isUnredactedNotificationsDisabled = isUnredactedNotificationsDisabled();
        if (!isSecureNotificationsDisabled && !isUnredactedNotificationsDisabled) {
            mToggleLockscreen.setOnPreferenceChangeListener(this);
            updateToggleLockscreenNotification();
        } else {
            mToggleLockscreen.setEnabled(false);
            mToggleLockscreen.setChecked(false);
        }
    }

    /**
     * Update toggle lockscreen notification
     *
     * @date Liuqin on 2017-03-23
     */
    private void updateToggleLockscreenNotification() {
        if (mToggleLockscreen == null) {
            return;
        }
        final boolean enabled = getLockscreenNotificationsEnabled();
        mToggleLockscreen.setChecked(enabled);
    }
    ...
    private void initOtherSettings() {
        PreferenceCategory category = (PreferenceCategory) findPreference(KEY_STATUSBAR);
        mToggleFoldNonPriorityNotification = (SwitchPreference) category.findPreference(KEY_TOGGLE_FOLD_NON_PRIORITY_NOTIFICATION);
        mToggleShowBatteryPercentage = (SwitchPreference) category.findPreference(KEY_TOGGLE_SHOW_BATTERY_PERCENTAGE);
        mToggleShowNetworkSpeed = (SwitchPreference) category.findPreference(KEY_TOGGLE_SHOW_NETWORK_SPEED);

        category = (PreferenceCategory) findPreference(KEY_NOTIFICATIONBAR);
        mToggleDragDownOnLockScreen = (SwitchPreference) category.findPreference(KEY_TOGGLE_DRAG_DOWN_ON_LOCK_SCREEN);
        mToggleUseMessageBox = (SwitchPreference) category.findPreference(KEY_TOGGLE_USE_MESSAGE_BOX);

        mToggleFoldNonPriorityNotification.setOnPreferenceChangeListener(this);
        mToggleShowBatteryPercentage.setOnPreferenceChangeListener(this);
        mToggleShowNetworkSpeed.setOnPreferenceChangeListener(this);
        mToggleDragDownOnLockScreen.setOnPreferenceChangeListener(this);
        mToggleUseMessageBox.setOnPreferenceChangeListener(this);
    ...
    /**
     * Update toggle reversely
     *
     * @param preference the preference
     * @param key        the key
     * @param def        the def
     * @date Liuqin on 2017-06-14
     */
    private void updateToggleReversely(SwitchPreference preference, String key, int def) {
        preference.setChecked(!isSettingsEnabled(key, def));
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if(preference instanceof SwitchPreference) {
            ...
            if (preference == mToggleLockscreen) {
                updateSettings(Settings.Secure.LOCK_SCREEN_SHOW_NOTIFICATIONS, enabled);
            } else if(preference == mToggleFoldNonPriorityNotification) {
                updateSettings(LocalSettings.Secure.NOTIFICATION_FOLD_NON_PRIORITY_NOTIFICATION, !enabled);
            } else if(preference == mToggleShowBatteryPercentage) {
                updateSettings(LocalSettings.Secure.NOTIFICATION_SHOW_BATTERY_PERCENTAGE, enabled);
            } else if(preference == mToggleShowNetworkSpeed) {
                updateSettings(LocalSettings.Secure.NOTIFICATION_SHOW_NETWORK_SPEED, enabled);
            } else if(preference == mToggleDragDownOnLockScreen) {
                updateSettings(LocalSettings.Secure.NOTIFICATION_DRAG_DOWN_ON_LOCK_SCREEN, enabled);
            } else if(preference == mToggleUseMessageBox) {
                updateSettings(LocalSettings.Secure.NOTIFICATION_USE_MESSAGE_BOX, enabled);
            }

...
public class DeviceInfoSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if(version!=null && version.length()>0 && !version.equals(Build.UNKNOWN)){
        	...
        	if(versions!=null && versions.length == 5){
        		...
        		setStringSummary(KEY_BUILD_NUMBER, realVersion);//（Build.DISPLAY);
        	}
        }
		...
		// Start by luqiang, eastaeon, 20160916 [del custom_build_version]
		// Remove for custom_build_version
		removePreference("custom_build_version");
		// End by luqiang, eastaeon, 20160916 [del custom_build_version]
        ...
        if (!isApkEnable(act, "com.adups.fota")) {
            removePreference("adupsfota_software_update");
        }
    }
    ...
    private boolean isApkEnable(Context ctx, String packageName) {
        PackageManager pm = ctx.getPackageManager();
        ApplicationInfo appInfo = null;
        try {
            pm.getPackageInfo(packageName, PackageManager.GET_ACTIVITIES);
        ...
        return appInfo != null ? appInfo.enabled : false;
    }
}

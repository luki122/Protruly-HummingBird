...
//            IndexColumns.DATA_ENTRIES
    ...
    public void updateFromClassNameResource(String className, boolean rebuild,
        ...
        if (WifiSettings.class.getName().equals(className)
                || BluetoothSettings.class.getName().equals(className)
                || SecuritySettings.class.getName().equals(className)) {
            return;
        }
    ...
    private String buildSearchWhereStringForColumns(String query, String[] columnNames) {
        ...
        sb.append("(");
        sb.append(buildSearchMatchStringForColumns(query, columnNames));
        sb.append(")");
        sb.append(" AND ");
    ...
    private void indexFromResource(Context context, SQLiteDatabase database, String localeStr,
        ...
        try {
            ...
            if (shouldNotIndexPreference(context, attrs)) {
                return;
            }
            ...
            while ((type = parser.next()) != XmlPullParser.END_DOCUMENT
                ...
                if (nodeName.equals(NODE_NAME_PREFERENCE_CATEGORY)) {
                    continue;
                }
    ...
    private void indexFromProvider(Context context, SQLiteDatabase database, String localeStr,
        ...
        if (rawList != null) {
            ...
            for (int i = 0; i < rawSize; i++) {
                ...
                final int itemIconResId = (raw.iconResId == 0) ? iconResId : raw.iconResId;
                final String itemClassName = (raw.className == null) ? className : raw.className;
                final int itemRank = (raw.rank == 0) ? rank : raw.rank;
                updateOneRowWithFilteredData(database, localeStr,
                        raw.title,
                        raw.summaryOn,
                        raw.summaryOff,
    ...
    private void updateOneRow(SQLiteDatabase database, String locale,
        ...
        if (intentTargetPackage != null && intentTargetPackage.equals("com.android.phone")) {
            return;
        }
     ...
     * @return the data should disable view
     * @date Liuqin on 2017-04-21
     */
    private boolean shouldNotIndexPreference(Context context, AttributeSet attrs) {
        return getDataBoolean(context, attrs,
                com.android.internal.R.styleable.Preference,
                com.android.internal.R.styleable.Preference_shouldDisableView);
    }
     ...
     * @date Liuqin on 2017-04-21
     */
    private boolean getDataBoolean(Context context, AttributeSet set, int[] attrs, int resId) {
        ...
        if (tv != null && tv.type == TypedValue.TYPE_INT_BOOLEAN) {
            if (tv.resourceId != 0) {
                result = context.getResources().getBoolean(tv.resourceId);
            } else {
                result = XmlUtils.convertValueToBoolean(tv.coerceToString(), false);
            }
        }
     ...
     * A basic AsyncTask for saving a Search query into the database
     */
    private class SaveSearchDeleteTask extends AsyncTask<String, Void, Long> {
        ...
        protected Long doInBackground(String... params) {
            ...
            try {
                if (!TextUtils.isEmpty(params[0])) {
                    ...
                    database.delete(Tables.TABLE_SAVED_QUERIES, null, null);
                }
            } catch (Exception e) {
                Log.d(LOG_TAG, "Cannot update saved Search queries", e);
            }
    ...
    public void deleteSavedQuery(String query) {
        final SaveSearchDeleteTask task = new SaveSearchDeleteTask();
        task.execute(query);
    }
}

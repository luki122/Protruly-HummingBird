...
public class PrivateVolumeSettings extends SettingsPreferenceFragment {
    ...
    private static final long MIN_TATAL_SISE = 50 * 1024;
    ...
    private List<hb.preference.PreferenceCategory> mHeaderPreferencePool = Lists.newArrayList();
...
//                result.value, result.units));
        final BytesResult resultFreeBytes = formatBytes(getResources(), freeBytes, 0);
        mSummary.setSummary(getString(R.string.storage_volume_header,
                getReadableRealStorageSize(context, totalBytes, 0),
                TextUtils.expandTemplate(getText(R.string.storage_size_large), resultFreeBytes.value, resultFreeBytes.units)));
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference pref) {
        ...
        if (itemTitleId != R.string.storage_detail_cached) {
    ...
    public static String getReadableRealStorageSize(Context context, long sizeBytes, int flag) {
        if (context == null) {
            return "";
        }
        BytesResult res = Formatter.formatBytes(context.getResources(), sizeBytes, flag);
        int realSize = adjustSize(Float.valueOf(res.value).intValue());
        res = new BytesResult(realSize + "", res.units, res.roundedBytes);
        return context.getString(com.android.internal.R.string.fileSizeSuffix,
                res.value, res.units);
    }

    private static int adjustSize(int similarSize) {
        int realSize = 2;
        while (realSize < similarSize) {
            realSize *= 2;
        }
        return realSize;
    }
    ...
    public static BytesResult formatBytes(Resources res, long sizeBytes, int flags) {
        ...
        roundedString = removeEndZero(roundedString);
    ...
    public static String formatFileSize(Context context, long sizeBytes) {
        ...
        return TextUtils.expandTemplate(context.getText(R.string.storage_size_large), res.value, res.units).toString();
    }

    private static String removeEndZero(String roundedString) {
        if (TextUtils.isEmpty(roundedString)
                || !roundedString.contains(".")) {
            return roundedString;
        }
        String result = roundedString;
        try {
            char ch;
            int count = 0;
            int N = roundedString.length();
            for (int i = N - 1; i >= 0; i--) {
                ch = roundedString.charAt(i);
                if (ch == '0') {
                    count++;
                } else if (ch == '.') {
                    count++;
            ...
            if (count > 0) {
                result = roundedString.subSequence(0, roundedString.length() - count) + "";
            }
        } catch (Exception e) {
            e.printStackTrace();
        }

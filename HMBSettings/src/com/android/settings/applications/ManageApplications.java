...
import hb.preference.PreferenceFrameLayout;
...
import com.android.settings.utils.HanziToPinyin;
...
public class ManageApplications extends InstrumentedFragment
    ...
    // The order which they appear is the order they will show when HbSpinner is present.
    ...
    private HbSpinner mFilterSpinner;
    ...
    private static final boolean IS_OPTION_MENU_SHOW_NOTIFICATION = false;
    private HbIndexBar mIndexbar;
    private int mIndexbarFirstPosition = -1;
    private List<Integer> mIndexbarLetterList = new ArrayList<>();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (className.equals(AllApplicationsActivity.class.getName())) {
            ...
            setToolbarTitle(R.string.usage_access_title);
            ...
            setToolbarTitle(R.string.system_alert_window_access_title);
        } else if (className.equals(WriteSettingsActivity.class.getName())) {
            mListType = LIST_TYPE_WRITE_SETTINGS;
            setToolbarTitle(R.string.write_settings_title);
        } else {
            mListType = LIST_TYPE_MAIN;
        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (mListContainer != null) {
            ...
            HbListView lv = (HbListView) mListContainer.findViewById(android.R.id.list);
            if (emptyView != null) {
    ...
    private void createHeader() {
        ...
        mFilterSpinner = (HbSpinner) mSpinnerHeader.findViewById(R.id.filter_spinner);
...
//            mFilterAdapter.enableFilter(FILTER_APPS_NO_PEEKING);
        ...
        if (mListType == LIST_TYPE_NOTIFICATION) {
            mApplications.setOverrideFilter(LocalSettings.NOTIFICATION_FILTER);
        }
    }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        if (mListType == LIST_TYPE_NOTIFICATION && !IS_OPTION_MENU_SHOW_NOTIFICATION) {
            return;
        }
    ...
    static class ApplicationsAdapter extends BaseAdapter implements Filterable,
        ...
        public void rebuild(boolean eraseold) {
            ...
            switch (mLastSortMode) {
                case R.id.sort_order_size:
                    ...
                    break;
                default:
                    mManageApplications.mIndexbarLetterList.clear();
                    comparatorObj = mManageApplications.ALPHA_COMPARATOR;
                    break;
            }
            switch (mManageApplications.mListType) {
                case LIST_TYPE_NOTIFICATION:
                    comparatorObj = LocalSettings.APP_NOTIFICATION_TYPE_COMPARATOR;
                    break;
            }
            ArrayList<ApplicationsState.AppEntry> entries
        ...
        public int getCount() {
            int count = mEntries != null ? mEntries.size() : 0;
            return count > 0 ? count + getHeaderCount() : 0;
        }

        public Object getItem(int position) {
            if(hasHeader()) {
                if (getItemViewType(position) == TYPE_ITEM) {
                    position = getItemRealPosition(position);
                } else {
                    return null;
                }
            }
        ...
        public ApplicationsState.AppEntry getAppEntry(int position) {
            return (ApplicationsState.AppEntry)getItem(position);
        }
        ...
        private View getItemView(int position, View convertView, ViewGroup parent) {
            ...
            synchronized (entry) {
                ...
                Drawable icon = LocalSettings.getIconDrawable(mContext, entry.info.packageName);
                if (icon == null) {
            ...
            switch (mManageApplications.mListType) {
                case LIST_TYPE_NOTIFICATION:
                    if (holder.entry.extraInfo != null
                            && holder.entry.extraInfo instanceof AppRow) {
                ...
                case LIST_TYPE_USAGE_ACCESS:
                    if (holder.entry.extraInfo != null
                            && holder.entry.extraInfo instanceof PermissionState) {
        ...
        public int getItemViewType(int position) {
            if(hasHeader()) {
                ...
                if (position == 0) {
                    ...
                    int pos = 0;
                    for (int i = 0; i < mHeaderSize.length; i++) {
                        pos += mHeaderSize[i] + 1;
                        if(position == pos) {
                            type = TYPE_HEADER;
                            break;
        ...
        public void notifyDataSetChanged() {
            setHeader();
            mManageApplications.indexbarSetLeterEnable();
        ...
        private void setHeader() {
            int headerCount = 0;
            if (mManageApplications.mListType == LIST_TYPE_NOTIFICATION) {
                if (LocalSettings.NOTIFICATION_FILTER.banned > 0) {
                    headerCount++;
                }
                if (LocalSettings.NOTIFICATION_FILTER.nonBanned > 0) {
                    headerCount++;
                }
                if (headerCount > 0) {
                    mHeaderSize = new int[headerCount];
                    if (headerCount == 2) {
                        mHeaderSize[0] = LocalSettings.NOTIFICATION_FILTER.banned;
                        mHeaderSize[1] = LocalSettings.NOTIFICATION_FILTER.nonBanned;
                    } else if (LocalSettings.NOTIFICATION_FILTER.nonBanned > 0) {
                        mHeaderSize[0] = LocalSettings.NOTIFICATION_FILTER.nonBanned;
                    } else {
                        mHeaderSize[0] = LocalSettings.NOTIFICATION_FILTER.banned;
                    }
                }
        ...
        private int getHeaderCount() {
            return mHeaderSize != null ? mHeaderSize.length : 0;
        }

        /**
         * Gets item real position.
         *
         * @param position the position
         * @return the item real position
         * @date Liuqin on 2017-03-23
         */
        private int getItemRealPosition(int position) {
            int realPosition = position;
            if(hasHeader()) {
                int totalItem = 0;
                for (int i = 0; i < mHeaderSize.length; i++) {
                    totalItem += mHeaderSize[i] + 1;
        ...
        private View getHeaderView(int position, View convertView, ViewGroup parent) {
            convertView = mManageApplications.mInflater.inflate(
                    LocalSettings.Layout.PREFERENCE_CATEGORY_MATERIAL, null);
            TextView titleView = (TextView) convertView.findViewById(android.R.id.title);
            if (titleView != null) {
                titleView.setText(getHeaderTitle(position));
            }
        ...
        private String getHeaderTitle(int position) {
            String title = null;
            if (mManageApplications.mListType == LIST_TYPE_NOTIFICATION) {
                if (mHeaderSize.length == 2) {
                    if (position == 0) {
                        title = mContext.getString(R.string.notification_disabled_count_header, mHeaderSize[0]);
                    } else {
                        title = mContext.getString(R.string.notification_enabled_count_header, mHeaderSize[1]);
                    }
                } else if (LocalSettings.NOTIFICATION_FILTER.banned > 0) {
                    title = mContext.getString(R.string.notification_disabled_count_header, mHeaderSize[0]);
                } else {
                    title = mContext.getString(R.string.notification_enabled_count_header, mHeaderSize[0]);
                }
            }
            return title;
        }

        public int getPositionForSelection(char section) {
            final int itemCount = getCount();
            AppEntry appEntry;
            for (int i = 0; i < itemCount; i++) {
                appEntry = (AppEntry) getItem(i);
                if (appEntry != null) {
                    char firstChar = mManageApplications.indexbarGetFirstChar(appEntry);
                    if ((firstChar == section)
                            || (section == '#' && !mManageApplications.indexbarIsAlpha(firstChar))) {
                        return i;
                    }
                }
            }
    ...
    private void setToolbarTitle(int resId) {
        ...
        Toolbar toolbar = activity != null ? activity.getToolbar() : null;
        if (toolbar != null) {
            toolbar.setTitle(resId);
        }
    }

    private void indexbarInit() {
        if (mListType != LIST_TYPE_NOTIFICATION) {
            mIndexbar = (HbIndexBar) mListContainer.findViewById(R.id.indexbar);
            mIndexbar.setVisibility(View.VISIBLE);
            mIndexbar.deleteLetter(0);
            mIndexbar.setOnSelectListener(new HbIndexBar.OnSelectListener() {
                @Override
                public void onSelect(int index, int layer, HbIndexBar.Letter letter) {
                    if (letter != null) {
                        int position = mApplications.getPositionForSelection(letter.text.charAt(0));
                        if (position >= 0) {
            ...
            mListView.setOnScrollListener(new AbsListView.OnScrollListener() {
                ...
                public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount,
                                     int totalItemCount) {
                    indexbarRefreshCurrentLetter(firstVisibleItem);
                }
            });
        }
    }

    private void indexbarSetLeterEnable(){
        if (mIndexbar != null) {
            int count = mApplications.getCount();
            if (count == 1) {
                AppEntry appEntry = (AppEntry) mApplications.getItem(0);
                if (appEntry != null) {
                    char ch = indexbarGetFirstChar(appEntry);
                    indexbarAddNewChar(ch);
                }
            }
            if (mIndexbarLetterList.size() > 0 && count > 0) {
                mIndexbar.setVisibility(View.VISIBLE);
                int N = mIndexbar.size();
                boolean isEnabled;
                for (int i = 0; i < N; i++) {
                    isEnabled = mIndexbarLetterList.contains(i);
                    mIndexbar.getLetter(i).enable = isEnabled;
                }
                mIndexbar.update();

                // refresh letter focus
                int firstPosition = mIndexbarFirstPosition;
                mIndexbarFirstPosition = -1;
                int maxIndex = count - 1;
                firstPosition = firstPosition > maxIndex ? maxIndex : firstPosition;
                indexbarRefreshCurrentLetter(firstPosition);
            } else {
    ...
    private void indexbarEnableLeter(){
        if (mIndexbar != null) {
            int N = mIndexbarLetterList.size();
            if (N > 0) {
                int[] letterArray = new int[N];
                for (int i = 0; i < N; i++) {
                    letterArray[i] = mIndexbarLetterList.get(i).intValue();
                }
                mIndexbar.setEnables(true, letterArray);
            }
        }
    }

    private void indexbarRefreshCurrentLetter(int firstPosition) {
        if (mIndexbar != null && mIndexbarFirstPosition != firstPosition
                && firstPosition < mApplications.getCount()) {
            AppEntry appEntry = (AppEntry) mApplications.getItem(firstPosition);
            char firstChar = indexbarGetFirstChar(appEntry);
            if (firstChar != 0) {
                int index;
                if (!indexbarIsAlpha(firstChar)) {
                    index = mIndexbar.size() - 1;
                } else {
                    index = firstChar - 'A';
                }
                mIndexbar.setFocus(index);
                mIndexbarFirstPosition = firstPosition;
            }
        }
    }

    private Comparator<ApplicationsState.AppEntry> ALPHA_COMPARATOR =
            new Comparator<ApplicationsState.AppEntry>() {
                @Override
                public int compare(ApplicationsState.AppEntry o1, ApplicationsState.AppEntry o2) {
                    char leftChar = indexbarGetFirstChar(o1);
                    char rightChar = indexbarGetFirstChar(o2);
                    indexbarAddNewChar(leftChar);
                    indexbarAddNewChar(rightChar);
                    boolean isLeftAlpha = indexbarIsAlpha(leftChar);
                    boolean isRightAlpha = indexbarIsAlpha(rightChar);
                    if (isLeftAlpha && isRightAlpha) {
                        return leftChar - rightChar;
                    } else if (isLeftAlpha && !isRightAlpha) {
                        return -1;
                    } else if (!isLeftAlpha && isRightAlpha) {
                        return 1;
                    } else {
    ...
    private char indexbarGetFirstChar(AppEntry appEntry) {
        char firstChar = 0;
        if (appEntry != null && !TextUtils.isEmpty(appEntry.label)) {
            if ("###".equals(appEntry.info.secondaryCpuAbi)) {
                firstChar = (char) appEntry.info.fullBackupContent;
            } else {
                String firstString = HanziToPinyin.getInstance().transliterate(appEntry.label.substring(0, 1));
                if (!TextUtils.isEmpty(firstString)) {
                    firstChar = firstString.charAt(0);
                } else {
                    firstChar = '#';
                }
                indexbarSetFirstChar(appEntry, firstChar);
            }
        }
        return firstChar;
    }

    private void indexbarSetFirstChar(AppEntry appEntry, char ch) {
        if (appEntry != null) {
            appEntry.info.fullBackupContent = ch;
            appEntry.info.secondaryCpuAbi = "###";
        }
    }

    private boolean indexbarIsAlpha(char firstChar) {
        return firstChar >= 'A' && firstChar <= 'Z';
    }

    private void indexbarAddNewChar(char firstChar) {
        if (mIndexbar == null) {
            return;
        }
        Integer index;
        ...
        if (!mIndexbarLetterList.contains(index)) {
            mIndexbarLetterList.add(index);
        }
    }

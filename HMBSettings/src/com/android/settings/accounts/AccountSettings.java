...
public class AccountSettings extends SettingsPreferenceFragment
    ...
    public static final String PROTRULY_ACCOUNT_TYPE = "com.protruly.AccountType";
    ...
    private Preference mProtrulyPreference;
    private PreferenceScreen mCustomProtrulyPreference;
    private long mLastJumpToProtrulyAccountTime = 0;
...
//        cleanUpPreferences();
    ...
    public boolean onPreferenceClick(Preference preference) {
        if (PROTRULY_ACCOUNT_TYPE.equals(preference.getKey())) {
            if (mProtrulyPreference != null) {
//                mProtrulyPreference.performClick(getPreferenceScreen());
                jumpToProtrulyLogin();
            } else {
                jumpToProtrulyLogin();
            }
    ...
    void updateUi() {
        ...
        if(mUm.isLinkedUser()) {
            ...
            initProtrulyAccount(!addCategory);
    ...
    private void updateProfileUi(final UserInfo userInfo, boolean addCategory,
        ...
        if (addCategory) {
            ...
            if (!isProtrulyAccountShow()) {
                profileData.preferenceGroup = parent;
            } else {
                profileData.preferenceGroup = getOthersPreferenceCategory(context);
            }
        }
...
//        preference.setIcon(R.drawable.ic_menu_add);
    ...
    private void updateAccountTypes(ProfileData profileData) {
        ...
        if (profileData.userInfo.isEnabled()) {
            ...
            boolean isProtrulyAccountExist = isProtrulyAccountShow();
            for (int i = 0; i < count; i++) {
                if (isProtrulyAccountExist
                        && PROTRULY_ACCOUNT_TYPE.equals(preferences.get(i).mAccountType)) {
                    mProtrulyPreference = preferences.get(i);
                    continue;
                }
            ...
            if (isProtrulyAccountExist) {
                updateProtrulyPreference(getContext(), profileData.userInfo.getUserHandle(), profileData.authenticatorHelper);
            }
        } else {
        ...
        for (int i = 0; i < accountTypes.length; i++) {
            ...
            if (skipToAccount) {
                ...
                accountTypePreferences.add(new AccountPreference(getActivity(), label,
                        ...
                        fragmentArguments,
                        helper.getDrawableForType(getActivity(), accountType), accountType));
            } else {
                final Bundle fragmentArguments = new Bundle();
    ...
    private class AccountPreference extends Preference implements OnPreferenceClickListener {
        ...
        public AccountPreference(Context context, CharSequence title, String titleResPackageName,
                ...
                Drawable icon, String accountType) {
            super(context);
            mTitle = title;
            ...
            setLayoutResource(LocalSettings.getPreferenceScreenLayout2(getContext()));
        }
            ...
            builder.setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ContentResolver.setMasterSyncAutomaticallyAsUser(mEnabling,
                            mIdentifier);
                    getTargetFragment().onPrepareOptionsMenu(((FullActivityBase)getActivity()).getToolbar().getMenu());
                }
            });
    ...
    private void initProtrulyAccount(boolean show) {
        if (show) {
            mCustomProtrulyPreference = (PreferenceScreen) getPreferenceScreen().findPreference(PROTRULY_ACCOUNT_TYPE);
            mCustomProtrulyPreference.setLayoutResource(LocalSettings.getPreferenceScreenLayout2(getContext()));
        } else {
            removePreference(PROTRULY_ACCOUNT_TYPE);
            mCustomProtrulyPreference = null;
        }
    }

    private boolean isProtrulyAccountShow() {
        return mCustomProtrulyPreference != null;
    }

    private PreferenceCategory getOthersPreferenceCategory(Context context) {
        PreferenceCategory category = new PreferenceCategory(context);
        category.setTitle(getString(R.string.account_others));
        category.setOrder(1);
        return category;
    }

    private Preference updateProtrulyPreference(Context context, UserHandle userHandle, AuthenticatorHelper helper) {
        if (mCustomProtrulyPreference == null) {
            return null;
        }
        final Account[] accounts = AccountManager.get(getActivity())
                .getAccountsByTypeAsUser(PROTRULY_ACCOUNT_TYPE, userHandle);
        String accountName = (accounts != null && accounts.length > 0)
                ? accounts[0].name : getString(R.string.account_not_login);

//        if (helper != null && helper.containsAccountType(PROTRULY_ACCOUNT_TYPE)) {
//            mCustomProtrulyPreference.setIcon(helper.getDrawableForType(getActivity(), PROTRULY_ACCOUNT_TYPE));
//        }
        mCustomProtrulyPreference.setStatus(accountName);
        mCustomProtrulyPreference.setOnPreferenceClickListener(this);
        mCustomProtrulyPreference.getLayoutResource();
        return mCustomProtrulyPreference;
    }

    private void jumpToProtrulyLogin() {
        if (isQuickToJump()) {
            return;
        }
        ...
        accountManager.addAccount(PROTRULY_ACCOUNT_TYPE, null, null,
    ...
    private boolean isQuickToJump() {
        boolean result = false;
        long currentTime = SystemClock.elapsedRealtime();
        if (currentTime - mLastJumpToProtrulyAccountTime < 1000) {
            result = true;
        } else {
            mLastJumpToProtrulyAccountTime = SystemClock.elapsedRealtime();
        }
        return result;
    }
}

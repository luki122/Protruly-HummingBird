...
public class GlowPadView extends View {
    ...
    public GlowPadView(Context context, AttributeSet attrs) {
        ...
        //add by lgy
        mHbPoint = new HbPoint(context); 
        mSlideAnimView = new SlideAnimView(context);
//        initGesture(this);
    }
...
//            mHandleDrawable = new TargetDrawable(getResources(), resourceId, 2);
            mHandleDrawable = new CenterTargetDrawable(getResources(), resourceId);
            mHandleDrawable.setState(TargetDrawable.STATE_INACTIVE);
        }
        /// @}
    }
...
//            }
    ...
    //modigy by lgy 
    protected void stopAndHideWaveAnimation() {
...
//        mPointCloud.glowManager.setY(mOuterRing.getY() + dy);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        //add by lgy 
        mHbPoint.setScale(mRingScaleFactor);
        mHbPoint.setCenter(mWaveCenterX, mWaveCenterY);   
        mSlideAnimView.setCenter(mWaveCenterX, mWaveCenterY);   
...
//           	mHbPoint.draw(canvas);	
           	mSlideAnimView.draw(canvas);	
        } else {
    ...
    private void startWaveAnimationV2() {
	    ...
	    mWaveAnimations.add(createArrowTweener());
	    mWaveAnimations.add(createCircleTweener(0));
	    
        mWaveAnimations.start();
    }   
    
    private SlideAnimView mSlideAnimView;
    private Tweener createArrowTweener() {
    	int duration = 1200;
    	PropertyValuesHolder pvh = PropertyValuesHolder.ofInt("distance",
 				-60, 140);
    	ObjectAnimator mArrowAnimator = ObjectAnimator.ofPropertyValuesHolder(
    			mSlideAnimView, pvh);
     	mArrowAnimator.setDuration(duration);
		mArrowAnimator.setInterpolator(new LinearInterpolator());
		mArrowAnimator.addUpdateListener(mUpdateListener);
//		mArrowAnimator.addListener(new AnimatorListenerAdapter() {
//            public void onAnimationEnd(Animator animator) {
//            	mSlideAnimView.setDistance(0);
//            }
//        });
		mArrowAnimator.setRepeatCount(ValueAnimator.INFINITE);
	    return new Tweener(mArrowAnimator);
    }

    private Tweener createCircleTweener(final int index) {
    	int duration = 1200;
    	PropertyValuesHolder pvha = PropertyValuesHolder.ofFloat("alpha",
 				0.2f, 0.7f, 0.2f);
    	ObjectAnimator mScaleAnimator = ObjectAnimator.ofPropertyValuesHolder(
    			mHandleDrawable.mCircleManager[index], pvha);
     	mScaleAnimator.setDuration(duration);
		mScaleAnimator.setInterpolator(new LinearInterpolator());
		mScaleAnimator.setRepeatCount(ValueAnimator.INFINITE);
	    return new Tweener(mScaleAnimator);
    }
    ...
    private float mDensity = 3f;
	 private void initGesture(View view) {
	        ...
	        gDetector = new GestureDetector(view.getContext(), new OnGestureListener() {
	            ...
	            public boolean onFling(MotionEvent e1, MotionEvent e2,
	                ...
	                if (y1 - y2 > 150 * mDensity) {
	                    final Call incomingCall = CallList.getInstance()
	                            .getIncomingCall();
	                    if (incomingCall != null) {
	                        Log.v(TAG, "onFling reject ");
	                        final InCallActivity activity = InCallApp.getInCallActivity();

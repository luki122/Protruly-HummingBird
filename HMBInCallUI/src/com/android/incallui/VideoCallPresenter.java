...
public class VideoCallPresenter extends Presenter<VideoCallPresenter.VideoCallUi> implements
    ...
    public void onStateChange(InCallPresenter.InCallState oldState,
        ...
        if (disconnectedCall != null &&
            ...
            /// M: if there is no any call contained the same video id with
            // disconnectedCall, it should clear the surface, otherwise ignore the action. @{
            if (findVideoCallByVideoId(callList, disconnectedCall) == null) {
                Log.d(this,"clear surface for disconnected call, " + disconnectedCall);
                disconnectSurfaces(disconnectedCall.getVideoCall());
            } else {
                Log.d(this,"ignore to clear surface. videoCall " + disconnectedCall);
            }
            /// @}
        } else {
            /// M: debugging log enhancement for VT call to clear surface.
            Log.d(this,"disconnectedCall " + disconnectedCall);
        }
        /// M: Avoid to clear surface more than once for the same VT call.
        mLastDisconnectedCall = disconnectedCall != null ? disconnectedCall : mLastDisconnectedCall;
        /// @}
    ...
    private void exitVideoModeForCall(Call call) {
        ...
        if (!Objects.equals(call, mPrimaryCall)) {
            Log.w(this, "Not exit video mode since is a different call, call: " + call
                    + " mPrimaryCall: " + mPrimaryCall);
            return;
        }
        showVideoUi(null, VideoProfile.STATE_AUDIO_ONLY, Call.State.ACTIVE);
        InCallPresenter.getInstance().setFullScreen(false);
    ...
    private Call findVideoCallByVideoId(CallList callList, Call original) {
        Bundle ori_extras = original.getTelecommCall().getDetails().getExtras();
        String ori_id = ori_extras != null ? ori_extras.getString(KEY_VIDEO_PROVIDER_ID) : null;
        if (TextUtils.isEmpty(ori_id)) {
            Log.d(this,"findVideoCallByVideoId, no extras or id, call is " + original);
            return null;
        } else {
            Log.d(this,"findVideoCallByVideoId, id= " + ori_id + ", original is  " + original);
        }
        ...
        for (android.telecom.Call telecomCall : list) {
            if (Objects.equals(original.getTelecommCall(), telecomCall)) {
                continue;
            }
            Call call = callList.getCallByTelecommCall(telecomCall);
            if (call.getState() == Call.State.DISCONNECTING
                    || call.getState() == Call.State.DISCONNECTED) {
                Log.d(this,"findVideoCallByVideoId, ignore the disconnected call " + call);
                continue;
            }
            ...
            if (!TextUtils.isEmpty(id) && ori_id.equals(id)) {
                Log.d(this,"findVideoCallByVideoId, found, id=" + id + ", call is" + call);
                return call;
            }
        }
        return null;

...
public class CallButtonPresenter extends Presenter<CallButtonPresenter.CallButtonUi>
    ...
    //add by lgy
    private boolean updateButtonsStateHb(Call call) {
        Log.v(this, "updateButtonsStateHb");
        final CallButtonUi ui = getUi();
        ...
        boolean enable = state == Call.State.ACTIVE||state == Call.State.ONHOLD;
        boolean enable2 = Call.State.isConnectingOrConnected(state);
         final boolean showHold = !showSwap && (state == Call.State.ONHOLD || state == Call.State.ACTIVE);
        boolean add = state == Call.State.IDLE || state == Call.State.ACTIVE;  
        
        Log.v(this, "showSwap = " + showSwap);
        Log.v(this, "showMerge = " + showMerge);
        
        ui.enableButton(BUTTON_HOLD, showHold && !showMerge);

        ui.showButton(BUTTON_AUDIO, true);
        ui.enableButton(BUTTON_AUDIO, enable2);   
        ui.showButton(BUTTON_MUTE, true);
        ui.enableButton(BUTTON_MUTE, enable);
        ui.showButton(BUTTON_ADD_CALL, !showSwap);
        ui.enableButton(BUTTON_ADD_CALL, add && !showMerge);
        ui.showButton(BUTTON_CONTACTS, !showMerge);
        ui.enableButton(BUTTON_CONTACTS, add && !showSwap);
        ui.showButton(BUTTON_DIALPAD, true);
        //zhangcj modify for button enable state when phone state
        ui.enableButton(BUTTON_DIALPAD, enable);   
        ui.showButton(BUTTON_MERGE, showMerge);
        ui.showButton(BUTTON_SWAP, showSwap);

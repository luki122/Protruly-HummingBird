            ...
            try {
                ...
                if (mCallerInfo == null) {
                    ...
                    if (cw.event == EVENT_EMERGENCY_NUMBER) {
                    	...
                    	//zhangcj modify in 20170426
                       // mCallerInfo = new CallerInfo().markAsEmergency(mQueryContext);
                    	  mCallerInfo = CallerInfo.getCallerInfo(mQueryContext, mQueryUri, cursor);
                    	  mCallerInfo.markAsEmergency(mQueryContext);
                        ...
                        mCallerInfo = CallerInfo.getCallerInfo(mQueryContext, mQueryUri, cursor);
                        //add by lgy
                        mCallerInfo.updataHbInfo(cw.number);
                        ...
                        if (ENABLE_UNKNOWN_NUMBER_GEO_DESCRIPTION) {
                            ...
                            if (TextUtils.isEmpty(mCallerInfo.name)) {
                                ...
                                */
                                ...
                                if (cw.cookie != null && cw.cookie instanceof Call) {
                                    subId = InCallUtils.getSubId((Call) cw.cookie);
                                }
                        ...
                        if (!TextUtils.isEmpty(cw.number)) {
                            ...
                            mCallerInfo.phoneNumber = PhoneNumberHelper.formatNumber(cw.number,
                                    ...
                                    TelephonyManagerUtils.getCurrentCountryIso(mQueryContext,
                                            Locale.getDefault()));
                            /**
                             * M: [log optimize]add log for debugging potential
                             * time-consuming flow. @{
                             */
                            long spentTime = SystemClock.currentThreadTimeMillis() - startTime;
                            if (spentTime > 1000) {
                                Log.e(this, "It spent too long to formatNumber: " + spentTime);
                            }
                            /** @} */
                        }
                    }
...
//zhangcj modfy for query in 20170426 start
...
//                .build();
        final Uri contactRef = Uri.withAppendedPath(Uri.parse("content://com.android.contacts/hb_phone_lookup_enterprise"),
				Uri.encode(info.phoneNumber)).buildUpon()
                .appendQueryParameter(PhoneLookup.QUERY_PARAMETER_SIP_ADDRESS,
                        String.valueOf(PhoneNumberHelper.isUriNumber(info.phoneNumber)))
                .build();
              //zhangcj modfy for query in 20170426 end
        if (DBG) {
        ...
        c.mHandler.startQuery(token,
                              ...
                              contactRef,  // uri
                              null,  // projection
                              null,/*"indicate_phone_or_sim_contact<0"*/ // selection
                              null,  // selectionArgs
                              null);  // orderBy
        return c;
    }

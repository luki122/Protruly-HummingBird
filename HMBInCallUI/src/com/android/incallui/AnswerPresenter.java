...
public class AnswerPresenter extends Presenter<AnswerPresenter.AnswerUi>
    ...
    private boolean showAnswerUi(boolean show) {
    	//add by lgy 
    	if(CallList.getInstance().hasCallWaiting() && show) {
    ...
    public void onCallChanged(Call call) {
        ...
        if (call.getState() != Call.State.INCOMING) {
            ...
            //zhangcj modfiy for call disconneted dialog maybe not close in 20170425
            getUi().dismissPendingDialogs();
	...
	public void rejectCallWithMessage(String message) {
		...
		//zhangcj add for modify 5709
		Uri callHandle = mCall.getHandle();
		// if (mCall.getHandle() != null &&
		// mCall.can(android.telecom.Call.Details.CAPABILITY_RESPOND_VIA_TEXT))
		// {
		if (isRespondViaSmsCapable(callHandle) && mCall.can(android.telecom.Call.Details.CAPABILITY_RESPOND_VIA_TEXT)) {
			TelecomAdapter.getInstance().sendMessageIfCallEnded(
					getUi().getContext(), mCall.getId(),
					mCall.getHandle().getSchemeSpecificPart(), message);
		}
		// / @}
		onDismissDialog();
	}
    ...
    //add by lgy 
    boolean mWithSms = true;
    public boolean  canSmsReject() {
    	return mWithSms;
    } 


   //zhangcj add 
	private boolean isRespondViaSmsCapable(Uri callHandle) {
		if (callHandle== null
				|| TextUtils.isEmpty(callHandle.getSchemeSpecificPart())) {
			// No incoming number known or call presentation is
			// "PRESENTATION_RESTRICTED", in
			// other words, the user should not be able to see the incoming
			// phone number.
			Toast.makeText(getUi().getContext(), getUi().getContext().getString(R.string.sms_reject_invalid_number_toast),
					Toast.LENGTH_SHORT).show();
			return false;
		}

		if (PhoneNumberUtils.isUriNumber(callHandle.toString())) {
			return false;
		}
		...
		if (SmsApplication.getDefaultRespondViaMessageApplication(getUi().getContext(),

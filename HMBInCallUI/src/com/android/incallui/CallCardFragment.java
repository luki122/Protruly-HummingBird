...
//        final Call call = calls.getFirstCall();
        ...
        if(calls.hasCallWaiting()) {
        	call = calls.getActiveCall();
        } 
...
//        if (TextUtils.isEmpty(number) && TextUtils.isEmpty(label)) {
    ...
    public void setCallState(
        ...
        if (CallUtils.isVideoCall(videoState)
        	...
        	//zhangcj delete for call active display
        ...
        if (callStateIcon != null) {
        	//zhangcj delete for call active display start
        	...
        	//zhangcj delete for call active display end
        } else {
    ...
    public void setPrimaryCallElapsedTime(boolean show, long duration) {
        ...
        DialpadFragment df = activity.getDialpadFragment();
        if (show) {
            ...
            if(df != null) {
            	if(df.getTime() != null) {
            		df.getTime().setText(callTimeElapsed);
            	}
            }
    ...
    private CallStateLabel getCallStateLabelFromState(int state, int videoState,
        ...
        switch  (state) {
            ...
            case Call.State.ACTIVE:
                ...
                /// @}
                	//zhangcj delete for call time display 
...
//        mCurrentSecondCallColor = secondCallColor;
    ...
    private TextView mArea, mMark, mTmsSlogan;
    
	private void initHbViews(View parent){
        mSimIcon = (ImageView) parent.findViewById(R.id.sim_icon);
        mArea = (TextView) parent.findViewById(R.id.area);
        mMark = (TextView) parent.findViewById(R.id.mark);
        mTmsSlogan = (TextView) parent.findViewById(R.id.tms_slogan);
        initGesture(parent);
	}
    ...
    public void setHbInfo(String area, MarkResult mark) {
        Log.v(this, "setHbInfo : area = " + area);
        Log.v(this, "setHbInfo : mark = " + mark);
		...
		if (mark != null) {
			...
			int count = mark.getTagCount();
            String countString = count == -1 ? getResources().getString(R.string.mark_by_user) : getResources().getString(R.string.mark_count, count);     	
			mMark.setText(" | " +countString + markName);
			mMark.setVisibility(View.VISIBLE);
			//zhangcj modify when usermark  mTmsSlogan view is gone
			if (count > 0) {
				mTmsSlogan.setVisibility(View.VISIBLE);
			}
		} else {
			mMark.setText("");
			mMark.setVisibility(View.GONE);
			mTmsSlogan.setVisibility(View.GONE);
		}
    }

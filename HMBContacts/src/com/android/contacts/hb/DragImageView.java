...
public class DragImageView extends ImageView {
	...
	private int hb_start_x,hb_start_y;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			hb_start_x=(int) event.getRawX();
			hb_start_y=(int) event.getRawY();
			onTouchDown(event);
			break;
		...
		case MotionEvent.ACTION_UP:
			Log.d(TAG,"ACTION_UP");
			mode = MODE.NONE;
			int hb_end_x= (int) event.getRawX();
			int hb_end_y= (int) event.getRawY();
			Log.d(TAG,"hb_start_x:"+hb_start_x+" hb_start_y:"+hb_start_y+" hb_end_x:"+hb_end_x+" hb_end_y:"+hb_end_y);
			if(Math.abs(hb_start_x-hb_end_x)<13&&Math.abs(hb_start_y-hb_start_y)<13){
				Log.d(TAG,"点击了");

...
public class CompactRawContactsEditorView extends LinearLayout implements View.OnClickListener {
    ...
    private static final String TAG = "CompactRawContactsEditorView-liumx";
    ...
    public interface Listener {
         ...
         */
        public void onContactCanSave(boolean nonEmpty);
    ...
    static final class NameEditorListener implements Editor.EditorListener {
        ...
        public void onRequest(int request) {
            if (request == Editor.EditorListener.FIELD_CHANGED) {
                ...
                hasName = (ContactsUtils.isGraphic(mValuesDelta.getDisplayName()));
            } else if (request == Editor.EditorListener.FIELD_TURNED_EMPTY) {
            	hasName = false;
                mValuesDelta.setSuperPrimary(false);
            } else if (request == Editor.EditorListener.FIELD_TURNED_NON_EMPTY) {
            	hasName = (ContactsUtils.isGraphic(mValuesDelta.getDisplayName()));
            }
            mListener.onContactCanSave(hasName || hasPhones());
        }
    ...
    private static ViewGroup mPhoneNumbers;
    private ViewGroup mEmails;
    private ViewGroup mOrganizations;
    private ViewGroup mNotes;
    private ViewGroup mOtherTypes;
    private Map<String, LinearLayout> mOtherTypesMap = new HashMap<>();
    private View mMoreFields;
    private Map<String, String> collapsedTypesMap = new HashMap<>();
//    private List<String> collapsedTypes = new ArrayList<>();
    private RawContactDelta defaultContactDelta;
    ...
    protected void onFinishInflate() {
        ...
        mOrganizations = (LinearLayout) findViewById(R.id.organizations);
        mNotes =  (LinearLayout) findViewById(R.id.notes);
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.more_fields && mListener != null ) {
        	final String[] menuItems = collapsedTypesMap.values().toArray(new String[collapsedTypesMap.size()]);
        	final String[] mimetypeItems = collapsedTypesMap.keySet().toArray(new String[collapsedTypesMap.size()]);
			...
			.setItems(menuItems, new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					String mimeType = mimetypeItems[which];
					LinearLayout ll = mOtherTypesMap.get(mimeType);
					...
					collapsedTypesMap.remove(mimeType);
					updateMoreMenu();
				}
			})
//			.setNegativeButton(getContext().getString(com.hb.R.string.cancel), null)
			.show();
        }
    }
    ...
    public void setEnabled(boolean enabled) {
        ...
        setEnabled(enabled, mOrganizations);
        setEnabled(enabled, mPhoneNumbers);
        setEnabled(enabled, mEmails);
        setEnabled(enabled, mNotes);
    ...
    public static boolean hasPhones() {
    	for (int i = 0; i < mPhoneNumbers.getChildCount(); i++) {
    		final View view = mPhoneNumbers.getChildAt(i);
			if (view instanceof KindSectionView) {
				final KindSectionView phoneEditorView = (KindSectionView) view;
    ...
    public void setState(RawContactDeltaList rawContactDeltas,
        ...
        defaultContactDelta = rawContactDeltas.get(0);
        setId(mViewIdGenerator.getId(defaultContactDelta, /* dataKind =*/ null,
        ...
        updateKindEditorEmptyFields(mOrganizations);
//        updateKindEditorIcons(mOrganizations);
...
//        updateKindEditorIcons(mEmails);
        updateKindEditorEmptyFields(mNotes);
...
//            updateKindEditorIcons(otherTypes.getValue());
        ...
        if (subscriberAccount.isAccountTypeIccCard(rawContactDeltas)
            ...
            mEmails.removeAllViews();
            mOrganizations.removeAllViews();
            mNotes.removeAllViews();
        }
        String displayName = getStructuredNameEditorView().getDisplayName();
        hasName = !TextUtils.isEmpty(displayName) && ContactsUtils.isGraphic(displayName) ;
        mListener.onContactCanSave(hasName || hasPhones());
        /// @}
        updateMoreMenu();
    }
...
//                    }
                    ...
                    kindSectionView.setListener(new KindSectionView.Listener() {
                        ...
                        public void onDeleteRequested(Editor editor) {
                            ...
                            updateKindEditorEmptyFields(mPhoneNumbers);
                            mListener.onContactCanSave(hasPhones() || hasName);
//                            updateKindEditorIcons(mPhoneNumbers);
                        }
...
//                    final LinearLayout otherTypeViewGroup;
                    final KindSectionView kindSectionView =
                            inflateKindSectionView(mOrganizations, dataKind, rawContactDelta);
                    kindSectionView.setListener(new KindSectionView.Listener() {
                        @Override
                        public void onDeleteRequested(Editor editor) {
                            if (kindSectionView.getEditorCount() == 1) {
                    ...
                    mOrganizations.addView(kindSectionView);
                    if (mOrganizations.getChildCount() > 1) {
                    	mOrganizations.removeViewAt(1);
                    }
                } else if (Note.CONTENT_ITEM_TYPE.equals(mimeType)) {
                  final KindSectionView kindSectionView =
                          inflateKindSectionView(mNotes, dataKind, rawContactDelta);
                  kindSectionView.setListener(new KindSectionView.Listener() {
                      @Override
                      public void onDeleteRequested(Editor editor) {
                          if (kindSectionView.getEditorCount() == 1) {
                  ...
                  mNotes.addView(kindSectionView);
                  if (mNotes.getChildCount() > 1) {
                	  mNotes.removeViewAt(0);
                  }
//                  mNotes.addView(kindSectionView);
              } else /*if (hasNonEmptyValuesDelta(rawContactDelta, mimeType, dataKind)) */{
//					if (hasNonEmptyValuesDelta(rawContactDelta, mimeType, dataKind) && collapsedTypesMap.containsKey(mimeType)) {
//						collapsedTypesMap.remove(mimeType);
//					}
                    final LinearLayout otherTypeViewGroup;
...
//                            updateKindEditorIcons(otherTypeViewGroup);
                    ...
                    updateKindEditorEmptyFields(otherTypeViewGroup);
                    if (!hasNonEmptyValuesDelta(rawContactDelta, mimeType, dataKind) ) {
                    	otherTypeViewGroup.setVisibility(View.GONE);
//                    	if (!collapsedTypes.contains(mimeType)) {
//                    		collapsedTypes.add(mimeType);
//                    	}
                    	if (!collapsedTypesMap.containsKey(mimeType)) {
                    		android.util.Log.e("liumx", "mimeType: "+ mimeType);
                    		collapsedTypesMap.put(mimeType, getContext().getString(dataKind.titleRes));
                    	}
                    }
                }
            }
...
//        Collapser.collapseList(result, ContactsApplicationEx.getContactsApplication());
        return result;
    }

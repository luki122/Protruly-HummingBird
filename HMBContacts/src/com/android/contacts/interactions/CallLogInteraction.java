...
//                TelecomManagerEx.EXTRA_SUGGESTED_PHONE_ACCOUNT_HANDLE, mPhoneAccountHandle);
    ...
    public String getViewBody(Context context) {
        ...
        android.util.Log.d(TAG,"numberType:"+numberType);
//        if (numberType == null) {                            liyang remove 2017-5-22
//            return null;
//        }
        
        return getCallTypeString(context);
...
//        Log.d(TAG, "accountName: " + accountName);
        ...
        return mValues.getAsString("sub_id");//modify by liyang 2017-5-11
    }
    /* @ } */
    ...
    private String formatDuration(Context mContext, long elapsedSeconds) {
		long minutes = 0;
		long seconds = 0;
    ...
    private String getCallTypeString(Context context) {
        ...
        String mDuration = formatDuration(context, getDuration());
        if (type == null) {
            return callType;
        }
        switch (type) {
            case Calls.INCOMING_TYPE:
//                callType = res.getString(R.string.hb_call_type_incoming)+" " +getDuration() + "秒";
                callType= getDuration()>0?res.getString(R.string.hb_call_type_incoming)+" " +mDuration:res.getString(R.string.hb_rejected_call);
                break;
            case Calls.MISSED_TYPE:
                callType = res.getString(R.string.hb_call_type_missed);
                break;
            case Calls.OUTGOING_TYPE:
                callType = res.getString(R.string.hb_call_type_outgoing) +" "+ mDuration;
                break;
        }
        return callType;
    }
	...
	public boolean betweenCall(long time) {
		long gap = time - getDate();
//		android.util.Log.i("liumx-betweenCall",
//				"time = " + time + ", date = " + getDate() + ", gap = " + gap + ",duration = " + getDuration());
		return gap >= 0 && gap <= getDuration() * 1000 + 2999;
	}
}

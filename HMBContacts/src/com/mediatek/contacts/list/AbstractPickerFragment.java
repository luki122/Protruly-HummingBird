...
ContactListMultiChoiceListener {
	...
	public boolean mIsSelectedNone = true;

	@Override
	public void onActivityCreated(Bundle savedInstanceState) {
		super.onActivityCreated(savedInstanceState);
		Log.i(TAG, "[onActivityCreated]");
		mSlectedItemsFormater = getActivity().getString(R.string.hb_menu_actionbar_selected_items);
	...
	protected void onItemClick(int position, long id) {
		...
		//			((ContactListMultiChoiceActivity)getActivity()).completeSelection();
		//		}
		//@}
	}
	...
	public boolean isSelectedAddInitial=false;
	@Override
	public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
		Log.i(TAG, "[onLoadFinished2]");
		...
		try{
			if (data != null && isSelectedAddInitial) {
				...
				while (data.moveToNext()) {
					...
					//默认全选
					getAdapter().getSelectedContactIds().add(data.getLong(ContactQuery.CONTACT_ID));
	...
	public void updateActionMode(int checkCount){
		if(mIsSelectedNone){
			actionMode.setTitle(getActivity().getString(R.string.hb_select_contacts));	
		}else{
			actionMode.setTitle(String.format(
					getActivity().getString(R.string.hb_menu_actionbar_selected_items),
					getAdapter().getSelectedContactIds().size()));
		}

		Log.d(TAG,"updateActionMode,checkCount:"+checkCount+" bottomBar:"+bottomBar);
		if(checkCount>0){
			if(bottomBar!=null && bottomBar.getVisibility()!=View.VISIBLE) bottomBar.setVisibility(View.VISIBLE);
			setIndexerHeight(getActivity().getResources().getDimensionPixelOffset(R.dimen.hb_index_bar_height1));
		}else{
			if(bottomBar!=null && bottomBar.getVisibility()!=View.GONE) bottomBar.setVisibility(View.GONE);
			setIndexerHeight(getActivity().getResources().getDimensionPixelOffset(R.dimen.hb_index_bar_height));
		}
	}


	private void setIndexerHeight(int height){
		RelativeLayout.LayoutParams params=(RelativeLayout.LayoutParams)mIndexBar.getLayoutParams();
		params.height=height;
		mIndexBar.setLayoutParams(params);
	}
	public void updateSelectedItemsView() {
		if(actionMode==null) return;
		Log.i(TAG, "[updateSelectedItemsView],getListView().getAdapter():"+getListView().getAdapter()+" isSearchMode():"+isSearchMode());
		if(/*isSearchMode() || */getListView().getAdapter()==null) return;
		...
		Log.i(TAG, "[updateSelectedItemsView]count1: " + count + ",checkCount:" + checkCount+" actionMode:"+actionMode);		
		...
		if(mIsSelectedAll){
			actionMode.setPositiveText(getActivity().getString(R.string.hb_actionmode_selectnone));
		}else{
			actionMode.setPositiveText(getActivity().getString(R.string.hb_actionmode_selectall));
		}
		if(count>0) actionMode.enableItem(ActionMode.POSITIVE_BUTTON,true);
		else actionMode.enableItem(ActionMode.POSITIVE_BUTTON,false);
	}
	...
	protected int getMultiChoiceLimitCount() {
		return TextUtils.equals(hbFilterString, "blacklist")?1000:DEFAULT_MULTI_CHOICE_MAX_COUNT;
	}

...
import com.android.providers.downloads.HbNetStateActivity.onDialogDismissListener;
...
//HB. Comments : marked blocking download task on waiting flag  , Engerineer : wxue , Date : 2017年6月6日 ,begin
 ...
 * from database query.
 */
public class DownloadInfo implements onDialogDismissListener{
    ...
    //HB. Comments : TODO , Engerineer : wxue , Date : 2017年6月7日 ,begin
    ...
    public enum NetworkState {
        ...
        //HB. Comments : diasble for waiting wifi , Engerineer : wxue , Date : 2017年6月6日 ,begin
        /**
         * The app requesting the download specific that it can only use wifi
         */
        UNUSABLE_FOR_WAITING_WIFI,
        //HB. end
    ...
    private boolean isReadyToDownload() {
        ...
        switch (mStatus) {
            ...
            //HB. Comments : waiting download , Engerineer : wxue , Date : 2017年6月5日 ,begin
            case Downloads.Impl.STATUS_WAITING_DOWNLOAD: 	
            //HB. end	
    ...
    private NetworkState checkIsNetworkTypeAllowed(int networkType, long totalBytes) {
        if (mIsPublicApi) {
            ...
            if (!allowAllNetworkTypes && (flag & mAllowedNetworkTypes) == 0) {
            	//HB. Comments : download fail for waiting wifi , Engerineer : wxue , Date : 2017年6月6日 ,begin
            	if(mAllowedNetworkTypes == DownloadManager.Request.NETWORK_WIFI){
            		return NetworkState.UNUSABLE_FOR_WAITING_WIFI;
            	}
            	//HB. end
                return NetworkState.TYPE_DISALLOWED_BY_REQUESTOR;
            }
    ...
    public boolean startDownloadIfReady(ExecutorService executor) {
        synchronized (this) {
            ...
            final boolean isActive = mSubmittedTask != null && !mSubmittedTask.isDone();
             //HB. Comments : TODO , Engerineer : wxue , Date : 2017年6月6日 ,begin
        	if(mThreadPoolExecutor == null){
        		mThreadPoolExecutor = (ThreadPoolExecutor)executor;
        	}
        	boolean isWaitingConfirm = HbDownManager.getInstance(mContext).isWaitingConfirm(this);
        	if(isWaitingConfirm){
        		return isReady;
        	}
        	//HB. end
        	//Log.i(TAG,"---startDownloadIfReady()--isReady = " + isReady + " isActive = " + isActive + " poolSize = " + mThreadPoolExecutor.getPoolSize()
        	//       + " id = " + mId + " mStatus = " + mStatus + " mControl = " + mControl);
            if (isReady && !isActive) {
            	//HB. Comments : TODO , Engerineer : wxue , Date : 2017年6月5日 ,begin
            	...
            	// 具备开始下载的条件后检查网络
            	NetworkInfo netInfo = mSystemFacade.getActiveNetworkInfo(mUid);
             	if(netInfo == null){
             		showNetStateDialog(HbNetStateActivity.DIALOG_TYPE_NO_NETWORK);
             		return isReady;
             	}else{
             		if(netInfo.isConnectedOrConnecting() && netInfo.getType() != ConnectivityManager.TYPE_WIFI){
             			showNetStateDialog(HbNetStateActivity.DIALOG_TYPE_MOBILE_CONNECT);
             			return isReady;
             		}
             	}
            	if(mThreadPoolExecutor.getPoolSize() >= mThreadPoolExecutor.getCorePoolSize()){
            		mStatus = Impl.STATUS_WAITING_DOWNLOAD;
            	}else if(mStatus != Impl.STATUS_RUNNING){
            		mStatus = Impl.STATUS_RUNNING;
            	}
            	 ContentValues values = new ContentValues();
                 values.put(Impl.COLUMN_STATUS, mStatus);
                 mContext.getContentResolver().update(getAllDownloadsUri(), values, null, null);
                 HbDownManager.getInstance(mContext).addDownloadInfo(this);
            	//HB. end
                
                LogUtil.i(TAG,"---mThreadPoolExecutor.getPoolSize() = " + mThreadPoolExecutor.getPoolSize() + " coreSzie = " + mThreadPoolExecutor.getCorePoolSize());
    ...
    //HB. Comments : TODO , Engerineer : wxue , Date : 2017年6月7日 ,begin
	private void showNetStateDialog(int dialogType) {
		HbDownManager.getInstance(mContext).addWaitConfirmedTask(this);
		Intent intent = new Intent(Intent.ACTION_VIEW);
		intent.setData(getAllDownloadsUri());
		LogUtil.i(TAG, "---showNetStateDialog()--dialogType = " + dialogType + " uri = " + getAllDownloadsUri());
		intent.putExtra("dialogType", dialogType);
		intent.setClassName(HbNetStateActivity.class.getPackage().getName(), HbNetStateActivity.class.getName());
    ...
    public void onDialogDismiss() {
    	...
    	LogUtil.i(TAG,"---onDialogDismiss()--- uri = " + getAllDownloadsUri() );
    	submitDownloadTask();
    }
    
    private boolean submitDownloadTask() {
        synchronized (this) {
            final boolean isReady = isReadyToDownload();
            final boolean isActive = mSubmittedTask != null && !mSubmittedTask.isDone();
            if (isReady && !isActive) {
            	if(mThreadPoolExecutor.getPoolSize() >= mThreadPoolExecutor.getCorePoolSize()){
                 ...
                 HbDownManager.getInstance(mContext).addDownloadInfo(this);
                 mTask = new DownloadThread(mContext, mSystemFacade, mNotifier, this);
                 mSubmittedTask = mThreadPoolExecutor.submit(mTask);
            }
            return isReady;
        }
    }

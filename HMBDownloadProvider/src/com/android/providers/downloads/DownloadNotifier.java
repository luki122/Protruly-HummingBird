...
public class DownloadNotifier {
    ...
    //HB. Comments : calculate download speed of each download task , Engerineer : wxue , Date : 2017年6月2日 ,begin
	private void calculateDownloadSpeed(Collection<DownloadInfo> downloads) {
		long current = 0;
		long total = 0;
		long speed = 0;
		...
		long[] ids = new long[downloads.size()];
		long[] speeds = new long[downloads.size()];
		int i = 0;
		synchronized (mDownloadSpeed) {
			for (DownloadInfo info : downloads) {
				if(info.mStatus ==Downloads.Impl.STATUS_RUNNING){
					ids[i] = info.mId;
					speeds[i] = mDownloadSpeed.get(info.mId);
					//Log.i(TAG, "---calculateDownloadSpeed()--id = " +ids[i] + " speed = " + speeds[i]);
					i++;
				}
			}
		}
		Intent intent = new Intent("com.android.providers.downloads.action.DOWNLOAD_SPEEDS");
		intent.putExtra("down_ids", ids);
		intent.putExtra("down_speeds", speeds);
    ...
    private void updateWithLocked(Collection<DownloadInfo> downloads) {
        ...
        //HB. Comments : call calculateDownloadSpeed , Engerineer : wxue , Date : 2017年6月5日 ,begin
        calculateDownloadSpeed(downloads);
        //HB. end
        ...
        for (String tag : clustered.keySet()) {
            ...
            if (type == TYPE_ACTIVE || type == TYPE_WAITING) {
                ...
                if (!sPendingIntents.containsKey(info.mId)) {
                     ...
                     Log.i(Constants.TAG, "sPendingIntents.put(), id: " + info.mId);
                }
                /// @}
            ...
            if (type == TYPE_COMPLETE) {
                ...
                if (sUpdateDoneItems.contains(info.mId)) {
                    Log.i(Constants.TAG, "sUpdateDoneItems contains,will continue, id: "
                            + info.mId);
                    continue;
                }
            }

...
public class DownloadThread implements Runnable {
    ...
    //HB. Comments : TODO , Engerineer : wxue , Date : 2017年7月4日 ,begin
    ...
    private class DownloadInfoDelta {
        ...
        private ContentValues buildContentValues() {
            ...
            //HB. Comments : TODO , Engerineer : wxue , Date : 2017年7月4日 ,begin
            values.put(Downloads.Impl.COLUMN_CONTROL, mInfo.mControl);
            //HB. end
    ...
    public void run() {
        ...
        //HB. Comments : modify status of waiting download thread , Engerineer : wxue , Date : 2017年6月5日 ,begin
        /*if (DownloadInfo.queryDownloadStatus(mContext.getContentResolver(), mId)
                == Downloads.Impl.STATUS_SUCCESS) {
            logDebug("Already finished; skipping");
            return;
        }*/
        int status = DownloadInfo.queryDownloadStatus(mContext.getContentResolver(), mId);
        LogUtil.i(TAG,"start run, id = " + mInfo.mId + " status = " + status);
        if (status == Downloads.Impl.STATUS_SUCCESS) {
            logDebug("Already finished; skipping");
            return;
        }
        if(status == Downloads.Impl.STATUS_PAUSED_BY_APP){
        	logDebug("Already paused by app; skipping");
            return;
        }
        if(status == Downloads.Impl.STATUS_WAITING_DOWNLOAD){
        	 status = Downloads.Impl.STATUS_RUNNING;
        	 mInfoDelta.mStatus = status;
      ...
      //HB. end
        ...
        try {
            ...
            if (Constants.MANY_ON_ENABLED) {
                ...
                ///M: add to fix 2567386. @{
                boolean cleartextTrafficPermitted = mSystemFacade
                        .isCleartextTrafficPermitted(mInfo.mUid);
                // Enforce the cleartext traffic opt-out for the UID.
                // This cannot be enforced earlier
            ...
            //HB. Comments : TODO , Engerineer : wxue , Date : 2017年7月3日 ,begin
            HbDownManager.getInstance(mContext).removeDownloadInfo(mId);
            //HB. end
            ...
            /// M: Add for fix GMS low memory issue 332710. @{
            Log.d(Constants.DL_ENHANCE, "after notifyDownloadCompleted"
    ...
    private void checkConnectivity() throws StopRequestException {
        ...
        if (networkUsable != NetworkState.OK) {
            ...
            //HB. Comments : request fail for waiting wifi , Engerineer : wxue , Date : 2017年6月6日 ,begin
            else if(networkUsable == NetworkState.UNUSABLE_FOR_WAITING_WIFI){
            	status = Downloads.Impl.STATUS_QUEUED_FOR_WIFI;
            }
            //HB. end
    ...
    private void finalizeDestinationFile(DownloadInfoDelta state) {
        if (state.mFileName != null) {
            ...
            Log.d(Constants.DL_DRM, "finalizeDestinationFile:MimeType is: "  + state.mMimeType +
                    ...
                    if (Helpers.isMtkDRMFLOrCDFile(state.mMimeType)) {
                        ...
                        try {
                            ...
                            if (result == OmaDrmClient.ERROR_NONE) {
                                ...
                                Log.d(Constants.DL_DRM, "finalizeDestinationFile:saverights return CID:"
                                        + strCID);
                            ...
                            mContext.getContentResolver().update(mInfo.getAllDownloadsUri(), values, null, null);
                            Log.e(Constants.DL_DRM, "Mark for delete DRM rights file");

                        } catch (IOException e) {
                            Log.e(Constants.DL_DRM, "save rights " + state.mFileName + " exception");
                        }
                    }
        ...
        if (state.mMimeType != null) {
            if (state.mMimeType.equals("application/vnd.oma.dd+xml")) {
                ...
                try {
                    ...
                    // There will update OMA_Download_Status, or the query will reuse
                    Log.e(Constants.LOG_OMA_DL, "DownloadThread: handleOmaDescriptorFile():" +
                            "New url failed" + state.mUri);
                }
                Log.i(Constants.LOG_OMA_DL, "DownloadThread: handleOmaDescriptorFile(): "

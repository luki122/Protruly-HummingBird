 ...
 */
public class FolderIcon extends FrameLayout implements FolderListener ,UninstallMode,UninstallMode.UninstallAnimation{
    @Thunk Launcher mLauncher;
    public Folder mFolder;
    public FolderInfo mInfo;
    @Thunk static boolean sStaticValuesDirty = true;

    public static final int NUM_ITEMS_IN_PREVIEW = SudokuFolderIconLayoutRule.MAX_NUM_ITEMS_IN_PREVIEW/*FeatureFlags.LAUNCHER3_LEGACY_FOLDER_ICON ?
            StackFolderIconLayoutRule.MAX_NUM_ITEMS_IN_PREVIEW :
            ClippedFolderIconLayoutRule.MAX_NUM_ITEMS_IN_PREVIEW*/;

    private CheckLongPressHelper mLongPressHelper;
    private StylusEventHelper mStylusEventHelper;
    private FolderIconMode mFolderIconMode = FeatureFlags.LAUNCHER3_LEGACY_FOLDER_ICON;
    ...
    public BubbleTextView getFolderName(){//lijun add
        return mFolderName;
    }
    ...
    private void init() {
        ...
        switch (mFolderIconMode){
            case ClippedFolderIconLayoutRule:
                ...
                break;
            case StackFolderIconLayoutRule:
                mPreviewLayoutRule = new StackFolderIconLayoutRule();
                break;
            case SudokuFolderIconLayoutRule:
                mPreviewLayoutRule = new SudokuFolderIconLayoutRule();
                break;
        }
    ...
    public static FolderIcon fromXml(int resId, Launcher launcher, ViewGroup group,
        ...
        if("".equals(folderInfo.title)) {
            folderName = launcher.getResources().getString(R.string.folder_hint_text);
        }else {
...
//UninstallMode add by liuzuo end
    ...
    private void computePreviewDrawingParams(int drawableSize, int totalSize) {
        if (mIntrinsicIconSize != drawableSize || mTotalWidth != totalSize ||
            ...
            mPreviewLayoutRule.init(mBackground.previewSize, mIntrinsicIconSize,
                    Utilities.isRtl(getResources()),getResources());
    ...
    public void startShakeAnimation(){
        Animation locateAnim = AnimationUtils.loadAnimation(mLauncher, R.anim.icon_shake_animation);
        this.startAnimation(locateAnim);
    }
    @Override
    ...
    public float getLocalCenterForIndex(int index, int curNumItems, int[] center) {
    ...
    public static class PreviewBackground {
        ...
        private static final float ACCEPT_SCALE_FACTOR = 1.08f;
        ...
        public void setup(DisplayMetrics dm, DeviceProfile grid, View invalidateDelegate,
                          int availableSpace, int topPadding , Resources resources) {
            mInvalidateDelegate = invalidateDelegate;

            final int previewSize = grid.folderIconSizePx;
            final int previewPadding = grid.folderIconPreviewPadding;
            mFolderIconbg = LauncherAppState.getInstance().getIconCache().getIconProvider().getIconFromManager(LauncherAppState.getInstance().getContext(),FOLDERICON_BG_PATH,R.drawable.foldericon_bg);
            int iconSize = previewSize - 2 * previewPadding;
            folderBgSize =  resources.getDimensionPixelSize(R.dimen.folder_bg_size);
        ...
        public void drawBackground(Canvas canvas, Paint paint) {
            Drawable folderIcon = mFolderIconbg;
            if(folderIcon == null){
                return;
            }
            canvas.save();
        ...
        private Rect getRect(Drawable folderIconbg) {
            Rect rect =null;
            if(folderIconbg!=null){
                int intrinsicWidth = (int) (folderBgSize);
                int intrinsicHeight =  (int) (folderBgSize);
                rect= new Rect((int) (intrinsicWidth*(1-mScale)),(int) (intrinsicHeight*(1-mScale)), (int) (intrinsicWidth*mScale),(int) (intrinsicHeight*mScale));
            }
            return rect;
        }
...
//        }
    ...
    public void updateItemDrawingParams(boolean animate) {
        ...
        for (int i = 0; i < mDrawingParams.size(); i++) {
            ...
            if(textView.getTag() instanceof  FolderPagedView.FolderAddInfo){
                mDrawingParams.remove(mDrawingParams.size() - 1);
                continue;
            }
            PreviewItemDrawingParams p = mDrawingParams.get(i);
            p.drawable = getTopDrawable(textView);

            if (!animate || FeatureFlags.LAUNCHER3_LEGACY_FOLDER_ICON!=FolderIconMode.StackFolderIconLayoutRule) {
    ...
    public void onTitleChanged(CharSequence title) {
        if(getResources().getString(R.string.folder_name).contentEquals((title))){
    ...
    public interface PreviewLayoutRule {
        ...
        public void init(int availableSpace, int intrinsicIconSize, boolean rtl,Resources resources);

        public int numItems();
        public boolean clipToBackground();
    }

    public void hideItem(final ShortcutInfo info, int index){
        final PreviewItemDrawingParams params = index < mDrawingParams.size() ?
                mDrawingParams.get(index) : null;
        if (params != null) params.hidden = true;
        postDelayed(new Runnable() {
    ...
    public void setFolderUnreadNum(int unreadNum) {
        if (BadgeController.DEBUG) {
            Log.d("FolderIcon", "setFolderUnreadNum: unreadNum = " + unreadNum + ", mInfo = " + mInfo
                    + ", this = " + this);
        }
        if (unreadNum <= 0) {
            mInfo.unreadNum = 0;
    ...
    /**
     * lijun add for unread ,don't do UI
     * Update unread number of the folder, the number is the total unread number
     * of all shortcuts in folder, duplicate shortcut will be only count once.
     */
    public void updateFolderUnreadNum(ArrayList<BadgeInfo> unreadApps) {
        ...
        if (BadgeController.DEBUG) {
            Log.d("FolderIcon", "updateFolderUnreadNum 1 end: unreadNumTotal = " + unreadNumTotal);
        }
        setFolderUnreadNum(unreadNumTotal);
    }

    /**
     * lijun add for unread ,called from setBadge
     */
    public void updateFolderUnreadNum(String packageName, int count, String shortcutCustomId) {
        final ArrayList<ShortcutInfo> contents = mInfo.contents;
        final int contentsCount = contents.size();
        int unreadNumTotal = 0;
        ...
        for (int i = 0; i < contentsCount; i++) {
            shortcutInfo = contents.get(i);
            componentName = shortcutInfo.intent.getComponent();
            boolean isCurrentShortCut = shortcutInfo.shortcutCustomId == null || shortcutInfo.shortcutCustomId.equals(shortcutCustomId);
            if (componentName != null && packageName.equals(componentName.getPackageName()) && isCurrentShortCut) {
                shortcutInfo.unreadNum = count;
                unreadNumTotal += shortcutInfo.unreadNum;
            }
        }
        if (BadgeController.DEBUG) {
     ...
     * lijun add for unread ,called from setBadge
     */
    public void updateFolderUnreadNum(BadgeInfo badgeInfo) {
        final ArrayList<ShortcutInfo> contents = mInfo.contents;
        final int contentsCount = contents.size();
        int unreadNumTotal = 0;
        ShortcutInfo shortcutInfo;
        ...
        for (int i = 0; i < contentsCount; i++) {
            shortcutInfo = contents.get(i);
            componentName = shortcutInfo.intent.getComponent();
            if (componentName != null && badgeInfo.pkgName.equals(componentName.getPackageName()) && shortcutInfo.shortcutCustomId == badgeInfo.shortcutCustomId) {
                shortcutInfo.unreadNum = badgeInfo.badgeCount;
                unreadNumTotal += shortcutInfo.unreadNum;
            }
        }
        if (BadgeController.DEBUG) {

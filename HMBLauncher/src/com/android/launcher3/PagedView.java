   ...
   // cyl add for cycle slide end	
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (pageCount > 0) {
			...
			Log.d("cyl", "dispatchDraw reordering = "+reordering);                            
            ...
            if (leftScreen != -1 && rightScreen != -1) {
                ...
                if (isNormalState() && getChildCount() > 1) {					
					...
					Log.d("cyl", "dispatchDraw: leftScreen = "+leftScreen +" rightScreen = "
						+rightScreen + " pageCount = "+pageCount + " offset = "+offset);    
                ...
                // cyl add for cycle slide end
                Log.d("cyl", "dispatchDraw: leftScreen = "+leftScreen +" rightScreen = "+rightScreen + " pageCount = "+pageCount);                            
                    for (int i = pageCount - 1; i >= 0; i--) {
						final View v = getPageAt(i);
						if (v == mDragView && reordering) continue;
						if (leftScreen <= i && i <= rightScreen && shouldDrawChild(v)) {
...
//                    onUnhandledTap(ev);
    ...
    protected void snapToPage(int whichPage, int delta, int duration, boolean immediate,
        ...
        //lijun add for overscroll
        if(!isNormalState() && FeatureFlags.OVERSCROLL_SPRINGBACK) {
       ...
       // cyl add for cycle slide start
        if ((isNormalState() && getChildCount() > 1) 
			&& ((mCurrentPage == 0 && whichPage == getChildCount() - 1 && getScrollX() < 0) 
			|| (mCurrentPage == getChildCount() - 1 && whichPage == 0 && (getScrollX() > mMaxScrollX)))) {
            int offset = getChildCount() * getViewportWidth() - screenDifference;
            if (delta > 0) {
                delta-= offset; 

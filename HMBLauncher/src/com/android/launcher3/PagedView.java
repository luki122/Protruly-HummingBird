...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    //lijun add for Reordering drag to switch
    private float mRawMotionDownX;
    private float mRawMotionX;
    private int mReorderingStartSide;//-1:left ; 0:middle ; 1:right
    private int leftMarginToRecording;
    private int rightMarginToRecording;
    private boolean firstReording = true;
    //lijun add end

    private boolean mCancelTap;

    protected int[] mPageScrolls;//lijun modify private to protected
    ...
    protected void init() {
        ...
        //setDefaultInterpolator(new BounceBackInterpolator());
        mCurrentPage = 0;
    ...
    public void scrollBy(int x, int y) {
        //lijun add for overscroll
        if (FeatureFlags.OVERSCROLL_SPRINGBACK) {
            int overScrollX = 0;
            if (mUnboundedScrollX > mMaxScrollX) {
                overScrollX = mUnboundedScrollX - mMaxScrollX;
            } else if (mUnboundedScrollX < 0) {
                overScrollX = -mUnboundedScrollX;
            }
            if(overScrollX != 0){
                int sign = 0;
                if (x > 0) {
                ...
                if (overScrollX < getViewportWidth() * 0.5f) {
                    float tempX = getViewportWidth() * 0.5f;
                    float surplusX = tempX - overScrollX;
                    int rx = Math.round(surplusX / tempX * x);
                    if(rx == 0 ){
                        x = sign;
                    }else{
                        x = rx;
    ...
    public void scrollTo(int x, int y) {
	   ...
	   // cyl add for cycle slide end
        ...
        } else if (isXAfterLastPage && !FeatureFlags.OVERSCROLL_SPRINGBACK) {//lijun add && !FeatureFlags.OVERSCROLL_SPRINGBACK
    ...
    protected void getVisiblePages(int[] range) {
        ...
        if (count > 0) {
		 ...
		 if (isNormalState() && count > 1) {
			...
			int x = getScrollX();
           if ((leftScreen == rightScreen) && (rightScreen == (count - 1))
    ...
    protected boolean shouldDrawChild(View child) {
        if (child == null) return false;//lijun add for NullPointException
   ...
   // cyl add for cycle slide end	
                ...
                if (isNormalState() && getChildCount() > 1) {
                    ...
                    if (rightScreen < leftScreen) {
                        if (getScrollX() >= mMaxScrollX) {
							...
							CellLayout leftChild = (CellLayout) getChildAt(leftScreen);
							float alpha = leftChild.getShortcutsAndWidgets().getAlpha();
							CellLayout rightChild = (CellLayout) getChildAt(rightScreen);
							rightChild.getShortcutsAndWidgets().setAlpha(1.0f - alpha);
							canvas.translate(+offset, 0);
                            drawScreen(canvas,rightScreen,drawingTime);
							boolean a = shouldDrawChild(getPageAt(rightScreen));
                            canvas.translate(-offset, 0);
                        } else {
                            drawScreen(canvas,rightScreen,drawingTime);
							CellLayout rightChild = (CellLayout) getChildAt(rightScreen);
							float alpha = rightChild.getShortcutsAndWidgets().getAlpha();
							CellLayout leftChild = (CellLayout) getChildAt(leftScreen);
							leftChild.getShortcutsAndWidgets().setAlpha(1.0f - alpha);
                            canvas.translate(-offset, 0);
                            drawScreen(canvas,leftScreen,drawingTime);
                            canvas.translate(+offset, 0);
                        }
    ...
    public void requestDisallowInterceptTouchEvent(boolean disallowIntercept) {
        if (disallowIntercept) {
            ...
            if(currentPage != null) {//lijun add for nullpointException
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                mRawMotionDownX = mRawMotionX = ev.getRawX();//lijun add for Reordering drag to switch
                /*
...
//        setEnableFreeScroll(true);//lijun remove
    }
     ...
     * @return
     */
    private int getDragOverPageIndex() {
        if (mDragView != null) {
            int dragX = (int) mRawMotionX;
            int minIndex = indexOfChild(mDragView);
            getFreeScrollPageRange(mTempVisiblePagesRange);

            if (firstReording && mReorderingStartSide == -1) {
                if (dragX > leftMarginToRecording) {
                    return getCurrentPage();
                }
                return minIndex;
            } else if (firstReording && mReorderingStartSide == 1) {
                if (dragX < rightMarginToRecording) {
                    return getCurrentPage();
                }
                return minIndex;
            } else {
                if (dragX < leftMarginToRecording) {
                    return ((getCurrentPage() - 1) >= mTempVisiblePagesRange[0]) ? (getCurrentPage() - 1) : mTempVisiblePagesRange[0];
                } else if (dragX > rightMarginToRecording) {
                    return ((getCurrentPage() + 1) <= mTempVisiblePagesRange[1]) ? (getCurrentPage() + 1) : mTempVisiblePagesRange[1];
                } else {
                    return minIndex;
                }
            }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_MOVE:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if(isNormalState() && (ev.getPointerCount() > 1 && ev.getPointerCount() > mMotionPointerCount)){
                   if (mTouchState == TOUCH_STATE_SCROLLING) {
                       snapToDestination();
                       onScrollInteractionEnd();
                   }
				   int ll = ev.getPointerCount();
                   resetTouchState();
                   return true;					
                }
				// cyl add for cycle slide end
                ...
                mRawMotionX = ev.getRawX();//lijun add for Reordering drag to switch
                ...
                //lijun modify for Reordering drag to switch
//                final int pageUnderPointIndex = getNearestHoverOverPageIndex();
                    ...
                    if (mTempVisiblePagesRange[0] <= pageUnderPointIndex &&
                        ...
                        mSidePageHoverRunnable = new Runnable() {
                            ...
                            public void run() {
                                ...
                                firstReording = false;//lijun add for Reordering drag to switch
        ...
        case MotionEvent.ACTION_UP:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (!mCancelTap) {
                    //lijun modify for homebutton click start
//                    onUnhandledTap(ev);
                    if(this instanceof Workspace && ((Workspace)this).checkOutsideTouchOnHomeButton(ev)){
    ...
    protected void resetTouchState() {//lijun modify private to protected
        releaseVelocityTracker();
        endReordering();
        mCancelTap = false;
        mTouchState = TOUCH_STATE_REST;
    ...
    protected void snapToPage(int whichPage, int delta, int duration, boolean immediate,
        ...
        //lijun add for overscroll
        if(FeatureFlags.OVERSCROLL_SPRINGBACK) {
            if (mUnboundedScrollX < 0 || mUnboundedScrollX > mMaxScrollX) {
       ...
       // cyl add for cycle slide start
        if ((isNormalState() && getChildCount() > 1) && ((mCurrentPage == 0 && whichPage == getChildCount() - 1)
                || (mCurrentPage == getChildCount() - 1 && whichPage == 0))) {
            int offset = getChildCount() * getViewportWidth();
            if (delta > 0) {
                delta-= offset; 
    ...
    public boolean startReordering(View v) {
        ...
        //lijun add start for Reordering drag to switch
        firstReording = true;
        Resources res = getResources();
        float scaleOverView = res.getInteger(R.integer.config_workspaceOverviewShrinkPercentage) / 100f;
        Launcher launcher = Launcher.getLauncher(getContext());
        DeviceProfile grid = launcher.getDeviceProfile();
        leftMarginToRecording = (int) ((grid.availableWidthPx - v.getMeasuredWidth() * scaleOverView) * 0.5f);
        rightMarginToRecording = grid.availableWidthPx - leftMarginToRecording;
        if (mRawMotionDownX < leftMarginToRecording) {
            mReorderingStartSide = -1;
        } else if (mRawMotionDownX > rightMarginToRecording) {
            mReorderingStartSide = 1;
        } else {
            mReorderingStartSide = 0;
        }
        //lijun add end

        //lijun add for aline button start
        if(this instanceof Workspace){
            ((Workspace)this).mLauncher.aLineButtonAnimateApha(0.0f);
        }
        ...
        if (mTempVisiblePagesRange[0] <= dragViewIndex &&
            ...
            mDragView.animate().scaleX(1.05f).scaleY(1.05f).setDuration(100).start();//lijun modify 1.15 to 1.05
            mDragViewBaselineLeft = mDragView.getLeft();
    ...
    void endReordering() {
        ...
        //lijun add for aline button start
        if(this instanceof Workspace){
            ((Workspace)this).mLauncher.aLineButtonAnimateApha(1.0f);
        }
        //lijun add for aline button end
    }

...
public class CellLayout extends ViewGroup implements BubbleTextShadowHandler ,UninstallMode{
    public static final int WORKSPACE_ACCESSIBILITY_DRAG = 2;
    public static final int FOLDER_ACCESSIBILITY_DRAG = 1;
    public static final float CELLLAYOUT_CONTENT_SCALE = 0.96f;
...
// cyl add for hotseat icon center end
    ...
    public void setFolderLeaveBehindCell(int x, int y) {
        ...
        mFolderLeaveBehind.setup(getResources().getDisplayMetrics(), grid, null,
                child.getMeasuredWidth(), child.getPaddingTop(),getResources());
   ...
   // cyl add public for special effect
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        //lijun add for homebutton
        if(isWorkspace){
            if(mHomeButton !=null){
                mHomeButton.layout(-30,-30,100,100);
            }
        }
        //lijun add end
    }
    ...
    public void visualizeDropLocation(View v, DragPreviewProvider outlineProvider, int cellX, int cellY,
        ...
        if (cellX != oldDragCellX || cellY != oldDragCellY) {
            ...
            if (resize) {
                ...
                if(v instanceof  BubbleTextView){
                    if (mLauncher.isLauncherArrangeMode() && mShortcutsAndWidgets != null) {
                        int numRows = mLauncher.getDeviceProfile().inv.numRows;
                        int numColumns = mLauncher.getDeviceProfile().inv.numColumns;
                        int offsetY = (int) (mShortcutsAndWidgets.getTranslationY() * 2 / (numRows - 1) * (numRows - 1 - cellY));
                        int i = numColumns / 2;
                        int copies;
                        if (i > 0) {
                            if (cellX / i == 0) {
                                ...
                                copies = (i - 1 - cellX % i) - 1;
                            }

                            int offsetX = (int) (mShortcutsAndWidgets.getWidth() * (1 - CELLLAYOUT_CONTENT_SCALE) / (numColumns / 2)) * copies;
                            left += offsetX;
                        }
                        top += offsetY;
                    }
                        top-= outlineProvider.getUninstallDrawableSize();
        ...
        for (int y = 0; y < getCountY(); y++) {
            for (int x = 0; x < getCountX(); x++) {
                ...
                if(child != null && child instanceof BubbleTextView) {
                    ...
                    if(show){
                        textView.showUninstallApp();
                    }else {
                        textView.hideUninstallApp();
                    }
                }else if(child != null && child instanceof LauncherAppWidgetHostView){
                    LauncherAppWidgetHostView hostView = (LauncherAppWidgetHostView) child;
                    if(lastHostView==hostView){
                        continue;
                    }
                    lastHostView=hostView;
                    if(show){
                        hostView.showUninstallApp();
                    }else {
                        hostView.hideUninstallApp();
                    }
                }else if(child!=null && child instanceof FolderIcon){
                    FolderIcon folderIcon = (FolderIcon) child;
                    if(show){
                        folderIcon.showUninstallApp();
                    }else {
                        folderIcon.hideUninstallApp();
                    }
                }
            }
    ...
    public LauncherAppWidgetHostView getOnClickWidgetView() {
        return mOnClickWidgetView;
    }

    public void setOnClickWidgetView(LauncherAppWidgetHostView onClickWidgetView) {
        Log.d(TAG,"setOnClickWidgetView");
...
//uninstallMode  add by liuzuo end
    ...
    */
     ...
     */
    public boolean getEmptyLayPlace(int minSpanX, int minSpanY, int spanX, int spanY, int[] result, int resultSpan[]) {
...
//        cv.drawOutline(outlineColor);
    ...
    private void pushIconByRow(CellAndSpan c, int countX, int countY, int whichEdge){
        if (whichEdge == ViewCluster.LEFT) {
            ...
            Log.v("edge", "left---addView: x=" + c.cellX + ", y=" + c.cellY);
            ...
            if (result != -1) {
                ...
                while(c.cellX == countX - 1 && c.cellY + c.spanY <= countY - 1) {
                    c.cellY = c.cellY + c.spanY;
                    c.cellX = -1;
                    int cellX = isCellInLauncherAppWidget(c.cellX + 1, c.cellX, ViewCluster.RIGHT);
                    if (cellX != -1) {
                        c.cellX = cellX;
                    }
                }
            }
            Log.v("edge", "right---addView: x=" + c.cellX + ", y=" + c.spanY);
        }
    }
    ...
    /**
     * *************************************
     * lijun add for topBar start
     */
    public void initTopBar() {
        isWorkspace = true;

        mHomeButton = (ImageButton) mLauncher.getLayoutInflater().inflate(R.layout.celllayout_home_button, this, false);
        mHomeButton.setOnClickListener(homeClickListener);
        mHomeButton.setTag("homebutton");
        mHomeButton.setBackground(null);
        mHomeButton.setVisibility(GONE);
        mHomeButton.setAlpha(0.0f);
        addView(mHomeButton);
    }
    ...
    public void onHomebuttonClick(){
        homeClickListener.onClick(mHomeButton);
    }
    ...
    private View.OnClickListener homeClickListener = new View.OnClickListener(){
        ...
        public void onClick(View v) {
            if (v == mHomeButton) {
                mLauncher.mWorkspace.setDefaultScreen(CellLayout.this);
            }
        }
    };
    ...
    public int getContentTranslationY(){
        ...
        int firstScaleHeith = (int) (mShortcutsAndWidgets.getMeasuredHeight() * (res.getInteger(R.integer.config_workspaceOverviewShrinkPercentage)/100f));
        return  (int) (firstScaleHeith*(1 - CELLLAYOUT_CONTENT_SCALE)*0.5f);
    }
    /**
     * lijun add for topBar end
     * *************************************
     */

    /**
     * lijun add for wallpaper change
     * @param colors
     */
    public void onColorChanged(int[] colors){
        if(mShortcutsAndWidgets != null && mShortcutsAndWidgets.getChildCount()>0){
            for(int i = 0 ; i < mShortcutsAndWidgets.getChildCount();i++){
                View child = mShortcutsAndWidgets.getChildAt(i);
                if(child instanceof BubbleTextView){
                    ((BubbleTextView) child).setTextColor(colors[0]);
                    if(isHotseat()){
                        ((BubbleTextView) child).setTextVisibility(false);
                    }
                }else if(child instanceof FolderIcon){
                    ((FolderIcon) child).getFolderName().setTextColor(colors[0]);
                    Folder folder = ((FolderIcon) child).getFolder();
                    if(folder!=null){
                        folder.updateColor(colors[0]);
                    }
                }
            }
            invalidate();
    ...
    public void cleanFolders() {
        if (mShortcutsAndWidgets != null && mShortcutsAndWidgets.getChildCount() > 0) {
            ArrayList<FolderIcon> removeFolderIcons = new ArrayList<>();
            for (int i = 0; i < mShortcutsAndWidgets.getChildCount(); i++) {
                View child = mShortcutsAndWidgets.getChildAt(i);
                if (child instanceof FolderIcon) {
                    ...
                    if (folder.getItemCount() <= 1) {
                        removeFolderIcons.add(folderIcon);
                    }
                }
            }
            for(FolderIcon folderIcon:removeFolderIcons){
                folderIcon.mFolder.replaceFolderWithFinalItem();
            }
        }
    }
}

...
public class Launcher extends Activity
        implements LauncherExterns, View.OnClickListener, OnLongClickListener,
                   ...
                   AccessibilityManager.AccessibilityStateChangeListener ,ColorManager.IWallpaperChange {//lijun add ColorManager.IWallpaperChange
    ...
    public enum State { NONE, WORKSPACE, WORKSPACE_SPRING_LOADED, APPS, APPS_SPRING_LOADED,
        WIDGETS, WIDGETS_SPRING_LOADED,FOLDER_IMPORT,WALLPAPER ,ICONARRANGE}//lijun add UNINSTALL_NORMAL mode
...
//            mWorkspace.exitUninstallMode();
    ...
    public void onBackPressed() {
        ...
        if (getOpenShortcutsContainer() != null) {
            ...
            showWorkspace(true);
        } else if (isWidgetsViewVisible() || isWallpaperMode()||isLauncherArrangeMode()) {//lijun add isWallpaperMode()
            //Icon Arrange begin
            if(isLauncherArrangeMode()){
                Folder openFolder = mWorkspace.getOpenFolder();
                if(openFolder!=null) {
...
//UninstallMode add by liuzuo end
    ...
    public boolean onLongClick(View v) {
        ...
        //liuzuo add end
        if (mState != State.WORKSPACE && mState!= State.WORKSPACE_SPRING_LOADED) return false;
...
//                Workspace.State.OVERVIEW, animated, postAnimRunnable);
        if (mState == State.WIDGETS || mState == State.WALLPAPER||mState == State.ICONARRANGE) {
            Folder.ICONARRANGING = false;
            showOverviewModeFromOverviewHidenMode(State.WORKSPACE, true);
        } else {
            mStateTransitionAnimation.startAnimationToWorkspace(mState, mWorkspace.getState(),
...
//            showWidgetsView(true, false);
    ...
    private void enterLauncherState(State launcherState,Workspace.State toWorkspaceState) {
        ...
        if (mState == State.APPS || mState == State.WIDGETS || mState == State.WALLPAPER) {//lijun add WALLPAPER
            return;
        }
        mStateTransitionAnimation.startAnimationToWorkspace(mState, mWorkspace.getState(),
                toWorkspaceState,
...
//        }
    ...
    public boolean isFullScreenMode() {
        if (isOverViewPanaelShowing() || isWallpaperMode() || isArrangeBarShowing() || isWidgetsViewVisible()) {
            return true;
        }
        return false;
    }

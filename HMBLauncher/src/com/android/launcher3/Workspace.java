...
import com.android.launcher3.pageindicators.PageIndicatorCube;
import com.android.launcher3.pageindicators.PageIndicatorDots;
import com.android.launcher3.pageindicators.PageIndicatorManager;
...
import static com.android.launcher3.Workspace.State.OVERVIEW_HIDDEN;
 ...
 */
 //PagedView  cyl modify for special effect :PagedView
public class Workspace extends SpecialEffectPagedView  
    ...
    private boolean mPrepareUninstallMode;//liuzuo add
    ...
    public boolean mAnimatingViewIntoPlace = false;
    ...
    //PagedView  cyl add for special effect start
    ...
    //PagedView  cyl add for special effect end
    ...
    public Workspace(Context context, AttributeSet attrs, int defStyle) {
        ...
        mWorkspaceFadeInAdjacentScreens = true;//grid.shouldFadeAdjacentWorkspaceScreens();
    ...
    public void onDragStart(DropTarget.DragObject dragObject, DragOptions options) {
        ...
        if (FeatureFlags.SHOW_PAGEINDICATOR_CUBE) {
            if (dragObject.dragSource instanceof Workspace && !isStartDragOnHotseat) {
                ...
                refreshviewCache(sourceIndex,sourceCell,sourceCell,false);
            } else {
                sourceIndex = -1;
            }
        }
        ...
        if (addNewPage) {
            ...
            if(mState == State.NORMAL){// && !(dragObject.dragSource instanceof Folder )
                //|| dragObject.dragSource instanceof AllAppsContainerView || source instanceof WidgetsContainerPageView
        ...
        if (!FeatureFlags.LAUNCHER3_LEGACY_WORKSPACE_DND&&mLauncher.getState()!= Launcher.State.ICONARRANGE) {
           ...
           if(!mLauncher.isWidgetsViewVisible()){ // cyl fix bug5057
             mPrepareUninstallMode = true;
           }           
            mLauncher.enterSpringLoadedDragMode(runnable);
        }
    }
    ...
    public void onDragEnd() {
        ...
        mPrepareUninstallMode = false; // cyl fix bug5057
...
//        }
    ...
    protected void initWorkspace() {
        ...
        mMaxDistanceForFolderCreation = (0.45f * grid.iconSizePx);
    ...
    public void initParentViews(View parent) {
        ...
        if (FeatureFlags.SHOW_PAGEINDICATOR_CUBE) {
            hidePageIndicator = (PageIndicator) parent.findViewById(R.id.page_indicator);
            ...
            if ((mPageIndicatorManager == null || !mPageIndicatorManager.initialized()) && mPageIndicatorViewId > -1) {
                mPageIndicatorManager = new PageIndicatorManager(this, mLauncher, (PageIndicatorDots) mPageIndicator, mPageIndicatorCube, getChildCount());
                mPageIndicatorManager.indicatorsAttachedToWindow();
            }
        }else{
            mPageIndicator.setVisibility(VISIBLE);
            hidePageIndicator = (PageIndicator) parent.findViewById(R.id.page_indicator_dots);
            mPageIndicatorCube.setVisibility(GONE);
        }
...
//        return numCustomPages();
    ...
    public void onChildViewAdded(View parent, View child) {
        ...
        if (FeatureFlags.SHOW_PAGEINDICATOR_CUBE) {
            ...
            if(isReordering(false)){
                mPageIndicatorCube.addMarker(pageIndex,getPageIndicatorMarkerForCube(pageIndex),false);
            }
    ...
    protected void removeMarkerForView() {
        ...
        // add/remove pages
        if(!noNeedToAddPageIndicatorMaker) {
            super.removeMarkerForView();
        }
    }

    private void removeCubeMarkerForView(int index) {
        if(mPageIndicatorCube!=null){
            mPageIndicatorCube.removeMarker(index,true);
        }
    }

    private void removeAllCubeMarker() {
    ...
    public void onViewRemoved(View child) {
        removeCubeMarkerForView(indexOfChild(child));
    ...
    public void addInScreen(View child, long container, long screenId,
...
//                    .ofFloat(mHotseatAlpha[HOTSEAT_STATE_ALPHA_INDEX], finalValue);
            ...
            if((isHotseatAnimating && mLauncher.getHotseat().getAlpha() < 1.0 && mLauncher.getHotseat().getAlpha() > 0)){
                isHotseatAnimating = false;
                animator = ValueAnimator
                        .ofFloat(mLauncher.getHotseat().getAlpha(), finalValue);
            }else{
                animator = ValueAnimator
                        .ofFloat(mHotseatAlpha[HOTSEAT_STATE_ALPHA_INDEX], finalValue);
            }
     ...
     *
     */
    public ValueAnimator createHotseatAlphaAnimatorNotIndicator(final float finalValue) {
        if (Float.compare(finalValue, mHotseatAlpha[HOTSEAT_STATE_ALPHA_INDEX]) == 0) {
            // Return a dummy animator to avoid null checks.
            return ValueAnimator.ofFloat(0, 0);
        } else {
    ...
    private void updatePageAlphaValues(int screenCenter) {
		// cyl add for bug2696 :mLauncher.isUninstallMode
        if (mLauncher.isUninstallMode ||mPrepareUninstallMode||
			(mWorkspaceFadeInAdjacentScreens &&
                //!workspaceInModalState() && // cyl del for set alpha
                !inModalStateSetAlpha() &&  // cyl add for set alpha
                !mIsSwitchingState)) {
    ...
    public boolean workspaceInModalState() {
        return mState != State.NORMAL
                ...
                &&mState!=OVERVIEW_HIDDEN;//liuzuo add for addIcon OVERVIEW_HIDDEN ; lijun add SPRING_LOADED
    }

  // cyl add for set alpha  start
  ...
  // cyl add for set alpha  start	
    ...
    @Thunk public void updateChildrenLayersEnabled(boolean force) {
...
||mLauncher.isLauncherArrangeMode()));//liuzuo add 
    }
    ...
    public boolean acceptDrop(DragObject d) {
	   ...
	   // cyl add for hotseat icon center
			boolean full = true;
			if(mLauncher.isHotseatLayout(dropTargetLayout) 
        ...
        if (d.dragSource != this) {
            ...
            if (dropTargetLayout != null && !d.cancelled) {
                ...
                float distance = dropTargetLayout.getDistanceFromCell(mDragViewVisualCenter[0],
                 ...
                 ItemInfo item = d.dragInfo;  // cyl add for hotseat icon center 
                 long itemContainer = item.container; // cyl add for hotseat icon center
               ...
               // ItemInfo item = d.dragInfo;  // cyl del for hotseat icon center
			   ...
			   boolean isFull = mLauncher.getHotseat().isFull();
                ...
                if (foundCell) {
                     ...
                     // cyl add for hotseat icon center start
                       if(toHotseat){
                            if(!dragFromHotseat(d.dragInfo)) {
                                ...
                                int index = mLauncher.getHotseat().getAppropriateIndex(mDragController.getCurTouchX());
                                mLauncher.getHotseat().onDrop(true, d.x, d.dragView, cell, true);
                                addInHotseat(cell, container, screenId, index, 0,
                    ...
                    if (!toHotseat) {  // cyl add for hotseat icon center 
                   ...
                   // cyl modify for hotseat icon center end
                    ...
                    if(toHotseat) {
                        ...
                        //mLauncher.showOutOfSpaceMessage(false);//liuzuo remove
                        if(layout!=null) {
                        ...
                        if(dragFromHotseat(d.dragInfo)) {
                            ...
                            addInHotseat(cell, info.container, info.cellX,
          ...
          // cyl add for hotseat icon center start
			  final ItemInfo itemInfo = (ItemInfo) cell.getTag();
               if(dragFromHotseat(d.dragInfo)){
                 ...
                 addInHotseat(cell, itemInfo.container, itemInfo.cellX,
                              itemInfo.cellX, itemInfo.cellY, itemInfo.spanX, itemInfo.spanY, itemInfo.cellX);
				 hasAddtoHotseat = true;
               }
               mLauncher.getHotseat().onDrop(false, 0, d.dragView, cell, true);
             }
		   ...
		   // cyl add for hotseat icon center end
            ...
            if (d.dragView.hasDrawn()) {
                ...
                if (isWidget) {
                  ...
                  if(!hasAddtoHotseat){ // cyl add for hotseat icon center 
   ...
   static private float squaredDistance(float[] point1, float[] point2) {
    ...
    public void onDragOver(DragObject d) {
        ...
        if (mDragTargetLayout != null) {
            ...
            if (mLauncher.isHotseatLayout(mDragTargetLayout)) {
                mapPointFromSelfToHotseatLayout(mLauncher.getHotseat(), mDragViewVisualCenter);					
                mLauncher.getHotseat().onEnterHotseat(mDragController.getCurTouchX(), (int)item.screenId, dragFromHotseat(d.dragInfo), mDragController.getCurTouchY());// cyl add for hotseat icon center
            } else {
...
//            manageFolderFeedback(mDragTargetLayout, mTargetCell, targetCellDistance, d);
            ...
            if (!nearestDropOccupied) {
			   if(!mLauncher.isHotseatLayout(mDragTargetLayout)){// cyl add for hotseat icon center
    ...
    private boolean manageFolderFeedback(CellLayout targetLayout,
            int[] targetCell, float distance, DragObject dragObject) {
        if (distance > mMaxDistanceForFolderCreation||mLauncher.isLauncherArrangeMode()) return false;
    ...
    class FolderCreationAlarmListener implements OnAlarmListener {
        ...
        public FolderCreationAlarmListener(CellLayout layout, int cellX, int cellY) {
            ...
            bg.setup(getResources().getDisplayMetrics(), grid, null,
                    cell.getMeasuredWidth(), cell.getPaddingTop(),getResources());
...
//        };
        ...
        if (info instanceof PendingAddItemInfo) {
           ...
           // cyl add for hotseat icon center start
            ...
            if(mLauncher.isHotseatLayout(cellLayout)) {
                int index = mLauncher.getHotseat().getAppropriateIndex(mDragController.getCurTouchX());
                addInHotseat(view, container, index, index, 0,info.spanX, info.spanY, index);
				view.setVisibility(INVISIBLE);
				mLauncher.getHotseat().onDrop(true, d.x, d.dragView, view, true);				
				hasAddtoHotseat = true;
            } else {// cyl add for hotseat icon center end
			...
			// cyl add for hotseat icon center :hasAddtoHotseat
    ...
    public void onDropExternalForPageIndicatorCube(final int[] touchXY, final ItemInfo dragInfo,
                                final CellLayout cellLayout, boolean insertAtFirst, DragObject d) {
...
//        };
    ...
    private void getFinalPositionForDropAnimation(int[] loc, float[] scaleXY,
        ...
        float cellLayoutScale = mLauncher.getDragLayer().getDescendantCoordRelativeToSelf(layout.getShortcutsAndWidgets(), loc, true);
...
//        loc[1] -= (dragView.getMeasuredHeight() - cellLayoutScale * r.height()) / 2;
        if(info instanceof PendingAddShortcutInfo){
            loc[0] -= (mLauncher.getDeviceProfile().cellWidthPx - cellLayoutScale * r.width()) / 2
                    - Math.ceil(layout.getUnusedHorizontalSpace() / 2f);
            loc[1] -= (mLauncher.getDeviceProfile().cellHeightPx - cellLayoutScale * r.height()) / 2;
        }else {
            loc[0] -= (dragView.getMeasuredWidth() - cellLayoutScale * r.width()) / 2
                    - Math.ceil(layout.getUnusedHorizontalSpace() / 2f);
            loc[1] -= (dragView.getMeasuredHeight() - cellLayoutScale * r.height()) / 2;
    ...
    public void onDropCompleted(final View target, final DragObject d,
        ...
        /*if(mDragInfo!=null && mDragInfo.cell instanceof UninstallMode.UninstallAnimation){
            Log.d("liuzuo556", "  onDropCompleted =" + (ItemInfo)mDragInfo.cell.getTag());
        ...
        if (success && !(beingCalledAfterUninstall && !mUninstallSuccessful)) {
           ...
           // cyl add for hotseat icon center start
            if(mLauncher.getHotseat().checkDragitem(mDragInfo.cell)) {
					...
					boolean flag = mLauncher.getHotseat().removeInvisibleView();
					if(flag){
					final View cell = mDragInfo.cell;
                    View parentCell = getParentCellLayoutForView(cell);
					 if (parentCell != null && parentCell instanceof CellLayout) {
                    ...
                    addInHotseat(mDragInfo.cell, info.container, info.cellX,
                            info.cellX, info.cellY, info.spanX, info.spanY, info.cellX);					
		...
		// cyl add for hotseat icon center :mDragInfo != null 
        ...
        if(target instanceof Folder){
            ...
            ArrayList<View> viewArrayList = folder.getItemsInReadingOrder();
            for(View view:viewArrayList){
                if(folder.getDragViewOfFolder()!=view&&view instanceof UninstallMode){
...
//            mDelayedResizeRunnable = null;
    ...
    public void removeWorkspaceItem(View v) {
        ...
        if (parentCell != null) {
            ...
            refreshviewCache(getPageIndexForScreenId(getIdForScreen(parentCell)),null,null,false);
        } else if (ProviderConfig.IS_DOGFOOD_BUILD) {
    ...
    public void removeItemsByMatcher(final ItemInfoMatcher matcher) {
        boolean removeHoeseatItem = false; 
...
//UninstallMod  add by liuzuo
    public void enterUninstallMode(){
        mLauncher.isUninstallMode = true;		
        showOrHideUnstallMode(true);
    }
    public void exitUninstallMode(){
        showOrHideUnstallMode(false);
        mLauncher.isUninstallMode = false;
    }
    private void showOrHideUnstallMode(boolean show) {
         /*
        workspace
         */
        int screenCount = getChildCount();
        for (int screen = 0; screen < screenCount; screen++) {
            ...
            if(child instanceof  CellLayout&&child instanceof  UninstallMode){
                UninstallMode layout = (UninstallMode) child;
                if (show) {
                    layout.showUninstallApp();
                } else {
                    layout.hideUninstallApp();
                }
            }
        }
        ...
        Folder openFolder = getOpenFolder();
        if(openFolder!=null){
        ArrayList<View> views = openFolder.getItemsInReadingOrder();
...
/*        if (openFolder != null && openFolder.getContent() != null) {
            ...
            for (int i = 0; i < pagedView.getChildCount(); i++) {
                Log.d("liuzuo885", "openFolder.size()=" + pagedView.getChildCount());
                CellLayout cellLayout = (CellLayout) pagedView.getChildAt(i);
                if (show) {
                    cellLayout.showUninstallApp();
                } else {
                    cellLayout.hideUninstallApp();
                }
            }

        }*/

        if (show && mDragInfo != null && mDragInfo.cell instanceof UninstallMode.UninstallAnimation &&! (mDragInfo.cell.getVisibility()==VISIBLE)) {
            UninstallMode.UninstallAnimation uninstallAnimationView = (UninstallMode.UninstallAnimation) mDragInfo.cell;
            Log.d(TAG, "uninstallAnimationView.stopShakeAnimation()" + (ItemInfo) mDragInfo.cell.getTag());
            uninstallAnimationView.stopShakeAnimation();


        }
        Folder folder = getOpenFolder();
        if (show && folder != null && folder.getmCurrentDragView() != null) {
            View view = folder.getmCurrentDragView();
            if (view instanceof UninstallMode.UninstallAnimation) {
                UninstallMode.UninstallAnimation uninstallAnimationView = (UninstallMode.UninstallAnimation) view;
                Log.d(TAG, "uninstallAnimationView.stopShakeAnimation()" + (ItemInfo) view.getTag());
                uninstallAnimationView.stopShakeAnimation();
            }
        }
    }
    ...
    public PageIndicatorCube.PageMarkerResources getPageIndicatorMarkerForCube(int pageIndex) {
        ...
        if(pageIndex == PageIndicatorCube.LEFT_INDICATOR_INDEX){//left scroll indicator
            if(isBlacktext){
                return new PageIndicatorCube.PageMarkerResources(R.drawable.ic_widgets_left_indicator_black);
            }else{
                return new PageIndicatorCube.PageMarkerResources(R.drawable.ic_widgets_left_indicator);
            }
        }else if(pageIndex == PageIndicatorCube.RIGHT_INDICATOR_INDEX){
            if(isBlacktext){
                return new PageIndicatorCube.PageMarkerResources(R.drawable.ic_widgets_right_indicator_black);
            }else{
                return new PageIndicatorCube.PageMarkerResources(R.drawable.ic_widgets_right_indicator);
            }
        }else if(screenId == EXTRA_EMPTY_SCREEN_ID){
            int count = mScreenOrder.size() - numCustomPages();
            if (count > 1) {
                return new PageIndicatorCube.PageMarkerResources(viewCaches[pageIndex]);
            }
        }
    ...
    public boolean onWidgetCellClick(ItemInfo item, WidgetCell v){
        ...
        if(!dropTargetLayout.findCellForSpan(mTargetCell,minSpanX,minSpanY)){
            ...
            if(!dropTargetLayout.findCellForSpan(mTargetCell,minSpanX,minSpanY)){
                ...
                // Update the model for the new screen
                dropTargetLayout = insertNewWorkspaceScreen(newId,getChildCount());
                screenId = newId;
                if(!dropTargetLayout.findCellForSpan(mTargetCell,minSpanX,minSpanY)){
                    stripEmptyScreens();
                    return false;
...
//            log = log + ", target:" + target[0] + "-" + target[1];
//        }
//        if (source != null && source.length >= 2) {
//            log = log + ", source:" + source[0] + "-" + source[1];
//        }
//        if(index == 0){
//            log = log+"||||why?";
//        }
//        Log.d("lijun","refreshviewCache "+log);
    ...
    private void refreshviewCacheLeftRight() {
        if (targetCubeIndex >= 0) {
            if (targetCubeIndex == sourceIndex) {
                refreshviewCache(targetCubeIndex, null, sourceCell, false);
            } else {
                refreshviewCache(targetCubeIndex, null, null, false);
            }
        }
    ...
    protected void updatePageIndicator() {
        ...
        if (FeatureFlags.SHOW_PAGEINDICATOR_CUBE && mPageIndicatorCube != null) {
            mPageIndicatorCube.setContentDescription(getPageIndicatorDescription());
            if (!isReordering(false)) {
                mPageIndicatorCube.setActiveMarker(getNextPage());
            }
        }
    }
    ...
    public void setDefaultScreen(long screenId) {
        defaultScreenId = screenId;
        if (mScreenOrder != null && mScreenOrder.size() > 0) {
            if (!mScreenOrder.contains(screenId)) {
                defaultScreenId = mScreenOrder.get(0);
            }
        }
        saveDefaultScreen();
        final int childCount = getChildCount();
        for (int i = 0; i < childCount; i++) {
            final CellLayout cl = (CellLayout) getChildAt(i);
            ImageButton homeButton = cl.getmHomeButton();
            if(homeButton == null)continue;
            if (getIdForScreen(cl) == defaultScreenId) {
                homeButton.setImageDrawable(mLauncher.getResources().getDrawable(R.drawable.ic_home_button_pressed));
            } else {
                homeButton.setImageDrawable(mLauncher.getResources().getDrawable(R.drawable.ic_home_button_normal));
            }
        }
        invalidate();
    }
    ...
    private void saveDefaultScreen(){
        ...
        sp.edit().putLong(HOMESCREEN_PREFERENCE_KEY, defaultScreenId).apply();
    }

    private void resetNewDefaultScreen() {
        if(mScreenOrder.contains(defaultScreenId))return;
        long id = getIdForScreen((CellLayout) getPageAt(0));
        if (mScreenOrder != null && mScreenOrder.size() > 0) {
            if (mScreenOrder.contains(id)) {
                defaultScreenId = id;
            } else {
                defaultScreenId = mScreenOrder.get(0);
            }
    ...
    /**
     * liuzuo add for invisible workspace
     */
...
/*        disableLayoutTransitions();
        ...
        //if(mState!=State.FOLDER_IMPORT) {
            ValueAnimator animator = createHotseatAlphaAnimatorNotIndicator(0f);
            animator.start();
        //}
        Log.d(TAG,"workspace invisible");
    }
    /**
     * liuzuo add for visible workspace
     */
...
/*        disableLayoutTransitions();
    ...
    public Folder getFolderForTag(final Object tag) {
        return (Folder) getFirstMatch(new ItemOperator() {
            ...
            @Override
            public boolean evaluate(ItemInfo info, View view) {
                return (view instanceof Folder) && (((Folder) view).getInfo() == tag)
                        && ((Folder) view).getInfo().opened;
            }
    ...
    //liuzuo add for home Button areas;
    private boolean isOutHomeButtonAreas(MotionEvent event, View view) {
        if ((mState == State.OVERVIEW || mState == OVERVIEW_HIDDEN) && view instanceof CellLayout) {
            Rect rect = new Rect();
            getPageAreaRelativeToDragLayer(rect);
    ...
    private boolean needClickOnHomeButton(MotionEvent event, View view) {
        if ((mState == State.OVERVIEW || mState == OVERVIEW_HIDDEN) && view instanceof CellLayout) {
            Rect rect = new Rect();
            getPageAreaRelativeToDragLayer(rect);
            rect.right = rect.left + 100;
    ...
    public void onColorChanged(int[] colors){
        ...
        for (int i = 0; i < screenCount; i++) {
            CellLayout cl = (CellLayout) getChildAt(i);
            cl.onColorChanged(colors);
        }
    }

    public boolean checkOutsideTouchOnHomeButton(MotionEvent ev) {
        CellLayout currentCellLayout = (CellLayout) getPageAt(getCurrentPage());
        if (needClickOnHomeButton(ev, currentCellLayout)) {
            currentCellLayout.onHomebuttonClick();
            return true;
        }
        return false;
    }
    public boolean findCellForSpanInScreen(int[] position,long screenId){
        CellLayout cellLayout = this.getScreenWithId(screenId);
        if(cellLayout!=null){
            return cellLayout.findCellForSpan(position,1,1);
        }
        return false;
    }


    public boolean findCellForSpanInCurrentPage(int[] position){
        return findCellForSpanInScreen(position,this.getScreenIdForPageIndex(this.getCurrentPage()));
    }

    public boolean findCellForSpanInCurrentPage(int[] position,ItemInfo info){
        if(findCellForSpanInCurrentPage(position)){
            info.screenId = getScreenIdForPageIndex(mCurrentPage);
            return true;
        }
        return false;
    }

    public boolean findSuitablePositionForView(int[] postion,ItemInfo info){

        if(findCellForSpanInCurrentPage(postion)){
            info.screenId = getScreenIdForPageIndex(mCurrentPage);
            return true;
        }else{
            int startPage = -500;
            long currentScreenId= getScreenIdForPageIndex(mCurrentPage);
            for(int i =0;i<mScreenOrder.size();i++){
                ...
                if(startPage!=-500){
                    if(findCellForSpanInScreen(postion,mScreenOrder.get(i))){
            ...
            if(startPage==-500){
                Log.e(TAG,"somethings exception here.",new Throwable());
            }
        }
        return false;
    }

    public boolean isOcupied(ItemInfo info){
        int screenId = (int)(info.screenId);
        CellLayout cellLayout = this.getScreenWithId(screenId);
        if(cellLayout!=null){
            return cellLayout.isOccupied(info.cellX,info.cellY);
        }
        return false;
    }


    public void addViewAndUpdateData(int[] position,final View newView,DragView dragView,View startView,boolean goBack,boolean animate){
        final ItemInfo info = (ItemInfo)newView.getTag();
        info.cellX=position[0];
        info.cellY=position[1];
        final CellLayout.LayoutParams lp = new CellLayout.LayoutParams(info.cellX,info.cellY,1,1);
        int index = goBack?this.getPageIndexForScreenId(info.screenId):this.getCurrentPage();
        CellLayout currentPage = (CellLayout)(this.getChildAt(index));
        lp.cellX = info.cellX;
        lp.cellY = info.cellY;
        info.container = LauncherSettings.Favorites.CONTAINER_DESKTOP;
        info.screenId = getIdForScreen(currentPage);
        if(currentPage!=null){
            int childId = mLauncher.getViewIdForItem(info);
            newView.setOnClickListener(mLauncher);
            Rect r = new Rect();
            ...
            Workspace.this.addInScreen(newView,LauncherSettings.Favorites.CONTAINER_DESKTOP,info.screenId ,lp.cellX,lp.cellY,1,1);
            if(animate){
                mLauncher.getDragLayer().animateViewIntoPosition2(dragView,newView,null,null);
            }
            if(!goBack) {
                LauncherModel.modifyItemInDatabase(mLauncher, info, LauncherSettings.Favorites.CONTAINER_DESKTOP, info.screenId,
    ...
    public void addViewAndUpdateData(int[] position,final View view,DragView dragView,View startView){
        addViewAndUpdateData(position,view,dragView,startView,false,true);
    }

    /**
     * 创建一个空文件夹
     */
    public FolderIcon createEmptFolder() {
        int index = getNextPage();
        CellLayout targetCellLayout = (CellLayout) getChildAt(index);
        if(targetCellLayout == null) {
            targetCellLayout = (CellLayout) getPageAt(getCurrentPage());
        }
        if(targetCellLayout == null) {
            return null;
        }
        ...
        if (targetCellLayout.findCellForSpan(position, 1, 1)) {
            final FolderInfo folderInfo = new FolderInfo();
            folderInfo.title = getResources().getString(R.string.new_folder_text);
            FolderIcon.PreviewBackground bg = new FolderIcon.PreviewBackground();
            DeviceProfile grid = mLauncher.getDeviceProfile();

            BubbleTextView bubbleTextView = findBubbleTextView();
            int width = bubbleTextView!=null?bubbleTextView.getMeasuredWidth():260;
            int topPadding = bubbleTextView!=null?bubbleTextView.getPaddingTop():17;
            bg.setup(getResources().getDisplayMetrics(), grid, null, width, topPadding, getResources());

            // The full preview background should appear behind the icon
            bg.isClipping = false;

            LauncherModel.addItemToDatabase(mLauncher, folderInfo, LauncherSettings.Favorites.CONTAINER_DESKTOP, screenId,
                    position[0], position[1]);

            FolderIcon folderIcon = FolderIcon.fromXml(R.layout.folder_icon, mLauncher,
                    targetCellLayout,
                    folderInfo, mIconCache);
            folderIcon.setFolderBackground(bg);

            addInScreen(folderIcon, LauncherSettings.Favorites.CONTAINER_DESKTOP, screenId, position[0], position[1], 1, 1,
                    mLauncher.isWorkspaceLocked());
            Log.i("folder","empty folder info = "+folderIcon.getFolderInfo());
            return folderIcon;
        } else {
            mLauncher.showOutOfSpaceMessage(false);
            return null;
...
//        cleanArrangeEmptyScreen();
    ...
    public boolean addEmptyScreenAsArrange() {
        if (mWorkspaceScreens.containsKey(arrangeAddScreenId)) {
            ...
            if (cellLayout.getShortcutsAndWidgets().getChildCount() > 0) {
                arrangeAddScreenId = LauncherSettings.Settings.call(getContext().getContentResolver(),
                        LauncherSettings.Settings.METHOD_NEW_SCREEN_ID)
                        .getLong(LauncherSettings.Settings.EXTRA_VALUE);
            }else {
                return false;
        ...
        int insertIndex = mScreenOrder.size();
        mLauncher.getModel().updateWorkspaceScreenOrder(mLauncher, mScreenOrder);
        CellLayout newCellLayout = insertNewWorkspaceScreen(arrangeAddScreenId, insertIndex);
        return true;
    }
    ...
    private void cleanArrangeEmptyScreen() {
        if (mWorkspaceScreens.containsKey(arrangeAddScreenId)) {
            CellLayout cellLayout = getScreenWithId(arrangeAddScreenId);
            if (cellLayout.getShortcutsAndWidgets().getChildCount() == 0) {
                if ((getScreenIdForPageIndex(getCurrentPage()) == arrangeAddScreenId)) {
                    int toPage = (getCurrentPage() - 1) >= 0 ? (getCurrentPage() - 1) : 0;
                    snapToPage(toPage);
                    setCurrentPage(toPage);
                }
                removeView(cellLayout);
                mWorkspaceScreens.remove(arrangeAddScreenId);
                mScreenOrder.remove(arrangeAddScreenId);
                mLauncher.getModel().updateWorkspaceScreenOrder(mLauncher, mScreenOrder);
            }
        }
    }
    ...
    private BubbleTextView findBubbleTextView() {
        for (CellLayout layout : mWorkspaceScreens) {
            for (int i = 0; i < layout.getChildCount(); i++) {
                ...
                if (v instanceof BubbleTextView) return (BubbleTextView) v;
            }
        }
        return null;
    }
    ...
    private boolean isNullOfLastScreen() {
        if (getPageCount() <= 0) return false;
        CellLayout cl = (CellLayout) getPageAt(getPageCount() - 1);
    ...
    public void addInHotseat(View child, long container, long screenId, int x, int y, int spanX, int spanY, int index) {
        if (container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
            if (getScreenWithId(screenId) == null) {
                Log.e(TAG, "Skipping child, screenId " + screenId + " not found");
                // DEBUGGING - Print out the stack trace to see where we are adding from
    ...
    public Runnable getmDelayedResizeRunnable() {
        return mDelayedResizeRunnable;
    }
  ...
  // cyl add for cycle slide start
   public void setCycleSlideFlag(){
     cycleSlideFlag  = mLauncher.getSharedPrefs().getBoolean(SpecialEffectPagedView.SPECIAL_EFFECT_CYCLE_SLIDE, true) && notHorizontal();        
   }
 ...
 // cyl add for special effect  start
    ...
    public void animateScrollEffect(boolean bStartAnim) {
        ...
        long animationDelay = bStartAnim ? 0L : 50L;
  ...
  // cyl add for special effect  end
     ...
     * @param unreadNum
     */
    public void updateComponentUnreadChanged(String packageName, int unreadNum, String shortcutCustomId) {
        if (LauncherBadgeProvider.DEBUG) {
            Log.d(TAG, "updateComponentUnreadChanged: packageName = " + packageName
                    + ", unreadNum = " + unreadNum);
        }
        ...
        for (ShortcutAndWidgetContainer layout : childrenLayouts) {
            ...
            for (int j = 0; j < childCount; j++) {
                ...
                if (tag instanceof ShortcutInfo) {
                    ...
                    final String pkgName = componentName == null? null:componentName.getPackageName();
                    final UserHandleCompat us = info.user;
                    if (LauncherBadgeProvider.DEBUG) {
                        Log.d(TAG, "updateComponentUnreadChanged 2: find packageName = "
                                + packageName + ",intent = " + intent + ",pkgName = "
                                + pkgName + ",user:"+us);
                    }
                    boolean isCurrentShortCut = info.shortcutCustomId == null || info.shortcutCustomId.equals(shortcutCustomId);
                    if (pkgName != null && pkgName.equals(packageName) && isCurrentShortCut) {
                        Log.d(TAG, "updateComponentUnreadChanged 1: find pkgName = "
                                + pkgName + ",tag = " + tag + ",j = " + j + ",cellX = "
                                + info.cellX + ",cellY = " + info.cellY);
                        info.unreadNum = unreadNum;
                        view.invalidate();
                    }
                } else if (tag instanceof FolderInfo) {
                    ((FolderIcon) view).updateFolderUnreadNum(packageName, unreadNum, shortcutCustomId);
                     view.invalidate();
                }
            }
        }
        ...
        for (ShortcutAndWidgetContainer layout : childrenLayouts) {
            ...
            for (int j = 0; j < childCount; j++) {
                ...
                if (tag instanceof ShortcutInfo&& view instanceof  BubbleTextView) {
                    ...
                    if(info.unreadNum>0&&!textView.isShowingUnread()) {
                        view.invalidate();
                    }
                }
                if (tag instanceof FolderInfo&& view instanceof  FolderIcon) {
                    FolderIcon textView = (FolderIcon) view;

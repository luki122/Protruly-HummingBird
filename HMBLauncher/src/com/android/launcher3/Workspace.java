...
public class Workspace extends PagedView
    ...
    public void onDragStart(DropTarget.DragObject dragObject, DragOptions options) {
        ...
        Runnable runnable =new Runnable() {
            @Override
            public void run() {
                if(FeatureFlags.UNINSTALL_MODE/*&&!mLauncher.isUninstallMode*/ && !mLauncher.isWidgetsViewVisible()&&mState!=State.FOLDER_IMPORT&&!mLauncher.isUninstallMode)
    ...
    public void onDragEnd() {
        ...
        if (!mDeferRemoveExtraEmptyScreen && (!mLauncher.isUnInstallMode()&& !mPrepareUninstallMode)) {
...
//        return numCustomPages();
    ...
    public CellLayout insertNewWorkspaceScreen(long screenId, int insertIndex) {
        ...
        if(mState == State.OVERVIEW_HIDDEN || (mState == State.SPRING_LOADED && mLauncher.mState == Launcher.State.WIDGETS_SPRING_LOADED)){//lijun add for newscreen background
...
//                    .ofFloat(mHotseatAlpha[HOTSEAT_STATE_ALPHA_INDEX], finalValue);
    ...
    public boolean workspaceInModalState() {
        return mState != State.NORMAL
                ...
                &&mState!=State.SPRING_LOADED;//liuzuo add for addIcon ; lijun add SPRING_LOADED
    }
...
//UninstallMod  add by liuzuo
    ...
    private void showOrHideUnstallMode(boolean show) {
        ArrayList<Long> workspaceScreens = LauncherModel.loadWorkspaceScreensDb(mLauncher);
        int childCount = workspaceScreens.size();
        /*
        workspace
         */
        for (int i = 0; i < childCount; i++) {
            CellLayout cellLayout = getScreenWithId(workspaceScreens.get(i));
            if (cellLayout instanceof UninstallMode) {
                UninstallMode layout = cellLayout;
                if (show) {
...
/*        disableLayoutTransitions();
    ...
    private boolean isOutHomeButtonAreas(MotionEvent event, View view) {
        if ((mState == State.OVERVIEW || mState == State.OVERVIEW_HIDDEN) && view instanceof CellLayout) {
            Rect rect = new Rect();
...
//        cleanArrangeEmptyScreen();
  ...
  protected boolean isNormalState(){
    if(FeatureFlags.CYCLE_SLIDE && mState == State.NORMAL){
	return true;
    }
	return false;
  }  

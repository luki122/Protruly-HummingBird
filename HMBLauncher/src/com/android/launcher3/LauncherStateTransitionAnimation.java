    ...
    public AnimatorSet startAnimationBetweenOverviewAndOverviewHiden(final Workspace.State toWorkspaceState,final Launcher.State launcherState, final boolean animated){
        ...
        if (toOverview) {
            if(launcherState == Launcher.State.WIDGETS){
                fromView = mLauncher.getWidgetsPanel();
            }else if(launcherState == Launcher.State.WALLPAPER){
                fromView = mLauncher.getmWallpaperPicker();
            ...
            }else if(launcherState == Launcher.State.SPECIALEFFECT){// cyl add for special effect
                fromView = mLauncher.getSpecialEffectPreview();
            }else if(launcherState == Launcher.State.WORKSPACE){
                return null;
            }
            toView = mLauncher.getOverviewPanel();
        }else {
            fromView = mLauncher.getOverviewPanel();
            if(launcherState == Launcher.State.WIDGETS){
                toView = mLauncher.getWidgetsPanel();
            }else if(launcherState == Launcher.State.WALLPAPER){
                toView = mLauncher.getmWallpaperPicker();
                mLauncher.refreshWallPaperList(true);
            }else if(launcherState == Launcher.State.ICONARRANGE){
        ...
        boolean initialized;
        if(launcherState == Launcher.State.WIDGETS){
            initialized =  mLauncher.getWidgetsButton() != null;
        }else if(launcherState == Launcher.State.WALLPAPER){

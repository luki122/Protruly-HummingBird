...
//                    previewWidth, recycle, null);
    ...
    public Bitmap generateWidgetPageViewPreview(Launcher launcher, LauncherAppWidgetProviderInfo info,
        ...
        boolean isInfoIcon = false;
        if (drawable == null) {
            try {
                isInfoIcon = true;
                drawable = mWidgetManager.loadIcon(info, mIconCache);
            } catch (Resources.NotFoundException e) {
        ...
        if(!isInfoIcon){
            if(info.spanX >= info.spanY){
                a = info.spanY/(float)info.spanX;
            }else {
                a = info.spanX/(float)info.spanY;
            }
        }
        int largeEdge = previewWidth>previewHeight?previewWidth:previewHeight;
        scale = (maxPreviewWidth - 2 * mProfileBadgeMargin * a) / (float) (largeEdge);
...
//            drawable.setBounds(x, 0, x + previewWidth, previewHeight);
        ...
        return mWidgetManager.getBadgeBitmap(info, preview, preview.getWidth(), preview.getHeight());
    }

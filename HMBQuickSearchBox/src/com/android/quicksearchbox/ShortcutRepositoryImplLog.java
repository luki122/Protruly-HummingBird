...
public class ShortcutRepositoryImplLog implements ShortcutRepository {
    ...
    /// M: 32 -> 33: update from JB to KK, 33->34:KK->L, 34->35:L->M, 35->36:M->N
    ...
    private static final String CLICKLOG_SQL = buildClickLogSql();//lijun add
...
// --------------------- Interface ShortcutRepository ---------------------
     ...
     * @param query
     */
    public void removeFromClickLogHistory(SuggestionCursor suggestions,final String query) {
        runTransactionAsync(new SQLiteTransaction() {
            @Override
            public boolean performTransaction(SQLiteDatabase db) {
                db.delete(ClickLog.TABLE_NAME, ClickLog.query.fullName + " = ?",
                        new String[]{ query });
                return true;
            }
        });
     ...
     * @param consumer
     */
    public void getClickLogs(final Collection<Corpus> allowedCorpora,final Consumer<ClickLogCursor> consumer){
        final long now = System.currentTimeMillis();
        mLogExecutor.execute(new Runnable() {
            public void run() {
                ClickLogCursor clickLogCursor = getClicLogCursor(allowedCorpora);
                Consumers.consumeCloseable(consumer, clickLogCursor);
            }
        });
    }

    private static String buildClickLogSql() {
        final String tables = ClickLog.TABLE_NAME;
        final String[] columns = ClickLog.COLUMNS;
        final String where = null;
        final String groupBy = null;
        final String having = null;
        final String orderBy = ClickLog.hit_time.name() + " DESC";
        final String limit = "5";//lijun here for limit clicklog size
        return SQLiteQueryBuilder.buildQueryString(
                false, tables, columns, where, groupBy, having, orderBy, limit);
    }
...
// -------------------------- end ShortcutRepository --------------------------
    ...
    //lijun add
    ClickLogCursor getClicLogCursor(Collection<Corpus> allowedCorpora){
        SQLiteDatabase db = mOpenHelper.getReadableDatabase();
        ...
        return new ClickLogCursor("",new SuggestionCursorImpl(allowedSources,"", cursor));
    }
    ...
    public class SuggestionCursorImpl extends CursorBackedSuggestionCursor {
        ...
        public String getQueryString(){
            return mCursor.getString(mCursor.getColumnIndex(ClickLog.query.name()));
        }
        public Long getHitTime(){
            return mCursor.getLong(mCursor.getColumnIndex(ClickLog.hit_time.name()));
        }
        ...
        Source getDefaultSuggestionSource(){
            if(mAllowedSources == null || mAllowedSources.size()<=0)return null;
            Set<String> keySet = mAllowedSources.keySet();
            String defaultKey = "";
            for(String s : keySet){
                if(s.contains("applications")){
                    defaultKey = s;
                }
            }
            if(mAllowedSources.get(defaultKey)!=null){
                return mAllowedSources.get(defaultKey);
            }else {
                for (Map.Entry<String, Source> sourceEntry : mAllowedSources.entrySet()) {
//                    String name = sourceEntry.getKey();
                    Source value = sourceEntry.getValue();
        ...
        public Source getSuggestionSource() {
            ...
            //lijun add start
            if (srcCol < 0) {
                return getDefaultSuggestionSource();
            }
            //lijun add end
            Cursor cursor = mCursor;
            String srcStr;
            try {
                srcStr = cursor.getString(srcCol);
            }catch (Exception e){
                Log.d(TAG,"getSuggestionSource Exception : " + e.toString());
                return getDefaultSuggestionSource();
            }
            if (srcStr == null) {
            ...
            int versionCode = cursor.getInt(Shortcuts.source_version_code.ordinal());
        ...
        public ComponentName getSuggestionIntentComponent() {
            //lijun add start
            if("_id".equals(mCursor.getColumnName(0))){
                return null;
            }
            //lijun add end
    ...
    void reportClickAtTime(SuggestionCursor suggestion,
        ...
        final String query = suggestion.getUserQuery();
     ...
     * @param query
     */
    public void reportClickForWebSearch(String query){
        final long now = System.currentTimeMillis();
        String intentKey = "com.android.quicksearchbox/.google.GoogleSearch/"+query;
        final ContentValues click = new ContentValues();
        click.put(ClickLog.intent_key.name(), intentKey);
        click.put(ClickLog.query.name(), query);
        click.put(ClickLog.hit_time.name(), now);
        click.put(ClickLog.corpus.name(), WebCorpus.WEB_CORPUS_NAME);

...
public class WebCorpus extends MultiSourceCorpus {
    ...
    public static final String WEB_CORPUS_NAME = "web";
    ...
    public CharSequence getSettingsDescription() {
        /// M: No access to bookmarks and history after Lollipop_MR1. @{
        if (!isSupportBrowserSearch()) {
             return getContext().getText(R.string.corpus_description_web_search_only);
        } else {
        /// @}
    ...
    protected List<Source> getSourcesToQuery(String query, boolean onlyCorpus) {
        ...
        /// M: Add check for Browser search supported.
        if (isSupportBrowserSearch() && mBrowserSource != null && query.length() > 0) {
    ...
    protected class WebResult extends Result {
        ...
        public void fill() {
            ...
            /// M: display all browser results, if supported. @{
            if (isSupportBrowserSearch() && (browserResult != null)) {
...
/**
 * M: Check if support external search for browser history and bookmarks.
 * @return     if browser search feasible, then return true, else return false
 */
    private boolean isSupportBrowserSearch() {

...
public class GoogleSearch extends Activity {
    ...
    private void handleWebSearchInternal(Intent intent) {
        SearchEngine searchEngineInfo = QsbApplication.get(this).getSearchEngine();
        ...
        String searchUri
                = QsbApplication.get(this).getSearchEngine().getSearchUriForQuery(query);
        if (searchUri == null) {
            Log.e(TAG, "Unable to get search URI for ");
        } else {
            /// M: plugin @{
            String searchEngineName = QsbApplication.get(this).getSearchEngine().getKeyWord();
    ...
    private boolean handleWebSearchInternal(Context context, String searchEngineName,
        String searchUri) {
        IWebSearchHandler handler = MPlugin.createInstance(IWebSearchHandler.class.getName(), getBaseContext());
        if (handler == null) {
            Log.e(TAG, "handleWebSearchInternal(), get IWebSearchHandler instance from MPlugin failed.");

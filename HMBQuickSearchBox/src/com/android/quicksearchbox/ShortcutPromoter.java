...
public class ShortcutPromoter extends AbstractPromoter {
    ...
    /*
     * M: If only show shortcuts, then get a certain amount of shortcuts by original order.
     */
    @VisibleForTesting
    void promoteShortcuts(SuggestionCursor shortcuts,
            ListSuggestionCursor promoted) {
        int shortcutCount = shortcuts == null ? 0 : shortcuts.getCount();
        if (shortcutCount == 0) return;
        // limit the shortcuts count
        int maxCount = getConfig().getShortcutsOnlyLimitedCount();
        for (int i = 0; i < shortcutCount && promoted.getCount() < maxCount; i++) {
            shortcuts.moveTo(i);
            Source source = shortcuts.getSuggestionSource();
            if (source != null && accept(shortcuts)) {
                promoted.add(new SuggestionPosition(shortcuts));

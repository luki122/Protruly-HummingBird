...
public class DefaultSuggestionViewFactory implements SuggestionViewFactory {
    ...
    public DefaultSuggestionViewFactory(Context context) {
        ...
        addFactory(new HistorySuggestionView.Factory(context));//lijun add
    ...
    public View getView(SuggestionCursor suggestion, String userQuery,
        ...
        //lijun add start
        if(suggestion instanceof ListSuggestionCursor && (((ListSuggestionCursor)suggestion).isHistory())){
            for (SuggestionViewFactory factory : mFactories) {//优先查找是否时历史记录
                if(factory instanceof HistorySuggestionView.Factory && factory.canCreateView(suggestion)){

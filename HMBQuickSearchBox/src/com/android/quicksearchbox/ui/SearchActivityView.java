...
import com.android.quicksearchbox.hotsearch.HotSearchHttpGet;
import com.android.quicksearchbox.hotsearch.HotSearchInfo;
...
public abstract class SearchActivityView extends RelativeLayout {
    ...
    //lijun add start
    private static Integer SUGGESTION_LIST_TYPE_HISTORY = new Integer(1);
    private static Integer SUGGESTION_LIST_TYPE_NORMAL = new Integer(2);
    ...
    protected SuggestionsListView<ExpandableListAdapter> mSuggestionsView;
    ...
    /// M:
    protected HotSearchView mHotSearchView;//lijun add for hot search
    ...
    protected void onFinishInflate() {
        ...
        /// M:
        mSearchGoButton = (TextView) mActionBarView.findViewById(R.id.search_go_btn);
        mVoiceSearchButton = (ImageButton) mActionBarView.findViewById(R.id.search_voice_btn);
        mVoiceSearchButton.setVisibility(View.GONE);//lijun add
        mVoiceSearchButton.setImageDrawable(getVoiceSearchIcon());
        mFootView = (TextView) inflate(getContext(),R.layout.expandadblelistview_foot,null);
        ...
        mHotSearchView = (HotSearchView) inflate(getContext(),R.layout.expandadblelistview_hotsearch,null);
//        mHotSearchView.loadData();
//        ((SuggestionsView)mSuggestionsView).addHeaderView(mHotSearchView,null,false);
...
//        }
        ...
        mSearchGoButton.setVisibility(View.GONE);//lijun add
...
//liuzuo add begin 
        ((SuggestionsView)mSuggestionsView).addFooterView(mFootView,null,false);
        mFootView.setOnClickListener(new SearchGoButtonClickListener());
        mQueryTextView.setClearDrawableVisible(false);
//liuzuo add end 
    }
    ...
    public void onResume() {
        if (getQuery().length() == 0 && mCorpus != null && mCorpus.isWebCorpus()) {
            mQueryTextView.setHint(
                    QsbApplication.get(getContext()).getSearchEngine().getLabel());
        }
    }
    ...
    protected SuggestionsAdapter<ExpandableListAdapter> createSuggestionsAdapter() {
        return new DelayingSuggestionsAdapter<ExpandableListAdapter>(new ExpandSuggestionsAdapter(
                getQsbApplication().getSuggestionViewFactory(),getContext()));
    }
...
//        updateVoiceSearchButton(queryEmpty);//lijun remove
        loadHotsearch(queryEmpty);//lijun add for hot search
    }

    protected void updateQueryTextView(boolean queryEmpty) {
        if (mCorpus == null) {
            mQueryTextView.setHint(getContext().getString(R.string.corpus_label_global_new));//lijun modify corpus_label_global to corpus_label_global_new
        } else if (mCorpus.isWebCorpus()) {
            mQueryTextView.setHint(QsbApplication.get(getContext()).getSearchEngine().getLabel());
        } else {
    ...
    private void updateSearchGoButton(boolean queryEmpty) {
        Log.d(TAG,"updateSearchGoButton queryEmpty : "+queryEmpty);
        onClickLogCountChanged(0);
        if (queryEmpty) {
          //  mSearchGoButton.setVisibility(View.INVISIBLE);
//liuzuo add begin
            mFootView.setText(R.string.clear_history_text);
            mFootView.setTag(SUGGESTION_LIST_TYPE_HISTORY);
            mFootView.setTextColor(getResources().getColor(R.color.clearhistory_text_color));
//liuzuo add end
        } else {
           // mSearchGoButton.setVisibility(View.VISIBLE);
            mFootView.setText(R.string.list_view_foot_text);
            mFootView.setTag(SUGGESTION_LIST_TYPE_NORMAL);
            mFootView.setTextColor(getResources().getColor(R.color.suggestion_group_expand_color));
//liuzuo add begin
//liuzuo add end
        }
    }
    ...
    /**
     * lijun add for hot search
     *
     * @param queryEmpty
     */
    public void updateHotsearchView(boolean queryEmpty) {
        boolean hasHeader = ((SuggestionsView) mSuggestionsView).getHeaderViewsCount() > 0;
        if (queryEmpty) {
            if (!hasHeader) {
                ((SuggestionsView) mSuggestionsView).addHeaderView(mHotSearchView, null, false);
            }
            ...
            if (hasHeader) {
                ((SuggestionsView) mSuggestionsView).removeHeaderView(mHotSearchView);
            }
        }
        ((SuggestionsView) mSuggestionsView).setBackground(null);
    }


    public void loadHotsearch() {
        loadHotsearch(isQueryEmpty());
    }
     ...
     * @param queryEmpty
     */
    public void loadHotsearch(boolean queryEmpty) {
        getActivity().hideWebLoadPanel();
        if(queryEmpty){
            try {
                new HotSearchHttpGet(mHotSearchView).execute(HotSearchInfo.getHotSearchUrl());
            }catch (Exception e){
                Log.d(TAG,"loadHotsearch error : " + e.toString());
                e.printStackTrace();
            }
        }else {
            getActivity().updateForQuery();
            updateHotsearchView(queryEmpty);
        }
    }
    ...
    private class SearchTextWatcher implements TextWatcher {
        ...
        public void afterTextChanged(Editable s) {
            ...
            mQueryTextView.setClearDrawableVisible(!empty);//liuzuo add
        }
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            if(s==null){
                needUpdate = showHistory?false:true;
            }else {
                String ss = s.toString();
                if (ss.equals(beforeText + " ") || (beforeText != null && beforeText.equals(s + " "))) {
                    needUpdate = false/*showHistory?false:true*/;
                } else {
                    needUpdate = true;
                }
            }
            Log.d(TAG,"onTextChanged needUpdate : " +needUpdate);
        }
    }
...
//            onSearchClicked(Logger.SEARCH_METHOD_BUTTON);
            if((Integer)view.getTag() == SUGGESTION_LIST_TYPE_HISTORY){
                onClearHistoryClick();
            }else {
                onSearchClicked(Logger.SEARCH_METHOD_BUTTON);
            }
    ...
    protected class SuggestionsObserver extends DataSetObserver {
        ...
        public void onChanged() {
            ...
            //liuzuo add for expand all in begining
            if(mSuggestionsView instanceof  SuggestionsView){
                SuggestionsView suggestionsView = (SuggestionsView) mSuggestionsView;
                if(mSuggestionsAdapter instanceof  DelayingSuggestionsAdapter&& ((DelayingSuggestionsAdapter) mSuggestionsAdapter).isFirstExpand()){
                    suggestionsView.expandAll();
                }
            }
            //liuzuo end

            //lijun add start
            if (mSuggestionsAdapter != null && mSuggestionsAdapter.getSuggestions() != null
                    && "".equals(mSuggestionsAdapter.getSuggestions().getQuery())
                    && mSuggestionsAdapter.getSuggestions().getClickLogCursor() != null) {
                ClickLogCursor clickLogCursor = mSuggestionsAdapter.getSuggestions().getClickLogCursor();
                onClickLogCountChanged(clickLogCursor.getCount());
                showHistory = true;
            }else {
                if (mSuggestionsAdapter != null && mSuggestionsAdapter.getSuggestions() != null) {
                    onClickLogCountChanged(mSuggestionsAdapter.getSuggestions().getResultCount());
                    showHistory = false;
                }
            }
            //lijun add end
    ...
    public void onClickLogCountChanged(int count){
        if(count<= 0){
    ...
    public void clearIconCache() {
        try {
            if (mSuggestionsView != null && mSuggestionsView instanceof SuggestionsView) {
                final SuggestionsView suggestionsView = ((SuggestionsView) mSuggestionsView);
                for (int i = 0; i < suggestionsView.getChildCount(); i++) {
                    View view = suggestionsView.getChildAt(i);
                    if (view != null && view instanceof DefaultSuggestionView) {
                        ((DefaultSuggestionView) view).clearIconCache();
                    }
                }
            }
        } catch (Exception e) {
            Log.e(TAG,"theme changed and clearIconCache failed!");
        }
    }
}

...
import com.android.quicksearchbox.Applications;
...
public class DefaultSuggestionView extends BaseSuggestionView {
    ...
    public void bindAsSuggestion(Suggestion suggestion, String userQuery) {
        ...
        //lijun add start for don't show apps at text2
        if (source != null && source.getName() != null && source.getName().contains("com.android.providers.applications")) {
            text2 = "";
        }
        //lijun add end
        if (text2 != null) {
...
//        mAsyncIcon1.set(suggestion.getSuggestionSource(), suggestion.getSuggestionIcon1());
        if (!mAsyncIcon1.setByIntentData(source, suggestion.getSuggestionIntentDataString())) {
            mAsyncIcon1.set(suggestion.getSuggestionSource(), suggestion.getSuggestionIcon1());
        }
        //lijun modify end
        mAsyncIcon2.set(suggestion.getSuggestionSource(), suggestion.getSuggestionIcon2());
        ComponentName componentName = suggestion.getSuggestionIntentComponent();
    ...
    private class AsyncIcon {
        ...
        //lijun add start
        private ComponentName mWantedComponent;
        //lijun add end
        ...
        public void set(final Source source, final String sourceIconId) {
            if (sourceIconId != null) {
                ...
                final Uri iconUri = source.getIconUri(sourceIconId);
                final String uniqueIconId = iconUri == null ? null : iconUri.toString();
                mWantedId = uniqueIconId;
                final Object tag = mView.getTag();
                final boolean viewChanged = (tag != null) && ((Integer) tag).equals(1);
                if(viewChanged){
                    mCurrentId = "";
                    clearDrawable();
                }
        ...
        /**
         * lijun add for theme icon
         */
        public boolean setByIntentData(final Source source, String dataString) {
            if (TextUtils.isEmpty(dataString)) {
                mWantedComponent = null;
                return false;
            }
            if(!"com.android.providers.applications/.ApplicationLauncher".equals(source.getName())){
                mWantedComponent = null;
                return false;
            }

            final ComponentName componentName = Applications.uriToComponentName(Uri.parse(dataString));
            if (componentName == null || componentName.getPackageName() == null) {
                mWantedComponent = null;
                return false;
            }

            IconManager iconManager = IconManager.getInstance(getContext(), true, true);

            Drawable drawable = iconManager.getIconDrawable(componentName, null);

            NowOrLater<Drawable> icon = new Now<Drawable>(drawable);
            if (drawable == null) {
                mWantedComponent = null;
                return false;
            }
            mWantedComponent = componentName;
            if (icon.haveNow()) {
                setDrawable2(icon.getNow(), componentName);
            } else {
                clearDrawable2();
                icon.getLater(new Consumer<Drawable>() {
                    public boolean consume(Drawable icon) {
                        // ensure we have not been re-bound since the request was made.
                        if (equalsComponentName(componentName, mWantedComponent)) {
                            setDrawable2(icon, componentName);
                            return true;
                        }
                        return false;
                    }
        ...
        private boolean equalsComponentName(ComponentName a, ComponentName b) {
            if (a != null && b != null) {
                if (a.getPackageName() != null && a.getPackageName().equals(b.getPackageName()) && a.getClassName() != null && a.getClassName().equals(b.getClassName())) {
                    return true;
                }
            }
            return false;
        ...
        //lijun add start
        private void setDrawable2(Drawable icon, ComponentName componentName) {
            mView.setTag(1);
            if(equalsComponentName(componentName,mWantedComponent)) {
                setViewDrawable(mView, icon);
            }
    ...
    protected void clearIconCache() {
        if(mAsyncIcon1!=null){
            mAsyncIcon1.mCurrentId = null;
            mAsyncIcon1.mWantedComponent = null;
            mAsyncIcon1.mView.setImageDrawable(null);
        }
    }
}

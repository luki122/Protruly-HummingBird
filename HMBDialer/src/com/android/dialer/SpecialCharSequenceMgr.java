...
//	private static final String HB_CLOSE_LOG = "*#*#789666#*#*";
//	private static final String HB_OPEN_LOG = "*#*#789667#*#*";
	...
	public static boolean handleChars(Context context, String input, EditText textField) {
		...
		if (ZteEngineerCode.handleZteEngineerCode(context, dialString)
				...
				/// M: for plug-in @{
				|| (ExtensionManager.getInstance().getDialPadExtension()!=null && ExtensionManager.getInstance().getDialPadExtension().handleChars(context,
						dialString))
				/// @}
				) {
			return true;
...
//		if(TextUtils.equals(input, HB_CLOSE_LOG)){//add by liyang:关闭日志暗码
////			System.setProperty("persist.sys.bugreportswitch","off");
//			Toast.makeText(context, "log switch turned off", Toast.LENGTH_LONG).show();
//		}else if(TextUtils.equals(input, HB_OPEN_LOG)){//add by liyang:打开日志暗码
////			System.setProperty("persist.sys.bugreportswitch","off");
//			Toast.makeText(context, "log switch turned on", Toast.LENGTH_LONG).show();
//		}
...
//			Log.d(TAG, "deviceIds1:"+deviceIds);
			if(deviceIds.size()>1){
				for(int i=0;i<deviceIds.size();i++){
					deviceIds.set(i, "IMEI"+(i+1)+":"+deviceIds.get(i));
				}
			}else if(deviceIds.size()>0) deviceIds.set(0, "IMEI:"+deviceIds.get(0));

			String meid=SystemProperties.get("gsm.mtk.meid");
			if(TextUtils.isEmpty(meid)) {
				if(!TextUtils.isEmpty(SystemProperties.get("gsm.mtk.meid1"))) meid=SystemProperties.get("gsm.mtk.meid1");
				else meid=SystemProperties.get("gsm.mtk.meid2");
			}

			if(!TextUtils.isEmpty(meid)) deviceIds.add("MEID:"+meid.toUpperCase());

...
public class CallLogQueryHandler extends NoNullCursorAsyncQueryHandler {
	...
	private void fetchCalls(int token, int callType, boolean newOnly, long newerThan,boolean isPrivate) {
		...
		Uri uri = getCallLogUri(mContext).buildUpon()
				.appendQueryParameter(Calls.LIMIT_PARAM_KEY, Integer.toString(limit))
				.build();
		Log.d(TAG,"liyangs-fetchCalls,uri:"+uri+" token:"+token+" selection:"+selection);
		startQuery(token, null, uri,
				CallLogQuery._PROJECTION, selection, selectionArgs.toArray(EMPTY_STRING_ARRAY),
				Calls.DEFAULT_SORT_ORDER);
		Log.d(TAG,"liyangs-fetchCalls4");
	}

	public static Uri getCallLogUri(Context context) {
		return TelecomUtil.hasReadWriteVoicemailPermissions(context) ? Uri.parse("content://call_log/calls_hb").buildUpon()
				.appendQueryParameter("allow_voicemails", "true")
				.build()
				: Uri.parse("content://call_log/calls_hb");
	}

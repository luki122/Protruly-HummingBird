...
import com.android.dialer.calllog.PhoneCallDetailsHelper.SimResources;
...
public class DialerPhoneNumberListAdapter extends PhoneNumberListAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (shortcutType >= 0) {
			ViewHolderShortCut viewHolderShortCut;
			ViewGroup v;
			if (convertView != null) {
				//				assignShortcutToView((ContactListItemView) convertView, shortcutType);
				v=(ViewGroup) convertView;
				viewHolderShortCut=(ViewHolderShortCut)v.getTag();
			} else {
				//				final ContactListItemView v = new ContactListItemView(getContext(), null);
				//				assignShortcutToView(v, shortcutType);
				v=(ViewGroup) LayoutInflater.from(getContext()).inflate(R.layout.hb_contacts_listview_item,null);
				v.removeViewAt(0);
				View inner =LayoutInflater.from(getContext()).inflate(com.hb.R.layout.list_item_1_line,null);
				v.addView(inner,0,new LayoutParams(LayoutParams.MATCH_PARENT,
						LayoutParams.WRAP_CONTENT));

				//				View devider=v.findViewById(R.id.devider);
				//				LinearLayout.LayoutParams params=(LinearLayout.LayoutParams) devider.getLayoutParams();
				//				params.setMarginEnd(getContext().getResources().getDimensionPixelOffset(R.dimen.contact_listview_header_padding_left));
				//				params.setMarginStart(getContext().getResources().getDimensionPixelOffset(R.dimen.contact_listview_header_padding_left));
				//				devider.setLayoutParams(params);

				viewHolderShortCut=new ViewHolderShortCut();
				viewHolderShortCut.textView=(TextView) inner.findViewById(android.R.id.text1);
				v.setTag(viewHolderShortCut);
			}
			hbAssignShortcutToView(viewHolderShortCut.textView, shortcutType);
			return v;
		} else {
			Cursor cursor=(Cursor) getItem(position);
			final int viewType = getViewType(cursor);
			View itemView;
			if(viewType!=VIEW_TYPE_CONTACT_HIDE_NAME) itemView=hbNewView(getContext(), cursor, position);
			else itemView=hbNewSingleView(getContext());

			//			bindContactView(itemView,getContext(),cursor);
			bindView(itemView, cursor, position,viewType);
			return itemView;
		}
	}

	private static class ViewHolderShortCut{
	...
	private void hbAssignShortcutToView(TextView v, int shortcutType) {
		final CharSequence text;
		final int drawableId;
		final Resources resources = getContext().getResources();
		final String number = getFormattedQueryString();
	...
	private final int VIEW_TYPE_CONTACT_HIDE_NAME = 2;
	...
	protected void bindView(View itemView, int partition, Cursor cursor, int position) {
		...
		final int viewType = getViewType(cursor);
		switch (viewType) {
		case VIEW_TYPE_CONTACT_HIDE_NAME:
	...
	protected void bindView(View itemView, Cursor cursor, int position, int viewType) {
	...
	protected View newView(Context context, int partition, Cursor cursor, int position,
			ViewGroup parent) {
		return hbNewView(context, cursor, position);
	...
	private static class ViewHolderHb {
		public TextView name;
		public ImageView sim_icon;
		public TextView datetime;
		public View call_log_first_list;
		//		public View call_log_second_list;
		//		public TextView call_type_text;
		public TextView number;
		public View item_more;
		public View primary_action_view;
		public QuickContactBadge quickContactBadge;
	}

	protected View hbNewView(Context context,Cursor cursor, int position){
		View view = View.inflate(context, R.layout.hb_call_log_list_item_slider, null);
		ViewHolderHb viewHolder = new ViewHolderHb();

		viewHolder.quickContactBadge = (QuickContactBadge) view
				.findViewById(R.id.quick_contact_photo);
		viewHolder.name = (TextView) view.findViewById(R.id.name);
		viewHolder.sim_icon = (ImageView) view.findViewById(R.id.sim_icon);
		viewHolder.datetime = (TextView) view.findViewById(R.id.datetime);
		viewHolder.call_log_first_list =view.findViewById(R.id.call_log_first_list);
		//		viewHolder.call_log_second_list =view.findViewById(R.id.call_log_second_list);
		//		viewHolder.call_type_text = (TextView) view.findViewById(R.id.call_type_text);
		viewHolder.number = (TextView) view.findViewById(R.id.number);
		viewHolder.item_more = view.findViewById(R.id.item_more);
		viewHolder.primary_action_view=view.findViewById(R.id.primary_action_view);

		view.setTag(viewHolder);
		return view;
	}

	protected View hbNewSingleView(Context context){
		View view = View.inflate(context, R.layout.hb_call_log_list_item_single_line, null);
		ViewHolderHb viewHolder = new ViewHolderHb();
	...
	private int getViewType(Cursor cursor) {
		...
		if (contactId > 0) {
			if(contactId==contactIdPre) retval=VIEW_TYPE_CONTACT_HIDE_NAME;
			else retval = VIEW_TYPE_CONTACT;
		} else if (callLogId > 0) {
...
//        }
	...
	private void bindContactView(View view, final Context context, Cursor cursor) {

		//		Log.d(TAG,"bindContactView");

		final ViewHolderHb viewHolder = (ViewHolderHb) view.getTag();
...
//		}
		...
		final Uri contactUri = getContactUri(cursor);
		...
		if (!TextUtils.isEmpty(displayName)) {
			...
			if (!TextUtils.isEmpty(formatNumber)) {
				...
				if (!TextUtils.isEmpty(highlight)) {
					SpannableStringBuilder style = highlightHyphen(highlight, formatNumber,
							number);
					setLabelAndNumber(viewHolder.number, /*label*/null, style);
				} else {
					setLabelAndNumber(viewHolder.number, /*label*/null,
							new SpannableStringBuilder(formatNumber));
				}
			}
		} else {
		...
		viewHolder.primary_action_view.setOnClickListener(new View.OnClickListener() {

			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				Log.d(TAG, "primary_action_view onclick");
		...
		viewHolder.item_more.setOnClickListener(new View.OnClickListener() {

			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				QuickContact.showQuickContact(context,viewHolder.quickContactBadge, contactUri,
						QuickContact.MODE_LARGE, null);
			}
		});

		//		RelativeLayout.LayoutParams params=(RelativeLayout.LayoutParams) viewHolder.call_log_second_list.getLayoutParams();
		//		if(viewType==VIEW_TYPE_CONTACT) {
		//			viewHolder.call_log_first_list.setVisibility(View.VISIBLE);
		//			params.addRule(RelativeLayout.BELOW, R.id.call_log_first_list); 
		//			Log.d(TAG,"bind VIEW_TYPE_CONTACT,displayName:"+displayName);
		//		} else {
		//			viewHolder.call_log_first_list.setVisibility(View.GONE);
		//			params.addRule(RelativeLayout.CENTER_VERTICAL);
		//			Log.d(TAG,"bind VIEW_TYPE_CONTACT_HIDE_NAME,displayName:"+displayName);
		//		}
		//		viewHolder.call_log_second_list.setLayoutParams(params);

		//		if(viewHolder.call_type_text!=null) viewHolder.call_type_text.setVisibility(View.GONE);
	}
...
//        mContactPhotoManager.loadThumbnail(viewHolder.quickContactBadge, 0, false, true, request);
	 ...
	 * @param cursor
	 */
	private void bindCallLogView(View view, final Context context, Cursor cursor) {
		...
		final long date = cursor.getLong(CALL_LOG_DATE_INDEX);
		final int indicate = cursor.getInt(INDICATE_PHONE_SIM_INDEX);
		String geocode = TextUtils.isEmpty(cursor.getString(CALL_GEOCODED_LOCATION_INDEX))?"":cursor.getString(CALL_GEOCODED_LOCATION_INDEX).replace(" ", "");
		...
		secondLine.append(getCallTypeText(context, type,1)+"  ");
		secondLine.append(geocode);
		...
		viewHolder.item_more.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.putExtra("mIsPrivate", false);
		...
		if(DialerApplication.isMultiSimEnabled){
			try{
				final int slotId=SubscriptionManager.getSlotId(cursor.getInt(PHONE_ACCOUNT_ID_INDEX));
				if (slotId == 1) {
					viewHolder.sim_icon.setBackground(mContext.getDrawable(R.drawable.hb_sim2_icon));
					viewHolder.sim_icon.setVisibility(View.VISIBLE);
				} else if (slotId == 0) {
					viewHolder.sim_icon.setBackground(mContext.getDrawable(R.drawable.hb_sim1_icon));
					viewHolder.sim_icon.setVisibility(View.VISIBLE);
				}else{
					viewHolder.sim_icon.setVisibility(View.GONE);
				}
				viewHolder.primary_action_view.setOnClickListener(new View.OnClickListener() {
...
//					intent.putExtra("slot",-1);//-1 不指定；0指定卡槽1拨号；1指定卡槽2拨号
	...
	private SpannableStringBuilder highlightString(String highlight, CharSequence target) {
		...
		for (int i = DS_MATCHED_DATA_INIT_POS; i + 1 < length; i += DS_MATCHED_DATA_DIVIDER) {
			...
			style.setSpan(/*new StyleSpan(Typeface.BOLD)*/new ForegroundColorSpan(Color.parseColor(getContext().getResources().getString(R.string.hb_highlight_color_string))), start, end,
	...
	private SpannableStringBuilder highlightHyphen(String highlight, String target, String origin) {
		...
		if (numberHighlightOffset != null && numberHighlightOffset.size() > 1) {
			style.setSpan(/*new StyleSpan(Typeface.BOLD)*/new ForegroundColorSpan(Color.parseColor(getContext().getResources().getString(R.string.hb_highlight_color_string))), numberHighlightOffset.get(0),

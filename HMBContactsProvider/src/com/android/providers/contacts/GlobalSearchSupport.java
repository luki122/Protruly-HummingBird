        ...
        public ArrayList<?> asList(String[] projection) {
            if (icon1 == null) {
                if (photoUri != null) {
                    ...
                    icon1 = String.valueOf(/*com.android.internal.*/R.drawable.hb_ic_contacts);
                }
            }
...
//            Log.i(TAG, " icon2 : " + icon2);
    ...
    public Cursor handleSearchSuggestionsQuery(SQLiteDatabase db, Uri uri, String[] projection,
        ...
        if (uri.getPathSegments().size() <= 1) {
            ...
            String searchClause = filterSpecialChars(uri.getLastPathSegment());
            if(TextUtils.isEmpty(searchClause)) return cursor;
    ...
    public String filterSpecialChars(String str){
    	// 清除掉所有特殊字符
    	String regEx="[`~!@#$%^&*()+=|{}':;',\\[\\].<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？]";
    	Pattern p = Pattern.compile(regEx);
    	Matcher m = p.matcher(str);
    	return m.replaceAll("").trim().replace("_", "").replace("-", "");
    } 
    ...
    private Cursor addSearchSuggestionsBasedOnFilter(MatrixCursor cursor, SQLiteDatabase db,
        ...
        if (haveFilter) {
            mContactsProvider.appendSearchIndexJoin(sb, filter, true,
                    ...
                    SNIPPET_ELLIPSIS, SNIPPET_MAX_TOKENS, false,false);
        }

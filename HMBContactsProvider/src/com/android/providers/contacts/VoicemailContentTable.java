...
public class VoicemailContentTable implements VoicemailTable.Delegate {
    ...
    public Uri insert(UriData uriData, ContentValues values) {
        ...
        try {
            ...
            if (rowId > 0) {
                ...
                if( DialerSearchSupport.getInstance(mContext)!=null) DialerSearchSupport.getInstance(mContext).handleCallLogsInserted(db, rowId,
                        strInsNumber);
    ...
    public int delete(UriData uriData, String selection, String[] selectionArgs) {
        ...
        if (count > 0) {
        	if( DialerSearchSupport.getInstance(mContext)!=null) DialerSearchSupport.getInstance(mContext).handleCallLogsDeleted(db);
        }
        return count;
        /** M: [VVM] trigger VVM for dialer search @} */
    }

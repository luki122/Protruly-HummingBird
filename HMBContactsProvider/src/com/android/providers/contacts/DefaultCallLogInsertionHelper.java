...
/*package*/ class DefaultCallLogInsertionHelper implements CallLogInsertionHelper {
    ...
    private DefaultCallLogInsertionHelper(Context context) {
        ...
        /// M: @{
        mContext = context;
        mTMSManager = (TMSManager)mContext.getSystemService(TMSManager.TMS_SERVICE);
        /// @}
    }
...
//        }
    	String location="";
    	try{
    	if(mTMSManager!=null && number!=null && !TextUtils.isEmpty(number.replace(" ", "")))
    		location=mTMSManager.getLocation(number.replace(" ", ""));
    	}catch(Exception e){
    		Log.d(TAG,"e:"+e);
    	}
    	return location;

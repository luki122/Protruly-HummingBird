...
import com.android.providers.hb.ContactsContract.Contacts.Photo;
import com.android.providers.hb.ContactsContract.Data;
import com.android.providers.hb.ContactsContract.Directory;
import com.android.providers.hb.ContactsContract.DisplayNameSources;
import com.android.providers.hb.ContactsContract.DisplayPhoto;
import com.android.providers.hb.ContactsContract.FullNameStyle;
import com.android.providers.hb.ContactsContract.Groups;
import com.android.providers.hb.ContactsContract.PhoneticNameStyle;
...
public class ContactsDatabaseHelper extends SQLiteOpenHelper {
	...
	public interface Views {
		...
		/// @}
		//add by lgy 
		public static final String HB_BLACK_VIEW = "view_black_not_white";
	}
	...
	public void onCreate(SQLiteDatabase db) {
		...
		if (ContactsProviderUtils.isSearchDbSupport()) {        	
			DialerSearchHelperForHb.getInstance().createDialerSearchTable(db);
			DialerSearchHelperForHb.getInstance().createDialerSearchView(db);
		}
	...
	public void updateRawContactDisplayName(SQLiteDatabase db, long rawContactId) {
		...
		try {
			while (c.moveToNext()) {
				...
				if (source == DisplayNameSources.STRUCTURED_NAME) {
					data1String=c.getString(RawContactNameQuery.DATA1);
		...
		if (bestDisplayNameSource == DisplayNameSources.STRUCTURED_NAME
			...
			//liyang modify for bug 650 begin
			boolean isAllNum = TextUtils.isEmpty(data1String)?false:data1String.replace("+", "").replace(" ", "").matches("[0-9]+");
//			Log.d(TAG,"isAllNum:"+isAllNum+" data1String:"+data1String);
			if(isAllNum) {
				displayNamePrimary=data1String;
				displayNameAlternative=data1String;
			}
			//liyang modify for bug 650 end
			...
			/** M: dialer search @{ */
			if (ContactsProviderUtils.isSearchDbSupport() && DialerSearchSupport.getInstance(mContext)!=null) {
		...
		Log.d(TAG,"sortKeyPrimary:"+sortKeyPrimary+" displayNamePrimary:"+displayNamePrimary);
		if (sortKeyPrimary != null) {
			phonebookBucketPrimary = localeUtils.getBucketIndex(sortKeyPrimary);
			Log.d(TAG, "phonebookBucketPrimary:"+phonebookBucketPrimary+" sortKeyPrimary:"+sortKeyPrimary);
			phonebookLabelPrimary = localeUtils.getBucketLabel(phonebookBucketPrimary);
		} else if(TextUtils.isEmpty(displayNamePrimary)){
			phonebookBucketPrimary = localeUtils.getBucketIndex(sortKeyPrimary);
			Log.d(TAG, "phonebookBucketPrimary1:"+phonebookBucketPrimary+" sortKeyPrimary:"+sortKeyPrimary);
			phonebookLabelPrimary = localeUtils.getBucketLabel(phonebookBucketPrimary);
		}
		...
		if (ContactsProviderUtils.isSearchDbSupport()) {
			...
			//			DialerSearchHelperForHb.getInstance().updateOrInsertNameDialerSearch(db,
			//					rawContactId, displayNamePrimary);			
			updateContactsPinyin(db,rawContactId,displayNamePrimary);
		}
		
//		//add by liyang for calllog name
//		CallLogProviderEx.getInstance(mContext).hbUpdateCallsName(db,
//				rawContactId, displayNamePrimary);
	}
	...
	public static final class BlackColumns implements BaseColumns { 		
	...
	public static final class WhiteColumns implements BaseColumns {
		public static final String LABLE = "lable";
		public static final String NAME = "white_name";
		public static final String NUMBER = "number";
	}
	...
	private void createRejectTables(SQLiteDatabase db) {
		db.execSQL("create table " + Tables.BLACK + "("
			    ...
			    + Calls.RAW_CONTACT_ID + " INTEGER DEFAULT NULL,"
			    + Calls.DATA_ID + " INTEGER DEFAULT NULL,"
				+ BlackColumns.NUMBER + " TEXT,"
				+ BlackColumns.REJECT + " INTEGER)" + ";");
		
		db.execSQL("CREATE UNIQUE INDEX black_number_index ON " + Tables.BLACK + " (" +
				BlackColumns.NUMBER +
				");");
		...
		db.execSQL("create table " + Tables.WHITE + "("
				+ WhiteColumns._ID + " INTEGER PRIMARY KEY autoincrement,"
				+ WhiteColumns.LABLE + " TEXT,"
				+ Calls.RAW_CONTACT_ID + " INTEGER DEFAULT NULL,"
			    + Calls.DATA_ID + " INTEGER DEFAULT NULL,"
				+ WhiteColumns.NAME + " TEXT,"
				+ WhiteColumns.NUMBER + " TEXT)" + ";");
		
		db.execSQL("CREATE UNIQUE INDEX white_number_index ON " + Tables.WHITE + " (" +
				WhiteColumns.NUMBER +
				");");
		
		String blackViewSelect = "select number , 1 as type from black union all select number, 0 as type from white";

		db.execSQL("CREATE VIEW " + Views.HB_BLACK_VIEW + " AS " + blackViewSelect);
	}

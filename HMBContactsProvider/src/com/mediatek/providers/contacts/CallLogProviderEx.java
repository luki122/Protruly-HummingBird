...
import com.hb.csp.contactsprovider.DialerSearchSupport.DialerSearchLookupColumns;
import com.hb.csp.contactsprovider.DialerSearchSupport.DialerSearchLookupType;
...
public class CallLogProviderEx {
	...
	static {
		...
		sCallsJoinDataViewProjectionMap.put("sub_id", "sub_id");
	}
...
//					+ Calls.RAW_CONTACT_ID + ">0) ");
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		try {
			...
			if (rowId > 0 && ContactsProviderUtils.isSearchDbSupport()) {
				if(mDialerSearchSupport!=null) mDialerSearchSupport.handleCallLogsInserted(db, rowId, strInsNumber);
			}
	...
	public int delete(Uri uri, String selection,
		...
		try {
			...
			if (count > 0 && ContactsProviderUtils.isSearchDbSupport()) {
				if(mDialerSearchSupport!=null) mDialerSearchSupport.handleCallLogsDeleted(db);
			}
	...
	public static final void notifyNewCallsCount(Context context) {
		...
		//send count=0 to clear the unread icon 
		        if (newCallsCount >= 0) {
		            ...
		            Log.d(TAG,"intent:"+newIntent+" action:"+newIntent.getAction()+" extra:"+newIntent.getExtras());
	...
	public void handleContactsDataInserted (SQLiteDatabase db,
		...
		try {
			...
			case ImsCall.CONTENT_ITEM_TYPE:
				...
				if (count > 0) {
					if(mDialerSearchSupport!=null) mDialerSearchSupport.handleCallLogsUpdated(db, false);
				}
				break;
		...
		try {
			...
			case ImsCall.CONTENT_ITEM_TYPE:
				...
				if (cntDelete > 0 || cntInsert > 0) {
					if(mDialerSearchSupport!=null) mDialerSearchSupport.handleCallLogsUpdated(db, (cntDelete > 0 ? true : false));
				}
				break;
	...
	public void handleContactsDataDeleted (SQLiteDatabase db) {
		try {
			...
			if(mDialerSearchSupport!=null) mDialerSearchSupport.handleCallLogsUpdated(db, true);
			}
		} catch (SQLiteException e) {
	...
	private int updateCallsWithNewCallable(SQLiteDatabase db,
		...
		//add by lgy 
		updateBlackWithNewCallable(db, rawContactId, dataId, callable);
		updateWhiteWithNewCallable(db, rawContactId, dataId, callable);
...
//	public int hbUpdateCallsName(SQLiteDatabase db,
...
//		int count = updateCallsWithNewCallable.executeUpdateDelete();
//		updateCallsWithNewCallable.close();
//		Log.d(TAG, "hbUpdateCallsName:" + updateCallsWithNewCallable+" name="+name);
//		return count;
//	}
	...
	private int updateOutdatedDataIdCalls(SQLiteDatabase db, String where) {
		//add by lgy 
		updateOutdatedDataIdBlack(db, where);
		updateOutdatedDataIdWhite(db, where);
	...
	//add by lgy
	private int updateBlackWithNewCallable(SQLiteDatabase db,
			long rawContactId, long dataId, String callable) {
		
		SQLiteStatement updateBlackWithNewCallable = db.compileStatement(
				" UPDATE " + Tables.BLACK
				+ " SET " + Calls.RAW_CONTACT_ID + "=?, "
				+ Calls.DATA_ID + "=?, "
				//+ "black_name =? "
		...
		updateBlackWithNewCallable.bindLong(1, rawContactId);
		updateBlackWithNewCallable.bindLong(2, dataId);
		int count = updateBlackWithNewCallable.executeUpdateDelete();
		updateBlackWithNewCallable.close();
		Log.d(TAG, "updateBlackWithNewCallable:" + count);
		return count;
	}
	
	private int updateWhiteWithNewCallable(SQLiteDatabase db,
			long rawContactId, long dataId, String callable) {
		
		SQLiteStatement updateWhiteWithNewCallable = db.compileStatement(
				" UPDATE " + Tables.WHITE
				+ " SET " + Calls.RAW_CONTACT_ID + "=?, "
				+ Calls.DATA_ID + "=?  "
				+ " WHERE " + buildCallLogSameNumberFilter(callable)
		...
		updateWhiteWithNewCallable.bindLong(1, rawContactId);
		updateWhiteWithNewCallable.bindLong(2, dataId);
		int count = updateWhiteWithNewCallable.executeUpdateDelete();
		updateWhiteWithNewCallable.close();
		Log.d(TAG, "updateWhiteWithNewCallable:" + count);
		return count;
	}
	
	
	private int updateOutdatedDataIdBlack(SQLiteDatabase db, String where) {
		SQLiteStatement updateCallsWithOldDataId = db.compileStatement(" UPDATE " + Tables.BLACK
				+ " SET " + Calls.DATA_ID + "=(SELECT " + Data._ID + " FROM " + Tables.DATA
				+ " WHERE " + Data.RAW_CONTACT_ID + " IN " + " (SELECT " + RawContacts._ID
				+ " FROM " + Tables.RAW_CONTACTS + " WHERE " + RawContacts.DELETED + "=0)"
				+ " AND ((" + mDbHelper.getMimeTypeId(Phone.CONTENT_ITEM_TYPE) + "="
				+ DataColumns.CONCRETE_MIMETYPE_ID + " AND PHONE_NUMBERS_EQUAL(" + Data.DATA1
				+ ", " + Tables.BLACK + "." + Calls.NUMBER + ")" + " ) OR ("
				+ DataColumns.CONCRETE_MIMETYPE_ID + " IN " + getUriCallableMimetypeIdsString()
				+ " AND " + Data.DATA1 + "=" + Tables.BLACK + "." + Calls.NUMBER + "))"
				+ " LIMIT 1), " + Calls.RAW_CONTACT_ID + "=0 " + " WHERE " + where);
		...
		if (count > 0) {
			SQLiteStatement updateRawContactId = db.compileStatement(" UPDATE " + Tables.BLACK
					+ " SET " + Calls.RAW_CONTACT_ID + "=(SELECT " + Data.RAW_CONTACT_ID + " FROM "
					+ Tables.DATA + " WHERE " + Data._ID + " = " + Tables.BLACK + "."
					+ Calls.DATA_ID + ")" + " WHERE " + Calls.DATA_ID + ">0 AND "
					+ Calls.RAW_CONTACT_ID + "=0");
	...
	private int updateOutdatedDataIdWhite(SQLiteDatabase db, String where) {
		SQLiteStatement updateCallsWithOldDataId = db.compileStatement(" UPDATE " + Tables.WHITE
				+ " SET " + Calls.DATA_ID + "=(SELECT " + Data._ID + " FROM " + Tables.DATA
				+ " WHERE " + Data.RAW_CONTACT_ID + " IN " + " (SELECT " + RawContacts._ID
				+ " FROM " + Tables.RAW_CONTACTS + " WHERE " + RawContacts.DELETED + "=0)"
				+ " AND ((" + mDbHelper.getMimeTypeId(Phone.CONTENT_ITEM_TYPE) + "="
				+ DataColumns.CONCRETE_MIMETYPE_ID + " AND PHONE_NUMBERS_EQUAL(" + Data.DATA1
				+ ", " + Tables.WHITE + "." + Calls.NUMBER + ")" + " ) OR ("
				+ DataColumns.CONCRETE_MIMETYPE_ID + " IN " + getUriCallableMimetypeIdsString()
				+ " AND " + Data.DATA1 + "=" + Tables.WHITE + "." + Calls.NUMBER + "))"
				+ " LIMIT 1), " + Calls.RAW_CONTACT_ID + "=0 " + " WHERE " + where);
		...
		if (count > 0) {
			SQLiteStatement updateRawContactId = db.compileStatement(" UPDATE " + Tables.WHITE
					+ " SET " + Calls.RAW_CONTACT_ID + "=(SELECT " + Data.RAW_CONTACT_ID + " FROM "
					+ Tables.DATA + " WHERE " + Data._ID + " = " + Tables.WHITE + "."
					+ Calls.DATA_ID + ")" + " WHERE " + Calls.DATA_ID + ">0 AND "
					+ Calls.RAW_CONTACT_ID + "=0");

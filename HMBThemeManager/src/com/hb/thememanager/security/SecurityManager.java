...
public class SecurityManager {
	...
	public static void startPay(Theme theme, Context context, Pay.PayResulteListener resultListener){
		if(!CommonUtil.hasNetwork(context)){
			ToastUtils.showShortToast(context,R.string.pay_when_no_network);
			return;
		}
		User user = User.getInstance(context);

		if(user.isLogin() && !TextUtils.isEmpty(user.getId())) {
			PayParams params = new PayParams.Builder(context).count(1)
					.description(theme.name)
					.designer(theme.designerId)
					.price((int) (Float.parseFloat(theme.price) * 100))
					.title(theme.name)
					.waresId(theme.id)
					.activity((Activity) context)
					.waresType(theme.type)
					.mobile(user.getPhone())
					.qlcId(Long.parseLong(user.getId()))
					.build();
			PayDialog dialog = new PayDialog(context, params);
			dialog.setPayResultListener(resultListener);
			dialog.show();
		}else{
			user.jumpLogin(null,(Activity)context);
		}
	}

...
public class LocalWallpaperAdapter extends AbsBaseAdapter<Wallpaper> {
...
//			applyStatus  = (ImageView)convertView.findViewById(R.id.wallpaper_item_applied);
		...
		public void bindDatas(int position, List<Wallpaper> themes) {
			...
			if(!TextUtils.isEmpty(wallpaperFileUrl)){
				getAdapter().getImageLoader().loadImage(wallpaperFileUrl, wallpaperImg);

			}
			boolean showEditBox = !getAdapter().getItem(position).isSystemTheme() && getAdapter().isEditMode();
			editCheckBox.setVisibility(showEditBox?View.VISIBLE:View.GONE);
		//	Log.d("selected","position->"+position+" selected->"+getAdapter().isSelected(position));
			editCheckBox.setChecked(getAdapter().isSelected(position));
//			applyStatus.setVisibility(adapter.themeApplied(wallpaper)?View.VISIBLE:View.GONE);
		}

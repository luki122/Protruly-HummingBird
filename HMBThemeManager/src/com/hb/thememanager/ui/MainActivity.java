...
import com.hb.thememanager.ui.fragment.FeedbackFragment;
import com.hb.thememanager.ui.fragment.SettingFragment;
import com.hb.thememanager.ui.fragment.AboutFragment;
...
public class MainActivity extends HbActivity implements FragmentManager.OnBackStackChangedListener{
     ...
     */
    public static final String EXTRA_THIRD_PART_FRAGMENT_ARGUMENTS = "theme_component";
	...
	public static final String[] ENTRY_FRAGMENTS = {
			LocalThemePkgListFragment.class.getName(),
			...
			LocalThemeDetailFragment.class.getName(),
			LocalThemeFontsListFragment.class.getName(),
			FontPkgDetailFragment.class.getName(),
			LocalFontDetailFragment.class.getName(),
			LocalRingtoneListFragment.class.getName(),
			LocalWallpaperListFragment.class.getName(),
	...
	private void switchPage(Intent intent){
		...
		if(LocalThemeDetailFragment.class.getName().equals(initialFragmentName)){
			int flag = getWindow().getDecorView().getSystemUiVisibility();
			getWindow().getDecorView().setSystemUiVisibility(flag|View.SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR_ADD_LINE);
		}
        ...
        if(!TextUtils.isEmpty(targetComponent)){
        	if(targetComponent.equals(Config.ThemeComponent.WALLPAPER)){
        		...
        		switchToFragment(LocalThemePkgListFragment.class.getName(), null, false, false, R.string.local_theme, null, false);
        	}else if(targetComponent.equals(Config.ThemeComponent.FEEDBACK)){
        		switchToFragment(FeedbackFragment.class.getName(), null, false, false, R.string.text_user_feedback, null, false);
        	}else if(targetComponent.equals(Config.ThemeComponent.SETTING)){
        		switchToFragment(SettingFragment.class.getName(), null, false, false, R.string.action_settings, null, false);
        	}
        }else{
	 ...
	 private void setTitleFromIntent(Intent intent) {
	        ...
	        if (initialTitleResId > 0) {
	            ...
	            if (initialTitleResPackageName != null) {
	                ...
	                setTitle(getString(mInitialTitleResId));
	            }
	        } else {
	            mInitialTitleResId = -1;
    ...
    public Fragment switchToFragment(String fragmentName, int contentViewId,Bundle args, boolean validate,
		...
		if(f instanceof FragmentKeyListener) {
			setFragmentKeyListener((FragmentKeyListener)f);
		}

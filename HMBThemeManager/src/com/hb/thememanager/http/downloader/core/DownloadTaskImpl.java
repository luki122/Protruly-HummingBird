...
import com.hb.thememanager.http.downloader.core.task.GetFileInfoTask;
import com.hb.thememanager.http.downloader.core.thread.DownloadThread;
...
//  private volatile boolean isComputerDownload;
  ...
  public DownloadTaskImpl(ExecutorService executorService, DownloadResponse downloadResponse,
                          DownloadInfo downloadInfo, DownloadConfig downloadConfig, DownloadTaskListener downloadTaskListener) {
  ...
  public void onSuccess(long size, boolean isSupportRanges) {
    ...
    if (isSupportRanges) {
      ...
      final int threads = downloadConfig.getEachDownloadThread();
    ...
    if (!isComputerDownload.get()) {
      synchronized (this) {
        if (!isComputerDownload.get()) {
          ...
          if ((currentTimeMillis - lastRefreshTime) > 60) {

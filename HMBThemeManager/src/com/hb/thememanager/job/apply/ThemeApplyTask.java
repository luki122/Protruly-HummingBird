...
public class ThemeApplyTask  extends IntentService{
	...
	private void applyWallpaperTheme(){
		...
		if(!TextUtils.isEmpty(wallpaperFilePath)){
			sStateManager.postState(ThemeState.STATE_START_APPLY);
			...
			sStateManager.postState(success?ThemeState.STATE_APPLIED:ThemeState.STATE_FAIL);
		}
	}
	...
	private void applyFontTheme(){
		final Theme font = mTheme;
		final String fontFilePath = font.themeFilePath;
		android.util.Log.e("cai","applyFontTheme : fontFilePath = "+fontFilePath);
		if(!TextUtils.isEmpty(fontFilePath)){
			sStateManager.postState(ThemeState.STATE_START_APPLY);
			HbFontsManager.getInstance().setFonts(fontFilePath, new HbFontsManager.OnFontsSetListener() {
				...
				public void onSetFinish(boolean success) {
					...
					if(success){
						SharePreferenceManager.setStringPreference(mContext,
								SharePreferenceManager.KEY_APPLIED_FONT_ID, font.id);
						if(needBackup()){
							SharePreferenceManager.setStringPreference(mContext,
									SharePreferenceManager.KEY_APPLIED_NORMAL_FONT_ID, font.id);
						}

					}
					sStateManager.postState(success?ThemeState.STATE_APPLY_SUCCESS:ThemeState.STATE_FAIL);
	...
	private void applyPkgTheme(){
		if(mTheme != null){
			...
			if(mTheme.isDefaultTheme()){
				...
				SharePreferenceManager.setStringPreference(mContext, SharePreferenceManager.KEY_APPLIED_THEME_ID, mTheme.id);
			...
			if(TextUtils.isEmpty(mTheme.themeFilePath)){
				sStateManager.postState(ThemeState.STATE_FILE_NOT_EXISTS);
				TLog.e(TAG,"apply theme:mTheme.themeFilePath is empty!!!");
				return;
			}
			...
			if(!themeFile.exists()){
				sStateManager.postState(ThemeState.STATE_FILE_NOT_EXISTS);
				TLog.e(TAG,"apply theme:target theme file is not exists!!!");
				return;
			}
			//准备开始应用
			startApply();
			if(!mTheme.isPaid()){
				tryThisTheme(themeFile);
				return;
			}
			extractThemeFile(themeFile);
			//应用结束
			endApply();
		}else{
			sStateManager.postState(ThemeState.STATE_FILE_NOT_EXISTS);
	...
	private boolean needBackup(){
		return  mTheme.isPaid() || mTheme.isFree() || mTheme.isSystemTheme();
	}


	private void extractThemeFile(File themeFile){
		try {
			...
			SharePreferenceManager.setStringPreference(mContext, SharePreferenceManager.KEY_APPLIED_THEME_ID, mTheme.id);
			if(needBackup()) {
				SharePreferenceManager.setStringPreference(mContext, SharePreferenceManager.KEY_APPLIED_NORMAL_THEME_ID, mTheme.id);
			}
		} catch (Exception e) {
			sStateManager.postState(ThemeState.STATE_FAIL);
	...
	private void tryThisTheme(File themeFile){
		extractThemeFile(themeFile);
		endApply();
	}
	...
	private void endApply(){
		...
		TLog.d(TAG,"apply theme finished->  id:"+mTheme.id+"  name:"+mTheme.name);
		try{
		...
		if(mEndTime - mStartTime < MAX_TIME_LIMIT){
			 mHandler.postDelayed(new Runnable() {
				...
				public void run() {
					...
					sStateManager.postState(ThemeState.STATE_APPLY_SUCCESS);
					startLauncher();
				}
			}, MAX_TIME_LIMIT);
		}else{
	...
	private void startLauncher(){
		if(mTheme.type == Theme.THEME_PKG) {

...
import com.hb.thememanager.job.ImageLoaderModule;
import com.hb.imageloader.RecyclingImageView;
import com.hb.thememanager.utils.Config;
public class ImageLoader  {

	//默认配置
    public static ImageLoaderConfig sDefaultConfig = new ImageLoaderConfig.Builder().
     ...
     * @param listener
     */
    public static void loadStringRes(ImageView view, String imageUrl, ImageLoaderConfig config, ImageLoaderListener listener) {
        load(view.getContext(), view, imageUrl, config, listener);
    }

    public static void loadFile(ImageView view, File file, ImageLoaderConfig config, ImageLoaderListener listener) {
        load(view.getContext(), view, file, config, listener);
    }

    public static void loadResId(ImageView view, Integer resourceId, ImageLoaderConfig config, ImageLoaderListener listener) {
        load(view.getContext(), view, resourceId, config, listener);
    }

    public static void loadUri(ImageView view, Uri uri, ImageLoaderConfig config, ImageLoaderListener listener) {
        load(view.getContext(), view, uri, config, listener);
    }

    public static void loadGif(ImageView view, String gifUrl, ImageLoaderConfig config, ImageLoaderListener listener) {
        load(view.getContext(), view, gifUrl, ImageLoaderConfig.parseBuilder(config).setAsGif(true).build(), listener);
    }

    public static void loadTarget(Context context, Object objUrl, ImageLoaderConfig config, final ImageLoaderListener listener) {
        load(context, null, objUrl, config, listener);
    }

    private static void load(Context context, ImageView view, Object objUrl, ImageLoaderConfig config, final ImageLoaderListener listener) {
        if (null == objUrl) {
...
//                    request.transform(new RotateTransformation(context, config.getRotateDegree()));
                ...
                if(config.getFormat() != null){
                	request.format(config.getFormat());
                }
                builder = request;
...
//            builder.dontAnimate();
            ...
            if (null != listener) {
                setListener(view,builder, listener);
            }
    ...
    private static void setListener(final View view ,GenericRequestBuilder request, final ImageLoaderListener listener) {
        request.listener(new RequestListener() {
            @Override
            public boolean onException(Exception e, Object model, Target target, boolean isFirstResource) {
                if (e.getMessage() != null && !e.getMessage().equals("divide by zero")) {
                    listener.onError();
                }
                return false;
            }
    ...
    private static void setTargetView(GenericRequestBuilder request, ImageLoaderConfig config, ImageView view) {
        //set targetView
     ...
     * @param listener
     */
    public static void loadBitmap(Context context, Object url, final BitmapLoaderListener listener) {
        if (url == null) {
            if (listener != null) {
                listener.onError();
            }
     ...
     * @param listener
     */
    public static void loadImageWithHighPriority(Object url, ImageView imageView, final ImageLoaderListener listener) {
        if (url == null) {
            if (listener != null) {
                listener.onError();
            }
    ...
    public static synchronized long getDiskCacheSize(Context context) {
        long size = 0L;
        File cacheDir = Config.getDiskCacheDir(Config.IMAGE_LOAD_CACHE);

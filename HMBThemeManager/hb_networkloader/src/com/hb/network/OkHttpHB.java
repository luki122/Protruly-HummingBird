...
import com.hb.network.body.ProgressRequestBody;
import com.hb.network.body.ResponseProgressBody;
import com.hb.network.response.DownloadResponseHandler;
import com.hb.network.response.GsonResponseHandler;
import com.hb.network.response.IResponseHandler;
import com.hb.network.response.JsonResponseHandler;
import com.hb.network.response.RawResponseHandler;
 ...
 * 依赖okhttp3，okio，gson
 */
public class OkHttpHB {

    private OkHttpClient mClient;
    private static OkHttpHB instance;

    private OkHttpHB(Context context) {
     ...
     * @return 获取实例
     */
    public static OkHttpHB getOkHttpHB(Context context) {
        if(instance == null) {
        	synchronized (OkHttpHB.class) {
    			if(instance == null) {
    	            instance = new OkHttpHB(context);
    			}
    		}
        }
        return instance;
    ...
    public void post(Context context, final String url, final Map<String, String> params, final IResponseHandler responseHandler) {
        ...
        mClient.newCall(request).enqueue(new MyCallback(new Handler(), responseHandler));
    }

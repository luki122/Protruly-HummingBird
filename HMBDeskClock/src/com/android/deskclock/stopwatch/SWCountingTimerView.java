 ...
 */
public class SWCountingTimerView extends View {
    ...
    private static final float HOURS_MINUTES_SPACING = 0.1f/*0.4f*/;
    // Ratio of the space leading the Hundredths
    private static final float HUNDREDTHS_SPACING = 0.f/*0.5f*/;
    ...
    private UnsignedTime mSplitColcon;
    private String splitColonStr = ":";
    ...
    Runnable mBlinkThread = new Runnable() {
        ...
        public void run() {
            mVisible = !mVisible;
            SWCountingTimerView.this.showTime(mVisible);
    ...
    public SWCountingTimerView(Context context) {
        this(context, null);
    }

    @SuppressWarnings("PMD")
    public SWCountingTimerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mAccessibilityManager =
                (AccessibilityManager) context.getSystemService(Context.ACCESSIBILITY_SERVICE);
        Resources r = context.getResources();
        mDefaultColor = mWhiteColor = r.getColor(R.color.stopwatch_text_color);
        mSecondTextColor = r.getColor(R.color.timer_cricle_second_text_color, null);
        mPressedColor = mAccentColor = r.getColor(R.color.stopwatch_pressed_text_color);
        mBigFontSize = r.getDimension(R.dimen.timer_circle_text_size);
        mSmallFontSize = r.getDimension(R.dimen.timer_circle_second_text_size);
        ...
        setTextColor(mDefaultColor, mSecondTextColor);

        // allDigits will contain ten digits: "0123456789" in the default locale
        final String allDigits = String.format("%010d", 123456789);
        mBigSeconds = new UnsignedTime(mPaintBigThin, HOURS_MINUTES_SPACING, allDigits);
        ...
        mSplitColcon = new UnsignedTime(mPaintBigThin, 0.2f, splitColonStr);
    ...
    protected void setTextColor(int textColor, int secondTextColor) {
        mPaintBigThin.setColor(textColor);
        mPaintMed.setColor(secondTextColor);
    }
    ...
    private void calcTotalTextWidth() {
        ...
        mTotalTextWidth += mSplitColcon.calcTotalWidth(splitColonStr);
    }
    ...
    /**
     * Calculate the square of the diameter to use in {@link SWCountingTimerView#setTotalTextWidth()}
     */
...
//        }
    ...
    public void onDraw(Canvas canvas) {
        ...
        mPaintBigThin.setColor(textColor);
        final int secondTextColor = mVirtualButtonPressedOn ? mPressedColor : mSecondTextColor;
        mPaintMed.setColor(secondTextColor);

        if (mHours != null) {
            xTextStart = mBigHours.draw(canvas, mHours, xTextStart, yTextStart);
            xTextStart = mSplitColcon.draw(canvas, splitColonStr, xTextStart, yTextStart);
        }
        if (mMinutes != null) {

...
public final class Timer {
    ...
    public static final long MAX_LENGTH = 24 * HOUR_IN_MILLIS;
    ...
    Timer elapse(long millis) {
        if (mState != PAUSED) {
            return null;
        }

        final long remainingTime = mRemainingTime - millis;
        if (remainingTime > 0) {
            return new Timer(mId, PAUSED, mLength, mTotalLength, Long.MIN_VALUE, remainingTime,
                    mLabel, mDeleteAfterUse);
        } else if (remainingTime > -HandleDeskClockApiCalls.RESET_EXPIRED_TIMER_DELAY_MILLIS) {
            return new Timer(mId, RUNNING, mLength, mTotalLength, now(), remainingTime, mLabel,
                    mDeleteAfterUse);
        } else {
            return new Timer(mId, EXPIRED, mLength, mTotalLength, now(), remainingTime, mLabel,
                    mDeleteAfterUse);
        }
    }

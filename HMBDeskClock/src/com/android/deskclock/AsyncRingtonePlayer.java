...
public class AsyncRingtonePlayer {
    ...
    private static final String INCALL_STATUS = "INCALL_STATUS";
    ...
    /** Plays the ringtone. */
    public void play(Uri ringtoneUri, boolean inCall) {
        LogUtils.d(TAG, "Posting play.");
        postMessage(EVENT_PLAY, ringtoneUri , inCall);
    }

    /** Stops playing the ringtone. */
    public void stop() {
        LogUtils.d(TAG, "Posting stop.");
        postMessage(EVENT_STOP, null, true);
    }
     ...
     */
    private void postMessage(int messageCode, Uri ringtoneUri, boolean inCall) {
        synchronized (this) {
            if (mHandler == null) {
            ...
            if (ringtoneUri != null) {
                ...
                bundle.putBoolean(INCALL_STATUS, inCall);
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    case EVENT_PLAY:
                        ...
                        final boolean inCall =  msg.getData().getBoolean(INCALL_STATUS);
                        getPlaybackDelegate().play(mContext, ringtoneUri, inCall);
                        break;
                    case EVENT_STOP:
    ...
    private interface PlaybackDelegate {
        void play(Context context, Uri ringtoneUri, boolean inCall);
    ...
    private static class MediaPlayerPlaybackDelegate implements PlaybackDelegate {
         ...
         */
        @Override
        public void play(final Context context, Uri ringtoneUri, boolean inCall) {
    ...
    private static class RingtonePlaybackDelegate implements PlaybackDelegate {
        ...
        public void play(Context context, Uri ringtoneUri, boolean inCall) {
            ...
            if (mRingtone == null
                   || (ringtoneUri != null && !AlarmClockFragment.isRingtoneExisted(context, ringtoneUri.toString()))) {

    ...
    private final BroadcastReceiver mStopPlayReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(POWER_OFF_ALARM_SNOOZE_ACITION)) {
				boolean isPoweroffMissed = intent.getBooleanExtra(AlarmStateManager.IS_POWEROFF_ALARM_MISSED, false);
//				if (count >= AlarmStateManager.POWEROFF_ALARM_MISSED_MAX_COUNT) {
//					AlarmStateManager.setPoweroffAlarmMissedCount(context, 0);
//					AlarmStateManager.setDismissState(context, mCurrentAlarm);
//					PowerOffAlarm.deleteRingtone(context, mCurrentAlarm);
//					shutDown(context);
//				} else {
//					AlarmStateManager.setPoweroffAlarmMissedCount(context, count);
...
//				}
                if (!isPoweroffMissed) {
                    mCurrentAlarm.mSnoozeTime = 0;
                }

                if (mCurrentAlarm.mSnoozeTime == 4) {
//                    AlarmStateManager.setDismissState(context, mCurrentAlarm);
                } else {
                    AlarmStateManager.setSnoozeState(context, mCurrentAlarm, false);
                }
    ...
    private void startAlarmKlaxon(AlarmInstance instance) {
        ...
        if (!PowerOffAlarm.bootFromPoweroffAlarm()) {
            ...
            if (inCall) {
                ...
                AlarmNotifications.updateAlarmNotification(getApplicationContext(), mCurrentAlarm);
            } else {
                AlarmNotifications.showAlarmNotification(getApplicationContext(), mCurrentAlarm);
            }
        } /// @}
        /**

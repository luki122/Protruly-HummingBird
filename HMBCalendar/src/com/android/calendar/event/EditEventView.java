...
import com.android.calendar.hb.EventLabelActivity;
import com.android.calendar.hb.EventRepeatActivity;
...
import hb.app.dialog.DateTimeDialog;
import hb.app.dialog.DateTimeDialog.OnDateTimeSetListener;
import hb.widget.DateTimePicker;
...
public class EditEventView implements View.OnClickListener, DialogInterface.OnCancelListener,
    ...
    TextView mEventEnd;
    HbSpinner mEventReminder;
    HbSpinner mEventRepeat;
    ...
    private List<Integer> mEventRepeatIndexs = new ArrayList<>();
    private List<String> mEventRepeatLabels = new ArrayList<>();

    private List<String> mDescriptionValues;
    private List<String> mDescriptionLabels;

    private ArrayList<Integer> mEventReminderValues;
    private ArrayList<String> mEventReminderLabels;
    ...
    private class DateTimeListener implements OnDateTimeSetListener {
        ...
        public void onDateTimeSet(DateTimePicker view, int year, int month, int monthDay,
            ...
            if (!CalendarUtils.isYearInRange(year)) {
                CalendarUtils.showToast(mActivity);
                return;
            }
            ...
            setDateTime(mEventBegin, startMillis);
            setDateTime(mEventEnd, endMillis);
        }
    }
    ...
    private void populateWhen() {
        ...
        mEventBegin.setOnClickListener(new DateTimeClickListener(mStartTime));
        mEventEnd.setOnClickListener(new DateTimeClickListener(mEndTime));
    }
    ...
    private void populateRepeats() {
        ...
        updateEventRepeat(repeatString);
    }
    ...
    public boolean isContentEmpty() {
        return isTitleEmpty() &&
                TextUtils.isEmpty(mDescriptionTextView.getText().toString().trim()) &&
                TextUtils.isEmpty(mRrule);
    }
    ...
    public void onClick(View view) {
        if (view == mRruleButton) {
            ...
            return;
        } else if (view == mEventLabel) {
            Intent intent = new Intent();
            intent.setClass(mActivity, EventLabelActivity.class);
            intent.putExtra(EventLabelActivity.KEY_EVENT_DESCRIPTION,
                    mDescriptionTextView.getText().toString());
            mActivity.startActivityForResult(intent,
                    EventLabelActivity.EVENT_LABEL_REQUEST_CODE);
            return;
        }
    ...
    public EditEventView(Activity activity, View view, EditDoneRunnable done,
        ...
        mEventBegin = (TextView) view.findViewById(R.id.event_begin);
        mEventEnd = (TextView) view.findViewById(R.id.event_end);
        mEventReminder = (HbSpinner) view.findViewById(R.id.event_reminder);
        mEventRepeat = (HbSpinner) view.findViewById(R.id.event_repeat);
        mEventLabel = (TextView) view.findViewById(R.id.event_label);
    ...
    public void setModel(CalendarEventModel model) {
        ...
        updateEventReminder();
        updateEventLabel(model.mDescription);
    ...
    private void prepareEventResources() {
        Resources res = mActivity.getResources();

        mEventReminderValues = loadIntegerArray(res, R.array.hb_reminder_minutes_values);
        mEventReminderLabels = loadStringArray(res, R.array.hb_reminder_minutes_labels);

        mDescriptionValues = Utils.loadDescriptionValues(res);
        mDescriptionLabels = Utils.loadDescriptionLabels(res);

        if (!mEventRepeatIndexs.isEmpty()) {
            mEventRepeatIndexs.clear();
        }
        if (!mEventRepeatLabels.isEmpty()) {
            mEventRepeatLabels.clear();
        }

        mEventRepeatIndexs.add(EditEventHelper.DOES_NOT_REPEAT);
        mEventRepeatLabels.add(res.getString(R.string.event_no_repeat));

        mEventRepeatIndexs.add(EditEventHelper.REPEATS_DAILY);
        mEventRepeatLabels.add(res.getString(R.string.event_daily));

        mEventRepeatIndexs.add(EditEventHelper.REPEATS_WEEKLY_ON_DAY);
        mEventRepeatLabels.add(res.getString(R.string.event_weekly));

        mEventRepeatIndexs.add(EditEventHelper.REPEATS_MONTHLY_ON_DAY);
        mEventRepeatLabels.add(res.getString(R.string.event_monthly));

        mEventRepeatIndexs.add(EditEventHelper.REPEATS_YEARLY);
        mEventRepeatLabels.add(res.getString(R.string.event_yearly));
    }

    private void updateEventReminder() {
        int index = 0;
        if (mModel.mReminders.size() > 0) {
            ReminderEntry reminder = mModel.mReminders.get(0);
            int minutes = reminder.getMinutes();
            EventViewUtils.addMinutesToList(mActivity, mEventReminderValues,
                    mEventReminderLabels, minutes);
            index = EventViewUtils.findMinutesInReminderList(mEventReminderValues, minutes);
        }

        final ArrayAdapter<String> adapter = new ArrayAdapter<String>(
                mActivity, R.layout.hb_edit_event_spinner_label, mEventReminderLabels);
        adapter.setDropDownViewResource(R.layout.hb_edit_event_spinner_item);

        mEventReminder.setAdapter(adapter);
        mEventReminder.setOnItemSelectedListener(new OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (hasAlarm) {
                    if (position == 0) {
                        ...
                        int minutes = mEventReminderValues.get(position);
                        int index = EventViewUtils.findMinutesInReminderList(mReminderMinuteValues, minutes);

                        LinearLayout reminderItem = mReminderItems.get(0);
        ...
        mEventReminder.setSelection(index);
    }

    private void updateEventRepeat(String repeatString) {
        int repeatIndex = EditEventHelper.DOES_NOT_REPEAT;
        if (!TextUtils.isEmpty(mRrule)) {
            repeatIndex = EventRepeatActivity.getRepeatIndex(mEventRecurrence);
            if (mEventRepeatIndexs.indexOf(repeatIndex) == -1) {
                mEventRepeatIndexs.add(repeatIndex);
                mEventRepeatLabels.add(repeatString);
            }
        }

        final ArrayAdapter<String> adapter = new ArrayAdapter<String>(
                mActivity, R.layout.hb_edit_event_spinner_label, mEventRepeatLabels);
        adapter.setDropDownViewResource(R.layout.hb_edit_event_spinner_item);

        mEventRepeat.setAdapter(adapter);
        mEventRepeat.setOnItemSelectedListener(new OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                int repeatIndex = mEventRepeatIndexs.get(position);
                String rruleString = EventRepeatActivity.getRecurrenceRule(
                        repeatIndex, mStartTime.toMillis(false), mStartTime.timezone,
                        Utils.getFirstDayOfWeekAsCalendar(mActivity));
                onRecurrenceSet(rruleString);
            }
        ...
        mEventRepeat.setSelection(mEventRepeatIndexs.indexOf(repeatIndex));
    }

    private void updateEventLabel(String eventDescription) {
        setEventLabel(eventDescription);

        mEventLabel.setOnClickListener(this);
    }

    void onEventDescriptionSet(String eventDescription) {
        setEventLabel(eventDescription);

        mDescriptionTextView.setText(eventDescription);
    }

    private void setEventLabel(String eventDescription) {
        if (!TextUtils.isEmpty(eventDescription)) {
            int index = mDescriptionValues.indexOf(eventDescription);
            if (index != -1) {
                mEventLabel.setText(mDescriptionLabels.get(index));
            } else {
                mEventLabel.setText(eventDescription);
            }
        } else {
            mEventLabel.setText(R.string.label_none);
        }
    }
    ...
    private void setViewStates(int mode) {
        ...
        if (mode == Utils.MODIFY_UNINITIALIZED || !EditEventHelper.canModifyEvent(mModel)) {
            ...
            mDescriptionGroup.setVisibility(View.GONE/*VISIBLE*/);

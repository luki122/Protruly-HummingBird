...
public class EventInfoFragment extends DialogFragment implements OnCheckedChangeListener,
    ...
    private TextView mEventReminder;
    ...
    public void onActionClick(View view) {
        ...
        if (viewId == R.id.action_cancel) {
            mActivity.onBackPressed();
        } else if (viewId == R.id.info_action_edit) {
            doEdit();
            mActivity.finish();
        } else if (viewId == R.id.info_action_delete) {
            mDeleteHelper =
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (mWindowStyle == DIALOG_WINDOW_STYLE) {
            ...
            mView = inflater.inflate(R.layout.hb_event_info_fragment, container, false);
        }
        ...
        mEventBegin = (TextView) mView.findViewById(R.id.event_begin);
        mEventEnd = (TextView) mView.findViewById(R.id.event_end);
        mEventReminder = (TextView) mView.findViewById(R.id.event_reminder);
        mEventLabel = (TextView) mView.findViewById(R.id.event_label);
    ...
    private void updateTime() {
        String eventBegin = DateUtils.formatDateTime(mContext, mStartMillis,
                DateUtils.FORMAT_SHOW_DATE
                | DateUtils.FORMAT_SHOW_YEAR
                | DateUtils.FORMAT_ABBREV_MONTH
                | DateUtils.FORMAT_SHOW_TIME);
        String eventEnd = DateUtils.formatDateTime(mContext, mEndMillis,
                DateUtils.FORMAT_SHOW_DATE
                | DateUtils.FORMAT_SHOW_YEAR
                | DateUtils.FORMAT_ABBREV_MONTH
                | DateUtils.FORMAT_SHOW_TIME);

        mEventBegin.setText(eventBegin);
        mEventEnd.setText(eventEnd);
    }

    private void updateEventReminder() {
        if (mHasAlarm && !mOriginalReminders.isEmpty()) {
            ReminderEntry reminder = mOriginalReminders.get(0);
            int index = EventViewUtils.findMinutesInReminderList(
                    mReminderMinuteValues, reminder.getMinutes());
            mEventReminder.setText(mReminderMinuteLabels.get(index));
        }
    }

    private void updateEventLabel(String eventDescription) {
        if (!TextUtils.isEmpty(eventDescription)) {
            List<String> descriptionValues = Utils.loadDescriptionValues(mActivity.getResources());
            List<String> descriptionLabels = Utils.loadDescriptionLabels(mActivity.getResources());

            int index = descriptionValues.indexOf(eventDescription);
            if (index != -1) {
                mEventLabel.setText(descriptionLabels.get(index));
            } else {

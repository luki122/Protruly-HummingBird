...
public class LunarUtils {
    ...
    private final String mLunarFestCHUXI;
    ...
    public String getLunarFestivalChineseString(int gregorianYear, int gregorianMonth, int gregorianDay) {
        ...
        int lunarDate[] = calculateLunarByGregorian(gregorianYear, gregorianMonth, gregorianDay);
        if (lunarDate[1] == 12 && lunarDate[2] >= 29) {
            Time time = new Time();
            time.set(gregorianDay + 1, gregorianMonth - 1, gregorianYear);
            time.normalize(true);

            int tempLunarDate[] = calculateLunarByGregorian(time.year, time.month + 1, time.monthDay);
            if (mLunarFestCHUNJIE.equals(
                    getLunarFestival(tempLunarDate[1], tempLunarDate[2], tempLunarDate[3]))) {
                return mLunarFestCHUXI;
            }
        }
        ...
        if (chineseStringBuilder.length() == 0) {
            boolean isLeapMonth = lunarDate[3] == LEAP_MONTH;
    ...
    private LunarUtils(Context context) {
        ...
        mLunarFestCHUXI = res.getString(R.string.lunar_fest_chuxi);

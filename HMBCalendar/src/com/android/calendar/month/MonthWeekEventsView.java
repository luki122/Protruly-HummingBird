...
import com.android.calendar.hb.legalholiday.LegalHoliday;
import com.android.calendar.hb.legalholiday.LegalHolidayUtils2;
...
public class MonthWeekEventsView extends SimpleWeekView {
    ...
    public MonthWeekEventsView(Context context) {
        ...
        isChineseEnvironment = CalendarUtils.isChineseEnv();
    }
    ...
    private void initMstView() {
        ...
        mMonthDayHighlightTextPadding = res.getInteger(R.integer.month_day_highlight_text_padding);
        ...
        int textSizeSolarMonthDay = res.getInteger(R.integer.month_day_text_size);
        int textSizeLunarMonthDay = res.getInteger(R.integer.month_day_other_text_size);

        if (mScale > 0 && mScale != 1.0F) {
            textSizeSolarMonthDay *= mScale;
            textSizeLunarMonthDay *= mScale;
        ...
        mLunarMonthDayPaint.setTextSize(textSizeLunarMonthDay);
        ...
        mLegalHolidayPaint = new Paint();
        mLegalHolidayPaint.setFakeBoldText(false);
        mLegalHolidayPaint.setAntiAlias(true);
        mLegalHolidayPaint.setTextSize(textSizeLunarMonthDay);
        mLegalHolidayPaint.setColor(mColorEventTag);
        mLegalHolidayPaint.setStyle(Style.FILL);
        mLegalHolidayPaint.setTextAlign(Align.LEFT);
    ...
    private void drawWeekNumber(Canvas canvas) {
        ...
        LegalHoliday legelHolidayUtils = LegalHolidayUtils2.getInstance();
    	...
    	for (int index = 0; index < mNumDays; ++index) {
			...
			if (isChineseEnvironment) {
				...
				int yLunarOffset = textBounds.height() + mMonthDayTextTopPadding;
                ...
                if (legalHolidayTags[index] == LegalHoliday.DAY_TYPE_HOLIDAY) {
                    highlightText = mStrLegalHoliday;
                    mLegalHolidayPaint.setColor(mColorEventTag);
                } else if (legalHolidayTags[index] == LegalHoliday.DAY_TYPE_WORKDAY) {
                    highlightText = mStrLegalWorkShift;
                    mLegalHolidayPaint.setColor(mColorLunarMonthDayDefault);
                }
                if (!TextUtils.isEmpty(highlightText)) {
                    mLegalHolidayPaint.getTextBounds(highlightText, 0, highlightText.length(), textBounds);
                    int inWidthHighlight = textBounds.width();

                    int xHighlightOffset = outRight - mMonthDayHighlightTextPadding - inWidthHighlight;
                    int yHighlightOffset = textBounds.height() + mMonthDayHighlightTextPadding;

                    canvas.drawText(highlightText, xHighlightOffset, yHighlightOffset, mLegalHolidayPaint);
                }
			}
    	}
    }
    ...
    private void drawEventsTag(Canvas canvas) {
        ...
        for (int i = 0; i < hasEvents.length; ++i) {
            if (hasEvents[i]) {
                ...
                if (mFocusDay[i] || (mHasToday && mTodayIndex == i)) {
                    p.setColor(mColorEventTag);
                } else {
                    p.setColor(mColorMonthDaySecondary);
                }
    ...
    private int adjustWeekNumberXOffset(String text, int xOffset) {
        ...
        if (text.equals("1")) {
            ...
            xOffset -= EXTRA_X_OFFSET_FOR_MONTH_NUMBER / 2;
        }

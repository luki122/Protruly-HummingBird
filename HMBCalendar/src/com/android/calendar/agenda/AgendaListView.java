...
public class AgendaListView extends ListView implements OnItemClickListener, OnItemLongClickListener {
    ...
    public boolean onItemLongClick(AdapterView<?> parent, View v, int position, long id) {
        if (id != -1) {
            ...
            mTime.set(startTime);
            CalendarController controller = CalendarController.getInstance(mContext);
            controller.sendEventRelatedEventWithExtra(this, EventType.DELETE_EVENT, item.id,
                    startTime, endTime, 0, 0, CalendarController.EventInfo.buildViewExtraLong(
                            Attendees.ATTENDEE_STATUS_NONE, item.allDay), holderStartTime);
        }
        return true;

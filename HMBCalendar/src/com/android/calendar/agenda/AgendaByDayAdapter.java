...
public class AgendaByDayAdapter extends BaseAdapter {
    ...
    public AgendaByDayAdapter(Context context) {
        mContext = context;
        mAgendaAdapter = new AgendaAdapter(context, R.layout.hb_agenda_item/*agenda_item*/);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (row.mType == TYPE_DAY) {
            ...
            if (holder == null) {
                ...
                agendaDayView = mInflater.inflate(R.layout.hb_agenda_day/*agenda_day*/, parent, false);
            ...
            //         mContext);
            String weekdayText = DateUtils.formatDateRange(mContext, mFormatter, millis, millis,
            ...
            int stringResid = R.string.agenda_date_weekday;
            if (row.mDay == mTodayJulianDay) {
                stringResid = R.string.agenda_date_weekday_today;
            } else if (row.mDay == mTodayJulianDay - 1) {
                stringResid = R.string.agenda_date_weekday_yesterday;
            } else if (row.mDay == mTodayJulianDay + 1) {
                stringResid = R.string.agenda_date_weekday_tomorrow;
            }
            dateViewText = mContext.getString(stringResid, dateViewText, weekdayText);
                ...
                if (prevStartDay == -1) {
                    ...
                    for (int currentDay = prevStartDay + 1; currentDay <= startDay; currentDay++) {
                        ...
                        while (iter.hasNext()) {
                            ...
                            boolean hideSeparatorInner = false;
                            ...
                            rowInfo.add(new RowInfo(TYPE_MEETING, currentDay, info.mPosition,
                                    ...
                                    infoEndTime, info.mInstanceId, info.mAllDay, hideSeparatorInner));
            ...
            for (int currentDay = prevStartDay + 1; currentDay <= dayAdapterInfo.end;
                ...
                while (iter.hasNext()) {
                    ...
                    rowInfo.add(new RowInfo(TYPE_MEETING, currentDay, info.mPosition,
                            ...
                            info.mInstanceId, info.mAllDay, hideSeparatorInner));
    ...
    private static class RowInfo {
        ...
        final boolean mHideSeparator;

        RowInfo(int type, int julianDay, int position, long id, long startTime, long endTime,
                long instanceId, boolean allDay, boolean hideSeparator) {
            mType = type;
            mDay = julianDay;
            mPosition = position;
            mEventId = id;

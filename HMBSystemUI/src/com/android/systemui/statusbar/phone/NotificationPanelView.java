...
public class NotificationPanelView extends PanelView implements
    ...
    private static final float HEADER_RUBBERBAND_FACTOR = 1.64f;//2.05f; //modify by chenhl for status_bar_header_height change
    ...
    protected void onFinishInflate() {
        ...
        //add by chenhl start
        mManagerContain = findViewById(R.id.dui_id_notify_manager_containt);
        //add by chenhl end
    }
    ...
    public void updateResources() {
        ...
        //add by chenhl start
        lp = (FrameLayout.LayoutParams) mManagerContain.getLayoutParams();
        if (lp.width != panelWidth) {
            lp.width = panelWidth;
            mManagerContain.setLayoutParams(lp);
        }
        //add by chenhl end
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        // Update Clock Pivot
        // modify by wxue 20170408 start
        //mKeyguardStatusView.setPivotX(getWidth() / 2);
        mKeyguardStatusView.setPivotX(0);
        // modify by wxue 20170408 end
    ...
    private final Runnable mAnimateKeyguardBottomAreaInvisibleEndRunnable = new Runnable() {
        ...
        public void run() {
            ...
            mIsKeyguardBottomAnimatorRun= false; //add by chenhl
        }
    };
    ...
    private void setKeyguardBottomAreaVisibility(int statusBarState,
        ...
        //modify by chenhl start
                ...
                /*|| statusBarState == StatusBarState.SHADE_LOCKED*/) {//delete  by chenhl
            mKeyguardBottomArea.animate().cancel();
            if (!mDozing) {
                ...
                mKeyguardBottomArea.animate()
                        ...
                        .withStartAction(mAnimateKeyguardBottomAreaStartRunnable)
                        .withEndAction(mAnimateKeyguardBottomEndRunnable)
                        .start();
            }
            //mKeyguardBottomArea.setAlpha(1f);
        }
        else if(statusBarState == StatusBarState.SHADE_LOCKED){
            mKeyguardBottomArea.animate().cancel();
            mKeyguardBottomArea.animate()
                    .alpha(0)
                    .setDuration(500)
    ...
    protected int getMaxPanelHeight() {
        ...
        maxHeight = Math.max(maxHeight,getHeight());//add by chenhl
        return maxHeight;
    }
    ...
    protected void onHeightUpdated(float expandedHeight) {
        ...
        // add by wxue for wallpaper slide 20140424 start
        mStatusBar.updateWallpaperViewHeight(expandedHeight);
        // add by wxue for wallpaper slide 20140424 end
    ...
    private int calculatePanelHeightShade() {
        ...
        //HB. Comments : calculate height , Engerineer : chenheliang , Date : 17-5-17 , begin
        maxHeight = getContext().getResources().getDimensionPixelOffset(R.dimen.hb_notification_height);
        //HB. end
        return maxHeight;
    }
    ...
    private void updateHeader() {
        if (mStatusBar.getBarState() == StatusBarState.KEYGUARD) {
        	// del by wxue 20170424 start
            //updateHeaderKeyguard();
            // del by wxue 20170424 end
        } else {
            updateHeaderShade();
        }
    }
    ...
    private float getHeaderTranslation() {
        ...
        //add by chenhl start
        translation = getExpandedHeight()-getMaxPanelHeight();
        if(translation<-mHeader.getHeight()){
            translation = -mHeader.getHeight();
        }
        //add by chenhl end
    ...
    private void updateHeaderKeyguardAlpha() {
        ...
        // modify by wxue for making mKeyguardStatusBar visible when bouncer showing 20170401 start
        /*mKeyguardStatusBar.setAlpha(Math.min(getKeyguardContentsAlpha(), alphaQsExpansion)
                * mKeyguardStatusBarAnimateAlpha);*/
        mKeyguardStatusBar.setAlpha(isBouncerShowing() ? 1.0f :Math.min(getKeyguardContentsAlpha(), alphaQsExpansion)
                * mKeyguardStatusBarAnimateAlpha);
        // modify by wxue for making mKeyguardStatusBar visible when bouncer showing 20170401 end
    ...
    private void updateKeyguardBottomAreaAlpha() {
        ...
        mKeyguardBottomArea.setY(mExpandedHeight - getMaxPanelHeight());
        mKeyguardBottomArea.getIndicationView().setAlpha(getIndicationViewAlpha());
        mStatusBar.setKeyguardChargingViewAlpha(getKeyguardChargingViewAlpha());
        // add by wxue 20170424 end
        //modify by chenhl start
        //if(!mIsKeyguardBottomAnimatorRun) {
    ...
    private void updateEmptyShadeView() {
        ...
        mNotificationStackScroller.updateEmptyShadeView(false);
        //modify by chenhl end
    }
    ...
    private void updateStackHeight(float stackHeight) {
        mNotificationStackScroller.setStackHeight(stackHeight,getMaxPanelHeight());
        updateKeyguardBottomAreaAlpha();
        //add by chenhl start
        updateBlueViewHeight(stackHeight,getMaxPanelHeight());
        //add by chenhl end
    }
    ...
    //add by chenhl start
    private DuiBlurNotificationBg mDuiBlurNotificationBg;
    private ValueAnimator mValueAnimator;
    private boolean mIsKeyguardBottomAnimatorRun=false;
    public void setBlueNotificationView(DuiBlurNotificationBg view){
        mDuiBlurNotificationBg = view;
    }
    private void updateBlueViewHeight(float h,float maxH){
        if(mDuiBlurNotificationBg!=null){
            if(h>maxH){
                h=maxH;
            }
            int hx=getHeight();
            if(mHeadsUpManager.hasPinnedHeadsUp()){
                if(isFullyCollapsed()) {
                    ...
                    if(h<mHeadsUpManager.getTopHeadsUpHeight()){
                        h=mHeadsUpManager.getTopHeadsUpHeight();
                    }
                }
            }
            mDuiBlurNotificationBg.updateBlueBgHight(h*hx/maxH,h-maxH);
        }
    }

    private void showBlurBgAnimator(){
        int max=mHeadsUpManager.getTopHeadsUpHeight();
        int start = mDuiBlurNotificationBg.getClipHeight();
        mValueAnimator = ValueAnimator.ofInt(start,max);
        mValueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                int h=(int)animation.getAnimatedValue();
                mDuiBlurNotificationBg.updateBlueBgHight(h,0);
            }
        });
        mValueAnimator.start();
    }

    private final Runnable mAnimateKeyguardBottomAreaStartRunnable = new Runnable() {
        @Override
        public void run() {
            mIsKeyguardBottomAnimatorRun= true;
        }
    };
    private final Runnable mAnimateKeyguardBottomEndRunnable = new Runnable() {
        @Override
        public void run() {
            mIsKeyguardBottomAnimatorRun= false;
        }
    };
    //add by chenhl end
    
    // add by wxue 20170426 start
    private float getIndicationViewAlpha(){
    	return 1- (getMaxPanelHeight() - mExpandedHeight) / 240;
    }
    
    private float getKeyguardChargingViewAlpha(){
    	float input =1- (getMaxPanelHeight() - mExpandedHeight) / 120;
    	if(input < 0){
    		input = 0;
    	}
    	return input * input;

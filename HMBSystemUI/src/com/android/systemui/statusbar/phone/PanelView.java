...
//add by wxue for NotificationPanelView visible when bouncer showing 20170401 start
import com.android.keyguard.KeyguardUpdateMonitor;
// add by wxue for NotificationPanelView visible when bouncer showing 20170401 end
...
public abstract class PanelView extends FrameLayout {
    ...
    private void endMotionEvent(MotionEvent event, float x, float y, boolean forceCancel) {
        ...
        if ((mTracking && mTouchSlopExceeded)
                /*|| Math.abs(x - mInitialTouchX) > mTouchSlop*/  //delete by chenhl
                || Math.abs(y - mInitialTouchY) > mTouchSlop
                /*|| event.getActionMasked() == MotionEvent.ACTION_CANCEL*/ //Three finger screenshot conflict
                || forceCancel) {
            float vel = 0f;
            float vectorVel = 0f;
            if (mVelocityTracker != null) {
    ...
    protected void flingToHeight(float vel, boolean expand, float target,
        ...
        if (expand) {
            ...
            mFlingAnimationUtils.apply(animator, mExpandedHeight, target, vel, getMaxPanelHeight()); //modify by chenhl
            if (expandBecauseOfFalsing) {
    ...
    private void startUnlockHintAnimationPhase1(final Runnable onAnimationFinished) {
        ...
        animator.start();
        mHeightAnimator = animator;
        // modify by wxue 20140424 start
        //mKeyguardBottomArea.getIndicationView().animate()
        mKeyguardBottomArea.animate()
        // modify by wxue 20140424 end
    ...
    // add by wxue for NotificationPanelView visible when bouncer showing 20170401 start
    protected boolean isBouncerShowing(){
    	KeyguardUpdateMonitor updateMonitor = KeyguardUpdateMonitor.getInstance(getContext());
    	//Log.i(TAG,"---isBouncerShowing()--bouncerShowing =" + updateMonitor.isKeyguardBouncer() + " statusBarState = " + mStatusBar.getBarState());
    	return updateMonitor.isKeyguardBouncer() && mStatusBar.getBarState() == StatusBarState.KEYGUARD;
    }
    // add by wxue for NotificationPanelView visible when bouncer showing 20170401 end
    
    protected void notifyBarPanelExpansionChanged() {
    	// modify by wxue for NotificationPanelView visible when bouncer showing 20170401 start
        /*mBar.panelExpansionChanged(this, mExpandedFraction, mExpandedFraction > 0f || mPeekPending
                || mPeekAnimator != null || mInstantExpanding || isPanelVisibleBecauseOfHeadsUp()
                || mTracking || mHeightAnimator != null);*/
    	/*Log.i(TAG,"---notifyBarPanelExpansionChanged()--mExpandedFraction = " + mExpandedFraction  + ""
    			+ " mPeekPending = " + mPeekPending + " mInstantExpanding = " + mInstantExpanding + " isPanelVisibleBecauseOfHeadsUp() = "
    			+ isPanelVisibleBecauseOfHeadsUp() + " mTracking = " + mTracking + " mHeightAnimator = " + mHeightAnimator + " isBouncerShowing() = " + isBouncerShowing());*/
    	mBar.panelExpansionChanged(this, mExpandedFraction, mExpandedFraction > 0f || mPeekPending
                || mPeekAnimator != null || mInstantExpanding || isPanelVisibleBecauseOfHeadsUp()
                || mTracking || mHeightAnimator != null || isBouncerShowing());
        // modify by wxue for NotificationPanelView visible when bouncer showing 20170401 end
    }

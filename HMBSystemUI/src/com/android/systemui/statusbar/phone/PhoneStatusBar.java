...
import com.android.systemui.statusbar.KeyguardChargingController;
//HB. end
...
import com.android.systemui.qs.QSPanelPullUp;
import com.android.systemui.qs.QSSlideBar;
import com.android.systemui.qs.ViewForQSPanelPullUp;
...
import com.android.systemui.statusbar.NetworkSpeedTracker;
import com.android.systemui.statusbar.NotificationData;
import com.android.systemui.statusbar.NetworkSpeedTracker.NetworkSpeedUpdateListener;
...
import com.dui.systemui.statusbar.DuiBlurNotificationBg;
import com.dui.systemui.statusbar.DuiNotificationOverflowContainer;
...
import static com.android.systemui.statusbar.phone.BarTransitions.MODE_SCREEN_SHOT;
import static com.android.systemui.statusbar.phone.BarTransitions.MODE_SEMI_TRANSPARENT;
import static com.android.systemui.statusbar.phone.BarTransitions.MODE_TRANSLUCENT;
import static com.android.systemui.statusbar.phone.BarTransitions.MODE_TRANSPARENT;
import static com.android.systemui.statusbar.phone.BarTransitions.MODE_WARNING;
import static com.android.systemui.statusbar.phone.NavigationBarView.HIDE_NAVIGATION_BAR;
...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
        ...
        StatusBarNotificationBoxStatusListener ,
        NetworkSpeedUpdateListener {
    ...
    //HB. Comments : remove lockscreen artwork , Engerineer : wxue , Date : 2017年6月27日 ,begin
    ...
    NetworkSpeedTracker mNetworkSpeedTracker;
    //speed text view in status bar , not in keyguard status bar
    ...
    private QSPanelPullUp mQuickSettingPanel;
    /**hb add by tangjun begin*/
    ...
    //HB. Comments : TODO , Engerineer : wxue , Date : 2017年6月19日 ,begin
    KeyguardChargingController mKeyguardChargingController;
    //HB. end
    ...
    private boolean mIsKeyguardStartActivity=false;
    //add by chenhl end
    ...
    // add by wxue 20170414 start
    private LockedWallpaperController mLockedWallpaperController;
    // add by wxue 20170414 end
    ...
    public void start() {
        ...
        //ShenQianfeng add begin
        mCustomStatusBarManager.registerObserver();
        //ShenQianfeng add end

        //add by chenhl start
        mContext.getContentResolver().registerContentObserver(Settings.Secure.getUriFor(HIDE_NAVIGATION_BAR),false,mHideNavigationbarObserver);
        mContext.getContentResolver().registerContentObserver(Settings.Secure.getUriFor(HB_HIDE_NAVIGATION_FOR_APP),false,mHideNavigationbarObserver);
        mContext.getContentResolver().registerContentObserver(Settings.Secure.getUriFor(HB_SCREEN_LONG_SHOT),false,mHideNavigationbarObserver);
        //add by chenhl end
    }
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        mStatusBarWindow.setPuLLDown(mKeyguardPullNotify);//add by chenhl
        ...
        //add by chenhl start
        mDuiBlurNotificationBg = (DuiBlurNotificationBg) mNotificationPanel.findViewById(R.id.dui_id_notify_bg);
        mDuiBlurNotificationBg.setActivityStarter(this);
        mNotificationPanel.setBlueNotificationView(mDuiBlurNotificationBg);
        mNotificationOverflowContainer = (DuiNotificationOverflowContainer) LayoutInflater.from(mContext).inflate(
                R.layout.dui_notification_overfow, mStackScroller, false);
        mNotificationOverflowContainer.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                mShowFirstNotify=false;
                updateRowStates();
            }
        });
		mNotificationOverflowContainer.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
                Intent intent = new Intent();
                intent.setClassName("com.android.settings","com.android.settings.SubSettings");
                intent.putExtra(":settings:show_fragment","com.android.settings.notification.NotificationStatusbarSettings");
                intent.putExtra(":settings:show_fragment_title",mContext.getString(R.string.hb_notification_and_statusbar));
                startActivity(intent,true);
                return true;
            }
        });
        mStackScroller.setOverflowContainer(mNotificationOverflowContainer);
        //add by chenhl end
        ...
        // add by wxue 20170414 start
        ImageView wallpaperImageView = (ImageView) mStatusBarWindow.findViewById(R.id.hb_wallpaper_image);
        ImageView bouncerImageView = (ImageView) mStatusBarWindow.findViewById(R.id.hb_bouncer_bg);
        mLockedWallpaperController = new LockedWallpaperController(mContext, wallpaperImageView, bouncerImageView);
        // add by wxue 20170414 end
        ...
        //add by chenhl start
        mClearView = mHeader.findViewById(R.id.dui_id_notify_close);
        ...
        //ShenQianfeng add begin
        //mIconController.setNotificationData(mNotificationData);
        mCustomStatusBarManager = new CustomStatusBarManager(mContext, mHandler, mIconController);
        mCustomStatusBarManager.setNotificationData(mNotificationData);
        mIconController.setCustomStatusBarManager(mCustomStatusBarManager);
        mCustomStatusBarManager.setStatusBarNotificationBoxStatusListener(this);
        //ShenQianfeng add end
        ...
        //ShenQianfeng add begin
        mNetworkSpeedTracker = new NetworkSpeedTracker(mContext);
        //add network speed listener for keyguard status bar 
        mKeyguardStatusBar.setNetworkSpeedTracker(mNetworkSpeedTracker);
        mNetworkSpeedTextView = (TextView)mStatusBarView.findViewById(R.id.network_speed);
        //add network speed listener for normal status bar
        mNetworkSpeedTracker.addUpdateListener(this);
        //ShenQianfeng add end
        ...
        mQSSlideBar =  new QSSlideBar(mContext);
        mQSSlideBar.init();
        mQuickSettingPanel = (QSPanelPullUp) mQSSlideBar.getQSPanelPanelView().getQuickSettingView();
        if (mQuickSettingPanel != null) {
            final QSTileHost qsh = new QSTileHost(mContext, this,
                    mBluetoothController, mLocationController, mRotationLockController,
                    mNetworkController, mZenModeController, mHotspotController,
                    mCastController, mFlashlightController,
            ...
            mQuickSettingPanel.setHost(qsh);
            mQuickSettingPanel.setTiles(qsh.getTiles());
            //mBrightnessMirrorController = new BrightnessMirrorController(mStatusBarWindow);
            //mQSPanel.setBrightnessMirror(mBrightnessMirrorController);
            //mHeader.setQSPanel(mQSPanel);
            qsh.setCallback(new QSTileHost.Callback() {
        ...
        /**hb tangjun add begin*/
        mQSSlideBar.setNavigationBarView(mNavigationBarView);
        ViewForQSPanelPullUp view = (ViewForQSPanelPullUp) mStatusBarWindow.findViewById(R.id.view_pullup);
        view.setDelegateView(mQSSlideBar.getHandlerBarView());
        view.setBar(this);
        mQSSlideBar.setBar(this);
        /**hb tangjun add end*/
        ...
        ((BatteryMeterView) mStatusBarView.findViewById(R.id.battery)).setBatteryController(
                mBatteryController);*/
        //Modify to:
        ((CustomBatteryMeterView) mStatusBarView.findViewById(R.id.battery)).setBatteryController(mBatteryController);
        //ShenQianfeng modify end
        mKeyguardStatusBar.setBatteryController(mBatteryController);
        mHeader.setNextAlarmController(mNextAlarmController);
        
        //HB. Comments : add charging icon , Engerineer : wxue , Date : 2017年6月19日 ,begin
        mKeyguardChargingController = new KeyguardChargingController(mContext,
                (KeyguardChargingView) mStatusBarWindow.findViewById(
                        R.id.keyguard_charging_view),mBatteryController);
        //HB. end
        ...
        //add by chenhl start
        filter.addAction(HIDE_NAVAGATIONBAR_ACTION);
        //add by chenhl end
    ...
    private void prepareNavigationBarView() {
        ...
        /**hb tangjun add begin*/
        mNavigationBarView.setDelegateView(mQSSlideBar.getHandlerBarView());
        /**hb tangjun add end*/
    }
    ...
    private void addNavigationBar() {
        ...
        //add by chenhl start
        int hide = Settings.Secure.getInt(mContext.getContentResolver(), HB_HIDE_NAVIGATION_FOR_APP, 1);
    ...
    public void addIcon(String slot, int index, int viewIndex, StatusBarIcon icon) {
        //Log.i(TAG, "PhoneStatusBar::addIcon slot:" + slot + " index:" + index + " viewIndex:" + viewIndex);
        mIconController.addSystemIcon(slot, index, viewIndex, icon);
    }

    public void updateIcon(String slot, int index, int viewIndex,
            StatusBarIcon old, StatusBarIcon icon) {
        //Log.i(TAG, "PhoneStatusBar::updateIcon slot:" + slot + " index:" + index + " viewIndex:" + viewIndex);
        mIconController.updateSystemIcon(slot, index, viewIndex, old, icon);
    }

    public void removeIcon(String slot, int index, int viewIndex) {
        //Log.i(TAG, "PhoneStatusBar::removeIcon slot:" + slot + " index:" + index + " viewIndex:" + viewIndex);
    ...
    public void addNotification(StatusBarNotification notification, RankingMap ranking,
        ...
        boolean isHeadsUped = mUseHeadsUp && shouldInterrupt(shadeEntry);//&&!mExpandedVisible;//modify by chenhl for headsup show
        if (isHeadsUped) {
    ...
    private void updateNotificationShade() {
        ...
        //Log.i(TAG, "updateNotificationShade---BEGIN");

        // Do not modify the notifications during collapse.
        if (isCollapsing()) {
            //Log.i(TAG, "updateNotificationShade---END 111111111");
            addPostCollapseAction(new Runnable() {
                @Override
                public void run() {
    ...
    private void updateQsExpansionEnabled() {
        //modify by chenhl for disable QS start
        ...
        //modify by chenhl for disable QS end
    }
    ...
    private void updateClearAll() {
        ...
        mClearView.setVisibility(showDismissView?View.VISIBLE:View.GONE);//add by chenhl
    }
    ...
    private void updateEmptyShadeView() {
        ...
        mHeader.updateNotifyTitleView(showEmptyShade);//add by chenhl
    }
    ...
    public void onUnlockMethodStateChanged() {
    	...
    	mLockedWallpaperController.setHasBouncer(isKeyguardCurrentlySecure());
    ...
    void makeExpandedVisible(boolean force) {
        ...
        if (mNavigationBarView != null&&mIsShowNavigationBar) //modify by chenhl
        ...
        updateRowStates();
        if(mState==StatusBarState.SHADE) {
            mDuiBlurNotificationBg.updateBg();
            if(mNavigationBarView!=null)
                mNavigationBarView.updateBarTranslation(mHeadsUpManager.hasPinnedHeadsUp()?false:true);
        }
        mDuiBlurNotificationBg.setmShowBg(true);
        //add by chenhl end
    }
    ...
    void makeExpandedInvisible() {
        ...
        //add by chenhl start
        mShowFirstNotify=true;
        if(mState==StatusBarState.SHADE&&mNavigationBarView!=null) {
            mNavigationBarView.updateBarTranslation(false);
        }
        mDuiBlurNotificationBg.setmShowBg(false);
        //add by chenhl end
    }
    ...
    public void setSystemUiVisibility(int vis, int mask) {
        ...
        if (diff != 0) {
            ...
            if(mNavigationBarView!=null){
                ...
                //add by chenhl start
                int screnshot = Settings.Secure.getInt(mContext.getContentResolver(), HB_SCREEN_LONG_SHOT, 0);
                mLastLightNavigationBar = light;
                if(screnshot==1){
                    light=true;
                }
                //add by chenhl end
                ...
                boolean lightAddLine = (vis & View.SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR_ADD_LINE) != 0;
                mNavigationBarView.setLightNavigationBarLineShow(lightAddLine);
                setLightNavigationBarAddLine(lightAddLine);
            }
            /**hb tangjun add end*/
    ...
    private void checkBarMode(int mode, int windowState, BarTransitions transitions,
        ...
        //add by chenhl start
        int screnshot = Settings.Secure.getInt(mContext.getContentResolver(), HB_SCREEN_LONG_SHOT, 0);
        if(screnshot==1&&transitions==mNavigationBarView.getBarTransitions()){
            mode = MODE_SCREEN_SHOT;
        }
        //add by chenhl end
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_CLOSE_SYSTEM_DIALOGS.equals(action)) {
                ...
                int screnshot = Settings.Secure.getInt(mContext.getContentResolver(), HB_SCREEN_LONG_SHOT, 0);
                if(screnshot==1) {
                    Intent intent1 = new Intent("hb_long_shot_stop_action");
            ...
            //add by chenhl start
            else if(HIDE_NAVAGATIONBAR_ACTION.equals(action)){
                int state = intent.getIntExtra("state",0);
                int from = intent.getIntExtra("from",0);
                Log.d("chenhl","HIDE_NAVAGATIONBAR_ACTION:"+state+" from:"+from);
                if(state==0){
                    hideNavigationBar(from);
                }else {
                    showNavigationBar();
                }
            }
    ...
    void updateResources() {
        ...
        //hb  tangjun add begin
        if (mQuickSettingPanel!= null) {
        	mQuickSettingPanel.updateResources();
        }
        //hb tangjun add end
    ...
    public void destroy() {
        ...
        /**hb tangjun add begin*/
        if (mQuickSettingPanel != null && mQuickSettingPanel.getHost() != null) {
        	mQuickSettingPanel.getHost().destroy();
        }
        /**hb tangjun add end*/
    }
    ...
    private void onLaunchTransitionFadingEnded() {
    	// add by wxue 20170419 start
    	mLockedWallpaperController.setLaunchTransitionOccluding(false);
    	// add by wxue 20170419 end
    ...
    public void fadeKeyguardAfterLaunchTransition(final Runnable beforeFading,
        ...
        Runnable hideRunnable = new Runnable() {
            ...
            public void run() {
                ...
                // add by wxue 20170419 start
            	mLockedWallpaperController.setLaunchTransitionOccluding(true);
            	// add by wxue 20170419 end
    ...
    public boolean hideKeyguard() {
        ...
        /**hb add by tangjun begin*/
        if (mQuickSettingPanel != null) {
        	mQuickSettingPanel.refreshAllTiles();
        }
        /**hb add by tangjun begin*/
    ...
    private void updateKeyguardState(boolean goingToFullShade, boolean fromShadeLocked) {
        if (mState == StatusBarState.KEYGUARD) {
		    mShowFirstNotify=true;//add by chenhl
        	// add by wxue 20170315 start
        	mKeyguardIndicationController.setRestingIndication(mContext.getString(R.string.keyguard_unlock));
        	// add by wxue 20170315 end
        ...
        //add by chenhl start
        //mClearView.setVisibility(mState != StatusBarState.KEYGUARD?View.VISIBLE:View.GONE);
        mDuiBlurNotificationBg.setBlurBgVisible(mState);
        updateHideState(mState == StatusBarState.KEYGUARD?true:mIsShowNavigationBar);
        //add by chenhl end
    }
    ...
    public boolean onBackPressed() {
        if (mStatusBarKeyguardViewManager.onBackPressed()) {
            //add by chenhl start
            mIsKeyguardStartActivity=false;
            //add by chenhl end
            return true;
        }
    ...
    private void showBouncer() {
        ...
        if(mState == StatusBarState.SHADE_LOCKED&&!mIsKeyguardStartActivity){

            mIsKeyguardStartActivity=false;
            mDuiBlurNotificationBg.setBlueKeyguardPulldown(true);
            goToKeyguard();
            instantExpandNotificationsPanel();
            return;
        }
        ...
        if (mState == StatusBarState.KEYGUARD || mState == StatusBarState.SHADE_LOCKED) {
            ...
            if(mStatusBarKeyguardViewManager.isBouncerShowing()){
                updateHideState(mIsShowNavigationBar);
            }
            //add by chenhl end
        }
    }
    ...
    public void setBarState(int state) {
        ...
        mState = state;
        // add by wxue 20170414 start
        mLockedWallpaperController.setBarState(state);
        mKeyguardChargingController.setBarState(state);
        // add by wxue 20170414 end
    ...
    public void onTrackingStarted() {
    	// add by wxue 20170426 start
        mLockedWallpaperController.onTrackingStarted();
        // add by wxue 20170426 end
        runPostCollapseRunnables();
    }
    ...
    public void onTrackingStopped(boolean expand) {
        if (mState == StatusBarState.KEYGUARD /*|| mState == StatusBarState.SHADE_LOCKED*/) {//modify by chenhl
        ...
        if(expand){
            if(mState==StatusBarState.SHADE) {
                if(mNavigationBarView!=null)
                    mNavigationBarView.updateBarTranslation(true);
            }
        }
        //add by chenhl end
        // add by wxue 20170426 start
        mLockedWallpaperController.onTrackingStopped(expand);
        // add by wxue 20170426 end
    }
    ...
    public void goToLockedShade(View expandView) {
        ...
        boolean fullShadeNeedsBouncer = !userAllowsPrivateNotificationsInPublic(mCurrentUserId)
                /*|| !mShowLockscreenNotifications*/; //modify by chenhl for delete lockscreennotifications switch on
    ...
    public void setBouncerShowing(boolean bouncerShowing) {
        ...
        disable(mDisabledUnmodified1, mDisabledUnmodified2, true /* animate */);
        //HB. Comments : dismiss charging icon when bouncer is showing , Engerineer : wxue , Date : 2017年6月20日 ,begin
        mKeyguardChargingController.setBouncerShowing(bouncerShowing);
        mLockedWallpaperController.setBouncerShowing(bouncerShowing);
        //HB. end
    }
    ...
    private boolean mFromAppHide=false;
    private long hideNavgationbarTime=0;
    private boolean mLastLightNavigationBar =false;

    @Override
    protected void updateKeygardPullSetting() {
        if(mStatusBarWindow!=null){
            mStatusBarWindow.setPuLLDown(mKeyguardPullNotify);
        }
    }

    private void hideNavigationBar(int from){
        if(mNavigationBarView==null||!mIsShowNavigationBar){
            return;
        }
        if(mNavigationBarView.isHideBar()){
            return;
        }
        hideNavgationbarTime = System.currentTimeMillis();
        mWindowManager.removeViewImmediate(mNavigationBarView);
        mIsShowNavigationBar=false;
        if(from==0) {
    ...
    private void showNavigationBar(){
        ...
        if(mState==StatusBarState.KEYGUARD&&!mStatusBarKeyguardViewManager.isBouncerShowing()&&!mFromAppHide){
            return;
        }
        long betTime= System.currentTimeMillis()-hideNavgationbarTime;
        if(betTime<500){
            return;
        }
        if(Settings.Secure.getInt(mContext.getContentResolver(), "hb_screen_long_shot_show_navigationbar", 0)==1){
            return;
        }
        mFromAppHide =false;
        if(!mIsShowNavigationBar&&mNavigationBarView!=null){
            mIsShowNavigationBar=true;
    ...
    final private ContentObserver mHideNavigationbarObserver = new ContentObserver(mHandler) {
        @Override
        public void onChange(boolean selfChange,Uri uri) {

            if(Settings.Secure.getUriFor(HIDE_NAVIGATION_BAR).equals(uri)) {
                int hide = Settings.Secure.getInt(mContext.getContentResolver(), HIDE_NAVIGATION_BAR, 0);
                if (mNavigationBarView != null) {
                    mNavigationBarView.updateHideView();
                }
                if (hide == 0) {
                    showNavigationBar();
                }
            }else if(Settings.Secure.getUriFor(HB_HIDE_NAVIGATION_FOR_APP).equals(uri)){
                int hide = Settings.Secure.getInt(mContext.getContentResolver(), HB_HIDE_NAVIGATION_FOR_APP, 1);
            ...
            }else if(Settings.Secure.getUriFor(HB_SCREEN_LONG_SHOT).equals(uri)){
                int screnshot = Settings.Secure.getInt(mContext.getContentResolver(), HB_SCREEN_LONG_SHOT, 0);
                if(mNavigationBarView!=null){
                    mNavigationBarView.getBarTransitions().transitionTo(screnshot==0?mNavigationBarMode:MODE_SCREEN_SHOT,false);
                    mNavigationBarView.setIconsDark2(screnshot==1?true:mLastLightNavigationBar);
                }
            }
        }
    };

    private void updateHideState(boolean show){
        if (show){
            //SystemProperties.set("hb.hide.navigationbar.sys", "1");
            Settings.Secure.putInt(mContext.getContentResolver(),"hb.hide.navigationbar.sys",1);
        }else{
            Settings.Secure.putInt(mContext.getContentResolver(),"hb.hide.navigationbar.sys",0);
            //SystemProperties.set("hb.hide.navigationbar.sys", "0");
        }
    }
    //add by chenhl end
    ...
    private boolean mIsLightNavigationBarAddLine = false;
    private void setLightNavigationBar(boolean lightNavigationBar) {
    	mIsLightNavigationBar = lightNavigationBar;
    }
    public boolean getLightNavigationBar() {
    	return mIsLightNavigationBar;
    }
    private void setLightNavigationBarAddLine(boolean lightNavigationBarAddLine) {
    	mIsLightNavigationBarAddLine = lightNavigationBarAddLine;
    }
    public boolean getLightNavigationBarAddLine() {
    	return mIsLightNavigationBarAddLine;
    }
    /**hb tangjun add end*/
    ...
    public void onStatusBarNotificationBoxStatusChange(boolean on) {
        Log.i(TAG, "PhoneStatusBar::onStatusBarNotificationBoxStatusChange on:" + on);
        mHandler.post(new Runnable(){
            @Override
            public void run() {
                updateNotifications();
    ...
    public void onNetworkSpeedUpdated(String speedText) {
        mNetworkSpeedTextView.setText(speedText);
    }

    @Override
    public void onNetworkSpeedClosed() {
        mNetworkSpeedTextView.setVisibility(View.GONE);
    }

    @Override
    public void onNetworkSpeedOpened() {
        mNetworkSpeedTextView.setVisibility(View.VISIBLE);
    }
    //ShenQianfeng add end
    
    // add by wxue 20170424 start
    public void updateWallpaperViewHeight(float expandedHeight ){
    	mLockedWallpaperController.updateWallpaperViewHeight(expandedHeight);
    }
    
    public void setCanShowBouncerWallpaper(){
    	mLockedWallpaperController.setHasBouncer(isKeyguardCurrentlySecure());
    }
    
    public void setKeyguardChargingViewAlpha(float alpha){
    	mKeyguardChargingController.setChargingScrimAlpha(alpha);
    }
    // add by wxue 20170424 end
}

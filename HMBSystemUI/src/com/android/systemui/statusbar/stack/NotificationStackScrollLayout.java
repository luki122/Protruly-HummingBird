...
public class NotificationStackScrollLayout extends ViewGroup
     ...
     */
    private float mLastSetStackHeight,mLastMaxH;
    ...
    private void initView(Context context) {
        ...
        //add by chenhl start
        mMarginBottomSize = getResources().getDimensionPixelSize(R.dimen.close_handle_underlap);
        mStackScrollAlgorithm.setHeadsUpManager(mHeadsUpManager);
        //add by chenhl end
    }
    ...
    private void requestAnimationOnViewResize() {
        if (mIsExpanded && mAnimationsEnabled) {
            ...
            //mNeedsAnimation = true; //delete by chenhl for bug 1278
        }
    }
     ...
     */
    public void setStackHeight(float height,float maxh) {
        ...
        //add by chenhl start
        mLastMaxH = maxh;
        // modify by wxue 20170424 start
        /* float bt = height-maxh;
            bt = bt>0?0:bt;
            mAmbientState.setTransBeween(bt);*/
        if(mPhoneStatusBar.getBarState() == StatusBarState.KEYGUARD){
            mAmbientState.setTransBeween(0);
        }else{
        	float bt = height-maxh;
            bt = bt>0?0:bt;
            mAmbientState.setTransBeween(bt);
         }
        // modify by wxue 20170424 end
        mAmbientState.setMaxStackH(maxh-mMarginBottomSize);
        updateClipLayout();  
        //add by chenhl end
    ...
    private int getScrollRange() {
        ...
        if (firstChild != null) {
            ...
            scrollRange = Math.max(0,contentHeight-getHeight());
    ...
    public void updateTopPadding(float qsHeight, int scrollY, boolean animate,
        ...
        setTopPadding(ignoreIntrinsicPadding ? (int) start : clampPadding((int) start),
                animate);
        setStackHeight(mLastSetStackHeight,mLastMaxH);
    }
    ...
    public void setSpeedBumpView(SpeedBumpView speedBumpView) {
        ...
        //addView(speedBumpView); //delete by chenhl
    }
    ...
    public void setDismissView(DismissView dismissView) {
        ...
        //addView(mDismissView); //delete by chenhl
    }
    ...
    //add by chenhl start
    private DuiNotificationOverflowContainer mNotifyOverflow;
    public void setOverflowContainer(DuiNotificationOverflowContainer overFlowContainer) {
        mNotifyOverflow = overFlowContainer;
        addView(mNotifyOverflow);
    }

    public void updateNotifyOverflowVisibility(boolean visible){
        int oldVisibility = mNotifyOverflow.willBeGone() ? GONE
                : mNotifyOverflow.getVisibility();
        final int newVisibility = visible ? VISIBLE : GONE;
        if (oldVisibility != newVisibility) {
            Runnable onFinishedRunnable = new Runnable() {
                @Override
                public void run() {
                    mNotifyOverflow.setVisibility(newVisibility);
                    mNotifyOverflow.setWillBeGone(false);
                    updateContentHeight();
                    notifyHeightChangeListener(mNotifyOverflow);
                }
            };
            if (!mAnimationsEnabled || !mIsExpanded) {
                mNotifyOverflow.cancelAppearDrawing();
                onFinishedRunnable.run();
            } else if (newVisibility != GONE) {
                mNotifyOverflow.performAddAnimation(0,
                        StackStateAnimator.ANIMATION_DURATION_STANDARD);
                mNotifyOverflow.setVisibility(newVisibility);
                mNotifyOverflow.setWillBeGone(false);
                updateContentHeight();
                ...
                mNotifyOverflow.performRemoveAnimation(
                        StackStateAnimator.ANIMATION_DURATION_STANDARD,
                        0.0f,
                        onFinishedRunnable);
                mNotifyOverflow.setWillBeGone(true);
            }
        }
    }
    //add by chenhl end
    ...
    public int getDismissViewHeight() {
        ...
        return 0;//height; delete by chenhl
    }
    ...
    public void updateClipLayout(){
    	int top=(int)(mAmbientState.geTtransBeween()+mTopPadding);
        if(top<0)top=0;
        mClipRect.set(0, top, getWidth(), getHeight());

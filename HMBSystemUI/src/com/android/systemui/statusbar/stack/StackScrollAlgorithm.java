...
public class StackScrollAlgorithm {
    ...
    public static final float DIMMED_SCALE = 1.05f;//0.95f; //modify by chenhl
    ...
    private boolean  IS_SCROLL = true; //add by chenhl
    ...
    public void getStackScrollState(AmbientState ambientState, StackScrollState resultState) {
        ...
        if(IS_SCROLL) {
            updatePositionsForState2(resultState, algorithmState, ambientState);
        }else {
        ...
        //updateClipping(resultState, algorithmState, ambientState);//modify by chenhl
    ...
    private void updateDimmedActivatedHideSensitive(AmbientState ambientState,
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            /*childViewState.scale = !mScaleDimmed || !dimmed || isActivatedChild
                    ? 1.0f
                    : DIMMED_SCALE;*/
            childViewState.scale=isActivatedChild?DIMMED_SCALE:1.0f;
            //modify by chenhl end
    ...
    private void updatePositionsForState2(StackScrollState resultState,
                                          StackScrollAlgorithmState algorithmState, AmbientState ambientState) {
        float currentYPosition = 0.0f,yPosition;
        int childCount = algorithmState.visibleChildren.size();
        boolean hasHeadUp=false;
        ExpandableNotificationRow topHeadsUpEntry = ambientState.getTopHeadsUpEntry();
        for (int i = 0; i < childCount; i++) {
            ExpandableView child = algorithmState.visibleChildren.get(i);
            StackViewState childViewState = resultState.getViewStateForView(child);
            ...
            childViewState.yTranslation += ambientState.getTopPadding()+ambientState.geTtransBeween()- algorithmState.scrollY+mCollapsedSize;

            yPosition = childViewState.yTranslation+childHeight-ambientState.geTtransBeween();

            if(yPosition>ambientState.getMaxStackH()&&ambientState.geTtransBeween()<0){
                float bt = yPosition-ambientState.getMaxStackH();
                if(bt<childHeight){
                    childViewState.height = (int)(childHeight-bt);
                }else{
            ...
            if(i==0 && child instanceof ExpandableNotificationRow){
                ExpandableNotificationRow row = (ExpandableNotificationRow) child;
                if(row.isHeadsUp()){
                    hasHeadUp = true;
                }
            }
            //adjust for headsup pull
            if(mHeadsUpManager!=null&&hasHeadUp) {
                float topPadding = mHeadsUpManager.getTopHeadsUpHeight();
                if (childViewState.yTranslation < topPadding) {
                    int delt = (int) (topPadding - childViewState.yTranslation);
                    if (delt < childHeight) {
                        childViewState.topOverLap = delt;
                    } else {
                        childViewState.topOverLap = childHeight;
                    }
                } else {
                    childViewState.topOverLap = 0;
                }
            }else{
                childViewState.topOverLap = 0;
            }
    ...
    private void updateHeadsUpStates(StackScrollState resultState,
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            //add by chenhl start
            if(IS_SCROLL) {
                //childState.yTranslation = childState.yTranslation - ambientState.getTopPadding();
               if (childState.yTranslation < 0) {
                    childState.yTranslation = 0;
                }
                if(ambientState.getStackTranslation()+ambientState.getTopPadding()<0){
                    childState.yTranslation += ambientState.getStackTranslation()+ambientState.getTopPadding();
                }
                algorithmState.itemsInBottomStack = childCount - 1;
                //ignore more headsup
                if(i==0)
    ...
    private void updateZValuesForState(StackScrollState resultState,
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (i < algorithmState.itemsInTopStack) {
                ...
                //modify by chenhl start
                if(IS_SCROLL){
                    childViewState.zTranslation=mZBasicHeight-mZDistanceBetweenElements;
                }else {

...
public abstract class BaseStatusBar extends SystemUI implements
    ...
    protected CustomStatusBarManager mCustomStatusBarManager;
    ...
    protected QSSlideBar mQSSlideBar;
    /**hb tangjun add end*/
    ...
    //add by chenhl start
    protected DuiNotificationOverflowContainer mNotificationOverflowContainer;
    protected DuiBlurNotificationBg mDuiBlurNotificationBg;
    //add by chenhl end
    ...
    //add by chenhl start
    private boolean mShowMessageBox;
    protected boolean mKeyguardPullNotify;
    private final Uri MESSAGEBOX_URI = Settings.Secure.getUriFor("notification_use_message_box");
    private final Uri KEYGUARD_PULL_URI = Settings.Secure.getUriFor("notification_drag_down_on_lock_screen");
    private final ContentObserver mDuiSettingsObserver = new ContentObserver(mHandler) {

        @Override
        public void onChange(boolean selfChange) {
            updateMessageBoxSetting();
            mKeyguardPullNotify = Settings.Secure.getIntForUser(mContext.getContentResolver(),
                    "notification_drag_down_on_lock_screen",
                    1,
                    mCurrentUserId) != 0;
            updateKeygardPullSetting();
        }

        @Override
        public void onChange(boolean selfChange, Uri uri) {

            if(uri.equals(MESSAGEBOX_URI)) {
                updateMessageBoxSetting();
            }
            if(uri.equals(KEYGUARD_PULL_URI)){
                mKeyguardPullNotify = Settings.Secure.getIntForUser(mContext.getContentResolver(),
                        "notification_drag_down_on_lock_screen",
                        1,
                        mCurrentUserId) != 0;
    ...
    private void updateMessageBoxSetting(){
        mShowMessageBox = Settings.Secure.getIntForUser(mContext.getContentResolver(),
                "notification_use_message_box",
                1,
                mCurrentUserId) != 0;
    }
    ...
    private final NotificationListenerService mNotificationListener =
            ...
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    for (StatusBarNotification sbn : notifications) {
                        //Log.i(TAG, "onListenerConnected isUpdate:false addNotification");
        ...
        public void onNotificationPosted(final StatusBarNotification sbn, final RankingMap rankingMap) {
            ...
            /*if (DEBUG)*/ Log.d(TAG, "onNotificationPosted: " + sbn + " rankingMap:" + rankingMap.toString());
            if (sbn != null) {
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        /*
                        Log.i(TAG, "onNotificationPosted StatusBarNotification sbn:" + sbn + 
                        ...
                        if (isUpdate) {
                            //Log.i(TAG, "onNotificationPosted isUpdate:true updateNotification");
                            updateNotification(sbn, rankingMap);
                        } else {
                            //Log.i(TAG, "onNotificationPosted isUpdate:false addNotification");
    ...
    public void start() {
        ...
        //add by chenhl start
        mContext.getContentResolver().registerContentObserver(MESSAGEBOX_URI,true,mDuiSettingsObserver,UserHandle.USER_ALL);
        mContext.getContentResolver().registerContentObserver(KEYGUARD_PULL_URI,true,mDuiSettingsObserver,UserHandle.USER_ALL);
        mDuiSettingsObserver.onChange(false);
        //add by chenhl end
    ...
    protected void applyColorsAndBackgrounds(StatusBarNotification sbn,
        ...
        //add by chenhl start
        if (entry.keyguardicon != null) {
            entry.keyguardicon.setTag(R.id.icon_is_pre_L, entry.targetSdk < Build.VERSION_CODES.LOLLIPOP);
        }
        //add by chenhl end
    }
    ...
    public void toggleRecentApps() {
    	//Log.d("222222", "---toggleRecentApps mQSSlideBar.getQSPanelPanelView().getQSPanelAlreadyShow() = " + mQSSlideBar.getQSPanelPanelView().getQSPanelAlreadyShow() );
    	/**hb tangjun mod begin*/
    	if(!mQSSlideBar.getQSPanelPanelView().getQSPanelAlreadyShow()) {
    		toggleRecents();
    	} else {
    		mQSSlideBar.getQSPanelPanelView().setToInitState();
    	}
        //toggleRecents();
    	/**hb tangjun mod end*/
    }
    ...
    void handleNotificationError(StatusBarNotification n, String message) {
        if(DEBUG) {
            Log.i(TAG, "handleNotificationError message:"+ message);
        }
    ...
    protected StatusBarNotification removeNotificationViews(String key, RankingMap ranking) {
        if(DEBUG) {
            Log.i(TAG, "removeNotificationViews key:"+ key);
        }
    ...
    protected NotificationData.Entry createNotificationViews(StatusBarNotification sbn) {
        ...
        NotificationData.Entry entry = new NotificationData.Entry(sbn, iconView,(StatusBarIconView)iconView.clone()); //modify by chenhl
    ...
    protected void updateRowStates() {
        ...
        mNotificationOverflowContainer.removeCount();//add by chenhl

        ArrayList<Entry> activeNotifications = mNotificationData.getActiveNotifications();
        final int N = activeNotifications.size();

        boolean canshobox =canShowBox(activeNotifications); //add by chenhl
        ...
        for (int i = 0; i < N; i++) {
            ...
            boolean isFirstNotify = shouldFirstNotify(entry);//add by chenhl
            if (/*(isLockscreenPublicMode() && !mShowLockscreenNotifications) ||*/ //delete by chenhl
                    (onKeyguard && (visibleNotifications >= maxKeyguardNotifications
                            || !showOnKeyguard || isInvisibleChild))) {
                entry.row.setVisibility(View.GONE);
                if (onKeyguard && showOnKeyguard && !isInvisibleChild&&isFirstNotify) {
                    mKeyguardIconOverflowContainer.getIconsView().addNotification(entry);
                }
            }
            //add by chenhl start
            else if(onKeyguard&&!isFirstNotify){
                entry.row.setVisibility(View.GONE);
                //mKeyguardIconOverflowContainer.getIconsView().addNotification(entry);
            }
            else if(!isFirstNotify&&mShowFirstNotify&&mShowMessageBox&&canshobox){
                entry.row.setVisibility(View.GONE);
                mNotificationOverflowContainer.addCount(entry.notification);
                if(mNotificationOverflowContainer.getCount()==0){
        ...
        mStackScroller.updateOverflowContainerVisibility(onKeyguard
                && mKeyguardIconOverflowContainer.getIconsView().getChildCount() > 0&&(visibleNotifications>=maxKeyguardNotifications));

        //add by chenhl start
        if(onKeyguard&&visibleNotifications>= maxKeyguardNotifications){
            mStackScroller.updateNotifyOverflowVisibility(false);
        }else{
            mNotificationOverflowContainer.updateString();
            mStackScroller.updateNotifyOverflowVisibility(mNotificationOverflowContainer.getCount()>0);
        }
        //add by chenhl end
        ...
        //add by chenhl
        mStackScroller.changeViewPosition(mNotificationOverflowContainer,
                mStackScroller.getChildCount()-3);
    }

    //add by chenhl start
    protected boolean mShowFirstNotify=true;
    private boolean shouldFirstNotify(Entry entry){
        try {
            int p = NotificationManager.getService().hbGetPriority(entry.notification.getPackageName(), entry.notification.getUid());
            //is headsup?
            boolean isHeadsup=entry.row.isPinned();
            return p == Notification.PRIORITY_MAX||isHeadsup;
        } catch (Exception e) {
            Log.w("chenhl", "Error calling NoMan", e);
            return false;
        }
    }

    private boolean canShowBox(ArrayList<Entry> Notifications){

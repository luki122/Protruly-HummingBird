...
public class WifiTile extends QSTile<QSTile.SignalState> {
    ...
    private boolean mRefreshFromStateChange = true;
    final Object mRefreshLock = new Object();
    ...
    protected void handleClick() {
    	/**hb tangjun add begin*/
    	if(!mRefreshFromStateChange) {
    		return;
    	}
    	mRefreshFromStateChange = false;
    	/**hb tangjun add end*/
        ...
        /**hb tangjun add begin*/
        mLastValue = mState.enabled;
        refreshState(null);
        //hb tangjun add for fix bug 6383 begin
        mHandler.postDelayed(new Runnable() {
			...
			public void run() {
				...
				mLastValue = true;
				mRefreshFromStateChange = true;
				refreshState(null);
			}
		}, 2000);
        //hb tangjun add for fix bug 6383 end
        /**hb tangjun add end*/
    }
    ...
    protected void handleUpdateState(SignalState state, Object arg) {
        ...
        /**hb tangjun add begin*/
        if(!mLastValue && !state.enabled && state.enabled == cb.enabled) {
        	state.icon = ResourceIcon.get(R.drawable.hb_wifi_waiting);
        	return;
        } else {
        	mLastValue = true;
        }
        ...
        if (!state.enabled) {
        	...
        	state.icon = ResourceIcon.get(R.drawable.hb_wifi_off);
        	...
        	state.icon = ResourceIcon.get(R.drawable.hb_wifi_on);

...
public class VolumeDialog {
    ...
    // add by wxue 20170518 start
    private int mLastOrientation ;
    // add by wxue 20170518 end
    ...
    public VolumeDialog(Context context, int windowType, VolumeDialogController controller,
        ...
        // modify by wxue 20170511 start
        //mActiveSliderTint = loadColorStateList(R.color.system_accent_color);
        //mInactiveSliderTint = loadColorStateList(R.color.volume_slider_inactive);
        mActiveSliderTint = loadColorStateList(R.color.hb_volume_slider_active);
        mInactiveSliderTint = loadColorStateList(R.color.hb_volume_slider_inactive);
        // modify by wxue 20170511 end
        ...
        addRow(AudioManager.STREAM_RING,
                R.drawable.hmb_volume_ringer, R.drawable.ic_volume_ringer_mute, true);
        addRow(AudioManager.STREAM_MUSIC,
                R.drawable.hmb_volume_media, R.drawable.hmb_volume_media_mute, true);
        addRow(AudioManager.STREAM_ALARM,
                R.drawable.hmb_volume_alarm, R.drawable.hmb_volume_alarm_mute, false);
        // modify by wxue 20170511 end
    ...
    private void updateWindowWidthH() {
        ...
        w -= mContext.getResources().getDimensionPixelSize(R.dimen.hb_volume_dialog_margin) * 2;
        // modify by wxue 20170511 end
        lp.width = w;
    ...
    private void addRow(int stream, int iconRes, int iconMuteRes, boolean important) {
        ...
        row.settingsButton.addOnLayoutChangeListener(new OnLayoutChangeListener() {
            ...
            public void onLayoutChange(View v, int left, int top, int right, int bottom,
                ...
                final boolean orientationChanged = mLastOrientation != orientation ? true : false;
                mLastOrientation = orientation;
                // add by wuxe 20170518 end
                // modify by wxue 20170518 start
                //if (moved) {
                if (moved || orientationChanged) {
                // modify by wxue 20170518 end
                ...
                if (row.stream == AudioManager.STREAM_RING) {
                    ...
                    if (mState.ringerModeInternal == AudioManager.RINGER_MODE_NORMAL) {
                        if (hasVibrator) {
                        	...
                        	if(mController.getIsVibrationEnabled()) {
                        		mController.setRingerMode(AudioManager.RINGER_MODE_VIBRATE, false);
                        		mController.getAudioManager().setStreamMute(AudioManager.STREAM_MUSIC, true);
                        	} else {
                        ...
                        mController.getAudioManager().setStreamVolume(AudioManager.STREAM_MUSIC,
                        		mController.getAudioManager().getLastAudibleStreamVolume(AudioManager.STREAM_MUSIC), 0);
                        /**hb tangjun add end*/
                        if (row.ss.level == 0) {
                	...
                	/**hb tangjun mod begin*/
                	//产品改成通话默认最小音量level为1，不是0
                	/*
                    final boolean vmute = row.ss.level == 0;
                    mController.setStreamVolume(stream, vmute ? row.lastAudibleLevel : 0);
                    */
                	if(row.stream == AudioManager.STREAM_VOICE_CALL || row.stream == AudioManager.STREAM_BLUETOOTH_SCO) {
                        final boolean vmute = row.ss.level == 1;
                        mController.setStreamVolume(stream, vmute ? row.lastAudibleLevel : 1);
                	} else {
                        final boolean vmute = row.ss.level == 0;
                        mController.setStreamVolume(stream, vmute ? row.lastAudibleLevel : 0);
                	}
    ...
    private void updateVolumeRowH(VolumeRow row) {
        ...
        */
        ...
        row.icon.setEnabled(true);
        /**hb tangjun mod end*/
        /**hb tangjun add for vibrate when silent begin*/
        int iconId = 0;
        if(mController.getIsVibrationEnabled()) {
        	iconId = R.drawable.hmb_volume_ringer_vibrate;
        } else {
        	iconId = R.drawable.hmb_volume_zen;
        }
        /**hb tangjun add for vibrate when silent end*/
        row.icon.setAlpha(iconEnabled ? 1 : 0.5f);
        final int iconRes =
        		// modify by wxue 20170511 start
                //isRingVibrate ? R.drawable.ic_volume_ringer_vibrate
        		//: isRingSilent || zenMuted ? row.cachedIconRes
//        		isRingVibrate ? R.drawable.hmb_volume_ringer_vibrate
//        				: isRingSilent || zenMuted ? R.drawable.hmb_volume_zen
        		//hb tangjun mod for vibrate when silent begin
        		isRingVibrate ? iconId
        				: isRingSilent || zenMuted ? iconId
        		//hb tangjun mod for vibrate when silent  end
                // modify by wxue 20170511 end 
        ...
        row.iconState =
                ...
                iconRes == R.drawable.hmb_volume_ringer_vibrate ? Events.ICON_STATE_VIBRATE
                // modify by wxue 20170511 end
        ...
        */
        final boolean enableSlider = !zenMuted && !isRingVibrate;
        /**hb tangjun mod for fix bug 6593 begin*/
//        final int vlevel = row.ss.muted && (/*isRingVibrate ||*/ !isRingStream && !zenMuted) ? 0
//                : row.ss.level;
        final int vlevel = (row.ss.muted && (/*isRingVibrate ||*/ !isRingStream && !zenMuted)) && (row.stream != AudioManager.STREAM_VOICE_CALL
        		&& row.stream != AudioManager.STREAM_BLUETOOTH_SCO) ? 0
        		: row.ss.level;
        /**hb tangjun mod for fix bug 6593 end*/
        updateVolumeRowSliderH(row, enableSlider, vlevel);
        /**hb tangjun mod end*/
    }
    ...
    private void updateVolumeRowSliderH(VolumeRow row, boolean enable, int vlevel) {
        ...
        /**hb tangjun mod for fix bug 8383 begin*/
        /*
        if (row.tracking) {
            return;  // don't update if user is sliding
        }
        */
        /**hb tangjun mod for fix bug 8383 end*/

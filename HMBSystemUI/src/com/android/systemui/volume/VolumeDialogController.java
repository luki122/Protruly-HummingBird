...
public class VolumeDialogController {
    ...
    private int mLastRingerModeInternal;
    ...
    public boolean getIsVibrationEnabled() {
    	if(mProfileManager != null) {
    		return mProfileManager.isVibrationEnabled("mtk_audioprofile_silent");
    	} else {
    ...
    private boolean updateRingerModeInternalW(int rm) {
        ...
        /**hb tangjun add for remove vibrate mode temporary begin*/
        if(rm == AudioManager.RINGER_MODE_VIBRATE && !getIsVibrationEnabled()) {
        	if(mLastRingerModeInternal != AudioManager.RINGER_MODE_SILENT) {
        		setRingerMode(AudioManager.RINGER_MODE_SILENT, false);
        		mAudio.setStreamMute(AudioManager.STREAM_MUSIC, true);
        	} else {
        		setRingerMode(AudioManager.RINGER_MODE_NORMAL, false);
        		mAudio.setStreamVolume(AudioManager.STREAM_MUSIC,
        				mAudio.getLastAudibleStreamVolume(AudioManager.STREAM_MUSIC), 0);
        	}
        }
        if(getIsVibrationEnabled()) {
        	if(rm == AudioManager.RINGER_MODE_SILENT) {
        		setRingerMode(AudioManager.RINGER_MODE_VIBRATE, false);
        	}
        ...
        mLastRingerModeInternal = rm;
        /**hb tangjun add for remove vibrate mode temporary end*/
